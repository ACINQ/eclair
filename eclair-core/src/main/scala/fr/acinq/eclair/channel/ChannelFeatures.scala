...
object ChannelFeatures {
  ...
  def apply(channelType: ChannelType, localFeatures: Features[InitFeature], remoteFeatures: Features[InitFeature]): ChannelFeatures = {
    ...
    val availableFeatures = Seq(Features.Wumbo, Features.UpfrontShutdownScript, Features.DualFunding).filter(f => Features.canUseFeature(localFeatures, remoteFeatures, f))
