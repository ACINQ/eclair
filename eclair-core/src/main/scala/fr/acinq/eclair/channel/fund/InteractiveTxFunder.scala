...
object InteractiveTxFunder {
  ...
  private case class FundTransactionResult(tx: Transaction, changePosition: Option[Int], excess_opt: Option[Satoshi] ) extends Command
  ...
  private def needsAdditionalFunding(fundingParams: InteractiveTxParams): Boolean = {
    if (fundingParams.isInitiator) {
      // We're the initiator, we always have to pay on-chain fees, even if we don't splice in or out.
      // If we're not paying those on-chain fees by lowering our channel contribution, we must add more funding.
...
private class InteractiveTxFunder(replyTo: ActorRef[InteractiveTxFunder.Response],
  ...
  def start(): Behavior[Command] = {
    ...
    if (!needsAdditionalFunding(fundingParams)) {
      ...
      if (fundingParams.isInitiator) {
        ...
        val fundingContributions = sortFundingContributions(fundingParams, sharedInput ++ previousWalletInputs, sharedOutput +: nonChangeOutputs, excess_opt = Some(0 sat))
        replyTo ! fundingContributions
        Behaviors.stopped
      } else {
        val nonChangeOutputs = fundingParams.localOutputs.map(txOut => Output.Local.NonChange(UInt64(0), txOut.amount, txOut.publicKeyScript))
        val fundingContributions = sortFundingContributions(fundingParams, previousWalletInputs, nonChangeOutputs, excess_opt = Some(0 sat))
        replyTo ! fundingContributions
        Behaviors.stopped
      }
    } else {
  ...
  private def fund(txNotFunded: Transaction, currentInputs: Seq[OutgoingInput], unusableInputs: Set[UnusableInput]): Behavior[Command] = {
    ...
    context.pipeToSelf(wallet.fundTransaction(txNotFunded, fundingParams.targetFeerate, replaceable = true, externalInputsWeight = sharedInputWeight, feeBudget_opt = feeBudget_opt)) {
      ...
      case Success(result) =>
        // TODO: detect if inputs are not p2wpkh and add appropriate dummy witness, or fail
        val dummyWitness = Script.witnessPay2wpkh(Transactions.PlaceHolderPubKey, ByteVector.fill(73)(0))
        val dummySignedTx = result.tx.copy(
          txIn = result.tx.txIn.filterNot(i => sharedInputWeight.contains(i.outPoint)).map(_.copy(witness = dummyWitness)),
          txOut = result.tx.txOut,
        )
        val dummyFee = Transactions.weight2fee(fundingParams.targetFeerate, dummySignedTx.weight() + sharedInputWeight.values.sum.toInt)
        val excess = result.fee - dummyFee
        val excess_opt = if (result.changePosition.isEmpty && excess > 0.sat) Some(excess) else None
        FundTransactionResult(result.tx, result.changePosition, excess_opt)
    }
    Behaviors.receiveMessagePartial {
      case FundTransactionResult(fundedTx, changePosition, excess_opt) =>
        ...
        if (lockedUnusableInputs.nonEmpty) {
          ...
          filterInputs(fundedTx, changePosition, currentInputs, unusableInputs, excess_opt)
        }
  ...
  private def filterInputs(fundedTx: Transaction, changePosition: Option[Int], currentInputs: Seq[OutgoingInput], unusableInputs: Set[UnusableInput], excess_opt: Option[Satoshi]): Behavior[Command] = {
