...
class Register extends Actor with ActorLogging {
  ...
  def main(channels: Map[ByteVector32, ActorRef], shortIds: Map[ShortChannelId, ByteVector32], channelsTo: Map[ByteVector32, PublicKey]): Receive = {
    ...
    case ShortChannelIdAssigned(_, channelId, shortChannelId, previousShortChannelId_opt) => previousShortChannelId_opt match {
      // ShortChannelId changed because of a block re-org.
