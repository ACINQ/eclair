...
trait ErrorHandlers extends CommonHandlers {
  ...
  def handleRemoteSpentCurrent(commitTx: Transaction, d: ChannelDataWithCommitments) = {
    ...
    context.system.eventStream.publish(TransactionPublished(d.channelId, remoteNodeId, commitTx, localCommitFee, remoteCommitFee, "remote-commit"))
  ...
  def handleRemoteSpentNext(commitTx: Transaction, d: ChannelDataWithCommitments) = {
    ...
    context.system.eventStream.publish(TransactionPublished(d.channelId, remoteNodeId, commitTx, localCommitFee, remoteCommitFee, "next-remote-commit"))
  ...
  def handleRemoteSpentOther(tx: Transaction, d: ChannelDataWithCommitments) = {
    ...
    Closing.RevokedClose.getRemotePerCommitmentSecret(d.commitments.channelParams, channelKeys, d.commitments.remotePerCommitmentSecrets, tx) match {
      case Some((commitmentNumber, remotePerCommitmentSecret)) =>
        ...
        context.system.eventStream.publish(TransactionPublished(d.channelId, remoteNodeId, tx, localCommitFee, remoteCommitFee, "revoked-commit"))
      ...
      case None => d match {
        case d: DATA_WAIT_FOR_REMOTE_PUBLISH_FUTURE_COMMITMENT =>
          ...
          context.system.eventStream.publish(TransactionPublished(d.channelId, remoteNodeId, tx, localCommitFee, remoteCommitFee, "future-remote-commit"))
