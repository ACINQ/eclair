...
final case class DATA_NEGOTIATING(commitments: Commitments,
  ...
  require(commitments.getContext == VersionSimplifiedCommitment || !commitments.localParams.isFunder || closingTxProposed.forall(!_.isEmpty), "funder must have at least one closing signature for every negotation attempt because it initiates the closing")
}
