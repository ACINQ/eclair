...
object InteractiveTxBuilder {
  ...
  object SharedFundingInput {
    def apply(commitment: Commitment): SharedFundingInput = commitment.commitmentFormat match {
      case _: SimpleTaprootChannelCommitmentFormat => Musig2Input(commitment)
      case _ => Multisig2of2Input(commitment)
    }
  }
  ...
  case class InteractiveTxParams(channelId: ByteVector32,
                                 ...
                                 currentCommitmentFormat: CommitmentFormat,
                                 nextCommitmentFormat: CommitmentFormat) {
  ...
  private case class InteractiveTxSession(toSend: Seq[Outgoing],
                                          ...
                                          secretNonces: Map[UInt64, LocalNonce] = Map.empty) {
...
private class InteractiveTxBuilder(replyTo: ActorRef[InteractiveTxBuilder.Response],
  ...
  private val fundingPubkeyScript: ByteVector = Funding.makeFundingScript(localFundingKey.publicKey, fundingParams.remoteFundingPubKey, fundingParams.nextCommitmentFormat).pubkeyScript
  ...
  // nonce used to sign the shared input if there is one
    ...
    session.toSend match {
      case (addInput: Input) +: tail =>
        ...
        val nextSecretNonces = addInput match {
          case i: Input.Shared if localNonce.isDefined => session.secretNonces + (i.serialId -> localNonce.get)
      ...
      case (addOutput: Output) +: tail =>
        ...
        receive(next)
      case Nil =>
        val txComplete = fundingParams.nextCommitmentFormat match {
          case _: SimpleTaprootChannelCommitmentFormat =>
            ...
            }.getOrElse(TxComplete(fundingParams.channelId))
          case _ =>
            TxComplete(fundingParams.channelId)
        }
  ...
  private def receiveInput(session: InteractiveTxSession, addInput: TxAddInput): Either[ChannelException, InteractiveTxSession] = {
    ...
    val session2 = input match {
      case i: Input.Shared if this.localNonce.isDefined => session1.copy(secretNonces = session1.secretNonces + (i.serialId -> localNonce.get))
      case _ => session1
    }
    Right(session2)
  }
  ...
  private def signCommitTx(session: InteractiveTxSession, completeTx: SharedTransaction): Behavior[Command] = {
    ...
    Funding.makeCommitTxs(channelParams,
      ...
      localCommitmentIndex = purpose.localCommitIndex, remoteCommitmentIndex = purpose.remoteCommitIndex, fundingParams.nextCommitmentFormat) match {
      case Left(cause) =>
        replyTo ! RemoteFailure(cause)
      ...
      case Right((localSpec, localCommitTx, remoteSpec, remoteCommitTx, sortedHtlcTxs)) =>
        ...
        val localSigOfRemoteTx = fundingParams.nextCommitmentFormat match {
        ...
        val tlvs: TlvStream[CommitSigTlv] = fundingParams.nextCommitmentFormat match {
          case _: SimpleTaprootChannelCommitmentFormat =>
          ...
          case _: SegwitV0CommitmentFormat => TlvStream.empty[CommitSigTlv]
        }
        val htlcSignatures = sortedHtlcTxs.map(_.sign(remoteCommitmentKeys, fundingParams.nextCommitmentFormat)).toList
        val localCommitSig = CommitSig(fundingParams.channelId, localSigOfRemoteTx, htlcSignatures, tlvs)
  ...
  private def signTx(session: InteractiveTxSession, unsignedTx: SharedTransaction): Unit = {
    ...
    val sharedSig_opt = fundingParams.sharedInput_opt.collect {
      ...
      case i: Musig2Input => ByteVector64.Zeroes
    }
    val sharedPartialSig_opt = fundingParams.sharedInput_opt.collect {
      case i: Musig2Input =>
        val Some(sharedInput) = (session.localInputs ++ session.remoteInputs).collectFirst { case i: Input.Shared => i }
        // there should be a single shared input
        val serialId = sharedInput.serialId
        ...
        log.debug(s"signFundingTx: creating partial sig $psig for ${tx.txid} inputIndex=$inputIndex fundingKey = ${fundingKey.publicKey} fundingTxIndex = ${i.fundingTxIndex} remoteFundingPubkey = ${i.remoteFundingPubkey} local nonce = ${localNonce.publicNonce} fundingTxIndex = ${i.fundingTxIndex} commitIndex = ${i.commitIndex} remote nonce = ${remoteNonce} spentOutputs = ${unsignedTx.spentOutputs}")
...
object InteractiveTxSigningSession {
        ...
        (partiallySignedTx.localSigs.previousFundingTxPartialSig_opt, remoteSigs.previousFundingTxPartialSig_opt) match {
          case (Some(localPartialSig), Some(remotePartialSig)) =>
            ...
            val aggSig = Musig2.aggregateTaprootSignatures(
              ...
              None)
            aggSig match {
              case Right(sig) => Some(Script.witnessKeyPathPay2tr(sig))
              case Left(error) =>
  ...
  case class WaitingForSigs(fundingParams: InteractiveTxParams,
    ...
    def receiveCommitSig(channelParams: ChannelParams, channelKeys: ChannelKeys, remoteCommitSig: CommitSig, currentBlockHeight: BlockHeight)(implicit log: LoggingAdapter): Either[ChannelException, InteractiveTxSigningSession] = {
      localCommit match {
        case Left(unsignedLocalCommit) =>
          ...
          val commitmentFormat = fundingParams.nextCommitmentFormat
          LocalCommit.fromCommitSig(channelParams, commitKeys, fundingTx.txId, fundingKey, fundingParams.remoteFundingPubKey, commitInput, remoteCommitSig, localCommitIndex, unsignedLocalCommit.spec, fundingParams.nextCommitmentFormat).map { signedLocalCommit =>
            if (shouldSignFirst(fundingParams.isInitiator, channelParams, fundingTx.tx)) {
              ...
              val commitment = Commitment(commitmentFormat, fundingTxIndex, remoteCommit.index, fundingParams.remoteFundingPubKey, fundingStatus, RemoteFundingStatus.NotLocked, signedLocalCommit, remoteCommit, None)
    ...
    def receiveTxSigs(channelParams: ChannelParams, channelKeys: ChannelKeys, remoteTxSigs: TxSignatures, currentBlockHeight: BlockHeight)(implicit log: LoggingAdapter): Either[ChannelException, SendingSigs] = {
      localCommit match {
        case Left(_) =>
