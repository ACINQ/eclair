...
object InteractiveTxBuilder {
  ...
  case class Succeeded(signingSession: InteractiveTxSigningSession.WaitingForSigs, commitSig: CommitSig, lease_opt: Option[LiquidityAds.Lease]) extends Response
        ...
        Behaviors.withMdc(Logs.mdc(remoteNodeId_opt = Some(channelParams.remoteParams.nodeId), channelId_opt = Some(fundingParams.channelId))) {
          Behaviors.receiveMessagePartial {
            case Start(replyTo) =>
              ...
              val liquidityPaymentTypeOk = liquidityLease_opt match {
                case Some(l) if !fundingParams.isInitiator => l.paymentDetails match {
                  case PaymentDetails.FromChannelBalance | _: PaymentDetails.FromChannelBalanceForFutureHtlc => true
                  ...
                  case _: PaymentDetails.FromFutureHtlc | _: PaymentDetails.FromFutureHtlcWithPreimage => nextRemoteBalance < l.fees.total
                }
                case _ => true
              }
              if (fundingParams.fundingAmount < fundingParams.dustLimit) {
                ...
                replyTo ! LocalFailure(InvalidLiquidityAdsPaymentType(channelParams.channelId, liquidityLease_opt.get.paymentDetails.paymentType, Set(LiquidityAds.PaymentType.FromChannelBalance, LiquidityAds.PaymentType.FromChannelBalanceForFutureHtlc)))
                Behaviors.stopped
              } else {
  ...
  private def validateTx(session: InteractiveTxSession): Either[ChannelException, SharedTransaction] = {
    ...
    previousTransactions.headOption match {
      ...
      case None =>
        ...
        val minimumFee = liquidityLease_opt.map(_.paymentDetails) match {
          case Some(paymentDetails) => paymentDetails match {
            case PaymentDetails.FromChannelBalance | _: PaymentDetails.FromChannelBalanceForFutureHtlc => feeWithoutWitness
            ...
            case _: PaymentDetails.FromFutureHtlc | _: PaymentDetails.FromFutureHtlcWithPreimage => feeWithoutWitness * 0.5
          }
  ...
  private def signFundingTx(completeTx: SharedTransaction, commitSig: CommitSig, localCommit: UnsignedLocalCommit, remoteCommit: RemoteCommit): Behavior[Command] = {
    ...
    Behaviors.receiveMessagePartial {
      case SignTransactionResult(signedTx) =>
        ...
        replyTo ! Succeeded(InteractiveTxSigningSession.WaitingForSigs(fundingParams, purpose.fundingTxIndex, signedTx, Left(localCommit), remoteCommit), commitSig, liquidityLease_opt)
        Behaviors.stopped
