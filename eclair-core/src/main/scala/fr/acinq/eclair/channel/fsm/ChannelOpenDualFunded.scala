    ...
    case Event(msg: InteractiveTxMessage, d: DATA_WAIT_FOR_DUAL_FUNDING_CREATED) =>
      msg match {
              ...
              if (txSession1.isComplete) {
                InteractiveTx.validateTx(txSession1, d.fundingParams) match {
                  case Right((completeTx, fundingOutputIndex)) =>
                    ...
                    Funding.makeFirstCommitTxs(keyManager, d.channelConfig, d.channelFeatures, d.channelId, d.localParams, d.remoteParams, d.fundingParams.fundingAmount, 0 msat, d.commitTxFeerate, fundingTx.hash, fundingOutputIndex, d.remoteFirstPerCommitmentPoint) match {
                      ...
                      case Left(cause) =>
                        InteractiveTx.rollback(d.txSession, wallet)
                        channelOpenReplyToUser(Left(LocalError(cause)))
                        handleLocalError(cause, d, None)
                    }
                  case Left(cause) =>
                    InteractiveTx.rollback(d.txSession, wallet)
                    channelOpenReplyToUser(Left(LocalError(cause)))
                    handleLocalError(cause, d, Some(msg))
                }
              } else {
        ...
        case _: TxAbort =>
          ...
          handleLocalError(DualFundingAborted(d.channelId), d, Some(msg))
        case _: TxSignatures =>
          InteractiveTx.rollback(d.txSession, wallet)
          channelOpenReplyToUser(Left(LocalError(InvalidFundingSignature(d.channelId, None))))
          handleLocalError(InvalidFundingSignature(d.channelId, None), d, Some(msg))
        case _: TxInitRbf =>
          InteractiveTx.rollback(d.txSession, wallet)
          channelOpenReplyToUser(Left(LocalError(InvalidRbfAttempt(d.channelId))))
          handleLocalError(InvalidRbfAttempt(d.channelId), d, Some(msg))
        case _: TxAckRbf =>
          InteractiveTx.rollback(d.txSession, wallet)
          channelOpenReplyToUser(Left(LocalError(InvalidRbfAttempt(d.channelId))))
          handleLocalError(InvalidRbfAttempt(d.channelId), d, Some(msg))
