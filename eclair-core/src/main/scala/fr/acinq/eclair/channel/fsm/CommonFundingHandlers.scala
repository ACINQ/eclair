...
trait CommonFundingHandlers extends CommonHandlers {
  ...
  def createShortIds(channelId: ByteVector32, realScidStatus: RealScidStatus, isPublic: Boolean): ShortIds = {
    ...
    context.system.eventStream.publish(ShortChannelIdAssigned(self, channelId, shortIds, remoteNodeId, isPublic))
    shortIds
  }
  ...
  def receiveChannelReady(shortIds: ShortIds, channelReady: ChannelReady, commitments: Commitments): DATA_NORMAL = {
    ...
    shortIds1.remoteAlias_opt.foreach(_ => context.system.eventStream.publish(ShortChannelIdAssigned(self, commitments.channelId, shortIds1, remoteNodeId, commitments.announceChannel)))
