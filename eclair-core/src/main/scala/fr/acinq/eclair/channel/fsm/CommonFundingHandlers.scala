  ...
  def createChannelReady(shortIds: ShortIds, params: ChannelParams): ChannelReady = {
    ...
    val tlvStream: TlvStream[ChannelReadyTlv] = params.commitmentFormat match {
      case SimpleTaprootChannelsStagingCommitmentFormat =>
        ...
        val nextLocalNonce = keyManager.commitmentNonce(params.localParams.fundingKeyPath, fundingTxIndex = 0, channelKeyPath, 1)
        TlvStream(ChannelReadyTlv.ShortChannelIdTlv(shortIds.localAlias), ChannelTlv.NextLocalNonceTlv(nextLocalNonce._2))
      case _ =>
