...
trait DualFundingHandlers extends CommonHandlers {
  ...
  def publishFundingTx(d: DATA_WAIT_FOR_DUAL_FUNDING_CONFIRMED): Unit = {
    d.fundingTx match {
      case _: InteractiveTx.PartiallySignedSharedTransaction =>
      ...
      case fundingTx: InteractiveTx.FullySignedSharedTransaction =>
        ...
        wallet.publishTransaction(fundingTx.signedTx).onComplete {
          case Success(_) =>
            context.system.eventStream.publish(TransactionPublished(d.commitments.channelId, remoteNodeId, fundingTx.signedTx, fundingTx.tx.fees, "funding"))
          ...
          case Failure(t) =>
            ...
            log.error(t, "error while committing funding tx: ") // tx may be published by our peer, we can't fail-fast
        }
    }
  }
