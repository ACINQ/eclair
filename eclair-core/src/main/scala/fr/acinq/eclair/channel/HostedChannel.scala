...
class HostedChannel(val nodeParams: NodeParams, remoteNodeId: PublicKey, router: ActorRef, relayer: ActorRef) extends FSM[State, HostedData] with FSMDiagnosticActorLogging[State, HostedData] {
  ...
  whenUnhandled {
      ...
      Try(commits.receiveFulfill(fulfill)) match {
        case Success(Right((commits1, origin, htlc))) =>
          ...
          commits.getError match {
            case Some(error) =>
              val commits2 = commits1.copy(fulfilledOutgoingHtlcLeftoverIds = commits1.fulfilledOutgoingHtlcLeftoverIds + htlc.id)
        ...
        case Failure(cause) => localSuspend(commits, ByteVector.view(cause.getMessage.getBytes)) replying s"Failed reason=${cause.getMessage}"
        case otherwise => stay replying s"Failed reason=$otherwise"
      }
    ...
    case Event(CMD_GETINFO, commits: HOSTED_DATA_COMMITMENTS) =>
      val pendingRelays = nodeParams.db.pendingRelay.listPendingRelay(commits.channelId)
      sender ! RES_HOSTED_GETINFO(remoteNodeId, commits.channelId, stateName, commits, commits.nextLocalSpec, pendingRelays)
      stay
  ...
  def restoreEmptyCommits(localLCSS: LastCrossSignedState, channelId: ByteVector32, isHost: Boolean): HOSTED_DATA_COMMITMENTS = {
    ...
    HOSTED_DATA_COMMITMENTS(remoteNodeId, ChannelVersion.STANDARD, localLCSS, futureUpdates = Nil, localCommitmentSpec, originChannels = Map.empty, channelId, isHost, channelUpdate,
      localError = None, remoteError = None, failedOutgoingHtlcLeftoverIds = Set.empty, fulfilledOutgoingHtlcLeftoverIds = Set.empty, overrideProposal = None)
  }
  ...
  def localSuspend(commits: HOSTED_DATA_COMMITMENTS, errCode: ByteVector): HostedFsmState = {
    ...
    if (stateName == OFFLINE) {
      // Never exit OFFLINE state except by an actual reconnection
  ...
  def failTimedoutOutgoing(blockHeight: Long, commits: HOSTED_DATA_COMMITMENTS): (HOSTED_DATA_COMMITMENTS, Set[UpdateAddHtlc]) = {
    val timedoutOutgoingHtlcs: Set[UpdateAddHtlc] = commits.timedOutOutgoingHtlcs(blockHeight) // Find timedout outgoing HTLCs which have not been failed or fulfilled by remote peer already
    ...
    (commits.copy(failedOutgoingHtlcLeftoverIds = timedoutOutgoingHtlcs.map(_.id) ++ commits.failedOutgoingHtlcLeftoverIds), timedoutOutgoingHtlcs)
  }
