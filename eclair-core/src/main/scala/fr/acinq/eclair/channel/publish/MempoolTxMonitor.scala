...
private class MempoolTxMonitor(nodeParams: NodeParams,
  ...
  private def waitForConfirmation(): Behavior[Command] = {
    ...
    context.system.eventStream ! EventStream.Publish(TransactionPublished(txPublishContext.channelId_opt.getOrElse(ByteVector32.Zeroes), txPublishContext.remoteNodeId, cmd.tx, localMiningFee = cmd.fee, remoteMiningFee = 0 sat, cmd.desc))
    Behaviors.receiveMessagePartial {
