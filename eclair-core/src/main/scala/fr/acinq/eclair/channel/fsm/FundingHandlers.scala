...
trait FundingHandlers extends CommonHandlers {
  ...
  def acceptFundingTx(commitments: Commitments, realScidStatus: RealScidStatus, localAlias: Alias): (ShortIds, ChannelReady) = {
    ...
    val shortIds = ShortIds(realScidStatus, localAlias, remoteAlias_opt = None)
