  ...
  private def addInputs(tx: ReplaceableTxWithWalletInputs, targetFeerate: FeeratePerKw, commitment: FullCommitment): Future[(ReplaceableTxWithWalletInputs, Satoshi, Seq[TxOut])] = {
    ...
    tx match {
      case anchorTx: ClaimLocalAnchorWithWitnessData => for {
        ...
        psbt = new Psbt(fundedTx.txInfo.tx)
        updatedPsbt <- bitcoinClient.utxoUpdatePsbt(psbt)
        spentUtxos = anchorTx.txInfo.input.txOut +: updatedPsbt.inputs.asScala.tail.map(_.getWitnessUtxo).map(kmp2scala).toSeq
      ...
      case htlcTx: HtlcWithWitnessData => for {
        ...
        psbt = new Psbt(fundedTx.txInfo.tx)
        updatedPsbt <- bitcoinClient.utxoUpdatePsbt(psbt)
        spentUtxos = htlcTx.txInfo.input.txOut +: updatedPsbt.inputs.asScala.tail.map(_.getWitnessUtxo).map(kmp2scala).toSeq
      } yield (fundedTx, amountIn, spentUtxos)
    }
  }
