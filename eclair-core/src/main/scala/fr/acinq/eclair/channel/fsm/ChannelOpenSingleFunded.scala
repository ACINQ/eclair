 ...
 */
    ...
    case Event(open: OpenChannel, d: DATA_WAIT_FOR_OPEN_CHANNEL) =>
      Helpers.validateParamsSingleFundedFundee(nodeParams, d.initFundee.channelType, d.initFundee.localParams.initFeatures, open, remoteNodeId, d.initFundee.remoteInit.features) match {
        ...
        case Right((channelFeatures, remoteShutdownScript)) =>
          ...
          val minimumDepth = params.minDepth(nodeParams.channelConf.minDepth)
          ...
          val accept = AcceptChannel(temporaryChannelId = open.temporaryChannelId,
            ...
            fundingPubkey = fundingPubkey,
            revocationBasepoint = channelKeys.revocationBasePoint,
            paymentBasepoint = d.initFundee.localParams.walletStaticPaymentBasepoint.getOrElse(channelKeys.paymentBasePoint),
            delayedPaymentBasepoint = channelKeys.delayedPaymentBasePoint,
            htlcBasepoint = channelKeys.htlcBasePoint,
            firstPerCommitmentPoint = channelKeys.commitmentPoint(0),
          ...
          setRemoteNextLocalNonces("from their open_channel", open.nexLocalNonce_opt.map(n => Map(TxId(ByteVector32.Zeroes) -> n)).getOrElse(Map.empty))
    ...
    case Event(accept: AcceptChannel, d@DATA_WAIT_FOR_ACCEPT_CHANNEL(init, open)) =>
      Helpers.validateParamsSingleFundedFunder(nodeParams, init.channelType, init.localParams.initFeatures, init.remoteInit.features, open, accept) match {
        ...
        case Right((channelFeatures, remoteShutdownScript)) =>
          ...
          val fundingPubkeyScript = Funding.makeFundingScript(localFundingKey.publicKey, accept.fundingPubkey, init.channelType.commitmentFormat).pubkeyScript
          ...
          setRemoteNextLocalNonces("received AcceptChannel", accept.nexLocalNonce_opt.map(n => Map(TxId(ByteVector32.Zeroes) -> n)).getOrElse(Map.empty))
  ...
  when(WAIT_FOR_FUNDING_INTERNAL)(handleExceptions {
    case Event(MakeFundingTxResponse(fundingTx, fundingTxOutputIndex, fundingTxFee), d@DATA_WAIT_FOR_FUNDING_INTERNAL(params, fundingAmount, pushMsat, commitTxFeerate, remoteFundingPubKey, remoteFirstPerCommitmentPoint, replyTo)) =>
      ...
      val localCommitmentKeys = LocalCommitmentKeys(params, channelKeys, localCommitIndex = 0)
      ...
      Funding.makeFirstCommitTxs(params, localFundingAmount = fundingAmount, remoteFundingAmount = 0 sat, localPushAmount = pushMsat, remotePushAmount = 0 msat, commitTxFeerate, fundingTx.txid, fundingTxOutputIndex, fundingKey, remoteFundingPubKey, localCommitmentKeys, remoteCommitmentKeys) match {
        ...
        case Right((localSpec, localCommitTx, remoteSpec, remoteCommitTx)) =>
          ...
          val fundingCreated = params.commitmentFormat match {
            case _: SimpleTaprootChannelCommitmentFormat =>
              val fakeFundingTxId = TxId(ByteVector32.Zeroes)
              val localNonce = NonceGenerator.verificationNonce(fakeFundingTxId, fundingKey, 0)
              val Right(psig) = remoteCommitTx.partialSign(fundingKey, remoteFundingPubKey, Map.empty, localNonce, Seq(localNonce.publicNonce, remoteNextLocalNonces(fakeFundingTxId)))
              FundingCreated(
            ...
            case _ =>
              val localSigOfRemoteTx = remoteCommitTx.sign(fundingKey, remoteFundingPubKey).sig
              FundingCreated(
                temporaryChannelId = temporaryChannelId,
                fundingTxId = fundingTx.txid,
                fundingOutputIndex = fundingTxOutputIndex,
  ...
  when(WAIT_FOR_FUNDING_CREATED)(handleExceptions {
    case Event(fc@FundingCreated(_, fundingTxId, fundingTxOutputIndex, _, _), d@DATA_WAIT_FOR_FUNDING_CREATED(params, fundingAmount, pushMsat, commitTxFeerate, remoteFundingPubKey, remoteFirstPerCommitmentPoint)) =>
      val temporaryChannelId = params.channelId
      val fundingKey = channelKeys.fundingKey(fundingTxIndex = 0)
      val localCommitmentKeys = LocalCommitmentKeys(params, channelKeys, localCommitIndex = 0)
      val remoteCommitmentKeys = RemoteCommitmentKeys(params, channelKeys, remoteFirstPerCommitmentPoint)
        ...
        case Right((localSpec, localCommitTx, remoteSpec, remoteCommitTx)) =>
          ...
          val isRemoveSigValid = fc.sigOrPartialSig match {
            ...
            case sig: ChannelSpendSignature.IndividualSignature =>
              localCommitTx.checkRemoteSig(fundingKey.publicKey, remoteFundingPubKey, sig)
          }
          isRemoveSigValid match {
            ...
            case true =>
              ...
              val fundingSigned = params.commitmentFormat match {
                case _: SimpleTaprootChannelCommitmentFormat =>
                  val localNonce = NonceGenerator.verificationNonce(TxId(ByteVector32.Zeroes), fundingKey, 0)
                  val Right(localPartialSigOfRemoteTx) = remoteCommitTx.partialSign(fundingKey, remoteFundingPubKey, Map.empty, localNonce, Seq(localNonce.publicNonce, remoteNextLocalNonces(TxId(ByteVector32.Zeroes))))
              ...
              val commitment = Commitment(
                ...
                localCommit = LocalCommit(0, localSpec, localCommitTx.tx.txid, localCommitTx.input, fc.sigOrPartialSig, htlcRemoteSigs = Nil),
              ...
              watchFundingConfirmed(commitment.fundingTxId, params.minDepth(nodeParams.channelConf.minDepth), delay_opt = None)
  ...
  when(WAIT_FOR_FUNDING_SIGNED)(handleExceptions {
    case Event(fundingSigned: FundingSigned, d@DATA_WAIT_FOR_FUNDING_SIGNED(params, remoteFundingPubKey, fundingTx, fundingTxFee, localSpec, localCommitTx, remoteCommit, fundingCreated, _)) =>
      ...
      val isRemoveSigValid = fundingSigned.sigOrPartialSig match {
        case psig: ChannelSpendSignature.PartialSignatureWithNonce =>
          ...
          localCommitTx.checkRemotePartialSignature(fundingPubkey, remoteFundingPubKey, psig, localNonce.publicNonce)
        case sig: ChannelSpendSignature.IndividualSignature =>
          localCommitTx.checkRemoteSig(fundingPubkey, remoteFundingPubKey, sig)
      }
      ...
      isRemoveSigValid match {
        ...
        case true =>
          val commitment = Commitment(
            ...
            remoteFundingStatus = RemoteFundingStatus.NotLocked,
            localCommit = LocalCommit(0, localSpec, localCommitTx.tx.txid, localCommitTx.input, fundingSigned.sigOrPartialSig, htlcRemoteSigs = Nil),
  ...
  when(WAIT_FOR_CHANNEL_READY)(handleExceptions {
    case Event(channelReady: ChannelReady, d: DATA_WAIT_FOR_CHANNEL_READY) =>
      ...
      val annSigs_opt = d1.commitments.all.find(_.fundingTxIndex == 0).flatMap(_.signAnnouncement(nodeParams, d1.commitments.params, channelKeys.fundingKey(fundingTxIndex = 0)))
