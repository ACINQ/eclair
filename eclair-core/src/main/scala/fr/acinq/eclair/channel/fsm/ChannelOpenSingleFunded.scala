 ...
 */
    ...
    case Event(MakeFundingTxResponse(fundingTx, fundingTxOutputIndex, fundingTxFee), d: DATA_WAIT_FOR_FUNDING_INTERNAL) =>
      ...
      Funding.makeFirstCommitTxs(d.channelParams, localFundingAmount = d.fundingAmount, remoteFundingAmount = 0 sat, localPushAmount = d.pushAmount, remotePushAmount = 0 msat, d.commitTxFeerate, fundingTx.txid, fundingTxOutputIndex, fundingKey, d.remoteFundingPubKey, localCommitmentKeys, remoteCommitmentKeys) match {
        ...
        case Right((localSpec, localCommitTx, remoteSpec, remoteCommitTx)) =>
          ...
          val fundingCreated = d.channelParams.commitmentFormat match {
            case _: SimpleTaprootChannelCommitmentFormat =>
              val fakeFundingTxId = TxId(ByteVector32.Zeroes)
              val localNonce = NonceGenerator.verificationNonce(fakeFundingTxId, fundingKey, 0)
              val Right(psig) = remoteCommitTx.partialSign(fundingKey, d.remoteFundingPubKey, Map.empty, localNonce, Seq(localNonce.publicNonce, remoteNextLocalNonces(fakeFundingTxId)))
              FundingCreated(
  ...
  when(WAIT_FOR_FUNDING_CREATED)(handleExceptions {
    case Event(fc@FundingCreated(_, fundingTxId, fundingTxOutputIndex, remoteSig, _), d: DATA_WAIT_FOR_FUNDING_CREATED) =>
      val temporaryChannelId = d.channelParams.channelId
      val fundingKey = channelKeys.fundingKey(fundingTxIndex = 0)
      val localCommitmentKeys = LocalCommitmentKeys(d.channelParams, channelKeys, localCommitIndex = 0)
      val remoteCommitmentKeys = RemoteCommitmentKeys(d.channelParams, channelKeys, d.remoteFirstPerCommitmentPoint)
        ...
        case Right((localSpec, localCommitTx, remoteSpec, remoteCommitTx)) =>
          ...
          isRemoveSigValid match {
            ...
            case true =>
              ...
              val fundingSigned = d.channelParams.commitmentFormat match {
                case _: SimpleTaprootChannelCommitmentFormat =>
                  val localNonce = NonceGenerator.verificationNonce(TxId(ByteVector32.Zeroes), fundingKey, 0)
                  val Right(localPartialSigOfRemoteTx) = remoteCommitTx.partialSign(fundingKey, d.remoteFundingPubKey, Map.empty, localNonce, Seq(localNonce.publicNonce, remoteNextLocalNonces(TxId(ByteVector32.Zeroes))))
      ...
      isRemoveSigValid match {
        ...
        case true =>
          val commitment = Commitment(
            ...
            remoteFundingStatus = RemoteFundingStatus.NotLocked,
            localCommit = LocalCommit(0, d.localSpec, d.localCommitTx.tx.txid, d.localCommitTx.input, fundingSigned.sigOrPartialSig, htlcRemoteSigs = Nil),
