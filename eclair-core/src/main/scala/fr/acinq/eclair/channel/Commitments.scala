...
case class Commitments(channelConfig: ChannelConfigOptions,
  ...
  require(channelType.paysDirectlyToWallet == localParams.walletStaticPaymentBasepoint.isDefined, s"localParams.walletStaticPaymentBasepoint must be defined only for commitments that pay directly to our wallet (channel_type=$channelType)")
...
object Commitments {
  ...
  def sendCommit(commitments: Commitments, keyManager: ChannelKeyManager)(implicit log: LoggingAdapter): Either[ChannelException, (Commitments, CommitSig)] = {
    ...
    commitments.remoteNextCommitInfo match {
      ...
      case Right(remoteNextPerCommitmentPoint) =>
        ...
        val (remoteCommitTx, htlcTxs) = makeRemoteTxs(keyManager, channelConfig, channelType, remoteCommit.index + 1, localParams, remoteParams, commitInput, remoteNextPerCommitmentPoint, spec)
  ...
  def receiveCommit(commitments: Commitments, commit: CommitSig, keyManager: ChannelKeyManager)(implicit log: LoggingAdapter): Either[ChannelException, (Commitments, RevokeAndAck)] = {
    ...
    val (localCommitTx, htlcTxs) = makeLocalTxs(keyManager, channelConfig, channelType, localCommit.index + 1, localParams, remoteParams, commitInput, localPerCommitmentPoint, spec)
  ...
  def makeLocalTxs(keyManager: ChannelKeyManager,
    ...
    val remotePaymentPubkey = if (channelType.features.hasFeature(Features.StaticRemoteKey)) {
    ...
    val outputs = makeCommitTxOutputs(localParams.isFunder, localParams.dustLimit, localRevocationPubkey, remoteParams.toSelfDelay, localDelayedPaymentPubkey, remotePaymentPubkey, localHtlcPubkey, remoteHtlcPubkey, localFundingPubkey, remoteParams.fundingPubKey, spec, channelType.commitmentFormat)
    ...
    val htlcTxs = Transactions.makeHtlcTxs(commitTx.tx, localParams.dustLimit, localRevocationPubkey, remoteParams.toSelfDelay, localDelayedPaymentPubkey, spec.feeratePerKw, outputs, channelType.commitmentFormat)
    (commitTx, htlcTxs)
  }
  ...
  def makeRemoteTxs(keyManager: ChannelKeyManager,
    ...
    val localFundingPubkey = keyManager.fundingPublicKey(localParams.fundingKeyPath).publicKey
    val localPaymentBasepoint = localParams.walletStaticPaymentBasepoint.getOrElse(keyManager.paymentPoint(channelKeyPath).publicKey)
    val localPaymentPubkey = if (channelType.features.hasFeature(Features.StaticRemoteKey)) {
    ...
    val outputs = makeCommitTxOutputs(!localParams.isFunder, remoteParams.dustLimit, remoteRevocationPubkey, localParams.toSelfDelay, remoteDelayedPaymentPubkey, localPaymentPubkey, remoteHtlcPubkey, localHtlcPubkey, remoteParams.fundingPubKey, localFundingPubkey, spec, channelType.commitmentFormat)
    ...
    val htlcTxs = Transactions.makeHtlcTxs(commitTx.tx, remoteParams.dustLimit, remoteRevocationPubkey, localParams.toSelfDelay, remoteDelayedPaymentPubkey, spec.feeratePerKw, outputs, channelType.commitmentFormat)
    (commitTx, htlcTxs)
  }
