...
case class Commitment(fundingTxIndex: Long,
  ...
  val maxHtlcAmount: MilliSatoshi = localCommitParams.maxHtlcValueInFlightMsat.min(remoteCommitParams.maxHtlcValueInFlightMsat)
  ...
  def canSendAdd(amount: MilliSatoshi, params: ChannelParams, changes: CommitmentChanges, feerates: FeeratesPerKw, feeConf: OnChainFeeConf): Either[ChannelException, Unit] = {
    ...
    val allowedHtlcValueInFlight = remoteCommitParams.maxHtlcValueInFlightMsat.min(localCommitParams.maxHtlcValueInFlightMsat)
  ...
  def canReceiveAdd(amount: MilliSatoshi, params: ChannelParams, changes: CommitmentChanges, feerates: FeeratesPerKw, feeConf: OnChainFeeConf): Either[ChannelException, Unit] = {
    ...
    if (localCommitParams.maxHtlcValueInFlightMsat < htlcValueInFlight) {
      return Left(HtlcValueTooHighInFlight(params.channelId, maximum = localCommitParams.maxHtlcValueInFlightMsat, actual = htlcValueInFlight))
    }
...
case class Commitments(params: ChannelParams,
  ...
  def receiveFee(fee: UpdateFee, feerates: FeeratesPerKw, feeConf: OnChainFeeConf)(implicit log: LoggingAdapter): Either[ChannelException, Commitments] = {
    if (params.localParams.paysCommitTxFees) {
      ...
      val localFeeratePerKw = feeConf.getCommitmentFeerate(feerates, params.remoteNodeId, active.head.commitmentFormat, active.head.capacity)
    ...
    remoteNextCommitInfo match {
      ...
      case Right(remoteNextPerCommitmentPoint) =>
        val (active1, sigs) = active.map(c => {
          val commitKeys = RemoteCommitmentKeys(params, channelKeys, remoteNextPerCommitmentPoint, c.commitmentFormat)
          c.sendCommit(params, channelKeys, commitKeys, changes, remoteNextPerCommitmentPoint, active.size)
        }).unzip
  ...
  def receiveCommit(commitSigs: CommitSigs, channelKeys: ChannelKeys)(implicit log: LoggingAdapter): Either[ChannelException, (Commitments, RevokeAndAck)] = {
    ...
    val active1 = active.zip(sigs).map { case (commitment, commit) =>
      val commitKeys = LocalCommitmentKeys(params, channelKeys, localCommitIndex + 1, commitment.commitmentFormat)
    ...
    remoteNextCommitInfo match {
      ...
      case Left(_) =>
        ...
        val (acceptedHtlcs, rejectedHtlcs) = {
          ...
          val localCommitDustExposure = DustExposure.computeExposure(localReduced, active.head.localCommitParams.dustLimit, active.head.commitmentFormat)
          ...
          val remoteCommitDustExposure = DustExposure.computeExposure(remoteReduced, active.head.remoteCommitParams.dustLimit, active.head.commitmentFormat)
          ...
          DustExposure.filterBeforeForward(
            maxDustExposure,
            localReduced,
            active.head.localCommitParams.dustLimit,
            localCommitDustExposure,
            remoteReduced,
            active.head.remoteCommitParams.dustLimit,
