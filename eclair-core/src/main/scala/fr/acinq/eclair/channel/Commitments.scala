...
case class Commitments(channelVersion: ChannelVersion,
                       ...
                       remoteChannelData: Option[ByteVector] = None) {
...
object Commitments {
  ...
  def receiveCommit(commitments: Commitments, commit: CommitSig, keyManager: KeyManager)(implicit log: LoggingAdapter): Try[(Commitments, RevokeAndAck)] = Try {
    ...
    val commitments1 = commitments.copy(localCommit = localCommit1, localChanges = ourChanges1, remoteChanges = theirChanges1, remoteChannelData = commit.channelData) // TODO: cap data size
