...
case class RemoteCommit(index: Long, spec: CommitmentSpec, txid: TxId, remotePerCommitmentPoint: PublicKey, localSig_opt: Option[CommitSig] = None) {
  def sign(keyManager: ChannelKeyManager, params: ChannelParams, fundingTxIndex: Long, remoteFundingPubKey: PublicKey, commitInput: InputInfo, remoteNonce_opt: Option[IndividualNonce])(implicit log: LoggingAdapter): CommitSig = {
    ...
    val (sig, tlvStream) = commitInput match {
      ...
      case _ =>
        val sig = keyManager.sign(remoteCommitTx, localFundingPubKey, TxOwner.Remote, params.commitmentFormat, Map.empty)
...
case class Commitment(fundingTxIndex: Long,
  ...
  def sendCommit(keyManager: ChannelKeyManager, params: ChannelParams, changes: CommitmentChanges, remoteNextPerCommitmentPoint: PublicKey, batchSize: Int, nextRemoteNonce_opt: Option[IndividualNonce])(implicit log: LoggingAdapter): (Commitment, CommitSig) = {
    ...
    val localFundingPubKey = keyManager.fundingPublicKey(params.localParams.fundingKeyPath, fundingTxIndex)
    val sig = remoteCommitTx.input match {
      case _: InputInfo.TaprootInput =>
        ByteVector64.Zeroes
      case _ =>
        keyManager.sign(remoteCommitTx, localFundingPubKey, TxOwner.Remote, params.commitmentFormat, Map.empty)
    }
    val partialSig: Set[CommitSigTlv] = remoteCommitTx.input match {
      case _: InputInfo.TaprootInput =>
        val localNonce = keyManager.signingNonce(localFundingPubKey.publicKey)
...
case class Commitments(params: ChannelParams,
    ...
    remoteNextCommitInfo match {
      ...
      case Right(remoteNextPerCommitmentPoint) =>
        ...
        val (active1, sigs) = active.map { c =>
          val remoteNonce_opt = c.commitInput match {
            case _: InputInfo.TaprootInput =>
  ...
  def receiveCommit(commits: Seq[CommitSig], keyManager: ChannelKeyManager)(implicit log: LoggingAdapter): Either[ChannelException, (Commitments, RevokeAndAck)] = {
    ...
    val tlvStream: TlvStream[RevokeAndAckTlv] = {
      val nonces = this.active.filter(_.commitInput.isInstanceOf[InputInfo.TaprootInput]).map(c => {
  ...
  def receiveRevocation(revocation: RevokeAndAck, maxDustExposure: Satoshi): Either[ChannelException, (Commitments, Seq[PostRevocationAction])] = {
    ...
    remoteNextCommitInfo match {
      ...
      case Left(_) if this.active.exists(_.commitInput.isInstanceOf[InputInfo.TaprootInput]) && revocation.nexLocalNonces.isEmpty => Left(MissingNextLocalNonce(channelId))
      case Left(_) =>
  ...
  def validateSeed(keyManager: ChannelKeyManager): Boolean = {
    active.forall { commitment =>
      ...
      commitment.commitInput match {
        ...
        case t: InputInfo.TaprootInput => t.internalKey == Scripts.Taproot.musig2Aggregate(localFundingKey, remoteFundingKey) && t.redeemPath.isInstanceOf[RedeemPath.KeyPath]
      }
    }
  }
