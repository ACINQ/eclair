...
case class Commitment(fundingTxIndex: Long,
  ...
  def sendCommit(params: ChannelParams, channelKeys: ChannelKeys, commitKeys: RemoteCommitmentKeys, changes: CommitmentChanges, remoteNextPerCommitmentPoint: PublicKey, batchSize: Int, nextRemoteNonce_opt: Option[IndividualNonce])(implicit log: LoggingAdapter): (Commitment, CommitSig) = {
    ...
    val sig = params.commitmentFormat match {
      case SimpleTaprootChannelCommitmentFormat => ByteVector64.Zeroes
...
case class Commitments(params: ChannelParams,
  ...
  def sendCommit(channelKeys: ChannelKeys, nextRemoteNonces: List[IndividualNonce] = List.empty)(implicit log: LoggingAdapter): Either[ChannelException, (Commitments, Seq[CommitSig])] = {
