...
object Helpers {
  ...
  object Closing {
    ...
    def checkClosingSignature(keyManager: KeyManager, commitments: Commitments, localScriptPubkey: ByteVector, remoteScriptPubkey: ByteVector, remoteClosingFee: Satoshi, remoteClosingSig: ByteVector64)(implicit log: LoggingAdapter): Try[Transaction] = {
      ...
      Transactions.checkSpendable(signedClosingTx).map(_ => signedClosingTx.tx).recover { case _ => throw InvalidCloseSignature(commitments.channelId, signedClosingTx.tx) }
    }

    def generateTx(desc: String)(attempt: Try[Either[TxGenerationSkipped, TransactionWithInputInfo]])(implicit log: LoggingAdapter): Option[TransactionWithInputInfo] = {
      attempt match {
