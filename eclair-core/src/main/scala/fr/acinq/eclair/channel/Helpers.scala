...
object Helpers {
   ...
   */
  def validateParamsFundee(nodeParams: NodeParams, features: Features, channelType: ChannelType, open: OpenChannel, remoteNodeId: PublicKey): Either[ChannelException, Unit] = {
  ...
  object Funding {
     ...
     */
    def makeFirstCommitTxs(keyManager: ChannelKeyManager, channelConfig: ChannelConfigOptions, channelType: ChannelType, temporaryChannelId: ByteVector32, localParams: LocalParams, remoteParams: RemoteParams, fundingAmount: Satoshi, pushMsat: MilliSatoshi, initialFeeratePerKw: FeeratePerKw, fundingTxHash: ByteVector32, fundingTxOutputIndex: Int, remoteFirstPerCommitmentPoint: PublicKey): Either[ChannelException, (CommitmentSpec, CommitTx, CommitmentSpec, CommitTx)] = {
      ...
      if (!localParams.isFunder) {
        ...
        val fees = commitTxTotalCost(remoteParams.dustLimit, remoteSpec, channelType.commitmentFormat)
      ...
      val (localCommitTx, _) = Commitments.makeLocalTxs(keyManager, channelConfig, channelType, 0, localParams, remoteParams, commitmentInput, localPerCommitmentPoint, localSpec)
      val (remoteCommitTx, _) = Commitments.makeRemoteTxs(keyManager, channelConfig, channelType, 0, localParams, remoteParams, commitmentInput, remoteFirstPerCommitmentPoint, remoteSpec)
  ...
  object Closing {
    ...
    def firstClosingFee(commitments: Commitments, localScriptPubkey: ByteVector, remoteScriptPubkey: ByteVector, feeEstimator: FeeEstimator, feeTargets: FeeTargets)(implicit log: LoggingAdapter): Satoshi = {
      ...
      val feeratePerKw = if (commitments.channelType.features.hasFeature(Features.AnchorOutputs)) {
        requestedFeerate
      } else {
    ...
    def checkClosingSignature(keyManager: ChannelKeyManager, commitments: Commitments, localScriptPubkey: ByteVector, remoteScriptPubkey: ByteVector, remoteClosingFee: Satoshi, remoteClosingSig: ByteVector64)(implicit log: LoggingAdapter): Either[ChannelException, ClosingTx] = {
      ...
      if (remoteClosingFee > lastCommitFeeSatoshi && !commitments.channelType.features.hasFeature(Features.AnchorOutputs)) {
    ...
    def claimRemoteCommitTxOutputs(keyManager: ChannelKeyManager, commitments: Commitments, remoteCommit: RemoteCommit, tx: Transaction, feeEstimator: FeeEstimator, feeTargets: FeeTargets)(implicit log: LoggingAdapter): RemoteCommitPublished = {
      import commitments.{channelConfig, channelType, commitInput, localParams, remoteParams}
      require(remoteCommit.txid == tx.txid, "txid mismatch, provided tx is not the current remote commit tx")
      val (remoteCommitTx, _) = Commitments.makeRemoteTxs(keyManager, channelConfig, channelType, remoteCommit.index, localParams, remoteParams, commitInput, remoteCommit.remotePerCommitmentPoint, remoteCommit.spec)
