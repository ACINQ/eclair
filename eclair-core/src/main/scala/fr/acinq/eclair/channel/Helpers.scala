...
object Helpers {
  ...
  object Closing {
    ...
    def claimCurrentLocalCommitTxOutputs(keyManager: ChannelKeyManager, commitments: Commitments, tx: Transaction, currentBlockHeight: BlockHeight, feeEstimator: FeeEstimator, feeTargets: FeeTargets)(implicit log: LoggingAdapter): LocalCommitPublished = {
      ...
      val htlcTxs: Map[OutPoint, Option[HtlcTx]] = claimLocalCommitHtlcTxOutputs(keyManager, commitments)
     ...
     */
    def claimLocalCommitHtlcTxOutputs(keyManager: ChannelKeyManager, commitments: Commitments)(implicit log: LoggingAdapter): Map[OutPoint, Option[HtlcTx]] = {
      import commitments._
      val channelKeyPath = keyManager.keyPath(localParams, channelConfig)
      val localPerCommitmentPoint = keyManager.commitmentPoint(channelKeyPath, commitments.localCommit.index.toInt)
     ...
     */
    def claimLocalCommitHtlcDelayedOutputs(localCommitPublished: LocalCommitPublished, keyManager: ChannelKeyManager, commitments: Commitments, tx: Transaction, feeEstimator: FeeEstimator, feeTargets: FeeTargets)(implicit log: LoggingAdapter): (LocalCommitPublished, Option[HtlcDelayedTx]) = {
      import commitments._
