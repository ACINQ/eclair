...
object Helpers {
  ...
  def validateParamsDualFundedNonInitiator(nodeParams: NodeParams,
                                           ...
                                           addFunding_opt: Option[LiquidityAds.AddFunding]): Either[ChannelException, (ChannelFeatures, Option[ByteVector], Option[LiquidityAds.WillFundLease])] = {
    // BOLT #2: if the chain_hash value, within the open_channel, message is set to a hash of a chain that is unknown to the receiver:
    // MUST reject the channel.
    if (nodeParams.chainHash != open.chainHash) return Left(InvalidChainHash(open.temporaryChannelId, local = nodeParams.chainHash, remote = open.chainHash))
    ...
    for {
      ...
      willFundLease_opt <- LiquidityAds.validateRequest(nodeParams.privateKey, open.temporaryChannelId, fundingScript, open.fundingFeerate, open.requestFunding_opt, addFunding_opt.flatMap(_.rates_opt))
    } yield (channelFeatures, script_opt, willFundLease_opt)
  }
  ...
  def validateParamsDualFundedInitiator(nodeParams: NodeParams,
                                        ...
                                        remoteFeatures: Features[InitFeature],
                                        open: OpenDualFundedChannel,
                                        accept: AcceptDualFundedChannel): Either[ChannelException, (ChannelFeatures, Option[ByteVector], Option[LiquidityAds.Lease])] = {
    validateChannelType(open.temporaryChannelId, channelType, open.channelFlags, open.channelType_opt, accept.channelType_opt, localFeatures, remoteFeatures) match {
      case Some(t) => return Left(t)
      case None => // we agree on channel type
    }
    ...
    for {
      ...
      lease_opt <- LiquidityAds.validateLease(open.requestFunding_opt, remoteNodeId, accept.temporaryChannelId, fundingScript, accept.fundingAmount, open.fundingFeerate, accept.willFund_opt)
    } yield {
      val channelFeatures = ChannelFeatures(channelType, localFeatures, remoteFeatures, open.channelFlags.announceChannel)
      (channelFeatures, script_opt, lease_opt)
    }
  }
