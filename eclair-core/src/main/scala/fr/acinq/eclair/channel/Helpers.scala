  ...
  object Closing {
    ...
    def claimCurrentLocalCommitTxOutputs(keyManager: KeyManager, commitments: Commitments, tx: Transaction, eventStream: EventStream)(implicit log: LoggingAdapter): LocalCommitPublished = {
      ...
      val successAndDelayedTxs = for {
        HtlcTxAndSigs(txinfo: HtlcSuccessTx, localSig, remoteSig) <- localCommit.publishableTxs.htlcTxsAndSigs if preimages.contains(txinfo.paymentHash)
        success <- generateTx("htlc-success")(Try { Transactions.addSigs(txinfo, localSig, remoteSig, preimages(txinfo.paymentHash)) })
      ...
      for ((paymentHash, amountMsat) <- inFlightHtlcs -- onChainHashes) {
        // Inform listeners about payments which are too small for on-chain settlement
        eventStream.publish(PaymentLostOnChain(MilliSatoshi(amountMsat), paymentHash))
      }
    ...
    def claimRemoteCommitTxOutputs(keyManager: KeyManager, commitments: Commitments, remoteCommit: RemoteCommit, tx: Transaction, eventStream: EventStream)(implicit log: LoggingAdapter): RemoteCommitPublished = {
      ...
      val txes = remoteCommit.spec.htlcs.collect {
        ...
        case DirectedHtlc(OUT, add: UpdateAddHtlc) if preimages.contains(add.paymentHash) => generateTx("claim-htlc-success")(Try {
          ...
          Transactions.addSigs(txinfo, sig, preimages(add.paymentHash))
        })
