...
case class InsufficientFunds                   (override val channelId: ByteVector32, amount: MilliSatoshi, missing: Satoshi, reserve: Satoshi, fees: Satoshi, isLocal: Boolean) extends ChannelException(channelId, s"insufficient funds: missing=$missing reserve=$reserve fees=$fees")
