...
case class DustLimitTooSmall                   (override val channelId: BinaryData, dustLimitSatoshis: Long, min: Long) extends ChannelException(channelId, s"dust_limit_satoshis=$dustLimitSatoshis is too small (min=$min)")
case class DustLimitTooLarge                   (override val channelId: BinaryData, dustLimitSatoshis: Long, max: Long) extends ChannelException(channelId, s"dust_limit_satoshis=$dustLimitSatoshis is too large (max=$max)")
case class DustLimitAboveOurReserve            (override val channelId: BinaryData, dustLimitSatoshis: Long, channelReserveSatoshis: Long) extends ChannelException(channelId, s"their dust_limit_satoshis=$dustLimitSatoshis is above our channelReserveSatoshis=$channelReserveSatoshis")
...
case class ChannelReserveNotMet                (override val channelId: BinaryData, toLocalMsat: Long, toRemoteMsat: Long, reserveSatoshis: Long) extends ChannelException(channelId, s"channel reserve is not met toLocalMast=$toLocalMsat toRemoteMsat=$toRemoteMsat reserveSat=$reserveSatoshis")
