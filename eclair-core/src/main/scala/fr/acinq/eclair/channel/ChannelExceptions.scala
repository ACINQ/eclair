...
case class TurboChannelOfferFromStranger       (override val channelId: ByteVector32, remoteNodeId: PublicKey) extends ChannelException(channelId, s"turbo channel proposal from remoteNodeId=${remoteNodeId.toString()} has been rejected because nodeId is not in whitelist")
