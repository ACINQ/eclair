...
object Channel {
  ...
  def maxAcceptedHtlcs(channelType: SupportedChannelType): Int = channelType match {
    ...
    case _: ChannelTypes.SimpleTaprootChannel | ChannelTypes.SimpleTaprootChannelsPhoenix => 483
...
class Channel(val nodeParams: NodeParams, val channelKeys: ChannelKeys, val wallet: OnChainChannelFunder with OnChainAddressCache, val remoteNodeId: PublicKey, val blockchain: typed.ActorRef[ZmqWatcher.Command], val relayer: ActorRef, val txPublisherFactory: Channel.TxPublisherFactory)(implicit val ec: ExecutionContext = ExecutionContext.Implicits.global)
  ...
  when(OFFLINE)(handleExceptions {
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: DATA_WAIT_FOR_DUAL_FUNDING_SIGNED) =>
      ...
      val nextFundingTlv = Set[ChannelReestablishTlv](ChannelReestablishTlv.NextFundingTlv(d1.signingSession.fundingTxId, d1.signingSession.retransmitRemoteCommitSig))
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: ChannelDataWithCommitments) =>
      ...
      val rbfTlv: Set[ChannelReestablishTlv] = d1 match {
        case d1: DATA_WAIT_FOR_DUAL_FUNDING_CONFIRMED => d1.status match {
          ...
          case _ => d1.latestFundingTx.sharedTx match {
            case _: InteractiveTxBuilder.PartiallySignedSharedTransaction => Set(ChannelReestablishTlv.NextFundingTlv(d1.latestFundingTx.sharedTx.txId, retransmitCommitSig = false))
      ...
      val channelReestablish = ChannelReestablish(
        channelId = d1.channelId,
        nextLocalCommitmentNumber = d1.commitments.localCommitIndex + 1,
        ...
        case syncSuccess: SyncResult.Success =>
          ...
          Helpers.Syncing.checkCommitNonces(channelReestablish, commitments1, pendingSplice_opt) match {
            ...
            case None =>
              ...
              sendQueue = sendQueue ++ spliceAnnSigs_opt.toSeq
  ...
  private def resendChannelReadyIfNeeded(channelReestablish: ChannelReestablish, d: DATA_NORMAL): (Option[ChannelReady], Option[AnnouncementSignatures]) = {
    d.commitments.lastLocalLocked_opt match {
      ...
      case Some(c) =>
        ...
        val channelReady_opt = if (notAnnouncedYet || notReceivedByRemote) {
