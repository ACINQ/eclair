              ...
              cmd match {
                case cmd: CMD_SPLICE => initiateSplice(cmd, d) match {
                  ...
                  case Right(spliceInit) =>
                    ...
                    val txFunder = context.spawnAnonymous(InteractiveTxFunder(remoteNodeId, fundingParams, dummyFundingPubkeyScript, purpose = InteractiveTxBuilder.SpliceTx(parentCommitment), wallet))
