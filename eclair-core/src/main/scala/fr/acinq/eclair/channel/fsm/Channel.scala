...
class Channel(val nodeParams: NodeParams, val channelKeys: ChannelKeys, val wallet: OnChainChannelFunder with OnChainPubkeyCache, val remoteNodeId: PublicKey, val blockchain: typed.ActorRef[ZmqWatcher.Command], val relayer: ActorRef, val txPublisherFactory: Channel.TxPublisherFactory)(implicit val ec: ExecutionContext = ExecutionContext.Implicits.global)
  ...
  def updateLocalClosingNonce(commitments: Commitments): TlvStream[ShutdownTlv] = commitments.channelParams.commitmentFormat match {
  ...
  when(OFFLINE)(handleExceptions {
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: ChannelDataWithCommitments) =>
      ...
      val myNextLocalNonces: Set[ChannelTlv.NextLocalNoncesTlv] = d.commitments.channelParams.commitmentFormat match {
        case _: SimpleTaprootChannelCommitmentFormat =>
  ...
  when(SYNCING)(handleExceptions {
    case Event(channelReestablish: ChannelReestablish, d: DATA_WAIT_FOR_FUNDING_CONFIRMED) =>
      if (d.commitments.channelParams.commitmentFormat.isInstanceOf[SimpleTaprootChannelCommitmentFormat]) {
          ...
          d.status match {
            case DualFundingStatus.RbfWaitingForSigs(signingSession) if signingSession.fundingTx.txId == fundingTxId =>
              if (channelReestablish.nextLocalCommitmentNumber == 0) {
                ...
                val commitSig = signingSession.remoteCommit.sign(d.commitments.channelParams, channelKeys, signingSession.fundingTxIndex, signingSession.fundingParams.remoteFundingPubKey, signingSession.commitInput, remoteNextLocalNonces.get(signingSession.fundingTx.txId))
            ...
            case _ if d.latestFundingTx.sharedTx.txId == fundingTxId =>
              ...
              if (channelReestablish.nextLocalCommitmentNumber == 0) {
                val commitSig = d.commitments.latest.remoteCommit.sign(d.commitments.channelParams, channelKeys, d.commitments.latest.fundingTxIndex, d.commitments.latest.remoteFundingPubKey, d.commitments.latest.commitInput, remoteNextLocalNonces.get(d.commitments.latest.fundingTxId))
              ...
              d.spliceStatus match {
                case SpliceStatus.SpliceWaitingForSigs(signingSession) if signingSession.fundingTx.txId == fundingTxId =>
                  if (channelReestablish.nextLocalCommitmentNumber == d.commitments.remoteCommitIndex) {
                    ...
                    val commitSig = signingSession.remoteCommit.sign(d.commitments.channelParams, channelKeys, signingSession.fundingTxIndex, signingSession.fundingParams.remoteFundingPubKey, signingSession.commitInput, channelReestablish.currentCommitNonce_opt)
                ...
                case _ if d.commitments.latest.fundingTxId == fundingTxId =>
                  d.commitments.latest.localFundingStatus match {
                    case dfu: LocalFundingStatus.DualFundedUnconfirmedFundingTx =>
                      ...
                      if (channelReestablish.nextLocalCommitmentNumber == d.commitments.remoteCommitIndex) {
                        log.info("re-sending commit_sig and tx_signatures for fundingTxIndex={} fundingTxId={}", d.commitments.latest.fundingTxIndex, d.commitments.latest.fundingTxId)
                        val commitSig = d.commitments.latest.remoteCommit.sign(d.commitments.channelParams, channelKeys, d.commitments.latest.fundingTxIndex, d.commitments.latest.remoteFundingPubKey, d.commitments.latest.commitInput, channelReestablish.currentCommitNonce_opt)
