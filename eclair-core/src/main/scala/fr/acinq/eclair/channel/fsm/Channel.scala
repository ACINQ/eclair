...
import fr.acinq.eclair.transactions.Transactions.{ClosingTx, CommitmentFormat, LegacySimpleTaprootChannelCommitmentFormat, LocalNonce, SimpleTaprootChannelCommitmentFormat, TaprootCommitmentFormat}
...
class Channel(val nodeParams: NodeParams, val channelKeys: ChannelKeys, val wallet: OnChainChannelFunder with OnChainPubkeyCache, val remoteNodeId: PublicKey, val blockchain: typed.ActorRef[ZmqWatcher.Command], val relayer: ActorRef, val txPublisherFactory: Channel.TxPublisherFactory)(implicit val ec: ExecutionContext = ExecutionContext.Implicits.global)
  ...
  def updateLocalClosingNonce(commitments: Commitments): TlvStream[ShutdownTlv] = commitments.params.commitmentFormat match {
      ...
      d.commitments.params.validateRemoteShutdownScript(remoteScriptPubKey) match {
        ...
        case Right(remoteShutdownScript) =>
          ...
          if (d.commitments.changes.remoteHasUnsignedOutgoingHtlcs) {
            ...
            if (d.commitments.latest.params.commitmentFormat.isInstanceOf[SimpleTaprootChannelCommitmentFormat]) {
            ...
            LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, isChannelCreation = false, msg.requestFunding_opt, nodeParams.liquidityAdsConfig.rates_opt, msg.useFeeCredit_opt) match {
              ...
              case Right(willFund_opt) =>
                ...
                val fundingParams = InteractiveTxParams(
                  ...
                  currentCommitmentFormat = parentCommitment.commitmentFormat,
                  nextCommitmentFormat = msg.commitmentFormat_opt.getOrElse(parentCommitment.commitmentFormat) // TODO: check that this ia a valid upgrade
                )
              ...
              LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, isChannelCreation = false, msg.requestFunding_opt, nodeParams.liquidityAdsConfig.rates_opt, feeCreditUsed_opt = None) match {
                ...
                case Right(willFund_opt) =>
                  ...
                  val fundingParams = InteractiveTxParams(
                    ...
                    rbf.latestFundingTx.fundingParams.currentCommitmentFormat,
                    rbf.latestFundingTx.fundingParams.nextCommitmentFormat,
                  )
                  val sessionId = randomBytes32()
                  val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
                    sessionId,
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: ChannelDataWithCommitments) =>
      ...
      d.commitments.active.collect {
        case c: Commitment if c.commitmentFormat.isInstanceOf[TaprootCommitmentFormat] =>
          ...
          c.fundingTxId -> NonceGenerator.verificationNonce(c.fundingTxId, localFundingKey, d.commitments.localCommitIndex + 1).publicNonce
      }
      ...
      val myNextLocalNonces: Set[ChannelTlv.NextLocalNoncesTlv] = d.commitments.params.commitmentFormat match {
        case _: SimpleTaprootChannelCommitmentFormat =>
          var nonces = collection.mutable.HashMap[TxId, IndividualNonce]()
          // send a nonce for each active commitment
          d.commitments.active.map { c =>
            val localFundingKey = channelKeys.fundingKey(c.fundingTxIndex)
        ...
        case _ => Set.empty[ChannelTlv.NextLocalNoncesTlv]
      }
  ...
  when(SYNCING)(handleExceptions {
    ...
    case Event(channelReestablish: ChannelReestablish, d: DATA_WAIT_FOR_DUAL_FUNDING_SIGNED) =>
      ...
      channelReestablish.nextFundingTxId_opt match {
        case Some(fundingTxId) if fundingTxId == d.signingSession.fundingTx.txId && channelReestablish.nextLocalCommitmentNumber == 0 =>
          ...
          val commitSig = d.signingSession.remoteCommit.sign(d.channelParams, d.signingSession.fundingParams.nextCommitmentFormat, channelKeys, d.signingSession.fundingTxIndex, d.signingSession.fundingParams.remoteFundingPubKey, d.signingSession.commitInput, remoteNextLocalNonces.get(d.signingSession.fundingTx.txId))
          ...
          d.status match {
            case DualFundingStatus.RbfWaitingForSigs(signingSession) if signingSession.fundingTx.txId == fundingTxId =>
              if (channelReestablish.nextLocalCommitmentNumber == 0) {
                ...
                val commitSig = signingSession.remoteCommit.sign(d.commitments.params, signingSession.fundingParams.nextCommitmentFormat, channelKeys, signingSession.fundingTxIndex, signingSession.fundingParams.remoteFundingPubKey, signingSession.commitInput, remoteNextLocalNonces.get(signingSession.fundingTx.txId))
    ...
    case Event(channelReestablish: ChannelReestablish, d: DATA_NORMAL) =>
      if (d.channelParams.commitmentFormat.isInstanceOf[SimpleTaprootChannelCommitmentFormat]) {
        channelReestablish.nextLocalNonces.size match {
          case s if s == d.commitments.active.size - 1 => ()
          case _ =>
            require(channelReestablish.nextLocalNonces.size >= d.commitments.active.size, "missing next local nonce")
            setRemoteNextLocalNonces("received ChannelReestablish", channelReestablish.nextLocalNonces)
        }
              ...
              d.spliceStatus match {
                case SpliceStatus.SpliceWaitingForSigs(signingSession) if signingSession.fundingTx.txId == fundingTxId =>
                  if (channelReestablish.nextLocalCommitmentNumber == d.commitments.remoteCommitIndex) {
                    ...
                    val commitSig = signingSession.remoteCommit.sign(d.commitments.params, signingSession.fundingParams.nextCommitmentFormat, channelKeys, signingSession.fundingTxIndex, signingSession.fundingParams.remoteFundingPubKey, signingSession.commitInput, channelReestablish.currentCommitNonce_opt)
