        ...
        case Right((commitments1, actions)) =>
          ...
          if (d.remoteShutdown.isDefined && !commitments1.changes.localHasUnsignedOutgoingHtlcs) {
            ...
            val tlvStream: TlvStream[ShutdownTlv] = if (d.commitments.params.commitmentFormat.useTaproot) {
              ...
              closingNonce = Some(keyManager.signingNonce(localFundingPubKey))
          ...
          if (d.commitments.changes.remoteHasUnsignedOutgoingHtlcs) {
            ...
            if (d.commitments.hasNoPendingHtlcsOrFeeUpdate) {
              ...
              if (d.commitments.params.localParams.paysClosingFees) {
                ...
                val (closingTx, closingSigned) = Closing.MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, localShutdown.scriptPubKey, remoteShutdownScript, nodeParams.currentBitcoinCoreFeerates, nodeParams.onChainFeeConf, d.closingFeerates, closingNonce, remoteShutdown.shutdownNonce_opt)
        ...
        case SpliceStatus.NonInitiatorQuiescent =>
          if (!d.commitments.isQuiescent) {
            ...
            val nextLocalNonces = if (d.commitments.latest.params.commitmentFormat.useTaproot) {
              List(
                keyManager.verificationNonce(localFundingPubKey, parentCommitment.localCommit.index)._2,
                keyManager.verificationNonce(localFundingPubKey, parentCommitment.localCommit.index + 1)._2,
              )
            } else {
              ...
              LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, isChannelCreation = false, msg.requestFunding_opt, nodeParams.liquidityAdsConfig.rates_opt, feeCreditUsed_opt = None) match {
                ...
                case Right(willFund_opt) =>
                  ...
                  val localNonces = if (d.commitments.latest.params.commitmentFormat.useTaproot) {
                    val localFundingPubKey = nodeParams.channelKeyManager.fundingPublicKey(d.commitments.params.localParams.fundingKeyPath, rbf.parentCommitment.fundingTxIndex + 1).publicKey
                    List(
                      keyManager.verificationNonce(localFundingPubKey, rbf.localCommitIndex)._2,
                      keyManager.verificationNonce(localFundingPubKey, rbf.localCommitIndex + 1)._2,
                    )
                  } else {
                    List.empty[IndividualNonce]
                  }
                  ...
                  val txBuilder = context.spawnAnonymous(InteractiveTxBuilder(
                    ...
                    wallet,
                    nextRemoteNonce_opt = msg.firstRemoteNonce
                  ))
                  this.pendingRemoteNextLocalNonce = msg.secondRemoteNonce
                  txBuilder ! InteractiveTxBuilder.Start(self)
    ...
    case Event(w: WatchFundingConfirmedTriggered, d: DATA_NORMAL) =>
      acceptFundingTxConfirmed(w, d) match {
        case Right((commitments1, commitment)) =>
          ...
          val localAnnSigs_opt = if (d.commitments.announceChannel && remoteLocked) commitment.signAnnouncement(nodeParams, commitments1.params) else None
          ...
          d.commitments.receiveCommit(sigs, keyManager) match {
            case Right((commitments1, revocation)) =>
              ...
              if (commitments1.hasNoPendingHtlcsOrFeeUpdate) {
                if (d.commitments.params.localParams.paysClosingFees) {
                  // we pay the closing fees, so we initiate the negotiation by sending the first closing_signed
                  val (closingTx, closingSigned) = Closing.MutualClose.makeFirstClosingTx(keyManager, commitments1.latest, localShutdown.scriptPubKey, remoteShutdown.scriptPubKey, nodeParams.currentBitcoinCoreFeerates, nodeParams.onChainFeeConf, closingFeerates, closingNonce, remoteShutdown.shutdownNonce_opt)
      ...
      c.feerates match {
        case Some(feerates) =>
          if (c.scriptPubKey.nonEmpty && !c.scriptPubKey.contains(d.localShutdown.scriptPubKey)) {
            ...
            log.info("updating our closing feerates: {}", feerates)
            val (closingTx, closingSigned) = Closing.MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, nodeParams.currentBitcoinCoreFeerates, nodeParams.onChainFeeConf, Some(feerates), closingNonce, d.remoteShutdown.shutdownNonce_opt)
  ...
  when(OFFLINE)(handleExceptions {
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: DATA_WAIT_FOR_DUAL_FUNDING_SIGNED) =>
      ...
      val myNextLocalNonce = if (d.channelParams.commitmentFormat.useTaproot) {
        ...
        val (_, publicNonce) = keyManager.verificationNonce(localFundingPubKey, 1)
    ...
    case Event(INPUT_RECONNECTED(r, localInit, remoteInit), d: ChannelDataWithCommitments) =>
      ...
      val myNextLocalNonces = if (d.commitments.params.commitmentFormat.useTaproot) {
        val nonces = d.commitments.active.map(c => {
          ...
          keyManager.verificationNonce(localFundingPubKey, d.commitments.localCommitIndex + 1)._2
        })
        ...
        case d: DATA_NORMAL => d.spliceStatus match {
          case w: SpliceStatus.SpliceWaitingForSigs if d.commitments.params.commitmentFormat.useTaproot =>
            ...
            val spliceNonces = List(
              keyManager.verificationNonce(localFundingPubKey, w.signingSession.localCommitIndex)._2,
              keyManager.verificationNonce(localFundingPubKey, w.signingSession.localCommitIndex + 1)._2,
            )
            Set(ChannelReestablishTlv.SpliceNoncesTlv(spliceNonces))
          case _ =>
            val localFundingPubKey = nodeParams.channelKeyManager.fundingPublicKey(d.commitments.params.localParams.fundingKeyPath, d.commitments.latest.fundingTxIndex).publicKey
            val spliceNonces = List(
              keyManager.verificationNonce(localFundingPubKey, d.commitments.localCommitIndex)._2,
              keyManager.verificationNonce(localFundingPubKey, d.commitments.localCommitIndex + 1)._2,
            )
            Set(ChannelReestablishTlv.SpliceNoncesTlv(spliceNonces))
        }
