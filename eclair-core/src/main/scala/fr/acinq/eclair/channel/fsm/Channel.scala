            ...
            LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, msg.requestFunding_opt, nodeParams.willFundRates_opt) match {
              ...
              case Right(willFundLease_opt) =>
                ...
                val spliceAck = SpliceAck(d.channelId,
                  fundingContribution = willFundLease_opt.map(_.lease.amount).getOrElse(0 sat),
                  fundingPubKey = localFundingPubKey,
                  pushAmount = 0.msat,
                  requireConfirmedInputs = nodeParams.channelConf.requireConfirmedInputsForDualFunding,
                  willFund_opt = willFundLease_opt.map(_.willFund)
                )
    ...
    case Event(msg: SpliceAck, d: DATA_NORMAL) =>
      d.spliceStatus match {
        case SpliceStatus.SpliceRequested(cmd, spliceInit) =>
          ...
          LiquidityAds.validateLease(spliceInit.requestFunding_opt, remoteNodeId, d.channelId, fundingScript, msg.fundingContribution, spliceInit.feerate, msg.willFund_opt) match {
            case Left(t) =>
              log.info("rejecting splice attempt: invalid liquidity ads lease ({})", t.getMessage)
