            ...
            case Right(rbf) =>
              ...
              LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, isChannelCreation = false, msg.requestFunding_opt, nodeParams.willFundRates_opt, feeCreditUsed_opt = None) match {
                ...
                case Right(willFund_opt) =>
                  ...
                  val localNonces = d.commitments.generateLocalNonces(keyManager, rbf.parentCommitment)
    ...
    case Event(remoteShutdown: Shutdown, d: DATA_NEGOTIATING_SIMPLE) =>
      d.status match {
        case status: ClosingNegotiation.WaitingForRemoteShutdown =>
          ...
          status.sendClosingFeerate_opt match {
            case Some(closingFeerate) =>
        ...
        case status: ClosingNegotiation.SigningTransactions =>
          ...
          val closingFeerate_opt = status.closingCompleteSent_opt.map(_.closingFeerate) match {
            case Some(closingFeerate) if status.closingSigReceived_opt.isEmpty => Some(closingFeerate)
            case _ => None
          }
          val status1 = ClosingNegotiation.WaitingForRemoteShutdown(localShutdown, closingFeerate_opt)
          self ! remoteShutdown
    ...
    case Event(c: CMD_CLOSE, d: DATA_NEGOTIATING_SIMPLE) =>
      ...
      d.status match {
        ...
        case _: ClosingNegotiation.SigningTransactions =>
          val status1 = ClosingNegotiation.WaitingForRemoteShutdown(localShutdown, Some(closingFeerate))
  ...
  when(SYNCING)(handleExceptions {
    ...
    case Event(_: ChannelReestablish, d: DATA_NEGOTIATING_SIMPLE) =>
      ...
      val status1 = d.status match {
        ...
        case _ => ClosingNegotiation.WaitingForRemoteShutdown(localShutdown, None)
      }
