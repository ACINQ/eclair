      ...
      aggregateSigs(commit) match {
        case Some(sigs) =>
          d.spliceStatus match {
            ...
            case _ if d.commitments.ignoreRetransmittedCommitSig(channelKeys, commit) =>
      ...
      d.commitments.params.validateRemoteShutdownScript(remoteScriptPubKey) match {
        ...
        case Right(remoteShutdownScript) =>
          ...
          if (d.commitments.changes.remoteHasUnsignedOutgoingHtlcs) {
            ...
            if (d.commitments.latest.params.commitmentFormat == SimpleTaprootChannelCommitmentFormat) {
        ...
        case SpliceStatus.NonInitiatorQuiescent =>
          if (!d.commitments.isQuiescent) {
            ...
            val sharedInput = d.commitments.latest.params.commitmentFormat match {
              case SimpleTaprootChannelCommitmentFormat => Musig2Input(parentCommitment)
            ...
            case Right(rbf) =>
              ...
              LiquidityAds.validateRequest(nodeParams.privateKey, d.channelId, fundingScript, msg.feerate, isChannelCreation = false, msg.requestFunding_opt, nodeParams.liquidityAdsConfig.rates_opt, feeCreditUsed_opt = None) match {
                ...
                case Right(willFund_opt) =>
                  ...
                  val sharedInput = d.commitments.latest.params.commitmentFormat match {
                    case SimpleTaprootChannelCommitmentFormat => Musig2Input(rbf.parentCommitment)
