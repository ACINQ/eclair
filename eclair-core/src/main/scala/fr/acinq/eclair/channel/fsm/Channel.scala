          ...
          if (d.commitments.changes.remoteHasUnsignedOutgoingHtlcs) {
            ...
            if (d.commitments.hasNoPendingHtlcsOrFeeUpdate) {
              ...
              if (Features.canUseFeature(d.commitments.params.localParams.initFeatures, d.commitments.params.remoteParams.initFeatures, Features.SimpleClose)) {
                ...
                val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, localShutdown.scriptPubKey, remoteShutdownScript, nodeParams.currentFeerates, nodeParams.onChainFeeConf, d.closingFeerates)
            ...
            case Right((commitments1, revocation)) =>
              ...
              if (commitments1.hasNoPendingHtlcsOrFeeUpdate) {
                if (Features.canUseFeature(d.commitments.params.localParams.initFeatures, d.commitments.params.remoteParams.initFeatures, Features.SimpleClose)) {
                  ...
                  // we are the channel initiator, need to initiate the negotiation by sending the first closing_signed
                  val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, commitments1.latest, localShutdown.scriptPubKey, remoteShutdown.scriptPubKey, nodeParams.currentFeerates, nodeParams.onChainFeeConf, closingFeerates)
          ...
          if (lastLocalClosingSigned_opt.exists(_.localClosingSigned.feeSatoshis == remoteClosingFee)) {
            ...
            c.feeRange_opt match {
              case Some(ClosingSignedTlv.FeeRange(minFee, maxFee)) if !d.commitments.params.localParams.isInitiator =>
                ...
                val localClosingFees = MutualClose.firstClosingFee(d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, nodeParams.currentFeerates, nodeParams.onChainFeeConf)
      ...
      c.feerates match {
        case Some(feerates) =>
          if (c.scriptPubKey.nonEmpty && !c.scriptPubKey.contains(d.localShutdown.scriptPubKey)) {
            ...
            val (closingTx, closingSigned) = MutualClose.makeFirstClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, nodeParams.currentFeerates, nodeParams.onChainFeeConf, Some(feerates))
  ...
  when(NEGOTIATING_SIMPLE)(handleExceptions {
    ...
    case Event(closingComplete: ClosingComplete, d: DATA_NEGOTIATING_SIMPLE) =>
      MutualClose.signSimpleClosingTx(keyManager, d.commitments.latest, d.localShutdown.scriptPubKey, d.remoteShutdown.scriptPubKey, closingComplete) match {
        ...
        case Right((signedClosingTx, closingSig)) =>
          ...
          stay() using d1 storing() calling doPublish(signedClosingTx, isInitiator = false) sending closingSig
      }
    ...
    case Event(closingSig: ClosingSig, d: DATA_NEGOTIATING_SIMPLE) =>
      MutualClose.receiveSimpleClosingSig(keyManager, d.commitments.latest, d.proposedClosingTxs.last, closingSig) match {
        ...
        case Right(signedClosingTx) =>
          val d1 = d.copy(publishedClosingTxs = d.publishedClosingTxs :+ signedClosingTx)
          stay() using d1 storing() calling doPublish(signedClosingTx, isInitiator = true)
      }
    ...
    case Event(WatchFundingSpentTriggered(tx), d: DATA_NEGOTIATING_SIMPLE) if d.findClosingTx(tx).nonEmpty =>
      if (!d.publishedClosingTxs.exists(_.tx.txid == tx.txid)) {
        ...
        // We need to publish it ourselves to record the fees and watch for confirmation.
        val closingTx = d.findClosingTx(tx).get
        stay() using d.copy(publishedClosingTxs = d.publishedClosingTxs :+ closingTx) storing() calling doPublish(closingTx, isInitiator = true)
      } else {
