...
class Channel(val nodeParams: NodeParams, val wallet: EclairWallet, remoteNodeId: PublicKey, blockchain: ActorRef, router: ActorRef, relayer: ActorRef, origin_opt: Option[ActorRef] = None)(implicit ec: ExecutionContext = ExecutionContext.Implicits.global) extends FSM[State, Data] with FSMDiagnosticActorLogging[State, Data] {
  ...
  when(WAIT_FOR_FUNDING_LOCKED)(handleExceptions {
    case Event(FundingLocked(_, nextPerCommitmentPoint), d@DATA_WAIT_FOR_FUNDING_LOCKED(commitments, shortChannelId, _)) =>
      ...
      val initialChannelUpdate = Announcements.makeChannelUpdate(nodeParams.chainHash, nodeParams.privateKey, remoteNodeId, shortChannelId, nodeParams.expiryDeltaBlocks, d.commitments.remoteParams.htlcMinimum,
        nodeParams.feeBase, nodeParams.feeProportionalMillionth, commitments.localCommit.spec.totalFunds, enable = Helpers.aboveReserve(d.commitments))
      // we need to periodically re-send channel updates, otherwise channel will be considered stale and get pruned by network
  ...
  when(NORMAL)(handleExceptions {
    ...
    case Event(c: CMD_ADD_HTLC, d: DATA_NORMAL) =>
      Try(Commitments.sendAdd(d.commitments, c, origin(c, sender), nodeParams.currentBlockHeight)) match {
        case Success(Right((commitments1, add))) =>
          if (c.commit) self ! CMD_SIGN
          handleCommandSuccess(sender, d.copy(commitments = commitments1)) sending add
        case Success(Left(error)) => handleCommandError(AddHtlcFailed(d.channelId, c.paymentHash, error, origin(c, sender), Some(d.channelUpdate), Some(c)), c)
        ...
        case Right(_) =>
          Try(Commitments.sendCommit(d.commitments, keyManager)) match {
            case Success((commitments1, commit)) =>
              ...
              context.system.eventStream.publish(AvailableBalanceChanged(self, d.channelId, d.shortChannelId, remoteNodeId, commitments1.commitInput.txOut.amount, commitments1.remoteParams.channelReserve, nextRemoteCommit.spec.toRemote, commitments1))
  ...
  def handleCommandError(cause: Throwable, cmd: Command): NormalFSMState = {
    ...
    context.system.eventStream.publish(ChannelErrorOccurred(self, Helpers.getChannelId(stateData), remoteNodeId, LocalError(cause), isFatal = false))
  ...
  def handleFundingPublishFailed(d: HasCommitments): NormalFSMState = {
    ...
    context.system.eventStream.publish(ChannelErrorOccurred(self, Helpers.getChannelId(stateData), remoteNodeId, LocalError(exc), isFatal = true))
  ...
  def handleLocalError(cause: Throwable, d: Data, msg: Option[Any]): NormalFSMState = {
    ...
    context.system.eventStream.publish(ChannelErrorOccurred(self, Helpers.getChannelId(stateData), remoteNodeId, LocalError(cause), isFatal = true))
