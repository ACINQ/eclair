...
class Channel(val nodeParams: NodeParams, val wallet: EclairWallet, remoteNodeId: PublicKey, blockchain: ActorRef, relayer: ActorRef, origin_opt: Option[ActorRef] = None)(implicit ec: ExecutionContext = ExecutionContext.Implicits.global) extends FSM[State, Data] with FSMDiagnosticActorLogging[State, Data] {
  ...
  when(WAIT_FOR_INIT_INTERNAL)(handleExceptions {
    ...
    case Event(c: CloseCommand, _) =>
      channelOpenReplyToUser(Right(ChannelCommandResponse.ChannelClosed(ByteVector32.Zeroes))) // for this corner case we answer with an all-zero channel id
      handleFastClose(c)
  ...
  when(WAIT_FOR_OPEN_CHANNEL)(handleExceptions {
    ...
    case Event(c: CloseCommand, _) => handleFastClose(c)
  ...
  when(NORMAL)(handleExceptions {
    case Event(c: CMD_ADD_HTLC, d: DATA_NORMAL) if d.localShutdown.isDefined || d.remoteShutdown.isDefined =>
      ...
      handleAddHtlcCommandError(c, d.channelId, error, origin(c), Some(d.channelUpdate))
    ...
    case Event(c: CMD_ADD_HTLC, d: DATA_NORMAL) =>
      Commitments.sendAdd(d.commitments, c, origin(c), nodeParams.currentBlockHeight, nodeParams.onChainFeeConf) match {
        ...
        case Failure(cause) => handleAddHtlcCommandError(c, d.channelId, cause, origin(c), Some(d.channelUpdate))
      }
    ...
    case Event(c: CMD_UPDATE_RELAY_FEE, d: DATA_NORMAL) =>
      ...
      // we use GOTO instead of stay because we want to fire transitions
      goto(NORMAL) using d.copy(channelUpdate = channelUpdate) storing() replying RES_SUCCESS(c)
...
//            relayer ! RES_ADD_FAILED(d.channelId, add.paymentHash, ChannelUnavailable(d.channelId), d.commitments.originChannels(add.id), Some(channelUpdate), None)
  ...
  when(OFFLINE)(handleExceptions {
    ...
    case Event(c: CMD_UPDATE_RELAY_FEE, d: DATA_NORMAL) =>
      ...
      stay using d.copy(channelUpdate = channelUpdate) storing() replying RES_SUCCESS(c)
  ...
  whenUnhandled {
    ...
    case Event(c: CMD_ADD_HTLC, d: HasCommitments) =>
      ...
      handleAddHtlcCommandError(c, d.channelId, error, origin(c), None) // we don't provide a channel_update: this will be a permanent channel failure
    ...
    case Event(c: CMD_FORCECLOSE.type, d) =>
      d match {
        case data: HasCommitments => handleLocalError(ForcedLocalCommit(data.channelId), data, Some(c)) replying RES_SUCCESS(c)
  ...
  def handleFastClose(c: CloseCommand) = {
    goto(CLOSED) replying RES_SUCCESS(c)
  }
  ...
  def handleCommandSuccess(c: Command, newData: Data) = {
    ...
    replyTo ! RES_SUCCESS(c)
    stay using newData
  }

  def handleAddHtlcCommandError(c: CMD_ADD_HTLC, channelId: ByteVector32, cause: Throwable, origin: Origin, channelUpdate: Option[ChannelUpdate]) = {
    log.warning(s"${cause.getMessage} while processing cmd=${c.getClass.getSimpleName} in state=$stateName")
    val replyTo = c match {
      case hasReplyTo: HasReplyTo if hasReplyTo.replyTo != ActorRef.noSender => hasReplyTo.replyTo
      case _ => sender
    }
    replyTo ! RES_ADD_FAILED(channelId, c.paymentHash, cause, origin, channelUpdate, Some(c))
  ...
  def handleAddDisconnected(c: CMD_ADD_HTLC, d: DATA_NORMAL) = {
    ...
    if (Announcements.isEnabled(d.channelUpdate.channelFlags)) {
      ...
      val error = ChannelUnavailable(d.channelId)
      handleAddHtlcCommandError(c, d.channelId, error, origin(c), Some(d.channelUpdate)) // can happen if we are in OFFLINE or SYNCING state (channelUpdate will have enable=false)
    }
  }
