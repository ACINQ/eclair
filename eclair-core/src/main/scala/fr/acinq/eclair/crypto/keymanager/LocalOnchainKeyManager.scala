...
import fr.acinq.bitcoin.scalacompat.{Block, ByteVector32, Crypto, DeterministicWallet, MnemonicCode}
...
class LocalOnchainKeyManager(entropy: ByteVector, chainHash: ByteVector32, passphrase: String = "") extends OnchainKeyManager with Logging {
  ...
  override def getPublicKey(keyPath: KeyPath): Crypto.PublicKey = {
    import fr.acinq.bitcoin.scalacompat.KotlinUtils._
    getPrivateKey(keyPath.keyPath).publicKey()
  }
