...
import fr.acinq.eclair.{Alias, BlockHeight, CltvExpiryDelta, Features, InitFeature, MilliSatoshi, MilliSatoshiLong, RealShortChannelId, UInt64, channel}
...
private[channel] object ChannelTypes4 {
  ...
  case class ChannelParams(channelId: ByteVector32,
    ...
    def remoteCommitParams(): channel.CommitParams = {
      val remoteMaxHtlcValueInFlight = remoteParams.maxHtlcValueInFlightMsat.toBigInt.min(MilliSatoshi.maxValue.toLong).toLong.msat
      channel.CommitParams(remoteParams.dustLimit, remoteParams.htlcMinimum, remoteMaxHtlcValueInFlight, remoteParams.maxAcceptedHtlcs, localParams.toSelfDelay)
    }
  }
