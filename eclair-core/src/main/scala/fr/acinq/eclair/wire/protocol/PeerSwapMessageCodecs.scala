...
import fr.acinq.eclair.json.PeerSwapJsonSerializers.formats
...
/**
 * Created by remyers on 29/03/2022.
 */
object PeerSwapMessageCodecs {
  ...
  val peerSwapMessageCodec: DiscriminatorCodec[PeerSwapMessage, Int] = discriminated[PeerSwapMessage].by(uint16)
  ...
  val peerSwapMessageCodecWithFallback: Codec[PeerSwapMessage] = discriminatorWithDefault(peerSwapMessageCodec, unknownPeerSwapMessageCodec.upcast)

  val meteredPeerSwapMessageCodec: Codec[PeerSwapMessage] = Codec[PeerSwapMessage](
    (msg: PeerSwapMessage) => KamonExt.time(Metrics.EncodeDuration.withTag(Tags.MessageType, msg.getClass.getSimpleName))(peerSwapMessageCodecWithFallback.encode(msg)),
    (bits: BitVector) => {
