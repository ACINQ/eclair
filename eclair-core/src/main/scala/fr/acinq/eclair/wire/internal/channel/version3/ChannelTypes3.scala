...
private[channel] object ChannelTypes3 {
  ...
  case class Commitments(channelId: ByteVector32,
    ...
    def migrate(): channel.Commitments = channel.Commitments(
      ...
      Seq(Commitment(channelFeatures.commitmentFormat, fundingTxIndex = 0, firstRemoteCommitIndex = 0, remoteFundingPubKey = remoteParams.fundingPubKey, localFundingStatus, remoteFundingStatus, localCommit, remoteCommit, remoteNextCommitInfo.left.toOption.map(w => NextRemoteCommit(w.sent, w.nextRemoteCommit)))),
      inactive = Nil,
