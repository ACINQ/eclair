...
import fr.acinq.eclair.{CltvExpiryDelta, Features, InitFeature, MilliSatoshi, MilliSatoshiLong, UInt64, channel}
...
private[channel] object ChannelTypes0 {
  ...
  case class Commitments(channelVersion: ChannelVersion,
    ...
    def migrate(): channel.Commitments = {
      ...
      val localCommitParams = CommitParams(localParams.dustLimit, localParams.htlcMinimum, localParams.maxHtlcValueInFlightMsat, localParams.maxAcceptedHtlcs, remoteParams.toSelfDelay)
      val remoteCommitParams = CommitParams(remoteParams.dustLimit, remoteParams.htlcMinimum, remoteParams.maxHtlcValueInFlightMsat.toBigInt.min(MilliSatoshi.maxValue.toLong).toLong.msat, remoteParams.maxAcceptedHtlcs, localParams.toSelfDelay)
