...
private[channel] object ChannelTypes0 {
  ...
  private def getPartialInputInfo(parentTx: Transaction, childTx: Transaction): InputInfo = {
    ...
    InputInfo(input, parentTx.txOut(input.index.toInt), InputSpendingInfo.Segwit(Nil))
  }
  ...
  case class RevokedCommitPublished(commitTx: Transaction, claimMainOutputTx: Option[Transaction], mainPenaltyTx: Option[Transaction], htlcPenaltyTxs: List[Transaction], claimHtlcDelayedPenaltyTxs: List[Transaction], irrevocablySpent: Map[OutPoint, TxId]) {
    def migrate(): channel.RevokedCommitPublished = {
      ...
      val claimHtlcDelayedPenaltyTxsNew = claimHtlcDelayedPenaltyTxs.map(tx => {
        ...
        ClaimHtlcDelayedOutputPenaltyTx(InputInfo(tx.txIn.head.outPoint, TxOut(Satoshi(0), Nil), InputSpendingInfo.Segwit(Nil)), tx)
      })
   ...
   */
  def migrateClosingTx(tx: Transaction): ClosingTx = ClosingTx(InputInfo(tx.txIn.head.outPoint, TxOut(Satoshi(0), Nil), InputSpendingInfo.Segwit(Nil)), tx, None)
