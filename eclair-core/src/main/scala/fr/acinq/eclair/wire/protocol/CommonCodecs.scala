...
object CommonCodecs {
  ...
  def isValidUtf8(s: String): Boolean = {
    val prohibited = Set(Character.CONTROL, Character.FORMAT, Character.SURROGATE, Character.PRIVATE_USE, Character.UNASSIGNED, Character.PARAGRAPH_SEPARATOR, Character.LINE_SEPARATOR).map(_.toInt)
