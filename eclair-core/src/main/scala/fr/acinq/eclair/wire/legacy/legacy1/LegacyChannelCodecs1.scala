...
private[wire] object LegacyChannelCodecs1 extends Logging {
  ...
  private[legacy1] def lengthDelimited[T](codec: Codec[T]): Codec[T] = variableSizeBytesLong(varintoverflow, codec).decodeOnly

  private[legacy1] val keyPathCodec: Codec[KeyPath] = ("path" | listOfN(uint16, uint32)).xmap[KeyPath](l => new KeyPath(l), keyPath => keyPath.path.toList).as[KeyPath].decodeOnly

  private[legacy1] val extendedPrivateKeyCodec: Codec[ExtendedPrivateKey] = (
  ...
  private[legacy1] val channelVersionCodec: Codec[ChannelVersion] = bits(ChannelVersion.LENGTH_BITS).as[ChannelVersion].decodeOnly

  /**
   * byte-aligned boolean codec
   */
  private[legacy1] val bool8: Codec[Boolean] = bool(8).decodeOnly

  private[legacy1] def localParamsCodec(channelVersion: ChannelVersion): Codec[LocalParams] = (
    ("nodeId" | publicKey) ::
  ...
  private[legacy1] val remoteParamsCodec: Codec[RemoteParams] = (
    ("nodeId" | publicKey) ::
      ("dustLimit" | satoshi) ::
      ("maxHtlcValueInFlightMsat" | uint64) ::
      ("channelReserve" | satoshi) ::
  ...
  private[legacy1] val htlcCodec: Codec[DirectedHtlc] = discriminated[DirectedHtlc].by(bool8)
  ...
  private[legacy1] val commitmentSpecCodec: Codec[CommitmentSpec] = (
    ("htlcs" | setCodec(htlcCodec)) ::
      ("feeratePerKw" | feeratePerKw) ::
      ("toLocal" | millisatoshi) ::
      ("toRemote" | millisatoshi)).as[CommitmentSpec].decodeOnly

  private[legacy1] val outPointCodec: Codec[OutPoint] = lengthDelimited(bytes.xmap(d => OutPoint.read(d.toArray), d => OutPoint.write(d)))

  private[legacy1] val txOutCodec: Codec[TxOut] = lengthDelimited(bytes.xmap(d => TxOut.read(d.toArray), d => TxOut.write(d)))

  private[legacy1] val txCodec: Codec[Transaction] = lengthDelimited(bytes.xmap(d => Transaction.read(d.toArray), d => Transaction.write(d)))

  private[legacy1] val closingTxCodec: Codec[ClosingTx] = txCodec.xmap(
    tx => LegacyChannelTypes.migrateClosingTx(tx),
    closingTx => closingTx.tx
  )

  private[legacy1] val inputInfoCodec: Codec[InputInfo] = (
  ...
  private[legacy1] val txWithInputInfoCodec: Codec[TransactionWithInputInfo] = discriminated[TransactionWithInputInfo].by(uint16)
  ...
  private[legacy1] val htlcTxAndSigsCodec: Codec[HtlcTxAndSigs] = (
  ...
  private[legacy1] val publishableTxsCodec: Codec[PublishableTxs] = (
  ...
  private[legacy1] val localCommitCodec: Codec[LocalCommit] = (
    ("index" | uint64overflow) ::
      ("spec" | commitmentSpecCodec) ::
      ("publishableTxs" | publishableTxsCodec)).as[LocalCommit].decodeOnly

  private[legacy1] val remoteCommitCodec: Codec[RemoteCommit] = (
    ("index" | uint64overflow) ::
      ("spec" | commitmentSpecCodec) ::
      ("txid" | bytes32) ::
      ("remotePerCommitmentPoint" | publicKey)).as[RemoteCommit].decodeOnly

  private[legacy1] val updateMessageCodec: Codec[UpdateMessage] = lengthDelimited(lightningMessageCodec.narrow[UpdateMessage](f => Attempt.successful(f.asInstanceOf[UpdateMessage]), g => g)).decodeOnly

  private[legacy1] val localChangesCodec: Codec[LocalChanges] = (
    ("proposed" | listOfN(uint16, updateMessageCodec)) ::
      ("signed" | listOfN(uint16, updateMessageCodec)) ::
      ("acked" | listOfN(uint16, updateMessageCodec))).as[LocalChanges].decodeOnly

  private[legacy1] val remoteChangesCodec: Codec[RemoteChanges] = (
    ("proposed" | listOfN(uint16, updateMessageCodec)) ::
      ("acked" | listOfN(uint16, updateMessageCodec)) ::
      ("signed" | listOfN(uint16, updateMessageCodec))).as[RemoteChanges].decodeOnly

  private[legacy1] val waitingForRevocationCodec: Codec[WaitingForRevocation] = (
    ("nextRemoteCommit" | remoteCommitCodec) ::
      ("sent" | lengthDelimited(commitSigCodec)) ::
      ("sentAfterLocalCommitIndex" | uint64overflow) ::
      ("reSignAsap" | bool8)).as[WaitingForRevocation].decodeOnly

  private[legacy1] val localColdCodec: Codec[Origin.LocalCold] = ("id" | uuid).as[Origin.LocalCold].decodeOnly

  private[legacy1] val localCodec: Codec[Origin.Local] = localColdCodec.xmap[Origin.Local](o => o: Origin.Local, o => Origin.LocalCold(o.id)).decodeOnly

  private[legacy1] val relayedColdCodec: Codec[Origin.ChannelRelayedCold] = (
    ("originChannelId" | bytes32) ::
      ("originHtlcId" | int64) ::
      ("amountIn" | millisatoshi) ::
      ("amountOut" | millisatoshi)).as[Origin.ChannelRelayedCold].decodeOnly

  private[legacy1] val relayedCodec: Codec[Origin.ChannelRelayed] = relayedColdCodec.xmap[Origin.ChannelRelayed](o => o: Origin.ChannelRelayed, o => Origin.ChannelRelayedCold(o.originChannelId, o.originHtlcId, o.amountIn, o.amountOut)).decodeOnly

  private[legacy1] val trampolineRelayedColdCodec: Codec[Origin.TrampolineRelayedCold] = listOfN(uint16, bytes32 ~ int64).as[Origin.TrampolineRelayedCold].decodeOnly

  private[legacy1] val trampolineRelayedCodec: Codec[Origin.TrampolineRelayed] = trampolineRelayedColdCodec.xmap[Origin.TrampolineRelayed](o => o: Origin.TrampolineRelayed, o => Origin.TrampolineRelayedCold(o.htlcs)).decodeOnly

  private[legacy1] val originCodec: Codec[Origin] = discriminated[Origin].by(uint16)
    .typecase(0x02, relayedCodec)
    .typecase(0x03, localCodec)
    .typecase(0x04, trampolineRelayedCodec)
    .decodeOnly

  private[legacy1] val originsMapCodec: Codec[Map[Long, Origin]] = mapCodec(int64, originCodec).decodeOnly

  private[legacy1] val spentMapCodec: Codec[Map[OutPoint, ByteVector32]] = mapCodec(outPointCodec, bytes32).decodeOnly

  private[legacy1] val commitmentsCodec: Codec[Commitments] = (
  ...
  private[legacy1] val closingTxProposedCodec: Codec[ClosingTxProposed] = (
    ("unsignedTx" | closingTxCodec) ::
      ("localClosingSigned" | lengthDelimited(closingSignedCodec))).as[ClosingTxProposed].decodeOnly

  private[legacy1] val localCommitPublishedCodec: Codec[LocalCommitPublished] = (
    ("commitTx" | txCodec) ::
  ...
  private[legacy1] val remoteCommitPublishedCodec: Codec[RemoteCommitPublished] = (
    ("commitTx" | txCodec) ::
  ...
  private[legacy1] val revokedCommitPublishedCodec: Codec[RevokedCommitPublished] = (
    ("commitTx" | txCodec) ::
      ("claimMainOutputTx" | optional(bool8, txCodec)) ::
