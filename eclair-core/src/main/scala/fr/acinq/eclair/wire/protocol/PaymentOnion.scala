...
object PaymentOnion {
  ...
  object IntermediatePayload {
    ...
    object ChannelRelay {
       ...
       */
      case class Blinded(records: TlvStream[OnionPaymentPayloadTlv], paymentRelayData: PaymentRelayData, nextPathKey: PublicKey) extends ChannelRelay with BlindedPayload {
        // @formatter:off
  ...
  object FinalPayload {
    ...
    object Standard {
      ...
      def createTrampolinePayload(amount: MilliSatoshi, totalAmount: MilliSatoshi, expiry: CltvExpiry, paymentSecret: ByteVector32, trampolinePacket: OnionRoutingPacket, trampolinePathKey_opt: Option[PublicKey], upgradeAccountability: Boolean): Standard = {
        val tlvs: Set[OnionPaymentPayloadTlv] = Set(
          Some(AmountToForward(amount)),
          Some(OutgoingCltv(expiry)),
          Some(PaymentData(paymentSecret, totalAmount)),
      ...
      def createLegacyTrampolinePayload(amount: MilliSatoshi, totalAmount: MilliSatoshi, expiry: CltvExpiry, paymentSecret: ByteVector32, trampolinePacket: OnionRoutingPacket, upgradeAccountability: Boolean): Standard = {
        val tlvs: Set[OnionPaymentPayloadTlv] = Set(
          Some(AmountToForward(amount)),
          Some(OutgoingCltv(expiry)),
          Some(PaymentData(paymentSecret, totalAmount)),
          Some(LegacyTrampolineOnion(trampolinePacket)),
          if (upgradeAccountability) Some(UpgradeAccountability()) else None
        ).flatten
        Standard(TlvStream(tlvs))
      }
     ...
     */
    case class Blinded(records: TlvStream[OnionPaymentPayloadTlv], blindedRecords: TlvStream[RouteBlindingEncryptedDataTlv]) extends FinalPayload with BlindedPayload {
      override val amount = records.get[AmountToForward].get.amount
  ...
  object TrampolineWithoutMppPayload {
    def create(amount: MilliSatoshi, expiry: CltvExpiry, trampolinePacket: OnionRoutingPacket, trampolinePathKey_opt: Option[PublicKey] = None, upgradeAccountability: Boolean = false): TrampolineWithoutMppPayload = {
      val tlvs: Set[OnionPaymentPayloadTlv] = Set(
        Some(AmountToForward(amount)),
        Some(OutgoingCltv(expiry)),
        trampolinePathKey_opt.map(k => PathKey(k)),
