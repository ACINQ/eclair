...
private[channel] object ChannelCodecs4 {
  ...
  private[version4] object Codecs {
     ...
     */
    case class EncodedCommitments(params: ChannelTypes4.ChannelParams,
      ...
      def toCommitments: Commitments = {
        Commitments(
          params = params.migrate(),
          changes = changes,
          active = active.map(_.migrate(params)),
          inactive = inactive.map(_.migrate(params)),
