...
import fr.acinq.bitcoin.scalacompat.{ByteVector32, OutPoint, Script, ScriptElt, ScriptWitness, Transaction, TxOut}
...
import fr.acinq.eclair.wire.internal.channel.version0.ChannelTypes0.{InputInfoWithRedeemScript, ClaimHtlcTimeoutWithRedeemScriptTx, HtlcSuccessWithRedeemScriptTx, HtlcTimeoutWithRedeemScriptTx}
...
private[channel] object ChannelCodecs4 {
  ...
  private[version4] object Codecs {
    ...
    val newInputInfoCodec: Codec[InputInfo] = (("outPoint" | outPointCodec) :: ("txOut" | txOutCodec)).as[InputInfo]
    ...
    val ripemdOfPaymentHashCodec: Codec[RipemdOfPaymentHash] = limitedSizeBytes(20, bytesStrict(20)).xmap[RipemdOfPaymentHash](d => RipemdOfPaymentHash(d), d => d.ripemdHash)

    val htlcSuccessTxCodec: Codec[HtlcSuccessTx] = (
      ("inputInfo" | newInputInfoCodec) :: ("tx" | txCodec) :: ("paymentHash" | bytes32) :: ("expiry" | cltvExpiry) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | blockHeightConfirmationTarget)
      ).as[HtlcSuccessTx]

    val htlcTimeoutTxCodec: Codec[HtlcTimeoutTx] = (
      ("inputInfo" | newInputInfoCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | blockHeightConfirmationTarget) :: ("ripemdOfPaymentHash" | ripemdOfPaymentHashCodec)
      ).as[HtlcTimeoutTx]

    val claimHtlcTimeoutTxCodec: Codec[ClaimHtlcTimeoutTx] = (
      ("inputInfo" | newInputInfoCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("ripemdOfPaymentHash" | ripemdOfPaymentHashCodec) :: ("expiry" | cltvExpiry) :: ("confirmationTarget" | blockHeightConfirmationTarget)
      ).as[ClaimHtlcTimeoutTx]

    val htlcSuccessTxWithRedeemScriptCodec: Codec[HtlcSuccessTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("paymentHash" | bytes32) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | blockHeightConfirmationTarget)
      ).as[HtlcSuccessWithRedeemScriptTx].xmap[HtlcSuccessTx](_.htlcSuccessTx, h => new HtlcSuccessWithRedeemScriptTx(h))

    val htlcTimeoutWithRedeemScriptTxCodec: Codec[HtlcTimeoutTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | blockHeightConfirmationTarget)
      ).as[HtlcTimeoutWithRedeemScriptTx].xmap(_.htlcTimeOutTx, h => new HtlcTimeoutWithRedeemScriptTx(h))

    private val htlcSuccessTxNoConfirmCodec: Codec[HtlcSuccessTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("paymentHash" | bytes32) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | defaultConfirmationTarget)
      ).as[HtlcSuccessWithRedeemScriptTx].xmap[HtlcSuccessTx](_.htlcSuccessTx, h => new HtlcSuccessWithRedeemScriptTx(h))

    private val htlcTimeoutTxNoConfirmCodec: Codec[HtlcTimeoutTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | defaultConfirmationTarget)
      ).as[HtlcTimeoutWithRedeemScriptTx].xmap(_.htlcTimeOutTx, h => new HtlcTimeoutWithRedeemScriptTx(h))
    ...
    val claimHtlcTimeoutWithRedeemScriptTxCodec: Codec[ClaimHtlcTimeoutTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | blockHeightConfirmationTarget)
      ).as[ClaimHtlcTimeoutWithRedeemScriptTx].xmap[ClaimHtlcTimeoutTx](_.claimHtlcTimeOutTx, h => new ClaimHtlcTimeoutWithRedeemScriptTx(h))
    ...
    private val claimHtlcTimeoutTxNoConfirmCodec: Codec[ClaimHtlcTimeoutTx] = (
      ("inputInfo" | inputInfoWithRedeemScriptCodec) :: ("tx" | txCodec) :: ("htlcId" | uint64overflow) :: ("confirmationTarget" | defaultConfirmationTarget)
      ).as[ClaimHtlcTimeoutWithRedeemScriptTx].xmap[ClaimHtlcTimeoutTx](_.claimHtlcTimeOutTx, h => new ClaimHtlcTimeoutWithRedeemScriptTx(h))
    ...
    val mainPenaltyTxCodec: Codec[MainPenaltyTx] = (("inputInfoIgnoreReemScriptCodec" | inputInfoCodec) :: ("tx" | txCodec)).as[MainPenaltyTx]

    val htlcPenaltyTxCodec: Codec[HtlcPenaltyTx] = (("inputInfoIgnoreReemScriptCodec" | inputInfoCodec) :: ("tx" | txCodec)).as[HtlcPenaltyTx]

    val claimHtlcDelayedOutputPenaltyTxCodec: Codec[ClaimHtlcDelayedOutputPenaltyTx] = (("inputInfoIgnoreReemScriptCodec" | inputInfoCodec) :: ("tx" | txCodec)).as[ClaimHtlcDelayedOutputPenaltyTx]
    ...
    def filterUnusedRemoteAnchorOutputTx(anchorTxs: List[ClaimAnchorOutputTx]): List[ClaimAnchorOutputTx] = anchorTxs.filter(_.confirmationTarget != ConfirmationTarget.Absolute(BlockHeight(42)))

    val htlcTxCodec: Codec[HtlcTx] = discriminated[HtlcTx].by(uint8)
      // Important: order matters!
      .typecase(0x13, htlcSuccessTxCodec)
      .typecase(0x14, htlcTimeoutTxCodec)
      .typecase(0x11, htlcSuccessTxWithRedeemScriptCodec)
      .typecase(0x12, htlcTimeoutWithRedeemScriptTxCodec)
    ...
    val claimHtlcTxCodec: Codec[ClaimHtlcTx] = discriminated[ClaimHtlcTx].by(uint8)
      ...
      .typecase(0x22, claimHtlcTimeoutWithRedeemScriptTxCodec)
