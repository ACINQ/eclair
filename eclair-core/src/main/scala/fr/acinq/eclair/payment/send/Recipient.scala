...
object BlindedRecipient {
   ...
   */
  def apply(invoice: Bolt12Invoice, paths: Seq[ResolvedPath], totalAmount: MilliSatoshi, expiry: CltvExpiry, customTlvs: Set[GenericTlv]): BlindedRecipient =
  ...
  def fromPaths(nodeId: PublicKey, features: Features[InvoiceFeature], totalAmount: MilliSatoshi, expiry: CltvExpiry, paths: Seq[ResolvedPath], customTlvs: Set[GenericTlv]): BlindedRecipient = {
    // If the total capacity of the paths is less than the amount that we want to send, it means the recipient expects
    // us to use the same path multiple times. To do that we clone the paths as many times as needed to have enough
    // capacity.
    val totalPathsCapacity = paths.map(_.paymentInfo.maxHtlc).sum
    val n = (totalAmount.toLong + totalPathsCapacity.toLong - 1) / totalPathsCapacity.toLong
