...
class TrampolinePaymentLifecycle private(nodeParams: NodeParams,
  ...
  private def sendPayment(channels: Seq[Peer.ChannelInfo], attemptNumber: Int): Behavior[Command] = {
    ...
    if (filtered.isEmpty) {
      ...
      waitForSettlement(remaining = 2, attemptNumber, Nil, trampolineHoldTimesPublished = false)
    }
  }

  private def waitForSettlement(remaining: Int, attemptNumber: Int, fulfilledParts: Seq[PartialPayment], trampolineHoldTimesPublished: Boolean): Behavior[Command] = {
    Behaviors.receiveMessagePartial {
      case CouldntAddHtlc(failure) =>
        ...
        if (remaining > 1) {
          ...
          waitForSettlement(remaining - 1, attemptNumber, fulfilledParts, trampolineHoldTimesPublished)
        } else {
      ...
      case HtlcSettled(result: HtlcResult, part, holdTimes, trampolineHoldTimes) =>
        ...
        if (!trampolineHoldTimesPublished && trampolineHoldTimes.nonEmpty) {
          context.system.eventStream ! EventStream.Publish(Router.ReportedTrampolineHoldTimes(trampolineHoldTimes))
        }
        result match {
          case fulfill: HtlcResult.Fulfill =>
            ...
            if (remaining > 1) {
              ...
              waitForSettlement(remaining - 1, attemptNumber, part +: fulfilledParts, trampolineHoldTimesPublished || trampolineHoldTimes.nonEmpty)
            } else {
          ...
          case fail: HtlcResult.Fail =>
            ...
            if (remaining > 1) {
              ...
              waitForSettlement(remaining - 1, attemptNumber, fulfilledParts, trampolineHoldTimesPublished || trampolineHoldTimes.nonEmpty)
            } else {
