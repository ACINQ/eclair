...
class Relayer(nodeParams: NodeParams, register: ActorRef, paymentHandler: ActorRef) extends Actor with ActorLogging {
  ...
  def main(channelUpdates: Map[ShortChannelId, OutgoingChannel], node2channels: mutable.HashMap[PublicKey, mutable.Set[ShortChannelId]] with mutable.MultiMap[PublicKey, ShortChannelId]): Receive = {
    case GetUsableBalances =>
      val state: Iterable[UsableBalances] = for {
        OutgoingChannel(nextNodeId, channelUpdate, commitments: Commitments) <- channelUpdates.values // Omit hosted channels by matching only normal commitments
        ub = UsableBalances(nextNodeId, channelUpdate.shortChannelId, commitments.availableBalanceForSend, commitments.availableBalanceForReceive, isPublic = commitments.announceChannel)
    ...
    case ForwardAdd(add, previousFailures) =>
      ...
      decryptPacket(add, nodeParams.privateKey, nodeParams.globalFeatures) match {
        ...
        case Left(failure) =>
          ...
          commandBuffer ! CommandBuffer.CommandSend(add.channelId, CMD_FAIL_HTLC(add.id, Right(failure), commit = true))
      }
    ...
    case Status.Failure(Register.ForwardShortIdFailure(Register.ForwardShortId(shortChannelId, CMD_ADD_HTLC(_, _, _, _, Upstream.Relayed(add), _, _)))) =>
      ...
      commandBuffer ! CommandBuffer.CommandSend(add.channelId, CMD_FAIL_HTLC(add.id, Right(UnknownNextPeer), commit = true))
    ...
    case ForwardFulfill(fulfill, to, add) =>
      to match {
        ...
        case Relayed(originChannelId, originHtlcId, amountIn, amountOut) =>
          commandBuffer ! CommandBuffer.CommandSend(originChannelId, CMD_FULFILL_HTLC(originHtlcId, fulfill.paymentPreimage, commit = true))
    ...
    case ForwardFail(fail, to, add) =>
      to match {
        ...
        case Local(_, Some(sender)) =>
          sender ! fail
        case Relayed(originChannelId, originHtlcId, _, _) =>
          commandBuffer ! CommandBuffer.CommandSend(originChannelId, CMD_FAIL_HTLC(originHtlcId, Left(fail.reason), commit = true))
      }
    ...
    case ForwardFailMalformed(fail, to, add) =>
      to match {
        ...
        case Local(_, Some(sender)) =>
          sender ! fail
        case Relayed(originChannelId, originHtlcId, _, _) =>
          commandBuffer ! CommandBuffer.CommandSend(originChannelId, CMD_FAIL_MALFORMED_HTLC(originHtlcId, fail.onionHash, fail.failureCode, commit = true))
      }
