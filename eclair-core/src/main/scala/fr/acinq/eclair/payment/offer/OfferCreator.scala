...
object OfferCreator {
  ...
  case class FirstNodeWrapper(nodeID: PublicKey) extends Command
  ...
  def apply(nodeParams: NodeParams, router: ActorRef, offerManager: typed.ActorRef[OfferManager.Command], defaultOfferHandler: typed.ActorRef[OfferManager.HandlerCommand]): Behavior[Command] =
    Behaviors.receivePartial {
      case (context, Create(replyTo, description_opt, amount_opt, expiry_opt, issuer_opt, firstNodeId_opt, hideNodeId)) =>
        new OfferCreator(context, replyTo, nodeParams, router, offerManager, defaultOfferHandler).init(description_opt, amount_opt, expiry_opt, issuer_opt, firstNodeId_opt, hideNodeId)
    }
}
