...
object OnTheFlyFunding {
  ...
  case class Config(wakeUpTimeout: FiniteDuration, proposalTimeout: FiniteDuration)
  ...
  def validateOpen(open: Either[OpenChannel, OpenDualFundedChannel], pendingOnTheFlyFunding: Map[ByteVector32, Pending]): ValidationResult = {
    open match {
      ...
      case Right(open) => open.requestFunding_opt match {
        case Some(requestFunding) => validate(open.temporaryChannelId, requestFunding, open.fundingFeerate, open.htlcMinimum, pendingOnTheFlyFunding)
  ...
  def validateSplice(splice: SpliceInit, htlcMinimum: MilliSatoshi, pendingOnTheFlyFunding: Map[ByteVector32, Pending]): ValidationResult = {
    splice.requestFunding_opt match {
      case Some(requestFunding) => validate(splice.channelId, requestFunding, splice.feerate, htlcMinimum, pendingOnTheFlyFunding)
      case None => ValidationResult.Accept(Set.empty)
    }
  }
  ...
  private def validate(channelId: ByteVector32,
    ...
    val fees = requestFunding.fees(feerate)
    ...
    private def checkChannelState(): Behavior[Command] = {
      ...
      Behaviors.receiveMessagePartial {
        ...
        case WrappedChannelInfo(_, data: DATA_NORMAL) =>
          nodeParams.db.onTheFlyFunding.getPreimage(paymentHash) match {
            case Some(preimage) =>
    ...
    private def relay(data: DATA_NORMAL): Behavior[Command] = {
      ...
      cmd.proposed.foldLeft(cmd.status.remainingFees) {
        case (remainingFees, p) =>
          ...
          val add = CMD_ADD_HTLC(cmdAdapter.toClassic, p.htlc.amount - htlcFees.amount, paymentHash, p.htlc.expiry, p.htlc.finalPacket, p.htlc.blinding_opt, Some(htlcFees), origin, commit)
          cmd.channel ! add
  ...
  object Codecs {
    ...
    private val upstreamChannel: Codec[Upstream.Hot.Channel] = (lengthDelimited(updateAddHtlcCodec) :: uint64overflow.as[TimestampMilli]).as[Upstream.Hot.Channel]
