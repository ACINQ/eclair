...
import fr.acinq.eclair.{FSMDiagnosticActorLogging, LongToBtcAmount, MilliSatoshi, NodeParams}
...
class MultiPartPaymentHandler(nodeParams: NodeParams, paymentHash: ByteVector32, paymentPreimage: ByteVector32, parent: ActorRef) extends FSMDiagnosticActorLogging[MultiPartPaymentHandler.State, MultiPartPaymentHandler.Data] {
  ...
  when(WAITING_FOR_FIRST_HTLC) {
    case Event(PaymentTimeout, _) =>
      goto(PAYMENT_FAILED) using PaymentFailed(0 msat, List.empty[(Long, PartialPayment, ActorRef)])
    ...
    case Event(MultiPartHtlc(totalAmountMsat, htlc), _) =>
      ...
      if (htlc.amountMsat >= totalAmountMsat) {
        ...
        goto(WAITING_FOR_MORE_HTLC) using WaitingForMoreHtlc(totalAmountMsat, htlc.amountMsat, (htlc.id, pp, sender) :: Nil)
      }
  }

  when(WAITING_FOR_MORE_HTLC) {
    case Event(PaymentTimeout, d: WaitingForMoreHtlc) =>
      goto(PAYMENT_FAILED) using PaymentFailed(d.paidAmount, d.parts)
    ...
    case Event(MultiPartHtlc(totalAmount, htlc), d: WaitingForMoreHtlc) =>
      ...
      if (totalAmount != d.totalAmount) {
        ...
        goto(PAYMENT_FAILED) using PaymentFailed(d.paidAmount, (htlc.id, pp, sender) :: d.parts)
  ...
  when(PAYMENT_FAILED) {
    ...
    case Event(MultiPartHtlc(_, htlc), PaymentFailed(paidAmountMsat, _)) =>
      sender ! CMD_FAIL_HTLC(htlc.id, Right(IncorrectOrUnknownPaymentDetails(paidAmountMsat, nodeParams.currentBlockHeight)), commit = true)
      stay
  ...
  onTransition {
    ...
    case _ -> PAYMENT_FAILED =>
      cancelTimer(PaymentTimeout.toString)
      nextStateData match {
        case PaymentFailed(paidAmountMsat, parts) =>
          parts.reverse.foreach { case (id, _, sender) => sender ! CMD_FAIL_HTLC(id, Right(IncorrectOrUnknownPaymentDetails(paidAmountMsat, nodeParams.currentBlockHeight)), commit = true) }
          // We expect the parent actor to send us a PoisonPill after receiving this message.
          parent ! MultiPartHtlcFailed(paymentHash)
        case d =>
...
object MultiPartPaymentHandler {
  ...
  case class MultiPartHtlcFailed(paymentHash: ByteVector32)
  // @formatter:on
  ...
  case class PaymentFailed(paidAmount: MilliSatoshi, parts: List[(Long, PartialPayment, ActorRef)]) extends Data
  // @formatter:on
