...
object MultiPartPaymentLifecycle {
  ...
  private def createChildPayment(nodeParams: NodeParams, request: SendPaymentRequest, childAmount: MilliSatoshi, channel: OutgoingChannel): SendPayment = {
    SendPayment(
      request.paymentHash,
   ...
   */
  def splitPayment(nodeParams: NodeParams, toSend: MilliSatoshi, localChannels: Seq[OutgoingChannel], networkStats: Option[NetworkStats], request: SendPaymentRequest, randomize: Boolean): (MilliSatoshi, Seq[SendPayment]) = {
