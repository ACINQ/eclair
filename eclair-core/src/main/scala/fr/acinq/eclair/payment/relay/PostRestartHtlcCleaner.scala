      ...
      .flatMap { c =>
        ...
        val htlcsToIgnore: Set[Long] = c match {
          case d: DATA_CLOSING =>
            ...
            val channelKeys = nodeParams.channelKeyManager.channelKeys(c.channelParams.channelConfig, c.channelParams.localParams.fundingKeyPath)
            ...
            val timedOutHtlcs: Set[Long] = (closingType_opt match {
              case Some(c: Closing.LocalClose) => confirmedTxs.flatMap(tx => Closing.trimmedOrTimedOutHtlcs(params.commitmentFormat, c.localCommit, htlcTxsMap, params.localParams.dustLimit, tx))
