...
object DefaultHandler {
  ...
  def makeRoutes(nodeParams: NodeParams, routes: Seq[Seq[Router.ChannelHop]]): Seq[InvoiceRequestActor.Route] = {
    ...
    (0 until nodeParams.offersConfig.paymentPathCount).map(i => {
      ...
      InvoiceRequestActor.Route(hops, nodeParams.channelConf.maxExpiryDelta, feeOverride = Some(RelayFees.zero), cltvOverride = Some(nodeParams.offersConfig.paymentPathCltvExpiryDelta))
    })
  }
}
