...
class PaymentLifecycle(nodeParams: NodeParams, id: UUID, router: ActorRef, register: ActorRef) extends FSM[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_PAYMENT_COMPLETE) {
    ...
    case Event(fail: UpdateFailHtlc, WaitingForComplete(s, c, _, failures, sharedSecrets, ignoreNodes, ignoreChannels, hops)) =>
      Sphinx.ErrorPacket.parsePacket(fail.reason, sharedSecrets) match {
        case Success(e@Sphinx.ErrorPacket(nodeId, failureMessage)) if nodeId == c.targetNodeId =>
        ...
        case Success(e@Sphinx.ErrorPacket(nodeId, failureMessage: Node)) =>
        ...
        case Success(e@Sphinx.ErrorPacket(nodeId, failureMessage: Update)) =>
...
object PaymentLifecycle {
  ...
  case class RemoteFailure(route: Seq[Hop], e: Sphinx.ErrorPacket) extends PaymentFailure
  ...
  def buildOnion(nodes: Seq[PublicKey], payloads: Seq[PerHopPayload], associatedData: ByteVector32): Sphinx.PacketAndSecrets = {
    ...
    Sphinx.PaymentPacket.makePacket(sessionKey, nodes, payloadsbin, associatedData)
  }
  ...
  def buildCommand(id: UUID, finalAmountMsat: Long, finalExpiry: Long, paymentHash: ByteVector32, hops: Seq[Hop]): (CMD_ADD_HTLC, Seq[(ByteVector32, PublicKey)]) = {
    ...
    CMD_ADD_HTLC(firstAmountMsat, paymentHash, firstExpiry, onion.packet.serialize, upstream = Left(id), commit = true) -> onion.sharedSecrets
  }
  ...
  def hasAlreadyFailedOnce(nodeId: PublicKey, failures: Seq[PaymentFailure]): Boolean =
    failures
      .collectFirst { case RemoteFailure(_, Sphinx.ErrorPacket(origin, u: Update)) if origin == nodeId => u.update }
      .isDefined
}
