...
class PaymentLifecycle(sourceNodeId: PublicKey, router: ActorRef, register: ActorRef) extends FSM[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_REQUEST) {
    case Event(c: SendPayment, WaitingForRequest) =>
      router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
  ...
  when(WAITING_FOR_PAYMENT_COMPLETE) {
    ...
    case Event(fail: UpdateFailHtlc, WaitingForComplete(s, c, _, failures, sharedSecrets, ignoreNodes, ignoreChannels, hops)) =>
      Sphinx.parseErrorPacket(fail.reason, sharedSecrets) match {
        ...
        case Failure(t) =>
          ...
          router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, ignoreNodes ++ blacklist, ignoreChannels, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
        ...
        case Success(e@ErrorPacket(nodeId, failureMessage: Node)) =>
          ...
          router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, ignoreNodes + nodeId, ignoreChannels, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
        ...
        case Success(e@ErrorPacket(nodeId, failureMessage: Update)) =>
          ...
          if (Announcements.checkSig(failureMessage.update, nodeId)) {
            ...
            router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, ignoreNodes, ignoreChannels, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
          } else {
        ...
        case Success(e@ErrorPacket(nodeId, failureMessage)) =>
          ...
          router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, ignoreNodes, ignoreChannels ++ faultyChannel.toSet, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
          goto(WAITING_FOR_ROUTE) using WaitingForRoute(s, c, failures :+ RemoteFailure(hops, e))
      }
    ...
    case Event(Status.Failure(t), WaitingForComplete(s, c, _, failures, _, ignoreNodes, ignoreChannels, hops)) =>
      if (failures.size + 1 >= c.maxAttempts) {
        ...
        router ! RouteRequest(sourceNodeId, c.targetNodeId, c.amountMsat, c.assistedRoutes, ignoreNodes, ignoreChannels + faultyChannel, wr_opt = c.wr_opt, maxFeeBaseMsat = c.maxFeeBaseMsat, maxFeePct = c.maxFeePct)
...
object PaymentLifecycle {
  ...
  /**
    ...
    */
  case class SendPayment(amountMsat: Long, paymentHash: BinaryData, targetNodeId: PublicKey, assistedRoutes: Seq[Seq[ExtraHop]] = Nil, finalCltvExpiry: Long = Channel.MIN_CLTV_EXPIRY, maxAttempts: Int = 5, maxFeeBaseMsat:Long = 21000, maxFeePct: Double = 0.03, wr_opt: Option[WeightRatios] = None) {
