...
object OutgoingPaymentPacket {
  ...
  private def buildHtlcFailure(nodeSecret: PrivateKey, reason: FailureReason, add: UpdateAddHtlc, holdTime: FiniteDuration): Either[CannotExtractSharedSecret, (ByteVector, ByteVector)] = {
    ...
    add.pathKey_opt match {
      ...
      case None =>
        ...
        buildHtlcFailure(nodeSecret, cmd.reason, add, holdTime).map {
          case (encryptedReason, attributionData) =>
            val tlvs: Set[UpdateFailHtlcTlv] = Set(
              if (useAttributableFailures) Some(UpdateFailHtlcTlv.AttributionData(attributionData)) else None
            ).flatten
