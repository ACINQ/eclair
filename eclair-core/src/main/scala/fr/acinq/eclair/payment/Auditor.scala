...
class Auditor(nodeParams: NodeParams) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case e: ChannelStateChanged =>
      // NB: order matters!
      e match {
        case ChannelStateChanged(_, channelId, _, remoteNodeId, WAIT_FOR_FUNDING_LOCKED, NORMAL, commitments: Commitments) =>
        ...
        case ChannelStateChanged(_, _, _, _, previousState, CLOSING, _) if previousState != WAIT_FOR_INIT_INTERNAL =>
