      ...
      Behaviors.withMdc(Logs.mdc(category_opt = Some(Logs.LogCategory.PAYMENT)), mdc) {
        Behaviors.receiveMessage {
          case Relay(nodeRelayPacket, originNode) =>
            ...
            children.get(childKey) match {
              ...
              case None =>
                ...
                val handler = context.spawn(NodeRelay.apply(nodeParams, context.self, register, reputationRecorder, relayId, nodeRelayPacket, outgoingPaymentFactory, triggerer), relayId.toString)
                context.log.debug("forwarding incoming htlc #{} from channel {} to new handler", htlcIn.id, htlcIn.channelId)
                handler ! NodeRelay.Relay(nodeRelayPacket, originNode)
                apply(nodeParams, register, reputationRecorder, outgoingPaymentFactory, triggerer, children + (childKey -> handler))
            }
          case RelayComplete(childHandler, paymentHash, paymentSecret) =>
            ...
            apply(nodeParams, register, reputationRecorder, outgoingPaymentFactory, triggerer, children - PaymentKey(paymentHash, paymentSecret))
