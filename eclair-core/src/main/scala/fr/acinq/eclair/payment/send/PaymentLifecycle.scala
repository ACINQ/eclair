...
class PaymentLifecycle(nodeParams: NodeParams, cfg: SendPaymentConfig, router: ActorRef, register: ActorRef) extends FSMDiagnosticActorLogging[PaymentLifecycle.State, PaymentLifecycle.Data] {
  ...
  when(WAITING_FOR_REQUEST) {
    case Event(c: SendPaymentToRoute, WaitingForRequest) =>
      ...
      c.route.fold(
        hops => router ! FinalizeRoute(c.amount, hops, c.recipient.extraEdges, paymentContext = Some(cfg.paymentContext)),
  ...
  when(WAITING_FOR_ROUTE) {
    case Event(RouteResponse(route +: _), WaitingForRoute(c, failures, ignore)) =>
      ...
      OutgoingPaymentPacket.buildOutgoingPayment(self, nodeParams.privateKey, cfg.upstream, paymentHash, route, c.recipient) match {
        case Success(payment) =>
          ...
          goto(WAITING_FOR_PAYMENT_COMPLETE) using WaitingForComplete(c, payment.cmd, failures, payment.sharedSecrets, ignore, route)
        case Failure(t) =>
  ...
  private def handleRemoteFail(d: WaitingForComplete, fail: UpdateFailHtlc) = {
    ...
    ((Sphinx.FailurePacket.decrypt(fail.reason, sharedSecrets) match {
      ...
      case Success(e@Sphinx.DecryptedFailurePacket(nodeId, _: InvalidOnionBlinding)) =>
        ...
        val failure = RemoteFailure(d.c.amount, cfg.fullRoute(route), e)
        val extraEdges1 = PaymentFailure.updateExtraEdges(failures :+ failure, c.recipient).extraEdges
        val ignore1 = PaymentFailure.updateIgnored(failure, ignore)
        c match {
          case _: SendPaymentToRoute =>
            log.error("unexpected retry during SendPaymentToRoute")
    ...
    if (cfg.recordPathFindingMetrics) {
      ...
      request match {
        case request: SendPaymentToNode =>
          context.system.eventStream.publish(PathFindingExperimentMetrics(cfg.paymentHash, request.amount, fees, status, duration, now, isMultiPart = false, request.routeParams.experimentName, cfg.recipientNodeId, request.recipient.extraEdges))
        case _: SendPaymentToRoute => ()
      }
    }
