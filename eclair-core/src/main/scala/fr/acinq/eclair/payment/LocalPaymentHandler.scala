...
class LocalPaymentHandler(nodeParams: NodeParams) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case ReceivePayment(amount_opt, desc, expirySeconds_opt, extraHops, fallbackAddress_opt, preimage_opt) =>
      Try {
        val paymentPreimage = preimage_opt.getOrElse(randomBytes32)
