...
class LocalPaymentHandler(nodeParams: NodeParams) extends Actor with ActorLogging {
  ...
  def run(hash2preimage: Map[BinaryData, (BinaryData, PaymentRequest)]): Receive = {
    ...
    case ReceivePayment(amount_opt, desc, expirySeconds_opt, extraHops, fallbackAddress) =>
      Try {
        ...
        val paymentRequest = PaymentRequest(nodeParams.chainHash, amount_opt, paymentHash, nodeParams.privateKey, desc, fallbackAddress = fallbackAddress, expirySeconds = Some(expirySeconds), extraHops = extraHops)
