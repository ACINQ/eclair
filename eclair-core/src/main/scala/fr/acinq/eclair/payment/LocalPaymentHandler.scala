  ...
  def run(hash2preimage: Map[BinaryData, PendingPaymentRequest]): Receive = {
    ...
    case ReceivePayment(amount_opt, desc, expirySeconds_opt, extraHops, fallbackAddressOpt) =>
      Try {
        ...
        val paymentRequest = PaymentRequest(nodeParams.chainHash, amount_opt, paymentHash, nodeParams.privateKey, desc, fallbackAddressOpt, expirySeconds = Some(expirySeconds), extraHops = extraHops)
