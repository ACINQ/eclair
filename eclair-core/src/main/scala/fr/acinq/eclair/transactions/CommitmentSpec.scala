...
final case class CommitmentSpec(htlcs: Set[DirectedHtlc], feeratePerKw: Long, toLocalMsat: Long, toRemoteMsat: Long) {
  val totalFunds = toLocalMsat + toRemoteMsat + htlcs.toSeq.map(_.add.amountMsat).sum.toLong
}
...
object CommitmentSpec {
  ...
  def addHtlc(spec: CommitmentSpec, direction: Direction, update: UpdateAddHtlc): CommitmentSpec = {
    ...
    direction match {
      case OUT => spec.copy(toLocalMsat = spec.toLocalMsat - htlc.add.amountMsat.toLong, htlcs = spec.htlcs + htlc)
      case IN => spec.copy(toRemoteMsat = spec.toRemoteMsat - htlc.add.amountMsat.toLong, htlcs = spec.htlcs + htlc)
    }
  }
  ...
  def fulfillHtlc(spec: CommitmentSpec, direction: Direction, htlcId: Long): CommitmentSpec = {
    spec.htlcs.find(htlc => htlc.direction != direction && htlc.add.id == htlcId) match {
      case Some(htlc) if direction == OUT => spec.copy(toLocalMsat = spec.toLocalMsat + htlc.add.amountMsat.toLong, htlcs = spec.htlcs - htlc)
      case Some(htlc) if direction == IN => spec.copy(toRemoteMsat = spec.toRemoteMsat + htlc.add.amountMsat.toLong, htlcs = spec.htlcs - htlc)
  ...
  def failHtlc(spec: CommitmentSpec, direction: Direction, htlcId: Long): CommitmentSpec = {
    spec.htlcs.find(htlc => htlc.direction != direction && htlc.add.id == htlcId) match {
      case Some(htlc) if direction == OUT => spec.copy(toRemoteMsat = spec.toRemoteMsat + htlc.add.amountMsat.toLong, htlcs = spec.htlcs - htlc)
      case Some(htlc) if direction == IN => spec.copy(toLocalMsat = spec.toLocalMsat + htlc.add.amountMsat.toLong, htlcs = spec.htlcs - htlc)
      case None => throw new RuntimeException(s"cannot find htlc id=${htlcId}")
    }
  }
