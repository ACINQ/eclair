...
object Transactions {
  ...
  def makeClosingTx(commitTxInput: InputInfo, localScriptPubKey: ByteVector, remoteScriptPubKey: ByteVector, localIsInitiator: Boolean, dustLimit: Satoshi, closingFee: Satoshi, spec: CommitmentSpec, sequence: Long = 0xffffffffL): ClosingTx = {
  ...
  def partialSign(txinfo: TransactionWithInputInfo, key: PrivateKey,
    ...
    val txHash = Transaction.hashForSigningTaprootKeyPath(txinfo.tx, inputIndex, Seq(txinfo.input.txOut), SigHash.SIGHASH_DEFAULT)
    // we compare x-only pubkeys here
    val publicKeys = Scripts.sort(Seq(localFundingPublicKey, remoteFundingPublicKey))
    println(s"creating partial signature for txid = ${txinfo.tx.txid} inputIndex = $inputIndex txHash = $txHash tx = ${txinfo.tx}")
    println(s"input = ${txinfo.input.txOut}")
    println(s"public keys = $publicKeys localNonce = ${localNonce._1.getData$bitcoin_kmp}  ${localNonce._2} remoteNextLocalNonce = $remoteNextLocalNonce")
