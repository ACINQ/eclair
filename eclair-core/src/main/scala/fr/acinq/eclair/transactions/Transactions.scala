...
object Transactions {
      ...
      if (inputIndex >= 0) {
        redeemInfo match {
          ...
          case t: RedeemInfo.Taproot =>
            val data = t match {
              case _: RedeemInfo.TaprootKeyPath => Transaction.hashForSigningTaprootKeyPath(tx, inputIndex = 0, Seq(input.txOut), sighash(txOwner, commitmentFormat))
              case s: RedeemInfo.TaprootScriptPath => Transaction.hashForSigningTaprootScriptPath(tx, inputIndex = 0, Seq(input.txOut), sighash(txOwner, commitmentFormat), s.leafHash)
            }
  ...
  private def makeHtlcTimeoutTx(commitTx: Transaction,
    ...
    if (amount < localDustLimit) {
      ...
      output.commitmentOutput.redeemInfo match {
        ...
        case t: RedeemInfo.TaprootScriptPath =>
          val input = InputInfo(OutPoint(commitTx, outputIndex), commitTx.txOut(outputIndex))
          val tree = t.scriptTree.asInstanceOf[ScriptTree.Branch] // Taproot.htlcDelayedScriptTree(localDelayedPaymentPubkey, toLocalDelay)
          val redeemInfo = RedeemInfo.TaprootScriptPath(t.internalKey, tree, tree.getLeft.hash())
          val tx = Transaction(
            version = 2,
            txIn = TxIn(input.outPoint, ByteVector.empty, getHtlcTxInputSequence(commitmentFormat)) :: Nil,
          ...
          Right(HtlcTimeoutTx(input, redeemInfo, tx, htlc.id, ConfirmationTarget.Absolute(BlockHeight(htlc.cltvExpiry.toLong))))
  ...
  private def makeHtlcSuccessTx(commitTx: Transaction,
    ...
    if (amount < localDustLimit) {
      ...
      output.commitmentOutput.redeemInfo match {
        ...
        case t: RedeemInfo.TaprootScriptPath =>
          val scriptTree: ScriptTree.Branch = t.scriptTree.asInstanceOf[ScriptTree.Branch]
          val input = InputInfo(OutPoint(commitTx, outputIndex), commitTx.txOut(outputIndex))
          val redeemInfo = RedeemInfo.TaprootScriptPath(t.internalKey, scriptTree, scriptTree.getRight.hash())
          ...
          Right(HtlcSuccessTx(input, redeemInfo, tx, htlc.paymentHash, htlc.id, ConfirmationTarget.Absolute(BlockHeight(htlc.cltvExpiry.toLong))))
    ...
    def makeUnsignedTx(input: InputInfo, redeemInfo: RedeemInfo.TaprootScriptPathOrSegwitV0): Either[TxGenerationSkipped, (InputInfo, RedeemInfo.TaprootScriptPathOrSegwitV0, Transaction)] = {
      ...
      val weight = redeemInfo match {
        case s: RedeemInfo.TaprootScriptPath =>
          val toLocalScriptTree = s.scriptTree.asInstanceOf[ScriptTree.Branch]
          val witness = Script.witnessScriptPathPay2tr(s.internalKey, toLocalScriptTree.getLeft.asInstanceOf[ScriptTree.Leaf], ScriptWitness(Seq(ByteVector64.Zeroes)), toLocalScriptTree)
          tx.updateWitness(0, witness).weight()
  ...
  def addSigs(claimRemoteDelayedOutputTx: ClaimRemoteDelayedOutputTx, localSig: ByteVector64): ClaimRemoteDelayedOutputTx = claimRemoteDelayedOutputTx.redeemInfo match {
    ...
    case t: RedeemInfo.TaprootScriptPath =>
      val witness = Script.witnessScriptPathPay2tr(t.internalKey, t.scriptTree.asInstanceOf[ScriptTree.Leaf], ScriptWitness(Seq(localSig)), t.scriptTree)
      claimRemoteDelayedOutputTx.copy(tx = claimRemoteDelayedOutputTx.tx.updateWitness(0, witness))
  }
  ...
  def addSigs(claimDelayedOutputTx: ClaimLocalDelayedOutputTx, localSig: ByteVector64): ClaimLocalDelayedOutputTx = claimDelayedOutputTx.redeemInfo match {
    ...
    case t: RedeemInfo.TaprootScriptPath =>
      val witness = Script.witnessScriptPathPay2tr(t.internalKey, t.scriptTree.asInstanceOf[ScriptTree.Branch].getLeft.asInstanceOf[ScriptTree.Leaf], ScriptWitness(Seq(localSig)), t.scriptTree)
      claimDelayedOutputTx.copy(tx = claimDelayedOutputTx.tx.updateWitness(0, witness))
  }
