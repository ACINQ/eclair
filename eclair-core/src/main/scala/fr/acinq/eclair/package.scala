...
package object eclair {
   ...
   *
   * Because the virtual transaction size is, as described in bip-0141, rounded *up*. Bitcoin-core uses this virtual size for its
   * mempool logic, which ends up rounding *down* the feerate.
   * We have vbyte = (weight + 3) // 4, we want :
   *     fee * 1000 // vbyte >= 1000
   *     fee * 1000 // ((weight + 3) * 4) >= 1000
   *     fee * 1000 // (weight + 3) >= 250
   *     (feerate * weight) * 1000 // (weight + 3) >= 250
   *
