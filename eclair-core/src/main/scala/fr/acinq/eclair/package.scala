...
package object eclair {
  ...
  def base58ToPublicKeyScript(address: String, chainHash: BinaryData): Seq[ScriptElt] = {
    Base58Check.decode(address) match {
      case (Base58.Prefix.PubkeyAddressTestnet, pubKeyHash) if chainHash == Block.TestnetGenesisBlock.hash || chainHash == Block.RegtestGenesisBlock.hash => Script.pay2pkh(pubKeyHash)
      case (Base58.Prefix.PubkeyAddress, pubKeyHash) if chainHash == Block.LivenetGenesisBlock.hash => Script.pay2pkh(pubKeyHash)
      case (Base58.Prefix.ScriptAddressTestnet, scriptHash) if chainHash == Block.TestnetGenesisBlock.hash || chainHash == Block.RegtestGenesisBlock.hash => OP_HASH160 :: OP_PUSHDATA(scriptHash) :: OP_EQUAL :: Nil
      ...
      case _ => throw new IllegalArgumentException("payment address does not match our blockchain")
    }
  }

  def bech32ToPublicKeyScript(address: String, chainHash: BinaryData): Seq[ScriptElt] = {
    Bech32.decodeWitnessAddress(address) match {
      case (_, version, _) if version != 0.toByte => throw new IllegalArgumentException(s"invalid version $version in bech32 address")
      case (_, _, bin) if bin.length != 20 && bin.length != 32 => throw new IllegalArgumentException("hash length in bech32 address must be either 20 or 32 bytes")
      case ("bc", _, bin) if chainHash == Block.LivenetGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
      case ("tb", _, bin) if chainHash == Block.TestnetGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
      case ("bcrt", _, bin) if chainHash == Block.RegtestGenesisBlock.hash => OP_0 :: OP_PUSHDATA(bin) :: Nil
    }
  }
  ...
  /**
    ...
    */
  def addressToPublicKeyScript(address: String, chainHash: BinaryData): Try[Seq[ScriptElt]] = Try(base58ToPublicKeyScript(address, chainHash)).orElse(Try(bech32ToPublicKeyScript(address, chainHash)))
}