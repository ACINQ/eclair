...
import fr.acinq.eclair.{Features, MilliSatoshi, MilliSatoshiLong, RealShortChannelId, TimestampMilli, ToMilliSatoshiConversion}
...
object PeerStatsTracker {
  ...
  case class Config(initPastEventsDelay: FiniteDuration, chunkPastEventsDelay: FiniteDuration)
  ...
  case class ChannelInfo(channelId: ByteVector32, shortChannelId_opt: Option[RealShortChannelId], capacity: Satoshi, canSend: MilliSatoshi, canReceive: MilliSatoshi, isPublic: Boolean)

  object ChannelInfo {
    def apply(commitments: Commitments): ChannelInfo = ChannelInfo(commitments.channelId, commitments.latest.shortChannelId_opt, commitments.latest.capacity, commitments.availableBalanceForSend, commitments.availableBalanceForReceive, commitments.announceChannel)
  }
...
private class PeerStatsTracker(config: PeerStatsTracker.Config, db: AuditDb, timers: TimerScheduler[PeerStatsTracker.Command], context: ActorContext[PeerStatsTracker.Command]) {
  ...
  private def start(channels: Seq[PersistentChannelData]): Behavior[Command] = {
    ...
    timers.startSingleTimer(LoadPastEvents(before = startedAt), config.initPastEventsDelay)
    ...
    Behaviors.receiveMessage {
      ...
      case LoadPastEvents(before) =>
        if (before <= (TimestampMilli.now() - Bucket.duration * BucketedPeerStats.bucketsCount)) {
          ...
          timers.startSingleTimer(LoadPastEvents(before - Bucket.duration), config.chunkPastEventsDelay)
