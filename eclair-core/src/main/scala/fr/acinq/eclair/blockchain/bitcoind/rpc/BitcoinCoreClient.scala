...
class BitcoinCoreClient(val rpcClient: BitcoinJsonRPCClient, val onchainKeyManager_opt: Option[OnchainKeyManager] = None) extends OnChainWallet with Logging {
  ...
  private def extractPublicKey(chainHash: ByteVector32, address: String)(implicit ec: ExecutionContext): Future[PublicKey] = {
    for {
      ...
      require(computed_opt.forall(_ == extracted), "cannot recompute pubkey generated by bitcoin core")
      require(computeP2WpkhAddress(extracted, chainHash) == address, "address does not match public key")
      extracted
    }
  }

  def getReceiveAddress(chainHash: ByteVector32, label: String)(implicit ec: ExecutionContext): Future[String] = for {
    JString(address) <- rpcClient.invoke("getnewaddress", label, "bech32")
    _ <- extractPublicKey(chainHash, address)
  } yield address

  def getP2wpkhPubkey(chainHash: ByteVector32)(implicit ec: ExecutionContext): Future[Crypto.PublicKey] = for {
    JString(address) <- rpcClient.invoke("getnewaddress", "", "bech32")
    pubKey <- extractPublicKey(chainHash, address)
  } yield pubKey

  /**
   * @return the public key hash of a bech32 raw change address.
   */
  def getChangeAddress(chainHash: ByteVector32)(implicit ec: ExecutionContext): Future[ByteVector] = for {
    JString(changeAddress) <- rpcClient.invoke("getrawchangeaddress", "bech32")
    pubKey <- extractPublicKey(chainHash, changeAddress)
  } yield {
    pubKey.hash160
  }
