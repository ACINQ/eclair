...
class BitcoinCoreClient(val rpcClient: BitcoinJsonRPCClient, val onChainKeyManager_opt: Option[OnChainKeyManager] = None) extends OnChainWallet with Logging {
  ...
  def makeFundingTx(pubkeyScript: ByteVector, amount: Satoshi, targetFeerate: FeeratePerKw, maxFundingFee: Satoshi)(implicit ec: ExecutionContext): Future[MakeFundingTxResponse] = {
    ...
    def verifyAndSign(tx: Transaction, fees: Satoshi, requestedFeeRate: FeeratePerKw): Future[MakeFundingTxResponse] = {
      ...
      for {
        ...
        _ = require(maxFundingFee >= fees, s"funding fees $fees are higher than the maximum allowed $maxFundingFee")
