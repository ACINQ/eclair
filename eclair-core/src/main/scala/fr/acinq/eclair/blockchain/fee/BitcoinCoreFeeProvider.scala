...
/**
  ...
  */
class BitcoinCoreFeeProvider(rpcClient: BitcoinJsonRPCClient, defaultFeerates: FeeratesPerByte)(implicit ec: ExecutionContext) extends FeeProvider with Logging{
  ...
  override def getFeerates: Future[FeeratesPerByte] = for {
    ...
    logger.info(s"bitcoind feerates=$feerates")
    feerates
  }
