  ...
  def running(tip: ElectrumClient.Header, watches: Set[Watch], scriptHashStatus: Map[BinaryData, String], block2tx: SortedMap[Long, Seq[Transaction]]): Receive = {
    ...
    case ElectrumClient.GetTransactionResponse(spendingTx) =>
      spendingTx.txIn.map(_.outPoint).map(outPoint => {
        watches.collect {
          ...
          case WatchSpentBasic(channel, txid, pos, _, event) if txid == outPoint.txid && pos == outPoint.index.toInt =>
            log.info(s"output ${txid}:$pos spent by transaction ${spendingTx.txid}")
    ...
    case PublishAsap(tx) =>
      ...
      if (csvTimeout > 0) {
        ...
        log.info(s"publishing ${Transaction.write(tx)}")
        client ! BroadcastTransaction(tx)
      }

    case ElectrumClient.BroadcastTransactionResponse(tx, error_opt) =>
      error_opt match {
        case Some(error) => log.error(s"broadcast of txid=${tx.txid} failed with error=$error")
        case None => log.info(s"brodcast of txid=${tx.txid} succeeded")
      }
