...
class BasicBitcoinJsonRPCClient(user: String, password: String, host: String = "127.0.0.1", port: Int = 8332, ssl: Boolean = false)(implicit system: ActorSystem) extends BitcoinJsonRPCClient {
  ...
  override def invoke(request: JsonRPCRequest)(implicit ec: ExecutionContext): Future[JValue] =
    for {
  ...
  override def invoke(requests: Seq[JsonRPCRequest])(implicit ec: ExecutionContext): Future[Seq[JValue]] =
    for {
