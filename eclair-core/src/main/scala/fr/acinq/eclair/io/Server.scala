 ...
 */
class Server(keyPair: KeyPair, peerConnectionConf: PeerConnection.PeerConnectionConf, switchboard: ActorRef, router: ActorRef, address: InetSocketAddress, bound: Option[Promise[Done]] = None) extends Actor with DiagnosticActorLogging {
...
object Server {
  ...
  def props(keyPair: KeyPair, peerConnectionConf: PeerConnection.PeerConnectionConf, switchboard: ActorRef, router: ActorRef, address: InetSocketAddress, bound: Option[Promise[Done]] = None): Props = Props(new Server(keyPair, peerConnectionConf, switchboard, router: ActorRef, address, bound))
