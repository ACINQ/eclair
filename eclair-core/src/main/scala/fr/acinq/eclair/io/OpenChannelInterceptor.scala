...
object OpenChannelInterceptor {
  def apply(replyTo: ActorRef[Any], plugin: InterceptOpenChannelPlugin, timeout: FiniteDuration, connectedData: ConnectedData, temporaryChannelId: ByteVector32, localParams: LocalParams, fundingAmount_opt: Option[Satoshi], open: Either[OpenChannel, OpenDualFundedChannel], channelType: SupportedChannelType): Behavior[Command] = {
    Behaviors.setup { context =>
      Behaviors.withTimers { timers =>
        timers.startSingleTimer(PluginTimeout, timeout)
        new OpenChannelInterceptor(replyTo, plugin, connectedData, temporaryChannelId, localParams, fundingAmount_opt, open, channelType, context).start()
      }
    }
  }
...
private class OpenChannelInterceptor(replyTo: ActorRef[Any], plugin: InterceptOpenChannelPlugin, connectedData: ConnectedData, temporaryChannelId: ByteVector32, localParams: LocalParams, fundingAmount_opt: Option[Satoshi], open: Either[OpenChannel, OpenDualFundedChannel], channelType: SupportedChannelType, context: ActorContext[OpenChannelInterceptor.Command]) {
  ...
  private def start(): Behavior[Command] = {
    ...
    plugin.getOpenChannelInterceptor ! InterceptOpenChannelReceived(pluginResponseAdapter, open, temporaryChannelId, localParams, fundingAmount_opt)
