...
class Peer(nodeParams: NodeParams, remoteNodeId: PublicKey, authenticator: ActorRef, watcher: ActorRef, router: ActorRef, relayer: ActorRef, wallet: EclairWallet) extends FSMDiagnosticActorLogging[Peer.State, Peer.Data] {
  ...
  when(CONNECTED, stateTimeout = nodeParams.pingInterval) {
    case Event(StateTimeout, d@ConnectedData(_, transport, _, _, _, _, _)) =>
      ...
      val disconnector = context.system.scheduler.scheduleOnce(10 seconds, transport, PoisonPill)
      transport ! ping
    ...
    case Event(pong@wire.Pong(data), d@ConnectedData(_, transport, _, _, _, _, expectedPong_opt)) =>
      transport ! TransportHandler.ReadAck(pong)
      // TODO: compute latency for remote peer ?
      expectedPong_opt match {
        case Some(ExpectedPong(ping, timeout)) =>
          log.debug(s"received pong with ${data.length} bytes, was expecting for ${ping.data.length} bytes")
          timeout.cancel
        case None =>
    ...
    case Event(msg: wire.RoutingMessage, ConnectedData(_, transport, _, _, _, behavior, _)) =>
      msg match {
    ...
    case Event(badMessage: BadMessage, data@ConnectedData(_, transport, _, _, _, behavior, _)) =>
...
object Peer {
  ...
  case class ExpectedPong(ping: Ping, timeout: Cancellable)
