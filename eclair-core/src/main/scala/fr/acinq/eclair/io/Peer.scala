...
class Peer(val nodeParams: NodeParams,
  ...
  private var pendingOnTheFlyFunding = nodeParams.db.onTheFlyFunding.listPending(remoteNodeId)
  ...
  whenUnhandled {
    ...
    case Event(current: CurrentBlockHeight, d) =>
      ...
      expired.foreach {
        case (paymentHash, pending) => pending.status match {
          case _: OnTheFlyFunding.Status.Proposed => ()
          case _: OnTheFlyFunding.Status.Funded => nodeParams.db.onTheFlyFunding.removePending(remoteNodeId, paymentHash)
        }
      }
    ...
    case Event(e: LiquidityLeaseSigned, _: ConnectedData) =>
      // We signed a liquidity lease purchased by our peer. At that point we're not 100% sure yet it will succeed: if
      ...
      val (paymentHashes, fees) = e.lease.paymentDetails match {
        ...
        case p: PaymentDetails.FromFutureHtlc => (p.paymentHashes, e.lease.fees.total)
        case p: PaymentDetails.FromFutureHtlcWithPreimage => (p.preimages.map(preimage => Crypto.sha256(preimage)), e.lease.fees.total)
      }
        ...
        .foreach(payment => {
          payment.status match {
            case status: OnTheFlyFunding.Status.Proposed =>
              ...
              log.info("liquidity lease signed for payment_hash={}, waiting to relay HTLCs (txId={}, fundingTxIndex={}, fees={})", payment.paymentHash, e.txId, e.fundingTxIndex, paymentFees)
            ...
            case status: OnTheFlyFunding.Status.Funded =>
              log.warning("liquidity lease was already signed for payment_hash={} (previousTxId={}, currentTxId={})", payment.paymentHash, status.txId, e.txId)
          }
        })
      stay()
...
object Peer {
  ...
  /** We signed a funding transaction where our peer purchased a liquidity lease. */
  case class LiquidityLeaseSigned(channelId: ByteVector32, txId: TxId, fundingTxIndex: Long, htlcMinimum: MilliSatoshi, lease: LiquidityAds.Lease)
