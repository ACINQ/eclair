...
class Peer(val nodeParams: NodeParams, remoteNodeId: PublicKey, authenticator: ActorRef, watcher: ActorRef, router: ActorRef, relayer: ActorRef, wallet: EclairWallet) extends FSMDiagnosticActorLogging[Peer.State, Peer.Data] {
  ...
  when(CONNECTED) {
    ...
    case Event(c: Peer.OpenChannel, d: ConnectedData) =>
      ...
      val channelFeeratePerKw = nodeParams.feeEstimator.getFeeratePerKw(target = nodeParams.feeTargets.commitmentBlockTarget)
      val fundingTxFeeratePerKw = c.fundingTxFeeratePerKw_opt.getOrElse(nodeParams.feeEstimator.getFeeratePerKw(target = nodeParams.feeTargets.fundingBlockTarget))
