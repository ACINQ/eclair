  ...
  when(CONNECTED) {
    dropStaleMessages {
      ...
      case Event(SpawnChannelInitiator(replyTo, c, channelConfig, channelType, localParams), d: ConnectedData) =>
        ...
        log.info(s"requesting a new channel with type=$channelType fundingAmount=${c.fundingAmount} dualFunded=$dualFunded pushAmount=${c.pushAmount_opt} fundingFeerate=$fundingTxFeerate maxFee=${c.maxFundingFee_opt} temporaryChannelId=$temporaryChannelId localParams=$localParams")
        channel ! INPUT_INIT_CHANNEL_INITIATOR(temporaryChannelId, c.fundingAmount, dualFunded, commitTxFeerate, fundingTxFeerate, c.maxFundingFee_opt, c.pushAmount_opt, requireConfirmedInputs, localParams, d.peerConnection, d.remoteInit, c.channelFlags_opt.getOrElse(nodeParams.channelConf.channelFlags), channelConfig, channelType, c.channelOrigin, replyTo)
...
object Peer {
  ...
  case class OpenChannel(remoteNodeId: PublicKey,
                         ...
                         maxFundingFee_opt: Option[Satoshi],
