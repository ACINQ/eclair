...
class BackupHandler private(databases: Databases, backupFile: File, backupScript_opt: Option[String]) extends Actor with RequiresMessageQueue[BoundedMessageQueueSemantics] with ActorLogging {
  ...
  def receive: Receive = {
    case persisted: ChannelPersisted =>
      ...
      Metrics.BackupCompleted.withoutTags().increment()
      Metrics.BackupDuration.withoutTags().record(end - start)
...
object BackupHandler {
  ...
  def props(databases: Databases, backupFile: File, backupScript_opt: Option[String]) =
    Props(new BackupHandler(databases, backupFile, backupScript_opt))
