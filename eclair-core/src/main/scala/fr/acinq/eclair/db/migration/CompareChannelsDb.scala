  ...
  private def compareChannelsTable(conn1: Connection, conn2: Connection): Boolean = {
    ...
    def hash1(rs: ResultSet): ByteVector = {
      ...
      val data_modified = channelDataCodec.decode(data.bits).require.value match {
        case c: ChannelData.WaitingForFundingConfirmed => channelDataCodec.encode(c.copy(waitingSince = BlockHeight(0))).require.toByteVector
        case c: ChannelData.Closing => channelDataCodec.encode(c.copy(waitingSince = BlockHeight(0))).require.toByteVector
        case _ => data
      }
      bytes(rs, "channel_id") ++
        data_modified ++
