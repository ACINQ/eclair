...
class PgChannelsDb(implicit ds: DataSource, lock: PgLock) extends ChannelsDb with Logging {
  ...
  inTransaction { pg =>
    using(pg.createStatement()) { statement =>
      ...
      def migration1112(statement: Statement): Unit = {
        ...
        val insertStatement = pg.prepareStatement("INSERT INTO closed_channels VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
        val batchSize = 50
        ...
        statement.executeUpdate("DELETE FROM local.channels WHERE is_closed=1")
  ...
  override def listClosedChannels(remoteNodeId_opt: Option[PublicKey], paginated_opt: Option[Paginated]): Seq[DATA_CLOSED] = withMetrics("channels/list-closed-channels", DbBackends.Postgres) {
    val sql = remoteNodeId_opt match {
      case Some(remoteNodeId) => s"SELECT data FROM local.closed_channels WHERE remote_node_id = '${remoteNodeId.toHex}' ORDER BY closed_at DESC"
      case None => "SELECT data FROM local.closed_channels ORDER BY closed_at DESC"
    }
    withLock { pg =>
      using(pg.prepareStatement(limited(sql, paginated_opt))) { statement =>
        statement.executeQuery().map { rs =>
          DATA_CLOSED(
            ...
            closingScript = rs.getByteVector32FromHex("closing_script"),
