...
class SqliteInboundFeesDb(val sqlite: Connection) extends InboundFeesDb with Logging {
  ...
  override def unsetInboundFees(nodeId: Crypto.PublicKey): Unit = withMetrics("inbound-fees/unset-inbound-fees", DbBackends.Sqlite) {
    using(sqlite.prepareStatement("DELETE FROM inbound_fees WHERE node_id=?")) { statement =>
      statement.setBytes(1, nodeId.value.toArray)
      statement.executeUpdate()
    }
  }
