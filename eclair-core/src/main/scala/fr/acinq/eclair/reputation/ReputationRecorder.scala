...
object ReputationRecorder {
  ...
  case class GetConfidence(replyTo: ActorRef[Confidence], upstream: Upstream.Hot, downstream_opt: Option[PublicKey], fee: MilliSatoshi) extends Command
  ...
  case class Confidence(value: Double, endorsement: Int)
  ...
  def run(): Behavior[Command] =
    Behaviors.receiveMessage {
      ...
      case GetConfidence(replyTo, upstream: Upstream.Hot.Channel, downstream_opt, fee) =>
        val incomingConfidence = incomingReputations(upstream.receivedFrom).getConfidence(fee, upstream.add.endorsement)
        val endorsement = incomingConfidence.toEndorsement
        val outgoingConfidence = downstream_opt.map(outgoingReputations(_).getConfidence(fee, endorsement)).getOrElse(1.0)
          ...
          upstream.received
            ...
            .map {
              case (nodeId, (amount, endorsement)) =>
                ...
                incomingReputations(nodeId).getConfidence(fee, endorsement)
            }
            .min
        val endorsement = incomingConfidence.toEndorsement
        val outgoingConfidence = downstream_opt.map(outgoingReputations(_).getConfidence(totalFee, endorsement)).getOrElse(1.0)
        replyTo ! Confidence(incomingConfidence min outgoingConfidence, endorsement)
        Behaviors.same
