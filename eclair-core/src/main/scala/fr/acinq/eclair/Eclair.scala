...
trait Eclair {
  ...
  def receive(description: String, amountMsat: Option[Long], expire: Option[Long], fallbackAddress: Option[String], preimage:Option[ByteVector32])(implicit timeout: Timeout): Future[PaymentRequest]
...
class EclairImpl(appKit: Kit) extends Eclair {
  ...
  override def receive(description: String, amountMsat: Option[Long], expire: Option[Long], fallbackAddress: Option[String], preimage:Option[ByteVector32])(implicit timeout: Timeout): Future[PaymentRequest] = {
    ...
    (appKit.paymentHandler ? ReceivePayment(description = description, amountMsat_opt = amountMsat.map(MilliSatoshi), expirySeconds_opt = expire, fallbackAddress = fallbackAddress, preimage = preimage)).mapTo[PaymentRequest]
  }
