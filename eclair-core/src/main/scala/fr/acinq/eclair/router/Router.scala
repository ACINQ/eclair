...
class Router(val nodeParams: NodeParams, watcher: ActorRef, initialized: Option[Promise[Done]] = None) extends FSMDiagnosticActorLogging[State, Data] {
  ...
  when(NORMAL) {
          ...
          Kamon.runWithSpan(Kamon.spanBuilder("process-validate-result").start(), finishSpan = true) {
            ...
            val publicChannel_opt = v match {
              case ValidateResult(c, Left(t)) =>
                ...
                remoteOrigins_opt.foreach(_.foreach(_.peerConnection ! Gossip.ValidationFailure(c)))
                None
              case ValidateResult(c, Right((tx, UtxoStatus.Unspent))) =>
                ...
                if (ok) {
                  ...
                  remoteOrigins_opt.foreach(_.foreach(_.peerConnection ! Gossip.InvalidAnnouncement(c)))
                  None
                } else {
                  watcher ! WatchSpentBasic(self, tx, outputIndex, BITCOIN_FUNDING_EXTERNAL_CHANNEL_SPENT(c.shortChannelId))
                  // TODO: check feature bit set
                  log.debug("added channel channelId={}", c.shortChannelId)
                  remoteOrigins_opt.foreach(_.foreach(_.peerConnection ! Gossip.Accepted(c)))
              ...
              case ValidateResult(c, Right((tx, fundingTxStatus: UtxoStatus.Spent))) =>
                if (fundingTxStatus.spendingTxConfirmed) {
                  ...
                  remoteOrigins_opt.foreach(_.foreach(_.peerConnection ! Gossip.ChannelClosed(c)))
                } else {
                  log.debug("ignoring shortChannelId={} tx={} (funding tx already spent but spending tx isn't confirmed)", c.shortChannelId, tx.txid)
                  remoteOrigins_opt.foreach(_.foreach(_.peerConnection ! Gossip.ChannelClosing(c)))
                }
    ...
    case Event(PeerRoutingMessage(origin, routingMessage: HasChainHash), _) if routingMessage.chainHash != nodeParams.chainHash =>
      origin.peerConnection ! TransportHandler.ReadAck(routingMessage)
    ...
    case Event(PeerRoutingMessage(origin, u: ChannelUpdate), d) =>
      origin.peerConnection ! TransportHandler.ReadAck(u)
      log.debug("received channel update for shortChannelId={}", u.shortChannelId)
      stay using handle(u, origin, d, remoteNodeId_opt = Some(origin.nodeId), peerConnection_opt = Some(origin.peerConnection))
    ...
    case Event(PeerRoutingMessage(origin, c: ChannelAnnouncement), d) =>
      ...
      if (d.channels.contains(c.shortChannelId)) {
        ...
        origin.peerConnection ! Gossip.Duplicate(c)
        stay
        ...
        origin.peerConnection ! Gossip.ChannelPruned(c)
        stay
      } else if (!Announcements.checkSigs(c)) {
        origin.peerConnection ! TransportHandler.ReadAck(c)
        log.warning("bad signature for announcement {}", c)
        origin.peerConnection ! Gossip.InvalidSignature(c)
        stay
      } else {
        ...
        stay using d.copy(awaiting = d.awaiting + (c -> Seq(origin)))
      }
    ...
    case Event(PeerRoutingMessage(origin, n: NodeAnnouncement), d: Data) =>
      origin.peerConnection ! TransportHandler.ReadAck(n)
      log.debug("received node announcement for nodeId={}", n.nodeId)
      stay using handle(n, origin, d)

    case Event(PeerRoutingMessage(origin, routingMessage@QueryChannelRange(chainHash, firstBlockNum, numberOfBlocks, extendedQueryFlags_opt)), d) =>
      origin.peerConnection ! TransportHandler.ReadAck(routingMessage)
    ...
    case Event(PeerRoutingMessage(origin, routingMessage@ReplyChannelRange(chainHash, _, _, _, shortChannelIds, _)), d) =>
      origin.peerConnection ! TransportHandler.ReadAck(routingMessage)
    ...
    case Event(PeerRoutingMessage(origin, routingMessage@QueryShortChannelIds(chainHash, shortChannelIds, _)), d) =>
      origin.peerConnection ! TransportHandler.ReadAck(routingMessage)
      ...
      Kamon.runWithContextEntry(remoteNodeIdKey, origin.nodeId.toString) {
        Kamon.runWithSpan(Kamon.spanBuilder("query-short-channel-ids").start(), finishSpan = true) {
          ...
          origin.peerConnection ! ReplyShortChannelIdsEnd(chainHash, 1)
          stay
        }
      }

    case Event(PeerRoutingMessage(origin, routingMessage: ReplyShortChannelIdsEnd), d) =>
      origin.peerConnection ! TransportHandler.ReadAck(routingMessage)
  ...
  def handle(n: NodeAnnouncement, origin: GossipOrigin, d: Data): Data =
    if (d.stash.nodes.contains(n)) {
      ...
      origin match {
        case RemoteGossip(peer, _) => peer ! Gossip.InvalidSignature(n)
        case LocalGossip =>
      }
      d
  ...
  def handle(u: ChannelUpdate, origin: GossipOrigin, d: Data, remoteNodeId_opt: Option[PublicKey] = None, peerConnection_opt: Option[ActorRef] = None): Data =
    if (d.channels.contains(u.shortChannelId)) {
      ...
      if (d.rebroadcast.updates.contains(u)) {
        ...
        origin match {
          case RemoteGossip(peer, _) => peer ! Gossip.InvalidSignature(u)
          case LocalGossip =>
        }
        d
  ...
  override def mdc(currentMessage: Any): MDC = {
    ...
    currentMessage match {
      ...
      case PeerRoutingMessage(origin, _) => Logs.mdc(category_opt, remoteNodeId_opt = Some(origin.nodeId))
