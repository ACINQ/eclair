...
case class RouteRequest(source: PublicKey, target: PublicKey, amountMsat: Long, assistedRoutes: Seq[Seq[ExtraHop]] = Nil, ignoreNodes: Set[PublicKey] = Set.empty, ignoreChannels: Set[ChannelDesc] = Set.empty, randomize: Boolean = false)
...
class Router(nodeParams: NodeParams, watcher: ActorRef, initialized: Option[Promise[Done]] = None) extends FSMDiagnosticActorLogging[State, Data] {
  ...
  when(NORMAL) {
    ...
    case Event(RouteRequest(start, end, amount, assistedRoutes, ignoreNodes, ignoreChannels, randomize), d) =>
      ...
      // if randomize is true we ask the router to make a random selection among the three best routes
      val numRoutes = if(randomize) DEFAULT_ROUTES_COUNT else 1
      findRoute(d.graph, start, end, amount, numRoutes = numRoutes, extraEdges = extraEdges, ignoredEdges = ignoredUpdates.toSet)
