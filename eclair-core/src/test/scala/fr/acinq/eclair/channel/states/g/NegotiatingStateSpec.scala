...
class NegotiatingStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv ClosingSigned (theirCloseFee != ourCloseFee)") { case (alice, bob, alice2bob, bob2alice, _, _) =>
    within(30 seconds) {
      ...
      awaitCond(alice.stateData.asInstanceOf[DATA_NEGOTIATING].closingTxProposed.map(_.localClosingSigned) == initialState.closingTxProposed.map(_.localClosingSigned) :+ aliceCloseSig2)
    }
  }
