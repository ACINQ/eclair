...
class NegotiatingStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv ClosingComplete (with concurrent shutdown)", Tag(ChannelStateTestsTags.SimpleClose)) { f =>
    ...
    // When Alice receives Bob's shutdown, she generates a new set of closing transactions using the new closing script.
    bob2alice.forward(alice, bobShutdown)
    val aliceClosingComplete2 = alice2bob.expectMsgType[ClosingComplete]
    alice2bob.forward(bob, aliceClosingComplete2)
    val bobClosingSig = bob2alice.expectMsgType[ClosingSig]
  ...
  test("recv WatchFundingSpentTriggered (signed closing tx)", Tag(ChannelStateTestsTags.SimpleClose)) { f =>
    ...
    alice2bob.forward(bob)
    val bobTx2 = alice2blockchain.expectMsgType[PublishFinalTx].tx
    alice2blockchain.expectWatchTxConfirmed(bobTx2.txid)
    val aliceTx2 = bob2blockchain.expectMsgType[PublishFinalTx].tx
    bob2blockchain.expectWatchTxConfirmed(aliceTx2.txid)
    assert(alice2blockchain.expectMsgType[PublishFinalTx].tx.txid == aliceTx2.txid)
    alice2blockchain.expectWatchTxConfirmed(aliceTx2.txid)
    assert(bob2blockchain.expectMsgType[PublishFinalTx].tx.txid == bobTx2.txid)
    bob2blockchain.expectWatchTxConfirmed(bobTx2.txid)
