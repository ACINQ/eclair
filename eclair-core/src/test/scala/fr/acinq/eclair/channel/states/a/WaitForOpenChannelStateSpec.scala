...
class WaitForOpenChannelStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with StateTestsHelperMethods {
  ...
  test("recv OpenChannel (funding over channel limit)") { f =>
    ...
    val error = bob2alice.expectMsgType[Error]
    assert(error.toAscii === Error(open.temporaryChannelId, InvalidFundingAmount(open.temporaryChannelId, highFunding, Bob.nodeParams.minFundingSatoshis, Bob.nodeParams.maxFundingSatoshis).getMessage).toAscii)
    awaitCond(bob.stateName == CLOSED)
  }
  ...
  test("recv OpenChannel (reserve below dust, zero-reserve channel)", Tag("zero_reserve")) { f =>
    import f._
    val open = alice2bob.expectMsgType[OpenChannel]
    assert(open.channelReserveSatoshis === Satoshi(0))
    alice2bob.forward(bob)
    awaitCond(bob.stateName == WAIT_FOR_FUNDING_CREATED)
  }
