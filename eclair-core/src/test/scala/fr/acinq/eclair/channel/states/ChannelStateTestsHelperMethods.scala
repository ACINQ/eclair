...
trait ChannelStateTestsBase extends Assertions with Eventually {
  ...
  def reachNormal(setup: SetupFixture, tags: Set[String] = Set.empty): Transaction = {
    ...
    val commitTxFeerate = if (tags.contains(ChannelStateTestsTags.AnchorOutputs) || tags.contains(ChannelStateTestsTags.AnchorOutputsZeroFeeHtlcTxs) || tags.contains(ChannelStateTestsTags.OptionSimpleTaprootStaging)) {
  ...
  def remoteClose(rCommitTx: Transaction, s: TestFSMRef[ChannelState, ChannelData, Channel], s2blockchain: TestProbe): RemoteCommitPublished = {
    ...
    val anchorTx_opt = closingData.commitments.params.commitmentFormat match {
      case _: AnchorOutputsCommitmentFormat | SimpleTaprootChannelCommitmentFormat => Some(s2blockchain.expectMsgType[PublishReplaceableTx])
