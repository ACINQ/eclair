...
trait ChannelStateTestsBase extends Assertions with Eventually {
    ...
    val (publishedHtlcSuccessTxs, publishedHtlcTimeoutTxs) = closingState.commitments.params.commitmentFormat match {
      ...
      case _: Transactions.AnchorOutputsCommitmentFormat | _: SimpleTaprootChannelCommitmentFormat =>
        ...
        val successTxs = publishedHtlcTxs.collect { case tx: ReplaceableHtlcSuccess =>
          assert(localCommitPublished.htlcs.get(tx.txInfo.input.outPoint).contains(IncomingHtlcId(tx.txInfo.htlcId)))
          tx.txInfo.tx
        }
        val timeoutTxs = publishedHtlcTxs.collect { case tx: ReplaceableHtlcTimeout =>
          assert(localCommitPublished.htlcs.get(tx.txInfo.input.outPoint).contains(OutgoingHtlcId(tx.txInfo.htlcId)))
          tx.txInfo.tx
        }
        (successTxs, timeoutTxs)
    }
  ...
  def remoteClose(rCommitTx: Transaction, s: TestFSMRef[ChannelState, ChannelData, Channel], s2blockchain: TestProbe, htlcSuccessCount: Int = 0, htlcTimeoutCount: Int = 0): (RemoteCommitPublished, PublishedForceCloseTxs) = {
    ...
    val publishedHtlcSuccessTxs = publishedClaimHtlcTxs.map(_.tx).collect { case tx: ReplaceableClaimHtlcSuccess =>
      assert(remoteCommitPublished.htlcs.get(tx.txInfo.input.outPoint).contains(OutgoingHtlcId(tx.txInfo.htlcId)))
      tx.txInfo.tx
    }
    assert(publishedHtlcSuccessTxs.size == htlcSuccessCount)
    val publishedHtlcTimeoutTxs = publishedClaimHtlcTxs.map(_.tx).collect { case tx: ReplaceableClaimHtlcTimeout =>
      assert(remoteCommitPublished.htlcs.get(tx.txInfo.input.outPoint).contains(IncomingHtlcId(tx.txInfo.htlcId)))
      tx.txInfo.tx
    }
    assert(publishedHtlcTimeoutTxs.size == htlcTimeoutCount)
