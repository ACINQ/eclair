...
class ClosingStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv GetTxResponse (funder, tx found)", Tag("funding_unconfirmed")) { f =>
    ...
    alice2blockchain.expectWatchOutputsSpent(Seq(claimMain.txIn.head.outPoint, anchorTx.input.outPoint))
    eventListener.expectMsgType[ChannelAborted]
  ...
  test("recv GetTxResponse (fundee, tx found)", Tag("funding_unconfirmed")) { f =>
    ...
    bob2blockchain.expectWatchOutputsSpent(Seq(claimMain.txIn.head.outPoint, anchorTx.input.outPoint))
    eventListener.expectMsgType[ChannelAborted]
  ...
  test("recv CMD_ADD_HTLC") { f =>
    ...
    val sender = TestProbe()
    val add = CMD_ADD_HTLC(sender.ref, 500000000 msat, ByteVector32(ByteVector.fill(32)(1)), cltvExpiry = CltvExpiry(300000), onion = TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv WatchFundingSpentTriggered (mutual close before converging)") { f =>
    testMutualCloseBeforeConverge(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }
  ...
  private def extractPreimageFromClaimHtlcSuccess(f: FixtureParam): Unit = {
    ...
    bob.commitments.latest.commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat =>
        // We don't need a dedicated anchor transaction, we'll use our main output instead.
        bob2blockchain.expectReplaceableTxPublished[ClaimRemoteMainOutputTx]
    }
  ...
  test("recv WatchOutputSpentTriggered (extract preimage from Claim-HTLC-success tx, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    extractPreimageFromClaimHtlcSuccess(f)
  }

  test("recv WatchOutputSpentTriggered (extract preimage from Claim-HTLC-success tx, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    extractPreimageFromClaimHtlcSuccess(f)
  }
  ...
  test("recv WatchOutputSpentTriggered (extract preimage from HTLC-success tx, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    extractPreimageFromHtlcSuccess(f)
  }

  test("recv WatchOutputSpentTriggered (extract preimage from HTLC-success tx, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    extractPreimageFromHtlcSuccess(f)
  }
  ...
  private def extractPreimageFromRemovedHtlc(f: FixtureParam): Unit = {
    ...
    alice ! WatchFundingSpentTriggered(rcp.commitTx)
    val (mainTx_opt, anchorTx_opt) = alice.commitments.latest.commitmentFormat match {
      case _: AnchorOutputsCommitmentFormat | _: SimpleTaprootChannelCommitmentFormat =>
        val anchorTx = alice2blockchain.expectReplaceableTxPublished[ClaimRemoteAnchorTx].tx
        val mainTx = alice2blockchain.expectFinalTxPublished("remote-main").tx
        (Some(mainTx), Some(anchorTx))
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat =>
        val mainTx = alice2blockchain.expectReplaceableTxPublished[ClaimRemoteMainOutputTx].tx
        (Some(mainTx), None)
    }
    ...
    mainTx_opt.foreach(tx => alice2blockchain.expectWatchOutputSpent(tx.txIn.head.outPoint))
    anchorTx_opt.foreach(tx => alice2blockchain.expectWatchOutputSpent(tx.txIn.head.outPoint))
  ...
  test("recv WatchOutputSpentTriggered (extract preimage for removed HTLC, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    extractPreimageFromRemovedHtlc(f)
  }

  test("recv WatchOutputSpentTriggered (extract preimage for removed HTLC, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    extractPreimageFromRemovedHtlc(f)
  }
  ...
  test("recv CMD_BUMP_FORCE_CLOSE_FEE (local commit)") { f =>
    import f._

    val (localCommitPublished, _) = localClose(alice, alice2blockchain)
    val initialState = alice.stateData.asInstanceOf[DATA_CLOSING]
  ...
  test("recv WatchTxConfirmedTriggered (local commit)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
  ...
  test("recv WatchTxConfirmedTriggered (local commit, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    testLocalCommitTxConfirmed(f, ZeroFeeCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (local commit, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testLocalCommitTxConfirmed(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  test("recv WatchTxConfirmedTriggered (local commit with fulfill only signed by local)") { f =>
    ...
    val aliceCommitTx = alice.signCommitTx()
    assert(aliceCommitTx.txOut.size == 5) // 2 main outputs + 2 anchor outputs + 1 htlc
  ...
  test("recv WatchTxConfirmedTriggered (local commit with fail not acked by remote)") { f =>
    ...
    // then we make alice unilaterally close the channel
    val (closingState, closingTxs) = localClose(alice, alice2blockchain)
    assert(closingState.commitTx.txOut.length == 4) // htlc has been removed (only main outputs and anchor outputs)
  ...
  test("recv WatchTxConfirmedTriggered (local commit followed by htlc settlement)") { f =>
    import f._
  ...
  test("recv INPUT_RESTORED (local commit)") { f =>
    ...
    assert(mainFeerate <= FeeratePerKw(15_000 sat))
    assert(alice2blockchain.expectReplaceableTxPublished[HtlcTimeoutTx].input.outPoint == htlcTimeoutTx.txIn.head.outPoint)
    ...
    closingTxs.anchorTx_opt.foreach(tx => alice2blockchain.expectWatchOutputSpent(tx.txIn.head.outPoint))
  ...
  test("recv INPUT_RESTORED (local commit with htlc-delayed transactions)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    import f._
  ...
  test("recv INPUT_RESTORED (htlcs claimed by both local and remote)") { f =>
    import f._
  ...
  test("recv WatchFundingSpentTriggered (remote commit)") { f =>
    ...
    assert(bobCommitTx.txOut.size == 4) // two main outputs and two anchor outputs
    val (closingState, _) = remoteClose(bobCommitTx, alice, alice2blockchain)
    assert(closingState.htlcOutputs.isEmpty)
  ...
  private def zeroFeeRemoteCommitWithSmallBalanceAndHtlcPending(f: FixtureParam): Unit = {
    import f._

    // Alice sends almost all of her balance to Bob.
    val aliceInitialBalance = alice.commitments.availableBalanceForSend
    addHtlc(aliceInitialBalance, alice, bob, alice2bob, bob2alice)
    crossSign(alice, bob, alice2bob, bob2alice)

    // The feerate is quite high for fast inclusion in the blockchain.
    alice.nodeParams.setBitcoinCoreFeerates(FeeratesPerKw.single(FeeratePerKw(20_000 sat)).copy(minimum = FeeratePerKw(253 sat), slow = FeeratePerKw(500 sat)))
    // Bob force-closes while the HTLC is pending.
    val bobCommitTx = bob.signCommitTx()
    alice ! WatchFundingSpentTriggered(bobCommitTx)
    // Alice claims the remote anchor, because her main output is too small to be used for CPFP.
    // She has a pending outgoing HTLC, which means some funds are at risk.
    val anchorTx = alice2blockchain.expectReplaceableTxPublished[ClaimRemoteAnchorTx]
    val mainTx = alice2blockchain.expectFinalTxPublished("remote-main")
    val htlcTimeoutTx = alice2blockchain.expectReplaceableTxPublished[ClaimHtlcTimeoutTx]
    alice2blockchain.expectWatchTxConfirmed(bobCommitTx.txid)
    alice2blockchain.expectWatchOutputsSpent(Seq(anchorTx.input.outPoint, mainTx.input, htlcTimeoutTx.input.outPoint))
    alice2blockchain.expectNoMessage(100 millis)
  }

  test("recv WatchFundingSpentTriggered (remote commit, zero-fee commitments with small balance)", Tag(ChannelStateTestsTags.ZeroFeeCommitments), Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    zeroFeeRemoteCommitWithSmallBalanceAndHtlcPending(f)
  }

  test("recv WatchFundingSpentTriggered (remote commit, taproot zero-fee commitments with small balance)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments), Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    zeroFeeRemoteCommitWithSmallBalanceAndHtlcPending(f)
  }

  test("recv CMD_BUMP_FORCE_CLOSE_FEE (remote commit)") { f =>
    import f._
  ...
  test("recv WatchTxConfirmedTriggered (remote commit, anchor outputs zero fee htlc txs)") { f =>
    import f._
    mutualClose(alice, bob, alice2bob, bob2alice, alice2blockchain, bob2blockchain)
    val initialState = alice.stateData.asInstanceOf[DATA_CLOSING]
  ...
  def testRemoteCommitTxWithHtlcsConfirmed(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    ...
    commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(bobCommitTx.txOut.length == 6) // two main outputs + one anchors + 3 HTLCs
    }
  ...
  test("recv WatchTxConfirmedTriggered (remote commit with multiple htlcs for the same payment, anchor outputs zero fee htlc txs)") { f =>
    testRemoteCommitTxWithHtlcsConfirmed(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (remote commit with multiple htlcs for the same payment, anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
  ...
  test("recv WatchTxConfirmedTriggered (remote commit with multiple htlcs for the same payment, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    testRemoteCommitTxWithHtlcsConfirmed(f, ZeroFeeCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (remote commit with multiple htlcs for the same payment, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testRemoteCommitTxWithHtlcsConfirmed(f, TaprootZeroFeeCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (remote commit) followed by htlc settlement") { f =>
    import f._
    // Bob sends 2 HTLCs to Alice that will be settled during the force-close: one will be fulfilled, the other will be failed.
  ...
  test("recv INPUT_RESTORED (remote commit)") { f =>
    ...
    closingTxs.mainTx_opt.foreach(tx => {
      val tx2 = alice2blockchain.expectFinalTxPublished("remote-main")
      assert(tx2.tx.txIn.head.outPoint == tx.txIn.head.outPoint)
    ...
    alice2blockchain.expectWatchOutputsSpent(Seq(closingTxs.mainTx_opt.get.txIn.head.outPoint, htlcTimeout.input.outPoint))
  }

  private def restartWithZeroFeeRemoteCommit(f: FixtureParam): Unit = {
    import f._

    val bobCommitTx = bob.signCommitTx()
    val (_, closingTxs) = remoteClose(bobCommitTx, alice, alice2blockchain)
    // Alice uses her main output to bump the commit fees instead of using a dedicated anchor transaction.
    assert(closingTxs.anchorTx_opt.isEmpty)
    // The remote commitment confirms independently (e.g. Bob spent the anchor output with his wallet inputs).
    alice ! WatchTxConfirmedTriggered(BlockHeight(0), 0, bobCommitTx)
    ...
    alice ! INPUT_RESTORED(beforeRestart)
    alice2blockchain.expectMsgType[SetChannelId]
    // We don't need to spend the anchor output anymore with our main transaction: it is a simple standalone transaction.
    val mainTx = alice2blockchain.expectFinalTxPublished("remote-main")
    assert(mainTx.input == closingTxs.mainTx_opt.get.txIn.head.outPoint)
    alice2blockchain.expectWatchOutputSpent(mainTx.input)
    alice2blockchain.expectNoMessage(100 millis)
  }

  test("recv INPUT_RESTORED (remote commit, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    restartWithZeroFeeRemoteCommit(f)
  }

  test("recv INPUT_RESTORED (remote commit, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    restartWithZeroFeeRemoteCommit(f)
  }
  ...
  test("recv WatchTxConfirmedTriggered (next remote commit, anchor outputs zero fee htlc txs)") { f =>
    import f._
  ...
  test("recv WatchTxConfirmedTriggered (next remote commit) followed by htlc settlement") { f =>
    import f._
    // Bob sends 2 HTLCs to Alice that will be settled during the force-close: one will be fulfilled, the other will be failed.
  ...
  test("recv INPUT_RESTORED (next remote commit)") { f =>
    import f._
    ...
    // then we should re-publish unconfirmed transactions
    val anchorTx = alice2blockchain.expectReplaceableTxPublished[ClaimRemoteAnchorTx]
    closingTxs.mainTx_opt.foreach(_ => alice2blockchain.expectFinalTxPublished("remote-main"))
  ...
  test("recv WatchTxConfirmedTriggered (future remote commit, anchor outputs zero fee htlc txs)") { f =>
    import f._
  ...
  test("recv INPUT_RESTORED (future remote commit)") { f =>
    ...
    // bob republishes his main transaction
    val mainTx = alice2blockchain.expectFinalTxPublished("remote-main")
    alice2blockchain.expectWatchTxConfirmed(bobCommitTx.txid)
    alice ! WatchTxConfirmedTriggered(BlockHeight(0), 0, bobCommitTx)
    alice ! WatchTxConfirmedTriggered(BlockHeight(0), 0, mainTx.tx)
  ...
  private def prepareRevokedClose(f: FixtureParam): RevokedCloseFixture = {
    import f._
    ...
    bob.commitments.latest.commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(bobCommit1.commitTx.txOut.size == 3) // 2 main outputs + 1 anchor
    }
    ...
    bob.commitments.latest.commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(bobCommit2.commitTx.txOut.size == 5)
    }
    ...
    bob.commitments.latest.commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(bobCommit3.commitTx.txOut.size == 7)
    }
    ...
    bob.commitments.latest.commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(bobCommit4.commitTx.txOut.size == 3)
    }

    RevokedCloseFixture(Seq(bobCommit1, bobCommit2, bobCommit3, bobCommit4), Seq(htlcAlice1, htlcAlice2), Seq(htlcBob1, htlcBob2))
  }

  case class RevokedCloseTxs(mainTx: Transaction, mainPenaltyTx: Transaction, htlcPenaltyTxs: Seq[Transaction])

  private def setupFundingSpentRevokedTx(f: FixtureParam, commitmentFormat: CommitmentFormat): (Transaction, RevokedCloseTxs) = {
    import f._

    val revokedCloseFixture = prepareRevokedClose(f)
    assert(alice.commitments.latest.commitmentFormat == commitmentFormat)
    ...
    val spentOutpoints = Seq(mainTx.input, mainPenaltyTx.input) ++ htlcPenaltyTxs.map(_.input)
    commitmentFormat match {
      ...
      case ZeroFeeCommitmentFormat | TaprootZeroFeeCommitmentFormat => assert(spentOutpoints.size == bobRevokedTx.txOut.size - 1) // we don't claim the anchor
    }
    ...
    (bobRevokedTx, RevokedCloseTxs(mainTx.tx, mainPenaltyTx.tx, htlcPenaltyTxs.map(_.tx)))
  }
  ...
  private def testFundingSpentRevokedTx(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    ...
    alice ! WatchTxConfirmedTriggered(BlockHeight(110), 2, closingTxs.mainTx)
    ...
    assert(closedAlice.closingAmount == closingTxs.mainTx.txOut.head.amount + closingTxs.mainPenaltyTx.txOut.head.amount + closingTxs.htlcPenaltyTxs.map(_.txOut.head.amount).sum)
  }
  ...
  test("recv WatchFundingSpentTriggered (one revoked tx, anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
  ...
  test("recv WatchFundingSpentTriggered (one revoked tx, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    testFundingSpentRevokedTx(f, ZeroFeeCommitmentFormat)
  }

  test("recv WatchFundingSpentTriggered (one revoked tx, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testFundingSpentRevokedTx(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  test("recv WatchFundingSpentTriggered (multiple revoked tx)", Tag(ChannelStateTestsTags.OptionSimpleTaprootPhoenix)) { f =>
    ...
    def broadcastBobRevokedTx(revokedTx: Transaction, htlcCount: Int, revokedCount: Int): RevokedCloseTxs = {
      ...
      RevokedCloseTxs(mainTx.tx, mainPenalty.tx, htlcPenaltyTxs.map(_.tx))
    }
    ...
    alice ! WatchTxConfirmedTriggered(BlockHeight(100), 2, closingTxs.mainTx)
  ...
  def testInputRestoredRevokedTx(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    ...
    alice2blockchain.expectMsgType[WatchFundingSpent]
    // then we should re-publish unconfirmed transactions
    alice2blockchain.expectFinalTxPublished("remote-main")
    ...
    alice2blockchain.expectWatchTxConfirmed(bobRevokedTx.txid)
    alice2blockchain.expectWatchOutputSpent(closingTxs.mainTx.txIn.head.outPoint)
  ...
  test("recv INPUT_RESTORED (one revoked tx, anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
  ...
  def testRevokedHtlcTxConfirmed(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    ...
    alice ! WatchTxConfirmedTriggered(BlockHeight(110), 1, mainTx.tx)
  ...
  test("recv WatchTxConfirmedTriggered (revoked htlc-success tx)") { f =>
    testRevokedHtlcTxConfirmed(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (revoked htlc-success tx, anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
  ...
  test("recv WatchTxConfirmedTriggered (revoked aggregated htlc tx)") { f =>
  ...
  test("recv WatchTxConfirmedTriggered (revoked aggregated htlc tx, zero-fee commitments)", Tag(ChannelStateTestsTags.ZeroFeeCommitments)) { f =>
    testRevokedAggregatedHtlcTxConfirmed(f, ZeroFeeCommitmentFormat)
  }

  test("recv WatchTxConfirmedTriggered (revoked aggregated htlc tx, taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testRevokedAggregatedHtlcTxConfirmed(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  def testInputRestoredRevokedHtlcTxConfirmed(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    ...
    // We re-publish closing transactions with a higher feerate.
    val mainTx2 = alice2blockchain.expectFinalTxPublished("remote-main")
  ...
  test("recv INPUT_RESTORED (revoked htlc transactions confirmed)") { f =>
  ...
  private def testRevokedTxConfirmed(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._
    assert(alice.commitments.latest.commitmentFormat == commitmentFormat)
    assert(bob.signCommitTx().txOut.size == 4)
    ...
    val (bobRevokedTx, htlcs1) = {
      ...
      crossSign(alice, bob, alice2bob, bob2alice)
      val bobCommitTx = bob.signCommitTx()
      assert(bobCommitTx.txOut.size == 6)
    ...
    val htlcs2 = {
      ...
      crossSign(bob, alice, bob2alice, alice2bob)
      assert(bob.signCommitTx().txOut.size == 7)
      Seq(htlc3, htlc4)
    }
  ...
  test("recv CMD_FORCECLOSE (max_closing_feerate override)") { f =>
    ...
    alice2blockchain.expectWatchTxConfirmed(commitTx.txid)
    alice2blockchain.expectWatchOutputsSpent(Seq(claimMain1.input, anchorTx.input.outPoint))
    ...
    alice2blockchain.expectWatchTxConfirmed(commitTx.txid)
    alice2blockchain.expectWatchOutputsSpent(Seq(claimMain2.input, anchorTx.input.outPoint))
