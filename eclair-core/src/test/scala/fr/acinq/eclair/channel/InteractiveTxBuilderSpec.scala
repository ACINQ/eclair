...
class InteractiveTxBuilderSpec extends TestKitBaseClass with AnyFunSuiteLike with BitcoindService with BeforeAndAfterAll {
  ...
  case class FixtureParams(fundingParamsA: InteractiveTxParams,
    ...
    def spawnTxBuilderAlice(wallet: OnChainWallet, fundingParams: InteractiveTxParams = fundingParamsA, lease_opt: Option[LiquidityAds.Lease] = None): ActorRef[InteractiveTxBuilder.Command] = system.spawnAnonymous(InteractiveTxBuilder(
      ByteVector32.Zeroes,
      nodeParamsA, fundingParams, channelParamsA,
      FundingTx(commitFeerate, firstPerCommitmentPointB, feeBudget_opt = None),
      0 msat, 0 msat, lease_opt,
      wallet))
    ...
    def spawnTxBuilderSpliceAlice(fundingParams: InteractiveTxParams, commitment: Commitment, wallet: OnChainWallet, lease_opt: Option[LiquidityAds.Lease] = None): ActorRef[InteractiveTxBuilder.Command] = system.spawnAnonymous(InteractiveTxBuilder(
      ByteVector32.Zeroes,
      nodeParamsA, fundingParams, channelParamsA,
      SpliceTx(commitment),
      0 msat, 0 msat, lease_opt,
    ...
    def spawnTxBuilderBob(wallet: OnChainWallet, fundingParams: InteractiveTxParams = fundingParamsB, lease_opt: Option[LiquidityAds.Lease] = None): ActorRef[InteractiveTxBuilder.Command] = system.spawnAnonymous(InteractiveTxBuilder(
      ByteVector32.Zeroes,
    ...
    def spawnTxBuilderSpliceBob(fundingParams: InteractiveTxParams, commitment: Commitment, wallet: OnChainWallet, lease_opt: Option[LiquidityAds.Lease] = None): ActorRef[InteractiveTxBuilder.Command] = system.spawnAnonymous(InteractiveTxBuilder(
      ByteVector32.Zeroes,
      nodeParamsB, fundingParams, channelParamsB,
      SpliceTx(commitment),
      0 msat, 0 msat, lease_opt,
  ...
  private def withFixture(fundingAmountA: Satoshi, utxosA: Seq[Satoshi], fundingAmountB: Satoshi, utxosB: Seq[Satoshi], targetFeerate: FeeratePerKw, dustLimit: Satoshi, lockTime: Long, requireConfirmedInputs: RequireConfirmedInputs, lease_opt: Option[LiquidityAds.Lease] = None)(testFun: Fixture => Any): Unit = {
    ...
    val fixtureParams = createFixtureParams(fundingAmountA, fundingAmountB, targetFeerate, dustLimit, lockTime, requireConfirmedInputs, nonInitiatorPaysCommitTxFees = lease_opt.nonEmpty)
    val alice = fixtureParams.spawnTxBuilderAlice(walletA, lease_opt = lease_opt)
    val bob = fixtureParams.spawnTxBuilderBob(walletB, lease_opt = lease_opt)
  ...
  test("initiator does not contribute -- on-the-fly funding") {
    ...
    val lease = LiquidityAds.Lease(fundingB, LiquidityAds.LeaseFees(2500 sat, 7500 sat), LiquidityAds.PaymentDetails.FromFutureHtlc(Nil), ByteVector64.Zeroes, LiquidityAds.FundingLeaseWitness.Basic(hex"deadbeef"))
    withFixture(0 sat, Nil, fundingB, utxosB, targetFeerate, 330 sat, 0, RequireConfirmedInputs(forLocal = false, forRemote = false), Some(lease)) { f =>
      import f._
  ...
  test("fund splice transaction from non-initiator without change output") {
    ...
    withFixture(fundingA, utxosA, fundingB, utxosB, targetFeerate, 660 sat, 0, RequireConfirmedInputs(forLocal = false, forRemote = false)) { f =>
      ...
      val lease = LiquidityAds.Lease(50_000 sat, LiquidityAds.LeaseFees(1000 sat, 1500 sat), LiquidityAds.PaymentDetails.FromChannelBalance, ByteVector64.Zeroes, LiquidityAds.FundingLeaseWitness.Basic(hex"deadbeef"))
      ...
      val fundingParamsA1 = spliceFixtureParams.fundingParamsA
      val fundingParamsB1 = spliceFixtureParams.fundingParamsB
      val aliceSplice = fixtureParams.spawnTxBuilderSpliceAlice(fundingParamsA1, commitmentA1, walletA, lease_opt = Some(lease))
      val bobSplice = fixtureParams.spawnTxBuilderSpliceBob(fundingParamsB1, commitmentB1, walletB, lease_opt = Some(lease))
      val fwdSplice = TypeCheckedForwarder(aliceSplice, bobSplice, alice2bob, bob2alice)
      ...
      assert(commitmentA2.localCommit.spec.toLocal == commitmentA1.localCommit.spec.toLocal - spliceFeeA - lease.fees.total)
      assert(commitmentB2.localCommit.spec.toLocal == commitmentB1.localCommit.spec.toLocal + fundingB + lease.fees.total)
      assert(targetFeerate * 0.9 <= spliceTxA1.feerate && spliceTxA1.feerate <= targetFeerate * 1.25)
      walletA.publishTransaction(spliceTxA1.signedTx).pipeTo(probe.ref)
      probe.expectMsg(spliceTxA1.txId)
    }
  ...
  test("invalid funding contributions for liquidity purchase") {
    val probe = TestProbe()
    val wallet = new SingleKeyOnChainWallet()
    val lease = LiquidityAds.Lease(500_000 sat, LiquidityAds.LeaseFees(5000 sat, 20_000 sat), LiquidityAds.PaymentDetails.FromChannelBalance, ByteVector64.Zeroes, LiquidityAds.FundingLeaseWitness.Basic(hex"deadbeef"))
    ...
    val bob = params.spawnTxBuilderBob(wallet, params.fundingParamsB, Some(lease))
    bob ! Start(probe.ref)
    ...
    val bobSplice = params.spawnTxBuilderSpliceBob(spliceParams, previousCommitment, wallet, Some(lease))
    ...
    val bobFutureHtlc = params.spawnTxBuilderBob(wallet, params.fundingParamsB, Some(lease.copy(paymentDetails = LiquidityAds.PaymentDetails.FromFutureHtlc(Nil))))
    ...
    val bobFutureHtlcWithBalance = params.spawnTxBuilderSpliceBob(spliceParams, previousCommitment, wallet, Some(lease.copy(fees = LiquidityAds.LeaseFees(1000 sat, 4000 sat), paymentDetails = LiquidityAds.PaymentDetails.FromFutureHtlc(Nil))))
