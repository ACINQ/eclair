...
class NormalQuiescentStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  private def initiateQuiescence(f: FixtureParam, sendInitialStfu: Boolean): TestProbe = {
    ...
    val cmd = CMD_SPLICE(sender.ref, spliceIn_opt = Some(SpliceIn(500_000 sat)), spliceOut_opt = Some(SpliceOut(100_000 sat, scriptPubKey)), requestFunding_opt = None, commitmentFormat_opt = None)
    alice ! cmd
  ...
  test("send stfu after pending local changes have been added") { f =>
    ...
    alice ! CMD_SPLICE(TestProbe().ref, spliceIn_opt = Some(SpliceIn(50_000 sat)), spliceOut_opt = None, requestFunding_opt = None, commitmentFormat_opt = None)
  ...
  test("initiate quiescence concurrently (no pending changes)") { f =>
    ...
    val sender = TestProbe()
    val cmd = CMD_SPLICE(sender.ref, spliceIn_opt = Some(SpliceIn(500_000 sat, pushAmount = 0 msat)), spliceOut_opt = None, requestFunding_opt = None, commitmentFormat_opt = None)
    alice ! cmd
    alice2bob.expectMsgType[Stfu]
    bob ! cmd
    bob2alice.expectMsgType[Stfu]
