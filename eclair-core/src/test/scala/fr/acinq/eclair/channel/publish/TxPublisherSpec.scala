...
import fr.acinq.eclair.transactions.Transactions.{ClaimLocalAnchorOutputTx, HtlcSuccessTx, InputInfo, InputSpendingInfo}
...
class TxPublisherSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("publish replaceable tx") { f =>
    ...
    val cmd = PublishReplaceableTx(ClaimLocalAnchorOutputTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, Nil, 0), confirmBefore), null, null)
    txPublisher ! cmd
  ...
  test("publish replaceable tx duplicate") { f =>
    ...
    val input = OutPoint(randomTxId(), 3)
    val anchorTx = ClaimLocalAnchorOutputTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, Nil, 0), ConfirmationTarget.Priority(ConfirmationPriority.Medium))
  ...
  test("stop publishing attempts when transaction confirms") { f =>
    ...
    val cmd3 = PublishReplaceableTx(ClaimLocalAnchorOutputTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, TxOut(20_000 sat, Nil) :: Nil, 0), ConfirmationTarget.Absolute(nodeParams.currentBlockHeight)), null, null)
    txPublisher ! cmd3
  ...
  test("publishing attempt fails (wallet input gone)") { f =>
    ...
    val cmd2 = PublishReplaceableTx(ClaimLocalAnchorOutputTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, TxOut(20_000 sat, Nil) :: Nil, 0), ConfirmationTarget.Absolute(nodeParams.currentBlockHeight)), null, null)
    txPublisher ! cmd2
  ...
  test("publishing attempt fails (not enough funds)") { f =>
    ...
    val cmd = PublishReplaceableTx(HtlcSuccessTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, Nil, 0), paymentHash, 3, ConfirmationTarget.Absolute(target)), null, null)
    txPublisher ! cmd
  ...
  test("publishing attempt fails (unconfirmed conflicting replaceable transaction)") { f =>
    ...
    val input = OutPoint(randomTxId(), 7)
    val paymentHash = randomBytes32()
    val cmd = PublishReplaceableTx(HtlcSuccessTx(InputInfo(input, TxOut(25_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, TxIn(input, Nil, 0) :: Nil, Nil, 0), paymentHash, 3, ConfirmationTarget.Absolute(nodeParams.currentBlockHeight)), null, null)
    txPublisher ! cmd
    val attempt1 = factory.expectMsgType[ReplaceableTxPublisherSpawned]
    attempt1.actor.expectMsgType[ReplaceableTxPublisher.Publish]
