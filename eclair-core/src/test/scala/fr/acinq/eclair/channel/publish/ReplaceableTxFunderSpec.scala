...
class ReplaceableTxFunderSpec extends TestKitBaseClass with AnyFunSuiteLike {
  ...
  test("adjust previous anchor transaction outputs") {
    ...
    val TxOutputAdjusted(feerateUpdate1) = adjustPreviousTxOutput(FundedTx(previousAnchorTx, 12000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(5000 sat), commitment)
    assert(feerateUpdate1.txInfo.tx.txIn == previousAnchorTx.txInfo.tx.txIn)
    assert(feerateUpdate1.txInfo.tx.txOut.length == 1)
    val TxOutputAdjusted(feerateUpdate2) = adjustPreviousTxOutput(FundedTx(previousAnchorTx, 12000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(6000 sat), commitment)
    ...
    val AddWalletInputs(previousTx) = adjustPreviousTxOutput(FundedTx(previousAnchorTx, 12000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(10000 sat), commitment)
  ...
  test("adjust previous htlc transaction outputs", Tag("fuzzy")) {
    ...
    for (initialHtlcTx <- Seq(initialHtlcSuccess, initialHtlcTimeout)) {
      ...
      // We can handle a small feerate update by lowering the change output.
      val TxOutputAdjusted(feerateUpdate1) = adjustPreviousTxOutput(FundedTx(previousTx, 15000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(5000 sat), commitment)
      ...
      val TxOutputAdjusted(feerateUpdate2) = adjustPreviousTxOutput(FundedTx(previousTx, 15000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(6000 sat), commitment)
      ...
      val AddWalletInputs(tx) = adjustPreviousTxOutput(FundedTx(previousTxNoChange, 25000 sat, FeeratePerKw(2500 sat), Map.empty), FeeratePerKw(5000 sat), commitment)
      ...
      for (_ <- 1 to 100) {
        ...
        adjustPreviousTxOutput(FundedTx(fuzzyPreviousTx, amountIn, FeeratePerKw(2500 sat), Map.empty), targetFeerate, commitment) match {
  ...
  test("adjust previous claim htlc transaction outputs") {
    ...
    for (claimHtlc <- Seq(claimHtlcSuccess, claimHtlcTimeout)) {
      ...
      for (i <- 1 to 100) {
        val targetFeerate = FeeratePerKw(250 * i sat)
        adjustPreviousTxOutput(FundedTx(claimHtlc, claimHtlc.txInfo.amountIn, FeeratePerKw(2500 sat), Map.empty), targetFeerate, commitment) match {
