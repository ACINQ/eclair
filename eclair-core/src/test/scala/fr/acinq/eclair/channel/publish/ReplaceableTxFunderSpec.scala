...
class ReplaceableTxFunderSpec extends TestKitBaseClass with AnyFunSuiteLike {
  ...
  private def createAnchorTx(): (CommitTx, ClaimLocalAnchorOutputTx) = {
    ...
    val commitTx = Transaction(
      2,
      Seq(TxIn(commitInput.outPoint, Script.write(commitInput.spendingInfo.asInstanceOf[InputSpendingInfo.Segwit].output.redeemScript), 0, Scripts.witness2of2(PlaceHolderSig, PlaceHolderSig, PlaceHolderPubKey, PlaceHolderPubKey))),
      Seq(TxOut(330 sat, Script.pay2wsh(anchorScript))),
      0
    )
    val anchorTx = ClaimLocalAnchorOutputTx(
      InputInfo(OutPoint(commitTx, 0), commitTx.txOut.head, InputSpendingInfo.Segwit(anchorScript)),
  ...
  private def createHtlcTxs(): (Transaction, HtlcSuccessWithWitnessData, HtlcTimeoutWithWitnessData) = {
    ...
    val htlcSuccess = HtlcSuccessWithWitnessData(HtlcSuccessTx(
      InputInfo(OutPoint(commitTx, 0), commitTx.txOut.head, InputSpendingInfo.Segwit(htlcSuccessScript)),
    ...
    val htlcTimeout = HtlcTimeoutWithWitnessData(HtlcTimeoutTx(
      InputInfo(OutPoint(commitTx, 1), commitTx.txOut.last, InputSpendingInfo.Segwit(htlcTimeoutScript)),
