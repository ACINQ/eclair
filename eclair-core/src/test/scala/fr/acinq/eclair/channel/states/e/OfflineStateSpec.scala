...
class OfflineStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("re-send lost htlc and signature after first commitment", Tag(IgnoreChannelUpdates)) { f =>
    ...
    alice ! CMD_ADD_HTLC(sender.ref, 1000000 msat, ByteVector32.Zeroes, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.maxScore, None, localOrigin(sender.ref))
  ...
  test("re-send lost revocation", Tag(IgnoreChannelUpdates)) { f =>
    ...
    //   | X-- sig ---|
    val sender = TestProbe()
    alice ! CMD_ADD_HTLC(ActorRef.noSender, 1000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.maxScore, None, localOrigin(sender.ref))
    val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  test("broadcast disabled channel_update while offline") { f =>
    ...
    alice ! CMD_ADD_HTLC(sender.ref, 4200 msat, randomBytes32(), CltvExpiry(123456), TestConstants.emptyOnionPacket, None, Reputation.maxScore, None, localOrigin(sender.ref))
