...
class OfflineStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("re-send update+sig after first commitment") { f =>
    ...
    sender.send(alice, CMD_ADD_HTLC(1000000 msat, ByteVector32.Zeroes, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, Upstream.Local(UUID.randomUUID())))
  ...
  test("re-send lost revocation") { f =>
    import f._
    val sender = TestProbe()
    sender.send(alice, CMD_ADD_HTLC(1000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, Upstream.Local(UUID.randomUUID())))
    val ab_add_0 = alice2bob.expectMsgType[UpdateAddHtlc]
    // add ->b
  ...
  test("broadcast disabled channel_update while offline") { f =>
    ...
    sender.send(alice, CMD_ADD_HTLC(4200 msat, randomBytes32, CltvExpiry(123456), Sphinx.emptyOnionPacket, Upstream.Local(UUID.randomUUID())))
