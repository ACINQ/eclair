...
class WaitForOpenDualFundedChannelStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv OpenDualFundedChannel", Tag(ChannelStateTestsTags.DualFunding)) { f =>
    import f._

    val open = alice2bob.expectMsgType[OpenDualFundedChannel]
    assert(open.upfrontShutdownScript_opt === None)
  ...
  test("recv OpenDualFundedChannel (funding too low)", Tag(ChannelStateTestsTags.DualFunding)) { f =>
    import f._
    val open = alice2bob.expectMsgType[OpenDualFundedChannel]
