...
class CommitmentsSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("correct values for availableForSend/availableForReceive (success case)") { f =>
    ...
    assert(ac1.availableBalanceForSend == a - p - htlcOutputFee) // * 2 as soon as htlc is sent, alice sees its balance decrease (more than the payment amount because of the commitment fees)
