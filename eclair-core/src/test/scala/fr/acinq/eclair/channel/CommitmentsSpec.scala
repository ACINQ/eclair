...
class CommitmentsSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("correct values for availableForSend/availableForReceive (failure case)") { f =>
    ...
    val cmdFail = CMD_FAIL_HTLC(0, Right(IncorrectOrUnknownPaymentDetails(p, BlockHeight(42))), useAttributableErrors = false, TimestampMilli.now())
  ...
  test("correct values for availableForSend/availableForReceive (multiple htlcs)") { f =>
    ...
    val cmdFail2 = CMD_FAIL_HTLC(1, Right(IncorrectOrUnknownPaymentDetails(p2, BlockHeight(42))), useAttributableErrors = false, TimestampMilli.now())
