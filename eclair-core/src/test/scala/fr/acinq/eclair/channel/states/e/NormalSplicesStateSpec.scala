...
class NormalSplicesStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv CMD_SPLICE (splice-in, liquidity ads)", Tag(ChannelStateTestsTags.Quiescence)) { f =>
    ...
    // Bob signed a liquidity lease.
    bobPeer.fishForMessage() {
      case l: LiquidityLeaseSigned =>
        assert(l.lease.paymentDetails == LiquidityAds.PaymentDetails.FromChannelBalance)
  ...
  test("recv CMD_SPLICE (splice-in, liquidity ads, below minimum funding amount)", Tag(ChannelStateTestsTags.Quiescence)) { f =>
    ...
    assert(alice2bob.expectMsgType[SpliceInit].requestFunding_opt.nonEmpty)
    alice2bob.forward(bob)
    assert(bob2alice.expectMsgType[TxAbort].toAscii.contains("liquidity ads selected lease rate does not match a lease we offer"))
    bob2alice.forward(alice)
    alice2bob.expectMsgType[TxAbort]
    alice2bob.forward(bob)
  }

  test("recv CMD_SPLICE (splice-in, liquidity ads, invalid funding lease)", Tag(ChannelStateTestsTags.Quiescence)) { f =>
    import f._

    val sender = TestProbe()
    val fundingRequest = LiquidityAds.RequestFunding(100_000 sat, LiquidityAds.FundingLease.Basic(10_000 sat, 200_000 sat, 0, 0, 0 sat), LiquidityAds.PaymentDetails.FromChannelBalance)
    val cmd = CMD_SPLICE(sender.ref, Some(SpliceIn(500_000 sat)), None, Some(fundingRequest))
    alice ! cmd
