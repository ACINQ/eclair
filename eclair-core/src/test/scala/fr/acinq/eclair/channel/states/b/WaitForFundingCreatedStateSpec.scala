...
class WaitForFundingCreatedStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    within(30 seconds) {
      alice ! INPUT_INIT_CHANNEL_INITIATOR(ByteVector32.Zeroes, fundingSatoshis, dualFunded = false, TestConstants.feeratePerKw, TestConstants.feeratePerKw, Some(pushMsat), aliceParams, alice2bob.ref, bobInit, ChannelFlags.Private, channelConfig, channelType)
  ...
  test("recv FundingCreated (funder can't pay fees)", Tag("funder_below_reserve")) { f =>
    ...
    val reserve = bobParams.requestedChannelReserve
