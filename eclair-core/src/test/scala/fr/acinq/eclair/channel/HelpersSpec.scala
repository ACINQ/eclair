...
class HelpersSpec extends TestKitBaseClass with AnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("check closing tx amounts above dust") {
    ...
    def toClosingTx(txOut: Seq[TxOut]): ClosingTx = {
      ClosingTx(InputInfo(OutPoint(TxId(ByteVector32.Zeroes), 0), TxOut(1000 sat, Nil), InputSpendingInfo.Segwit(Nil)), Transaction(2, Nil, txOut, 0), None)
    }
  ...
  test("tell closing type") {
    ...
    ).map(tx => ClosingTx(InputInfo(tx.txIn.head.outPoint, TxOut(10_000 sat, Nil), InputSpendingInfo.Segwit(Nil)), tx, None))
