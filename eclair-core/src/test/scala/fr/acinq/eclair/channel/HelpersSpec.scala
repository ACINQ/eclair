...
class HelpersSpec extends TestKitBaseClass with AnyFunSuiteLike with ChannelStateTestsBase {
  ...
  def setupHtlcs(testTags: Set[String] = Set.empty): Fixture = {
    ...
    val aliceMppCmd = CMD_ADD_HTLC(TestProbe().ref, 15_000_000 msat, htlca1a.paymentHash, htlca1a.cltvExpiry, htlca1a.onionRoutingPacket, None, Reputation.maxScore, None, Origin.Hot(TestProbe().ref, Upstream.Local(UUID.randomUUID())))
    ...
    val bobMppCmd = CMD_ADD_HTLC(TestProbe().ref, 17_000_000 msat, htlcb1a.paymentHash, htlcb1a.cltvExpiry, htlcb1a.onionRoutingPacket, None, Reputation.maxScore, None, Origin.Hot(TestProbe().ref, Upstream.Local(UUID.randomUUID())))
