...
class HelpersSpec extends TestKitBaseClass with AnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("check closing tx amounts above dust") {
    ...
    def toClosingTx(txOut: Seq[TxOut]): ClosingTx = {
      ClosingTx(InputInfo(OutPoint(TxId(ByteVector32.Zeroes), 0), TxOut(1000 sat, Nil)), RedeemInfo.SegwitV0(Nil), Transaction(2, Nil, txOut, 0), None)
    }
  ...
  test("tell closing type") {
    ...
    val dummyRedeemInfo = RedeemInfo.SegwitV0(Nil)
    ...
    ).map(tx => ClosingTx(InputInfo(tx.txIn.head.outPoint, TxOut(10_000 sat, Nil)), dummyRedeemInfo, tx, None))
    ...
    assert(Closing.isClosingTypeAlreadyKnown(
      DATA_CLOSING(
        ...
        localCommitPublished = Some(LocalCommitPublished(
          commitTx = tx2.tx,
          claimMainDelayedOutputTx = Some(ClaimLocalDelayedOutputTx(tx3.input, dummyRedeemInfo, tx3.tx)),
          htlcTxs = Map.empty,
    ...
    assert(Closing.isClosingTypeAlreadyKnown(
      DATA_CLOSING(
        ...
        nextRemoteCommitPublished = Some(RemoteCommitPublished(
          commitTx = tx4.tx,
          claimMainOutputTx = Some(ClaimP2WPKHOutputTx(tx5.input, dummyRedeemInfo, tx5.tx)),
          claimHtlcTxs = Map.empty,
          claimAnchorTxs = Nil,
    ...
    assert(Closing.isClosingTypeAlreadyKnown(
      DATA_CLOSING(
        ...
        futureRemoteCommitPublished = Some(RemoteCommitPublished(
          commitTx = tx4.tx,
          claimMainOutputTx = Some(ClaimRemoteDelayedOutputTx(tx5.input, dummyRedeemInfo, tx5.tx)),
          claimHtlcTxs = Map.empty,
          claimAnchorTxs = Nil,
          irrevocablySpent = Map.empty
        )),
    ...
    assert(Closing.isClosingTypeAlreadyKnown(
      DATA_CLOSING(
        ...
        revokedCommitPublished =
          RevokedCommitPublished(
            commitTx = tx2.tx,
            claimMainOutputTx = Some(ClaimP2WPKHOutputTx(tx3.input, dummyRedeemInfo, tx3.tx)),
            mainPenaltyTx = None,
            htlcPenaltyTxs = Nil,
