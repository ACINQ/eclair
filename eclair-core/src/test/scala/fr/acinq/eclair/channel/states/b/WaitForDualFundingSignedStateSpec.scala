...
class WaitForDualFundingSignedStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    within(30 seconds) {
      ...
      alice ! initChannelInitiator(TestConstants.fundingSatoshis, aliceChannelParams, aliceCommitParams, bobCommitParams, channelType, dualFunded = true, requestFunding_opt = requestFunding_opt, pushAmount_opt = initiatorPushAmount)
      bob ! initChannelNonInitiator(bobChannelParams, bobCommitParams, aliceCommitParams, channelType, dualFunded = true, fundingContribution_opt = bobContribution, pushAmount_opt = nonInitiatorPushAmount)
