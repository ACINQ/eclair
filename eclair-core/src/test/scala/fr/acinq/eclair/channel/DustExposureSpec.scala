...
class DustExposureSpec extends AnyFunSuiteLike {
  ...
  def createHtlc(id: Long, amount: MilliSatoshi): UpdateAddHtlc = {
    UpdateAddHtlc(ByteVector32.Zeroes, id, amount, randomBytes32(), CltvExpiry(500), TestConstants.emptyOnionPacket, None, accountable = false, None)
  }
  ...
  test("filter incoming htlcs before forwarding") {
    ...
    assert(DustExposure.computeExposure(initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat) == 0.msat)
    assert(DustExposure.contributesToDustExposure(IncomingHtlc(createHtlc(0, 9000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    assert(DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 9000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    ...
    assert(DustExposure.contributesToDustExposure(IncomingHtlc(createHtlc(0, 9500.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 9500.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 10000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 10000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat))
    ...
    assert(DustExposure.computeExposure(updatedSpec, dustLimit, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat) == 18500.sat.toMilliSatoshi)
    ...
    val (accepted, rejected) = DustExposure.filterBeforeForward(25000 sat, updatedSpec, dustLimit, 10000.sat.toMilliSatoshi, initialSpec, dustLimit, 15000.sat.toMilliSatoshi, receivedHtlcs, Transactions.UnsafeLegacyAnchorOutputsCommitmentFormat)
