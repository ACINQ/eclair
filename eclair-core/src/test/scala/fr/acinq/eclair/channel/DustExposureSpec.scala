...
class DustExposureSpec extends AnyFunSuiteLike {
  ...
  test("filter incoming htlcs before forwarding") {
    val dustLimit = 10_000.sat
    val initialSpec = CommitmentSpec(Set.empty, FeeratePerKw(10000 sat), 0 msat, 0 msat)
    assert(DustExposure.computeExposure(initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat) == 0.msat)
    assert(DustExposure.contributesToDustExposure(IncomingHtlc(createHtlc(0, 9000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    assert(DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 9000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    ...
    assert(DustExposure.contributesToDustExposure(IncomingHtlc(createHtlc(0, 9500.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 9500.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(IncomingHtlc(createHtlc(0, 10000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    assert(!DustExposure.contributesToDustExposure(OutgoingHtlc(createHtlc(0, 10000.sat.toMilliSatoshi)), initialSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat))
    ...
    assert(DustExposure.computeExposure(updatedSpec, dustLimit, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat) == 18500.sat.toMilliSatoshi)
    ...
    val (accepted, rejected) = DustExposure.filterBeforeForward(25000 sat, updatedSpec, dustLimit, 10000.sat.toMilliSatoshi, initialSpec, dustLimit, 15000.sat.toMilliSatoshi, receivedHtlcs, Transactions.ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
