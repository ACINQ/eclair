...
class NormalStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv CMD_ADD_HTLC (incrementing ids)") { f =>
    ...
    val h = randomBytes32
    for (i <- 0 until 10) {
      sender.send(alice, CMD_ADD_HTLC(50000000 msat, h, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
      sender.expectMsg("ok")
      val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  test("recv CMD_ADD_HTLC (relayed htlc)") { f =>
    ...
    val cmd = CMD_ADD_HTLC(originHtlc.amountMsat - 10000.msat, h, originHtlc.cltvExpiry - CltvExpiryDelta(7), Sphinx.emptyOnionPacket, upstream = Right(originHtlc))
  ...
  test("recv CMD_ADD_HTLC (expiry too small)") { f =>
    ...
    val add = CMD_ADD_HTLC(500000000 msat, randomBytes32, expiryTooSmall, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (expiry too big)") { f =>
    ...
    val add = CMD_ADD_HTLC(500000000 msat, randomBytes32, expiryTooBig, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (value too small)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(50 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(MilliSatoshi(Int.MaxValue), randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds, missing 1 msat)") { f =>
    ...
    val add = CMD_ADD_HTLC(initialState.commitments.availableBalanceForSend + 1.msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(bob, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs and 0 balance)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    sender.send(alice, CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(200000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(67600000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(1000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs 2/2)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    sender.send(alice, CMD_ADD_HTLC(300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (over max inflight htlc value)") { f =>
    ...
    val sender = TestProbe()
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(151000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(bob, add)
  ...
  test("recv CMD_ADD_HTLC (over max accepted htlcs)") { f =>
    ...
    for (i <- 0 until 30) {
      sender.send(alice, CMD_ADD_HTLC(10000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
      sender.expectMsg("ok")
      alice2bob.expectMsgType[UpdateAddHtlc]
    }
    val add = CMD_ADD_HTLC(10000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (over capacity)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add1 = CMD_ADD_HTLC(TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    ...
    val add2 = CMD_ADD_HTLC(TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
  ...
  test("recv CMD_ADD_HTLC (after having received Shutdown)") { f =>
    ...
    val add1 = CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add1)
    sender.expectMsg("ok")
    ...
    val add2 = CMD_ADD_HTLC(100000000 msat, randomBytes32, CltvExpiry(300000), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    // messages cross
    alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  val b2a_2 = bobMinOffer + 10.sat // will be only be in bob tx
    ...
    sender.send(alice, CMD_ADD_HTLC(a2b_1.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    sender.send(alice, CMD_ADD_HTLC(a2b_2.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    sender.send(bob, CMD_ADD_HTLC(b2a_1.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
    sender.send(bob, CMD_ADD_HTLC(b2a_2.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
