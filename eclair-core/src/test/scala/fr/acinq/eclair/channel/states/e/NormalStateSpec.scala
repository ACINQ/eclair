...
class NormalStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with StateTestsHelperMethods {
  ...
  test("recv CurrentBlockCount (fulfilled signed htlc ignored by upstream peer)") { f =>
    ...
    assert(Bob.nodeParams.fulfillSafetyBeforeTimeout.min.toInt < 9 && 9 < Bob.nodeParams.fulfillSafetyBeforeTimeout.max.toInt)
  ...
  test("recv CurrentBlockCount (fulfilled proposed htlc ignored by upstream peer)") { f =>
    ...
    sender.expectMsg(ChannelCommandResponse.Ok)
    bob2alice.expectMsgType[UpdateFulfillHtlc]
    sender.send(bob, CurrentBlockCount((htlc.cltvExpiry - Bob.nodeParams.fulfillSafetyBeforeTimeout.min).toLong))
