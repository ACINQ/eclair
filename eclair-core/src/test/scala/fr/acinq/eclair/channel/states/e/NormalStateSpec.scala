...
class NormalStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv CMD_ADD_HTLC (relayed htlc)") { f =>
    ...
    val origin = Origin.Hot(sender.ref, Upstream.Hot.Channel(originHtlc, TimestampMilli.now(), randomKey().publicKey, 0.1, upgradeAccountability = false))
  ...
  test("recv CMD_ADD_HTLC (trampoline relayed htlc)") { f =>
    ...
    val origin = Origin.Hot(sender.ref, Upstream.Hot.Trampoline(List(originHtlc1, originHtlc2).map(htlc => Upstream.Hot.Channel(htlc, TimestampMilli.now(), randomKey().publicKey, 0.1, upgradeAccountability = false))))
