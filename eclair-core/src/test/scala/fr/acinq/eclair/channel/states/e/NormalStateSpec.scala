...
class NormalStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv CMD_ADD_HTLC (empty origin)") { f =>
    ...
    val h = randomBytes32
    val add = CMD_ADD_HTLC(50000000 msat, h, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
  ...
  test("recv CMD_ADD_HTLC (incrementing ids)") { f =>
    ...
    val h = randomBytes32
    for (i <- 0 until 10) {
      sender.send(alice, CMD_ADD_HTLC(50000000 msat, h, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
      sender.expectMsg("ok")
      val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  test("recv CMD_ADD_HTLC (relayed htlc)") { f =>
    ...
    val sender = TestProbe()
    val h = randomBytes32
    val originHtlc = UpdateAddHtlc(channelId = randomBytes32, id = 5656, amountMsat = 50000000 msat, cltvExpiry = CltvExpiryDelta(144).toCltvExpiry, paymentHash = h, onionRoutingPacket = Sphinx.emptyOnionPacket)
  ...
  test("recv CMD_ADD_HTLC (value too small)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(50 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(MilliSatoshi(Int.MaxValue), randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds, missing 1 msat)") { f =>
    ...
    val add = CMD_ADD_HTLC(initialState.commitments.availableBalanceForSend + 1.msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(bob, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs and 0 balance)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    sender.send(alice, CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(200000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(67600000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(1000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs 2/2)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    sender.send(alice, CMD_ADD_HTLC(300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    sender.send(alice, CMD_ADD_HTLC(300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (over max inflight htlc value)") { f =>
    ...
    val sender = TestProbe()
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(151000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(bob, add)
  ...
  test("recv CMD_ADD_HTLC (over max accepted htlcs)") { f =>
    ...
    for (i <- 0 until 30) {
      sender.send(alice, CMD_ADD_HTLC(10000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
      sender.expectMsg("ok")
      alice2bob.expectMsgType[UpdateAddHtlc]
    }
    val add = CMD_ADD_HTLC(10000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add)
  ...
  test("recv CMD_ADD_HTLC (over capacity)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add1 = CMD_ADD_HTLC(TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    ...
    val add2 = CMD_ADD_HTLC(TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
  ...
  test("recv CMD_ADD_HTLC (after having received Shutdown)") { f =>
    ...
    val add1 = CMD_ADD_HTLC(500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID()))
    sender.send(alice, add1)
    sender.expectMsg("ok")
  ...
  test("recv UpdateAddHtlc") { f =>
    ...
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, 150000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket)
    bob ! htlc
  ...
  test("recv UpdateAddHtlc (unexpected id)") { f =>
    ...
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 42, 150000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket)
    bob ! htlc.copy(id = 0)
    bob ! htlc.copy(id = 1)
    bob ! htlc.copy(id = 2)
    bob ! htlc.copy(id = 3)
  ...
  test("recv UpdateAddHtlc (value too small)") { f =>
    import f._
    val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, 150 msat, randomBytes32, cltvExpiry = CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket)
  ...
  test("recv UpdateAddHtlc (insufficient funds)") { f =>
    import f._
    val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, MilliSatoshi(Long.MaxValue), randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket)
    alice2bob.forward(bob, htlc)
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (insufficient funds w/ pending htlcs 1/2)") { f =>
    import f._
    val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 0, 400000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 1, 200000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 2, 167600000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 3, 10000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (insufficient funds w/ pending htlcs 2/2)") { f =>
    import f._
    val tx = bob.stateData.asInstanceOf[DATA_NORMAL].commitments.localCommit.publishableTxs.commitTx.tx
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 0, 300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 1, 300000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 2, 500000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (over max inflight htlc value)") { f =>
    ...
    alice2bob.forward(alice, UpdateAddHtlc(ByteVector32.Zeroes, 0, 151000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
  ...
  test("recv UpdateAddHtlc (over max accepted htlcs)") { f =>
    ...
    // Bob accepts a maximum of 30 htlcs
    for (i <- 0 until 30) {
      alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, i, 1000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    }
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 30, 1000000 msat, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket))
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv CMD_SIGN (check htlc info are persisted)") { f =>
    ...
    sender.send(alice, CMD_ADD_HTLC(a2b_1.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    sender.send(alice, CMD_ADD_HTLC(a2b_2.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    sender.send(bob, CMD_ADD_HTLC(b2a_1.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
    sender.send(bob, CMD_ADD_HTLC(b2a_2.toMilliSatoshi, randomBytes32, CltvExpiryDelta(144).toCltvExpiry, Sphinx.emptyOnionPacket, upstream = Left(UUID.randomUUID())))
    sender.expectMsg("ok")
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
