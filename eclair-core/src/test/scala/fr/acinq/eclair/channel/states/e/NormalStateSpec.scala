...
class NormalStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  private def testRecvCmdAddHtlcEmptyOrigin(f: FixtureParam): Unit = {
    ...
    val h = randomBytes32()
    val add = CMD_ADD_HTLC(sender.ref, 50000000 msat, h, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (incrementing ids)") { f =>
    ...
    val h = randomBytes32()
    for (i <- 0 until 10) {
      alice ! CMD_ADD_HTLC(sender.ref, 500000 msat, h, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
      sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
      val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  test("recv CMD_ADD_HTLC (relayed htlc)") { f =>
    ...
    val sender = TestProbe()
    val h = randomBytes32()
    val originHtlc = UpdateAddHtlc(channelId = randomBytes32(), id = 5656, amountMsat = 50000000 msat, cltvExpiry = CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), paymentHash = h, onionRoutingPacket = TestConstants.emptyOnionPacket, pathKey_opt = None, accountable = false, fundingFee_opt = None)
    val origin = Origin.Hot(sender.ref, Upstream.Hot.Channel(originHtlc, TimestampMilli.now(), randomKey().publicKey, 0.1))
    val cmd = CMD_ADD_HTLC(sender.ref, originHtlc.amountMsat - 10_000.msat, h, originHtlc.cltvExpiry - CltvExpiryDelta(7), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, origin)
    alice ! cmd
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
    assert(htlc.id == 0 && htlc.paymentHash == h)
  ...
  test("recv CMD_ADD_HTLC (trampoline relayed htlc)") { f =>
    ...
    val sender = TestProbe()
    val h = randomBytes32()
    val originHtlc1 = UpdateAddHtlc(randomBytes32(), 47, 30000000 msat, h, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
    val originHtlc2 = UpdateAddHtlc(randomBytes32(), 32, 20000000 msat, h, CltvExpiryDelta(160).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
    ...
    val cmd = CMD_ADD_HTLC(sender.ref, originHtlc1.amountMsat + originHtlc2.amountMsat - 10000.msat, h, originHtlc2.cltvExpiry - CltvExpiryDelta(7), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, origin)
    alice ! cmd
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    val htlc = alice2bob.expectMsgType[UpdateAddHtlc]
    assert(htlc.id == 0 && htlc.paymentHash == h)
  ...
  test("recv CMD_ADD_HTLC (expiry too small)") { f =>
    ...
    val add = CMD_ADD_HTLC(sender.ref, 500000000 msat, randomBytes32(), expiryTooSmall, TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (value too small)") { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add = CMD_ADD_HTLC(sender.ref, 50 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (0 msat)") { f =>
    ...
    val add = CMD_ADD_HTLC(sender.ref, 0 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! add
  ...
  test("recv CMD_ADD_HTLC (HTLC dips into remote funder fee reserve)", Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    import f._
    val sender = TestProbe()
    addHtlc(772_000_000 msat, alice, bob, alice2bob, bob2alice)
    ...
    val htlcs = (1 to 9).map { _ =>
      bob ! CMD_ADD_HTLC(sender.ref, 12_000_000 msat, randomBytes32(), CltvExpiry(400144), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
      sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    ...
    val failedAdd = CMD_ADD_HTLC(sender.ref, 11_000_000 msat, randomBytes32(), CltvExpiry(400144), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! failedAdd
    val error = RemoteCannotAffordFeesForNewHtlc(channelId(bob), failedAdd.amount, missing = 200 sat, 20_000 sat, 8_200 sat)
  ...
  test("recv CMD_ADD_HTLC (HTLC dips into remote funder channel reserve)", Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    ...
    alice.underlyingActor.nodeParams.setBitcoinCoreFeerates(FeeratesPerKw.single(FeeratePerKw(3_500 sat)))
    bob.underlyingActor.nodeParams.setBitcoinCoreFeerates(FeeratesPerKw.single(FeeratePerKw(3_500 sat)))
    updateFee(FeeratePerKw(3_500 sat), alice, bob, alice2bob, bob2alice)

    // At this point alice has the minimal amount to sustain a channel.
    // Alice maintains an extra reserve to accommodate for a few more HTLCs, so the first few HTLCs should be allowed.
    val htlcs = (1 to 4).map { _ =>
      bob ! CMD_ADD_HTLC(sender.ref, 25_000_000 msat, randomBytes32(), CltvExpiry(400144), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
      sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
      val add = bob2alice.expectMsgType[UpdateAddHtlc]
      bob2alice.forward(alice, add)
      add
    ...
    // But this one will dip alice below her reserve: we must wait for the previous HTLCs to settle before sending any more.
    val failedAdd = CMD_ADD_HTLC(sender.ref, 25_000_000 msat, randomBytes32(), CltvExpiry(400144), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! failedAdd
    val error = RemoteCannotAffordFeesForNewHtlc(channelId(bob), failedAdd.amount, missing = 206 sat, 20_000 sat, 8_206 sat)
    sender.expectMsg(RES_ADD_FAILED(failedAdd, error, Some(bob.stateData.asInstanceOf[DATA_NORMAL].channelUpdate)))
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs and 0 balance)", Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    alice ! CMD_ADD_HTLC(sender.ref, 500_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice ! CMD_ADD_HTLC(sender.ref, 200_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice ! CMD_ADD_HTLC(sender.ref, 70_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(sender.ref, 1_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
    val error = InsufficientFunds(channelId(alice), amount = 1_000_000 msat, missing = 1580 sat, reserve = 20_000 sat, fees = 5_190 sat)
    sender.expectMsg(RES_ADD_FAILED(add, error, Some(initialState.channelUpdate)))
    alice2bob.expectNoMessage(100 millis)
  }
  ...
  test("recv CMD_ADD_HTLC (insufficient funds w/ pending htlcs 2/2)", Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    alice ! CMD_ADD_HTLC(sender.ref, 300_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice ! CMD_ADD_HTLC(sender.ref, 300_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    val add = CMD_ADD_HTLC(sender.ref, 500_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
    val error = InsufficientFunds(channelId(alice), amount = 500_000_000 msat, missing = 329_720 sat, reserve = 20_000 sat, fees = 4_760 sat)
    sender.expectMsg(RES_ADD_FAILED(add, error, Some(initialState.channelUpdate)))
    alice2bob.expectNoMessage(100 millis)
  }
  ...
  test("recv CMD_ADD_HTLC (over remote max inflight htlc value)", Tag(ChannelStateTestsTags.AliceLowMaxHtlcValueInFlight)) { f =>
    ...
    val add = CMD_ADD_HTLC(sender.ref, 151_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! add
  ...
  test("recv CMD_ADD_HTLC (over remote max inflight htlc value with duplicate amounts)", Tag(ChannelStateTestsTags.AliceLowMaxHtlcValueInFlight)) { f =>
    ...
    assert(initialState.commitments.latest.localCommitParams.maxHtlcValueInFlight == UInt64(initialState.commitments.latest.capacity.toMilliSatoshi.toLong))
    assert(initialState.commitments.latest.remoteCommitParams.maxHtlcValueInFlight == UInt64(150_000_000))
    val add = CMD_ADD_HTLC(sender.ref, 75_500_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! add
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    bob2alice.expectMsgType[UpdateAddHtlc]
    val add1 = CMD_ADD_HTLC(sender.ref, 75_500_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! add1
    val error = HtlcValueTooHighInFlight(channelId(bob), maximum = UInt64(150_000_000), actual = 151_000_000 msat)
  ...
  test("recv CMD_ADD_HTLC (over remote max accepted htlcs)") { f =>
    ...
    for (_ <- 0 until 30) {
      alice ! CMD_ADD_HTLC(sender.ref, 10_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
      sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
      alice2bob.expectMsgType[UpdateAddHtlc]
    }
    val add = CMD_ADD_HTLC(sender.ref, 10_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (over local max accepted htlcs)") { f =>
    ...
    for (_ <- 0 until 30) {
      bob ! CMD_ADD_HTLC(sender.ref, 500_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
      sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
      bob2alice.expectMsgType[UpdateAddHtlc]
    }
    val add = CMD_ADD_HTLC(sender.ref, 500_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    bob ! add
  ...
  test("recv CMD_ADD_HTLC (over max dust htlc exposure)") { f =>
    ...
    assert(Transactions.offeredHtlcTrimThreshold(aliceCommitments.latest.localCommitParams.dustLimit, aliceCommitments.latest.localCommit.spec, aliceCommitments.latest.commitmentFormat) == 1100.sat)
    assert(Transactions.receivedHtlcTrimThreshold(aliceCommitments.latest.localCommitParams.dustLimit, aliceCommitments.latest.localCommit.spec, aliceCommitments.latest.commitmentFormat) == 1100.sat)
    assert(Transactions.offeredHtlcTrimThreshold(aliceCommitments.latest.remoteCommitParams.dustLimit, aliceCommitments.latest.localCommit.spec, aliceCommitments.latest.commitmentFormat) == 1000.sat)
    assert(Transactions.receivedHtlcTrimThreshold(aliceCommitments.latest.remoteCommitParams.dustLimit, aliceCommitments.latest.localCommit.spec, aliceCommitments.latest.commitmentFormat) == 1000.sat)

    // Alice sends HTLCs to Bob that add 10 000 sat to the dust exposure:
    (1 to 20).foreach { _ => addHtlc(500.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice) }
    addHtlc(15_000.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice) // way above the trimmed threshold -> not included in the dust exposure
    crossSign(alice, bob, alice2bob, bob2alice)

    // Bob sends HTLCs to Alice that add 14 500 sat to the dust exposure:
    (1 to 29).foreach { _ => addHtlc(500.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob) }
    addHtlc(18_000.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob) // way above the trimmed threshold -> not included in the dust exposure
    crossSign(bob, alice, bob2alice, alice2bob)

    // HTLCs that take Alice's dust exposure above her threshold are rejected.
    val dustAdd = CMD_ADD_HTLC(sender.ref, 501.sat.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! dustAdd
    sender.expectMsg(RES_ADD_FAILED(dustAdd, LocalDustHtlcExposureTooHigh(channelId(alice), 25_000.sat, 25_001.sat.toMilliSatoshi), Some(initialState.channelUpdate)))

    // HTLCs that don't contribute to dust exposure are accepted.
    alice ! CMD_ADD_HTLC(sender.ref, 25_000.sat.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
  }
  ...
  test("recv CMD_ADD_HTLC (over max dust htlc exposure with pending local changes)") { f =>
    ...
    // Alice sends HTLCs to Bob that add 22 500 sat to the dust exposure.
    // She signs them but Bob doesn't answer yet.
    (1 to 25).foreach { _ => addHtlc(900.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice) }
    alice ! CMD_SIGN(Some(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_SIGN]]
    alice2bob.expectMsgType[CommitSig]

    // Alice sends HTLCs to Bob that add 1 800 sat to the dust exposure.
    addHtlc(900.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice)
    addHtlc(900.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice)

    // HTLCs that take Alice's dust exposure above her threshold are rejected.
    val add = CMD_ADD_HTLC(sender.ref, 701.sat.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (over max dust htlc exposure in local commit only with pending local changes)") { f =>
    import f._
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    assert(alice.underlyingActor.nodeParams.onChainFeeConf.feerateToleranceFor(bob.underlyingActor.nodeParams.nodeId).dustTolerance.maxExposure == 25_000.sat)
    ...
    // HTLCs that take Alice's dust exposure above her threshold are rejected.
    val add = CMD_ADD_HTLC(sender.ref, 1050.sat.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (over max dust htlc exposure in remote commit only with pending local changes)") { f =>
    import f._
    val sender = TestProbe()
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
  ...
  test("recv CMD_ADD_HTLC (over capacity)", Tag(ChannelStateTestsTags.NoMaxHtlcValueInFlight)) { f =>
    ...
    val sender = TestProbe()
    val initialState = alice.stateData.asInstanceOf[DATA_NORMAL]
    val add1 = CMD_ADD_HTLC(sender.ref, TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add1
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice ! CMD_SIGN()
    ...
    val add2 = CMD_ADD_HTLC(sender.ref, TestConstants.fundingSatoshis.toMilliSatoshi * 2 / 3, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add2
    val error = InsufficientFunds(channelId(alice), add2.amount, 562193 sat, 20000 sat, 4330 sat)
  ...
  test("recv CMD_ADD_HTLC (after having sent Shutdown)") { f =>
    ...
    val add = CMD_ADD_HTLC(sender.ref, 500000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
  ...
  test("recv CMD_ADD_HTLC (after having received Shutdown)") { f =>
    ...
    val add1 = CMD_ADD_HTLC(sender.ref, 50000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add1
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    ...
    val add2 = CMD_ADD_HTLC(sender.ref, 10000000 msat, randomBytes32(), CltvExpiry(300000), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    // messages cross
    alice2bob.expectMsgType[UpdateAddHtlc]
  ...
  test("recv UpdateAddHtlc") { f =>
    import f._
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, 150000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
    bob ! htlc
  ...
  test("recv UpdateAddHtlc (unexpected id)") { f =>
    ...
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 42, 150000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
    bob ! htlc.copy(id = 0)
    bob ! htlc.copy(id = 1)
    bob ! htlc.copy(id = 2)
    bob ! htlc.copy(id = 3)
  ...
  test("recv UpdateAddHtlc (value too small)") { f =>
    import f._
    val tx = bob.signCommitTx()
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, 150 msat, randomBytes32(), cltvExpiry = CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
  ...
  test("recv UpdateAddHtlc (insufficient funds)") { f =>
    import f._
    val tx = bob.signCommitTx()
    val htlc = UpdateAddHtlc(ByteVector32.Zeroes, 0, MilliSatoshi(Long.MaxValue), randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
    alice2bob.forward(bob, htlc)
    val error = bob2alice.expectMsgType[Error]
    assert(new String(error.data.toArray) == InsufficientFunds(channelId(bob), amount = MilliSatoshi(Long.MaxValue), missing = 9223372036078675L sat, reserve = 20000 sat, fees = 3900 sat).getMessage)
    awaitCond(bob.stateName == CLOSING)
    // channel should be advertised as down
    assert(channelUpdateListener.expectMsgType[LocalChannelDown].channelId == bob.stateData.asInstanceOf[DATA_CLOSING].channelId)
    bob2blockchain.expectFinalTxPublished(tx.txid)
  ...
  test("recv UpdateAddHtlc (insufficient funds w/ pending htlcs)") { f =>
    import f._
    val tx = bob.signCommitTx()
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 0, 400000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 1, 300000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 2, 100000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv UpdateAddHtlc (insufficient funds w/ pending htlcs 1/2)") { f =>
    ...
    val tx = bob.signCommitTx()
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 0, 400000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 1, 200000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 2, 167600000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 3, 10000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    val error = bob2alice.expectMsgType[Error]
    assert(new String(error.data.toArray) == InsufficientFunds(channelId(bob), amount = 10000000 msat, missing = 2790 sat, reserve = 20000 sat, fees = 5190 sat).getMessage)
    awaitCond(bob.stateName == CLOSING)
    // channel should be advertised as down
    assert(channelUpdateListener.expectMsgType[LocalChannelDown].channelId == bob.stateData.asInstanceOf[DATA_CLOSING].channelId)
    bob2blockchain.expectFinalTxPublished(tx.txid)
  ...
  test("recv UpdateAddHtlc (insufficient funds w/ pending htlcs 2/2)") { f =>
    import f._
    val tx = bob.signCommitTx()
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 0, 300000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 1, 300000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 2, 500000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    val error = bob2alice.expectMsgType[Error]
    assert(new String(error.data.toArray) == InsufficientFunds(channelId(bob), amount = 500000000 msat, missing = 324760 sat, reserve = 20000 sat, fees = 4760 sat).getMessage)
    awaitCond(bob.stateName == CLOSING)
    // channel should be advertised as down
    assert(channelUpdateListener.expectMsgType[LocalChannelDown].channelId == bob.stateData.asInstanceOf[DATA_CLOSING].channelId)
    bob2blockchain.expectFinalTxPublished(tx.txid)
  ...
  test("recv UpdateAddHtlc (over max inflight htlc value)", Tag(ChannelStateTestsTags.AliceLowMaxHtlcValueInFlight)) { f =>
    ...
    alice2bob.forward(alice, UpdateAddHtlc(ByteVector32.Zeroes, 0, 151_000_000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
  ...
  test("recv UpdateAddHtlc (over max accepted htlcs)") { f =>
    ...
    for (i <- 0 until 30) {
      alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, i, 1000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    }
    alice2bob.forward(bob, UpdateAddHtlc(ByteVector32.Zeroes, 30, 1000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None))
    val error = bob2alice.expectMsgType[Error]
  ...
  test("recv CMD_SIGN (two identical htlcs in each direction)") { f =>
    import f._
    val sender = TestProbe()
    val add = CMD_ADD_HTLC(sender.ref, 10000000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    alice ! add
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
  ...
  test("recv CMD_SIGN (check htlc info are persisted)") { f =>
    ...
    assert(Alice.nodeParams.channelConf.dustLimit > Bob.nodeParams.channelConf.dustLimit + 10.sat)
    ...
    val a2b_1 = Alice.nodeParams.channelConf.dustLimit + 10.sat // will be in alice and bob tx
    val a2b_2 = Alice.nodeParams.channelConf.dustLimit + 20.sat // will be in alice and bob tx
    val b2a_1 = Alice.nodeParams.channelConf.dustLimit + 10.sat // will be in alice and bob tx
    val b2a_2 = Bob.nodeParams.channelConf.dustLimit + 10.sat // will be only be in bob tx
    alice ! CMD_ADD_HTLC(sender.ref, a2b_1.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    alice ! CMD_ADD_HTLC(sender.ref, a2b_2.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    alice2bob.expectMsgType[UpdateAddHtlc]
    alice2bob.forward(bob)
    bob ! CMD_ADD_HTLC(sender.ref, b2a_1.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
    bob ! CMD_ADD_HTLC(sender.ref, b2a_2.toMilliSatoshi, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
    bob2alice.expectMsgType[UpdateAddHtlc]
    bob2alice.forward(alice)
  ...
  test("recv CMD_SIGN (going above balance threshold)", Tag(ChannelStateTestsTags.NoPushAmount), Tag(ChannelStateTestsTags.ChannelsPublic), Tag(ChannelStateTestsTags.DoNotInterceptGossip), Tag(ChannelStateTestsTags.AdaptMaxHtlcAmount)) { f =>
    ...
    val (p5, htlc5) = addHtlc(452_000_000 msat, alice, bob, alice2bob, bob2alice)
    crossSign(alice, bob, alice2bob, bob2alice)
    fulfillHtlc(htlc5.id, p5, bob, alice, bob2alice, alice2bob)
    crossSign(bob, alice, bob2alice, alice2bob)
    assert(alice.stateData.asInstanceOf[DATA_NORMAL].commitments.latest.localCommit.spec.toLocal == 37_500_000.msat)
    ...
    assert(bob.stateData.asInstanceOf[DATA_NORMAL].commitments.latest.localCommit.spec.toLocal == 962_500_000.msat)
    bobListener.expectNoMessage(100 millis)
  ...
  test("recv CommitSig (multiple htlcs in both directions, anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testRecvCommitSigMultipleHtlcs(f)
  }

  test("recv CommitSig (multiple htlcs in both directions, phoenix taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaprootPhoenix)) { f =>
    testRecvCommitSigMultipleHtlcs(f)
  }

  def testRecvCommitSigMultipleHtlcZeroFees(f: FixtureParam): Unit = {
    import f._
  ...
  test("recv CommitSig (multiple htlcs in both directions, anchor outputs zero fee htlc txs)") { f =>
    testRecvCommitSigMultipleHtlcZeroFees(f)
  }

  test("recv CommitSig (multiple htlcs in both directions, taproot zero fee htlc txs)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testRecvCommitSigMultipleHtlcZeroFees(f)
  }
  ...
  test("recv CommitSig (multiple htlcs in both directions, taproot zero fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testRecvCommitSigMultipleHtlcZeroFees(f)
  }
  ...
  test("recv CommitSig (only fee update)") { f =>
    ...
    alice ! CMD_UPDATE_FEE(TestConstants.anchorOutputsFeeratePerKw + FeeratePerKw(1000 sat), commit = false)
    alice ! CMD_SIGN()

    // actual test begins (note that channel sends a CMD_SIGN to itself when it receives RevokeAndAck and there are changes)
    val updateFee = alice2bob.expectMsgType[UpdateFee]
    assert(updateFee.feeratePerKw == TestConstants.anchorOutputsFeeratePerKw + FeeratePerKw(1000 sat))
    alice2bob.forward(bob)
    alice2bob.expectMsgType[CommitSig]
    alice2bob.forward(bob)
    bob2alice.expectMsgType[RevokeAndAck]
  ...
  test("recv CommitSig (two htlcs received with same r)") { f =>
    ...
    alice ! CMD_ADD_HTLC(sender.ref, 50000000 msat, h, CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, Reputation.Score.max(accountable = false), None, localOrigin(sender.ref))
    sender.expectMsgType[RES_SUCCESS[CMD_ADD_HTLC]]
  ...
  test("recv CommitSig (simple taproot channels, missing partial signature)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    import f._
    addHtlc(50000000 msat, alice, bob, alice2bob, bob2alice)
    val tx = bob.signCommitTx()
    ...
    alice ! CMD_SIGN()
    val commitSig = alice2bob.expectMsgType[CommitSig]
    val commitSigMissingPartialSig = commitSig.copy(tlvStream = commitSig.tlvStream.copy(records = commitSig.tlvStream.records.filterNot(_.isInstanceOf[CommitSigTlv.PartialSignatureWithNonceTlv])))
    bob ! commitSigMissingPartialSig
    val error = bob2alice.expectMsgType[Error]
    assert(new String(error.data.toArray).startsWith("invalid commitment signature"))
    awaitCond(bob.stateName == CLOSING)
    bob2blockchain.expectFinalTxPublished(tx.txid)
  ...
  test("recv CommitSig (simple taproot channels, invalid partial signature)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    ...
    val invalidPsig = CommitSigTlv.PartialSignatureWithNonceTlv(psig.copy(partialSig = psig.partialSig.reverse))
    val commitSigWithInvalidPsig = commitSig.copy(tlvStream = commitSig.tlvStream.copy(records = commitSig.tlvStream.records.filterNot(_.isInstanceOf[CommitSigTlv.PartialSignatureWithNonceTlv]) + invalidPsig))
  ...
  test("recv RevokeAndAck (simple taproot channels, invalid nonce)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    import f._
    // Alice sends an HTLC to Bob.
    val (r, add) = addHtlc(50000000 msat, alice, bob, alice2bob, bob2alice)
    alice ! CMD_SIGN()
    alice2bob.expectMsgType[CommitSig]
    alice2bob.forward(bob)
    ...
    alice2bob.expectMsgType[RevokeAndAck]
    alice2bob.forward(bob)
    // Bob will force-close when receiving Alice's next commit_sig.
    val commitTx = bob.signCommitTx()
    fulfillHtlc(add.id, r, bob, alice, bob2alice, alice2bob)
    bob ! CMD_SIGN()
    bob2alice.expectMsgType[CommitSig]
    bob2alice.forward(alice)
    alice2bob.expectMsgType[RevokeAndAck]
  ...
  test("recv RevokeAndAck (over max dust htlc exposure)") { f =>
    ...
    // Alice sends HTLCs to Bob that add 24 000 sat to the dust exposure:
    (1 to 24).foreach(_ => addHtlc(1000.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice))
    crossSign(alice, bob, alice2bob, bob2alice)

    // Bob sends HTLCs to Alice that overflow the dust exposure:
    val (_, dust1) = addHtlc(750.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob)
    val (_, dust2) = addHtlc(500.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob)
    val (_, nonDust) = addHtlc(20_000.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob) // way above the trimmed threshold -> not included in the dust exposure
    crossSign(bob, alice, bob2alice, alice2bob)

    // Alice forwards HTLCs that fit in the dust exposure.
    alice2relayer.expectMsgAllOf(
      RelayForward(dust1, TestConstants.Bob.nodeParams.nodeId, 3.0 / 30),
      RelayForward(nonDust, TestConstants.Bob.nodeParams.nodeId, 3.0 / 30),
    )
    alice2relayer.expectNoMessage(100 millis)
    // And instantly fails the others.
    assert(alice2bob.expectMsgType[UpdateFailHtlc].id == dust2.id)
    alice2bob.expectMsgType[CommitSig]
    alice2bob.expectNoMessage(100 millis)
  }
  ...
  test("recv RevokeAndAck (over max dust htlc exposure with pending local changes)") { f =>
    ...
    (1 to 10).foreach(_ => addHtlc(1000.sat.toMilliSatoshi, bob, alice, bob2alice, alice2bob))
    crossSign(bob, alice, bob2alice, alice2bob)
    ...
    (1 to 10).foreach(_ => addHtlc(1000.sat.toMilliSatoshi, alice, bob, alice2bob, bob2alice))
    ...
    (1 to 5).foreach(_ => alice2relayer.expectMsgType[RelayForward])
    alice2relayer.expectNoMessage(100 millis)
  ...
  test("recv RevokeAndAck (over max dust htlc exposure in local commit only with pending local changes)", Tag(ChannelStateTestsTags.HighDustLimitDifferenceAliceBob)) { f =>
    import f._
  ...
  test("recv RevokeAndAck (over max dust htlc exposure in remote commit only with pending local changes)", Tag(ChannelStateTestsTags.HighDustLimitDifferenceBobAlice)) { f =>
    import f._
  ...
  def testRevokeAndAckHtlc(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._

    assert(alice.commitments.latest.commitmentFormat == commitmentFormat)
    assert(bob.commitments.latest.commitmentFormat == commitmentFormat)

    def aliceToRemoteScript(): ByteVector = {
      val toRemoteAmount = alice.commitments.latest.localCommit.spec.toRemote
    ...
    bob2alice.expectMsgType[RevokeAndAck]
    bob2alice.forward(alice)
    awaitCond(alice.commitments.remoteNextCommitInfo.isRight)
    ...
    alice2bob.expectMsgType[RevokeAndAck]
    alice2bob.forward(bob)
    awaitCond(bob.commitments.remoteNextCommitInfo.isRight)
  ...
  test("recv RevokeAndAck (one htlc sent)") { f =>
    testRevokeAndAckHtlc(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv RevokeAndAck (one htlc sent, anchor_outputs_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testRevokeAndAckHtlc(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }

  test("recv RevokeAndAck (one htlc sent, option_simple_taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
  ...
  test("recv RevokeAndAck (one htlc sent, taproot_zero_fee_commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testRevokeAndAckHtlc(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  private def testReceiveCmdFulfillHtlc(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._
  ...
  test("recv CMD_FULFILL_HTLC") { f =>
    testReceiveCmdFulfillHtlc(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv CMD_FULFILL_HTLC (anchor_outputs_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testReceiveCmdFulfillHtlc(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }
  ...
  test("recv CMD_FULFILL_HTLC (taproot_zero_fee_commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testReceiveCmdFulfillHtlc(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  private def testUpdateFulfillHtlc(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._
  ...
  test("recv UpdateFulfillHtlc") { f =>
    testUpdateFulfillHtlc(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv UpdateFulfillHtlc (anchor_outputs_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testUpdateFulfillHtlc(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }

  test("recv UpdateFulfillHtlc (option_simple_taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testUpdateFulfillHtlc(f, ZeroFeeHtlcTxSimpleTaprootChannelCommitmentFormat)
  }
  ...
  test("recv UpdateFulfillHtlc (invalid preimage)") { f =>
    ...
    alice2blockchain.expectReplaceableTxPublished[ClaimLocalAnchorTx]
    alice2blockchain.expectFinalTxPublished("local-main-delayed")
    alice2blockchain.expectReplaceableTxPublished[HtlcTimeoutTx]
    alice2blockchain.expectWatchTxConfirmed(tx.txid)
  }
  ...
  test("recv CMD_FAIL_HTLC") { f =>
    testCmdFailHtlc(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv CMD_FAIL_HTLC (anchor_outputs_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testCmdFailHtlc(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }
  ...
  test("recv CMD_FAIL_HTLC (taproot_zero_fee_commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    testCmdFailHtlc(f, TaprootZeroFeeCommitmentFormat)
  }
  ...
  private def testUpdateFailHtlc(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._
  ...
  test("recv UpdateFailHtlc") { f =>
    testUpdateFailHtlc(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv UpdateFailHtlc (anchor_outputs_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testUpdateFailHtlc(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }

  test("recv UpdateFailHtlc (option_simple_taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testUpdateFailHtlc(f, ZeroFeeHtlcTxSimpleTaprootChannelCommitmentFormat)
  }
  ...
  private def testCmdUpdateFee(f: FixtureParam, commitmentFormat: CommitmentFormat): Unit = {
    import f._
  ...
  test("recv CMD_UPDATE_FEE") { f =>
    testCmdUpdateFee(f, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  }

  test("recv CMD_UPDATE_FEE (anchor_output_phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testCmdUpdateFee(f, UnsafeLegacyAnchorOutputsCommitmentFormat)
  }

  test("recv CMD_UPDATE_FEE (simple taproot channel)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testCmdUpdateFee(f, ZeroFeeHtlcTxSimpleTaprootChannelCommitmentFormat)
  }
  ...
  test("recv UpdateFee (two in a row)") { f =>
    import f._
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    val fee1 = UpdateFee(ByteVector32.Zeroes, TestConstants.anchorOutputsFeeratePerKw * 0.8)
    bob ! fee1
    val fee2 = UpdateFee(ByteVector32.Zeroes, TestConstants.anchorOutputsFeeratePerKw * 1.2)
    bob ! fee2
    awaitCond(bob.stateData == initialState
  ...
  test("recv UpdateFee (when sender is not funder)") { f =>
    import f._
    val tx = alice.signCommitTx()
    alice ! UpdateFee(ByteVector32.Zeroes, TestConstants.anchorOutputsFeeratePerKw * 1.2)
    alice2bob.expectMsgType[Error]
    awaitCond(alice.stateName == CLOSING)
    // channel should be advertised as down
    assert(channelUpdateListener.expectMsgType[LocalChannelDown].channelId == alice.stateData.asInstanceOf[DATA_CLOSING].channelId)
  ...
  test("recv UpdateFee (taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    import f._
    val tx = bob.signCommitTx()
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    bob ! UpdateFee(ByteVector32.Zeroes, TestConstants.anchorOutputsFeeratePerKw)
  ...
  test("recv UpdateFee (remote feerate is too high)") { f =>
    import f._
  ...
  test("recv UpdateFee (remote feerate is too small)") { f =>
    ...
    val initialState = bob.stateData.asInstanceOf[DATA_NORMAL]
    assert(initialState.commitments.latest.localCommit.spec.commitTxFeerate == TestConstants.anchorOutputsFeeratePerKw)
    val add = UpdateAddHtlc(ByteVector32.Zeroes, 0, 2500000 msat, randomBytes32(), CltvExpiryDelta(144).toCltvExpiry(currentBlockHeight), TestConstants.emptyOnionPacket, None, accountable = false, None)
  ...
  test("recv CMD_CLOSE (no pending htlcs) (anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testCmdClose(f, None)
  }

  test("recv CMD_CLOSE (no pending htlcs) (simple taproot channel)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testCmdClose(f, None)
  }
  ...
  test("recv CMD_CLOSE (with native segwit script)") { f =>
  ...
  test("recv Shutdown (no pending htlcs) (anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testShutdown(f, None)
  }
  ...
  test("recv Shutdown (with unsigned fee update)") { f =>
    import f._
    val sender = TestProbe()
    alice ! CMD_UPDATE_FEE(FeeratePerKw(3_000 sat), commit = true)
    alice2bob.expectMsgType[UpdateFee]
    alice2bob.forward(bob)
  ...
  test("recv Shutdown (with native segwit script)") { f =>
  ...
  test("recv Shutdown (with signed htlcs)") { f =>
    testShutdownWithHtlcs(f)
  }

  test("recv Shutdown (with signed htlcs) (anchor outputs phoenix)", Tag(ChannelStateTestsTags.AnchorOutputsPhoenix)) { f =>
    testShutdownWithHtlcs(f)
  }
  ...
  test("recv CurrentBlockCount (fulfilled signed htlc ignored by upstream peer)") { f =>
    ...
    bob2blockchain.expectReplaceableTxPublished[ClaimLocalAnchorTx]
    bob2blockchain.expectFinalTxPublished("local-main-delayed")
    assert(bob2blockchain.expectReplaceableTxPublished[HtlcSuccessTx].input.outPoint == htlcSuccessTx.input.outPoint)
  ...
  test("recv CurrentFeerate (when funder, doesn't trigger and UpdateFee with taproot zero-fee commitments)", Tag(ChannelStateTestsTags.TaprootZeroFeeCommitments)) { f =>
    import f._
    assert(alice.commitments.latest.localCommit.spec.commitTxFeerate == FeeratePerKw(0 sat))
    val event = CurrentFeerates.BitcoinCore(FeeratesPerKw.single(FeeratePerKw(5000 sat)).copy(minimum = FeeratePerKw(250 sat)))
    alice.setBitcoinCoreFeerates(event.feeratesPerKw)
  ...
  test("recv Error") { f =>
    ...
    alice2blockchain.expectFinalTxPublished(aliceCommitTx.txid)
    assert(aliceCommitTx.txOut.size == 8) // two main outputs, two anchor outputs and 4 pending htlcs
    awaitCond(alice.stateName == CLOSING)
    ...
    val htlcTx1 = alice2blockchain.expectReplaceableTxPublished[HtlcSuccessTx].sign()
    val htlcTx2 = alice2blockchain.expectReplaceableTxPublished[HtlcTimeoutTx].sign()
    val htlcTx3 = alice2blockchain.expectReplaceableTxPublished[HtlcTimeoutTx].sign()
    ...
    alice2blockchain.expectWatchTxConfirmed(aliceCommitTx.txid)
    alice2blockchain.expectWatchOutputsSpent(Seq(localCommitPublished.localOutput_opt, localCommitPublished.anchorOutput_opt).flatten ++ localCommitPublished.htlcOutputs.toSeq)
    alice2blockchain.expectNoMessage(100 millis)
    ...
    Seq(htlcTx1, htlcTx2, htlcTx3).foreach { htlcTx =>
      ...
      alice ! WatchTxConfirmedTriggered(BlockHeight(2701), 3, htlcTx)
      val htlcDelayedTx = alice2blockchain.expectFinalTxPublished("htlc-delayed")
      Transaction.correctlySpends(htlcDelayedTx.tx, htlcTx :: Nil, ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
  ...
  test("recv Error (anchor outputs zero fee htlc txs)") { f =>
    testErrorAnchorOutputsWithHtlcs(f)
  }

  test("recv Error (simple taproot channel)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testErrorAnchorOutputsWithHtlcs(f)
  }

  test("recv Error (anchor outputs zero fee htlc txs, fee-bumping for commit txs without htlcs disabled)", Tag(ChannelStateTestsTags.DontSpendAnchorWithoutHtlcs)) { f =>
  ...
  test("recv Error (anchor outputs zero fee htlc txs without htlcs)") { f =>
    testErrorAnchorOutputsWithoutHtlcs(f, commitFeeBumpDisabled = false)
  }

  test("recv Error (simple taproot channel without htlcs)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    testErrorAnchorOutputsWithoutHtlcs(f, commitFeeBumpDisabled = false)
  }

  test("recv Error (anchor outputs zero fee htlc txs without htlcs, fee-bumping for commit txs without htlcs disabled)", Tag(ChannelStateTestsTags.DontSpendAnchorWithoutHtlcs)) { f =>
  ...
  test("recv Error (nothing at stake)", Tag(ChannelStateTestsTags.NoPushAmount)) { f =>
    ...
    assert(bobCommitTx.txOut.size == 2) // alice's main output and anchor output
    alice2blockchain.expectNoMessage(100 millis)
  ...
  test("recv WatchFundingConfirmedTriggered (public channel, zero-conf)", Tag(ChannelStateTestsTags.ChannelsPublic), Tag(ChannelStateTestsTags.DoNotInterceptGossip), Tag(ChannelStateTestsTags.ZeroConf)) { f =>
    import f._
  ...
  test("recv WatchFundingConfirmedTriggered (private channel, zero-conf)", Tag(ChannelStateTestsTags.ZeroConf)) { f =>
    import f._
  ...
  test("recv INPUT_DISCONNECTED (with pending htlcs, taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    import f._
    ...
    val (rb, htlcB) = addHtlc(25_000_000 msat, bob, alice, bob2alice, alice2bob)
    bob ! CMD_SIGN()
    bob2alice.expectMsgType[CommitSig]
    bob2alice.forward(alice)
    val revA1 = alice2bob.expectMsgType[RevokeAndAck] // not received by Bob
    alice2bob.expectMsgType[CommitSig] // not received by Bob
    val (_, htlcA2) = addHtlc(10_000_000 msat, alice, bob, alice2bob, bob2alice) // not signed by either Alice or Bob

    alice ! INPUT_DISCONNECTED
    val addSettledA = alice2relayer.expectMsgType[RES_ADD_SETTLED[Origin, HtlcResult]]
    assert(addSettledA.htlc == htlcA2)
    assert(addSettledA.result.isInstanceOf[HtlcResult.DisconnectedBeforeSigned])
    alice2relayer.expectNoMessage(100 millis)
    awaitCond(alice.stateName == OFFLINE)
    bob ! INPUT_DISCONNECTED
    bob2relayer.expectNoMessage(100 millis)
    awaitCond(bob.stateName == OFFLINE)

    // Alice and Bob finish signing the HTLCs on reconnection.
    ...
    assert(alice2bob.expectMsgType[ChannelReestablish].nextCommitNonces == revA1.nextCommitNonces)
    alice2bob.forward(bob)
    bob2alice.expectMsgType[ChannelReestablish]
    bob2alice.forward(alice)
    assert(alice2bob.expectMsgType[RevokeAndAck].nextCommitNonces == revA1.nextCommitNonces)
    alice2bob.forward(bob)
    assert(alice2bob.expectMsgType[UpdateAddHtlc].paymentHash == htlcA1.paymentHash)
    alice2bob.forward(bob)
    alice2bob.expectMsgType[CommitSig]
    alice2bob.forward(bob)
    bob2alice.expectMsgType[RevokeAndAck]
    ...
    // Alice and Bob fulfill the pending HTLCs.
    fulfillHtlc(htlcA1.id, ra1, bob, alice, bob2alice, alice2bob)
    fulfillHtlc(htlcB.id, rb, alice, bob, alice2bob, bob2alice)
    crossSign(alice, bob, alice2bob, bob2alice)
  }

  test("recv INPUT_DISCONNECTED (missing nonces, taproot)", Tag(ChannelStateTestsTags.OptionSimpleTaproot)) { f =>
    import f._
    ...
    alice ! INPUT_RECONNECTED(bob, aliceInit, bobInit)
    bob ! INPUT_RECONNECTED(alice, bobInit, aliceInit)
    val channelReestablish = alice2bob.expectMsgType[ChannelReestablish]
    assert(channelReestablish.nextCommitNonces.size == 1)
    bob2alice.expectMsgType[ChannelReestablish]
    alice2bob.forward(bob, channelReestablish.copy(tlvStream = TlvStream(channelReestablish.tlvStream.records.filterNot(_.isInstanceOf[ChannelReestablishTlv.NextLocalNoncesTlv]))))
    bob2alice.expectMsgType[Error]
  }
