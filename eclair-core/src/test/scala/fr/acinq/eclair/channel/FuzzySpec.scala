...
class FuzzySpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase with Logging {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    within(30 seconds) {
      ...
      bobRegister ! bob
      // no announcements
      alice ! INPUT_INIT_CHANNEL_INITIATOR(ByteVector32.Zeroes, TestConstants.fundingSatoshis, dualFunded = false, TestConstants.feeratePerKw, TestConstants.feeratePerKw, fundingTxFeeBudget_opt = None, Some(TestConstants.initiatorPushAmount), requireConfirmedInputs = false, requestFunding_opt = None, Alice.channelParams, 330 sat, 1 msat, MilliSatoshi.maxValue, 483, CltvExpiryDelta(144), pipe, bobInit, channelFlags, ChannelConfig.standard, ChannelTypes.Standard(), replyTo = system.deadLetters)
      alice2blockchain.expectMsgType[TxPublisher.SetChannelId]
      bob ! INPUT_INIT_CHANNEL_NON_INITIATOR(ByteVector32.Zeroes, None, dualFunded = false, None, requireConfirmedInputs = false, Bob.channelParams, 330 sat, 1 msat, MilliSatoshi.maxValue, 483, CltvExpiryDelta(144), pipe, aliceInit, ChannelConfig.standard, ChannelTypes.Standard())
