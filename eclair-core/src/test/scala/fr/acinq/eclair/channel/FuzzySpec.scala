...
class FuzzySpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase with Logging {
  ...
  class SenderActor(sendChannel: TestFSMRef[ChannelState, ChannelData, Channel], paymentHandler: ActorRef, latch: CountDownLatch, count: Int) extends Actor with ActorLogging {
    ...
    def buildCmdAdd(paymentHash: ByteVector32, recipient: ClearRecipient): CMD_ADD_HTLC = {
      ...
      OutgoingPaymentPacket.buildCommand(randomKey(), self, Upstream.Local(UUID.randomUUID()), paymentHash, Route(amount, ChannelHop(null, null, recipient.nodeId, null) :: Nil, recipient), amount, amount, expiry).get._1
    }
    ...
    def initiatePaymentOrStop(remaining: Int): Unit =
      if (remaining > 0) {
        ...
        context become {
          case req: Bolt11Invoice =>
            sendChannel ! buildCmdAdd(req.paymentHash, req.recipient)
            context become {
