...
class FuzzySpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase with Logging {
  ...
  class SenderActor(sendChannel: TestFSMRef[ChannelState, ChannelData, Channel], paymentHandler: ActorRef, latch: CountDownLatch, count: Int) extends Actor with ActorLogging {
    ...
    def buildCmdAdd(paymentHash: ByteVector32, dest: PublicKey, paymentSecret: ByteVector32): CMD_ADD_HTLC = {
      ...
      OutgoingPaymentPacket.buildCommand(randomKey(), self, Upstream.Local(UUID.randomUUID()), paymentHash, ChannelHop(null, null, dest, null) :: Nil, None, amount, amount, expiry, paymentSecret, None, Nil, Nil).get._1
    }
