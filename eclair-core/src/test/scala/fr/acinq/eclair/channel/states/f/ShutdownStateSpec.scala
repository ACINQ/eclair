...
class ShutdownStateSpec extends TestkitBaseClass with StateTestsHelperMethods {
  ...
  test("recv CurrentFeerate (when funder, triggers an UpdateFee)") { f =>
    ...
    alice2bob.expectMsg(UpdateFee(initialState.commitments.channelId, event.feeratesPerKw.feePerBlock(feeConfOfChannel(alice).feeTargets.commitmentBlockTarget)))
  }
