...
import fr.acinq.eclair.{BlockHeight, CltvExpiry, CltvExpiryDelta, MilliSatoshiLong, TestConstants, TestKitBaseClass, TimestampMilli, randomBytes32}
...
class ShutdownStateSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  test("recv UpdateFulfillHtlc (unknown htlc id)") { f =>
    import f._
    val tx = alice.stateData.asInstanceOf[DATA_SHUTDOWN].commitments.latest.localCommit.commitTxAndRemoteSig.commitTx.tx
  ...
  test("recv CMD_FAIL_HTLC") { f =>
    import f._
    val initialState = bob.stateData.asInstanceOf[DATA_SHUTDOWN]
    bob ! CMD_FAIL_HTLC(1, Right(PermanentChannelFailure()), useAttributableErrors = false, TimestampMilli.now())
  ...
  test("recv CMD_FAIL_HTLC (unknown htlc id)") { f =>
    ...
    val sender = TestProbe()
    val initialState = bob.stateData.asInstanceOf[DATA_SHUTDOWN]
    val c = CMD_FAIL_HTLC(42, Right(PermanentChannelFailure()), useAttributableErrors = false, TimestampMilli.now(), replyTo_opt = Some(sender.ref))
    bob ! c
    sender.expectMsg(RES_FAILURE(c, UnknownHtlcId(channelId(bob), 42)))
    assert(initialState == bob.stateData)
  }
  ...
  test("recv CMD_FORCECLOSE") { f =>
    ...
    val aliceCommitTx = alice.stateData.asInstanceOf[DATA_SHUTDOWN].commitments.latest.localCommit.commitTxAndRemoteSig.commitTx.tx
