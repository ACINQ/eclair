...
trait StateTestsHelperMethods extends TestKitBase with fixture.TestSuite with ParallelTestExecution {
  ...
  def init(nodeParamsA: NodeParams = TestConstants.Alice.nodeParams, nodeParamsB: NodeParams = TestConstants.Bob.nodeParams, wallet: EclairWallet = new TestWallet): SetupFixture = {
    ...
    val alice: TestFSMRef[State, Data, Channel] = TestFSMRef(new Channel(nodeParamsA.copy(turboWhitelist = Set(nodeParamsB.nodeId)), wallet, Bob.nodeParams.nodeId, alice2blockchain.ref, router.ref, relayerA.ref))
    val bob: TestFSMRef[State, Data, Channel] = TestFSMRef(new Channel(nodeParamsB.copy(turboWhitelist = Set(nodeParamsA.nodeId)), wallet, Alice.nodeParams.nodeId, bob2blockchain.ref, router.ref, relayerB.ref))
