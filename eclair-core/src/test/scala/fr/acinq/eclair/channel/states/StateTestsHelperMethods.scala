...
trait StateTestsHelperMethods extends TestKitBase {
  ...
  def reachNormal(setup: SetupFixture, tags: Set[String] = Set.empty): Unit = {
    ...
    val channelType = if (tags.contains(StateTestsTags.AnchorOutputs)) {
    ...
    alice ! INPUT_INIT_FUNDER(ByteVector32.Zeroes, fundingSatoshis, pushMsat, initialFeeratePerKw, TestConstants.feeratePerKw, None, aliceParams, alice2bob.ref, bobInit, channelFlags, channelConfig, channelType)
