...
class RustyTestsSpec extends TestKit(ActorSystem("test")) with Matchers with fixture.FunSuiteLike with BeforeAndAfterAll {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    val alice: TestFSMRef[State, Data, Channel] = TestFSMRef(new Channel(Alice.nodeParams.copy(feeEstimator = feeEstimator), wallet, Bob.nodeParams.nodeId, alice2blockchain.ref, router.ref, relayer))
    val bob: TestFSMRef[State, Data, Channel] = TestFSMRef(new Channel(Bob.nodeParams.copy(feeEstimator = feeEstimator), wallet, Alice.nodeParams.nodeId, bob2blockchain.ref, router.ref, relayer))
