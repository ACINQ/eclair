  ...
  def exec(script: List[String], a: ActorRef, b: ActorRef): Unit = {
    ...
    script match {
      case offer(x, amount, rhash) :: rest =>
        resolve(x) ! CMD_ADD_HTLC(MilliSatoshi(amount.toInt), ByteVector32.fromValidHex(rhash), CltvExpiry(144), Sphinx.emptyOnionPacket, Upstream.Local(UUID.randomUUID()))
        exec(rest, a, b)
