...
class LiquidityDbSpec extends AnyFunSuite {
  ...
  test("add/list/remove pending on-the-fly funding proposals") {
    forAllDbs { dbs =>
      ...
      val upstream = Seq(
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 7, 25_000_000 msat, paymentHash1, CltvExpiry(750_000), randomOnion(), None, accountable = false, None), TimestampMilli(0), randomKey().publicKey, 0.1),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 0, 1 msat, paymentHash1, CltvExpiry(750_000), randomOnion(), Some(randomKey().publicKey), accountable = false, None), TimestampMilli.now(), randomKey().publicKey, 0.1),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 561, 100_000_000 msat, paymentHash2, CltvExpiry(799_999), randomOnion(), None, accountable = false, None), TimestampMilli.now(), randomKey().publicKey, 0.1),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 1105, 100_000_000 msat, paymentHash2, CltvExpiry(799_999), randomOnion(), None, accountable = false, None), TimestampMilli.now(), randomKey().publicKey, 0.1),
      )
