...
class AuditDbSpec extends AnyFunSuite {
  ...
  test("add/list transaction events") {
    forAllDbs { dbs =>
      ...
      val channelId2 = randomBytes32()
      val remoteNodeId = randomKey().publicKey
      val p1a = TransactionPublished(channelId1, remoteNodeId, Transaction(2, Nil, Seq(TxOut(50_000 sat, Script.pay2wpkh(remoteNodeId))), 0), 50 sat, 0 sat, "funding", now - 10.seconds)
      val p1b = TransactionPublished(channelId1, remoteNodeId, Transaction(2, Nil, Seq(TxOut(100_000 sat, Script.pay2wpkh(remoteNodeId))), 0), 75 sat, 25 sat, "splice", now - 5.seconds)
      val p2 = TransactionPublished(channelId2, remoteNodeId, Transaction(2, Nil, Seq(TxOut(200_000 sat, Script.pay2wpkh(remoteNodeId))), 0), 0 sat, 0 sat, "local-close", now - 1.seconds)
  ...
  test("add/list payment events") {
    forAllDbs { dbs =>
      ...
      val e3 = ChannelPaymentRelayed(paymentHash1, PaymentEvent.IncomingPayment(channelId1, remoteNodeId1, 42000 msat, now - 3.seconds), PaymentEvent.OutgoingPayment(channelId2, remoteNodeId2, 1000 msat, now))
      ...
      val e7 = ChannelPaymentRelayed(paymentHash2, PaymentEvent.IncomingPayment(channelId1, remoteNodeId1, 13000 msat, now - 5.seconds), PaymentEvent.OutgoingPayment(channelId2, remoteNodeId2, 11000 msat, now + 4.milli))
      val e8 = ChannelPaymentRelayed(paymentHash2, PaymentEvent.IncomingPayment(channelId2, remoteNodeId2, 15000 msat, now - 4.seconds), PaymentEvent.OutgoingPayment(channelId1, remoteNodeId1, 12500 msat, now + 5.milli))
  ...
  test("stats") {
    forAllDbs { dbs =>
      ...
      db.add(TransactionPublished(c2, n2, Transaction(2, Nil, Seq(TxOut(5000 sat, hex"12345")), 0), 200 sat, 100 sat, "funding"))
      db.add(TransactionConfirmed(c2, n2, Transaction(2, Nil, Seq(TxOut(5000 sat, hex"12345")), 0)))
      db.add(TransactionPublished(c2, n2, Transaction(2, Nil, Seq(TxOut(4000 sat, hex"00112233")), 0), 300 sat, 200 sat, "mutual"))
      db.add(TransactionConfirmed(c2, n2, Transaction(2, Nil, Seq(TxOut(4000 sat, hex"00112233")), 0)))
      db.add(TransactionPublished(c3, n3, Transaction(2, Nil, Seq(TxOut(8000 sat, hex"deadbeef")), 0), 400 sat, 50 sat, "funding"))
      db.add(TransactionConfirmed(c3, n3, Transaction(2, Nil, Seq(TxOut(8000 sat, hex"deadbeef")), 0)))
      db.add(TransactionPublished(c4, n4, Transaction(2, Nil, Seq(TxOut(6000 sat, hex"0000000000")), 0), 500 sat, 0 sat, "funding"))
      ...
      db.add(TransactionPublished(c2, n2, Transaction(2, Nil, Seq(TxOut(5000 sat, hex"12345")), 0), 1000 sat, 0 sat, "funding")) // duplicate
      db.add(TransactionPublished(c4, n4, Transaction(2, Nil, Seq(TxOut(4500 sat, hex"1111222233")), 0), 500 sat, 150 sat, "funding")) // unconfirmed
  ...
  ignore("relay stats performance", Tag("perf")) {
    forAllDbs { dbs =>
      ...
      channelIds.foreach(channelId => {
        ...
        db.add(TransactionPublished(channelId, nodeId, fundingTx, 100 sat, 0 sat, "funding"))
  ...
  test("migrate audit db to v14") {
    ...
    val txPublished = TransactionPublished(channelId1, remoteNodeId1, fundingTx, 200 sat, 100 sat, "funding")
