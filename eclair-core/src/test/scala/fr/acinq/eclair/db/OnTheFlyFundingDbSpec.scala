...
class OnTheFlyFundingDbSpec extends AnyFunSuite {
  ...
  test("add/list/remove pending proposals") {
    forAllDbs { dbs =>
      ...
      val upstream = Seq(
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 7, 25_000_000 msat, paymentHash1, CltvExpiry(750_000), randomOnion(), None, None), TimestampMilli(0), randomKey().publicKey),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 0, 1 msat, paymentHash1, CltvExpiry(750_000), randomOnion(), Some(randomKey().publicKey), None), TimestampMilli.now(), randomKey().publicKey),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 561, 100_000_000 msat, paymentHash2, CltvExpiry(799_999), randomOnion(), None, None), TimestampMilli.now(), randomKey().publicKey),
        Upstream.Hot.Channel(UpdateAddHtlc(randomBytes32(), 1105, 100_000_000 msat, paymentHash2, CltvExpiry(799_999), randomOnion(), None, None), TimestampMilli.now(), randomKey().publicKey),
      )
