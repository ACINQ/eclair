...
class SqlitePaymentsDbSpec extends FunSuite {
  ...
  test("handle version migration v1 -> v3") {
    ...
    // add a few rows
    val ps1 = OutgoingPayment(id = UUID.randomUUID(), paymentHash = ByteVector32(hex"0f059ef9b55bb70cc09069ee4df854bf0fab650eee6f2b87ba26d1ad08ab114f"), None, amount = 12345 msat, fee = 0 msat, createdAt = 12345, None, PENDING)
    ...
    preMigrationDb.updateOutgoingPayment(ps1.id, OutgoingPaymentStatus.SUCCEEDED, preimage = Some(ByteVector32.Zeroes), fee = 1000L msat)
    assert(preMigrationDb.getOutgoingPayment(ps1.id).head.fee.toLong === 1000L)
  }
  ...
  test("add/retrieve/update sent payments") {
    ...
    val s1 = OutgoingPayment(id = UUID.randomUUID(), paymentHash = ByteVector32(hex"0f059ef9b55bb70cc09069ee4df854bf0fab650eee6f2b87ba26d1ad08ab114f"), None, amount = 12345 msat, fee = 0 msat, createdAt = 12345, None, PENDING)
    val s2 = OutgoingPayment(id = UUID.randomUUID(), paymentHash = ByteVector32(hex"08d47d5f7164d4b696e8f6b62a03094d4f1c65f16e9d7b11c4a98854707e55cf"), None, amount = 12345 msat, fee = 0 msat, createdAt = 12345, None, PENDING)
    ...
    db.updateOutgoingPayment(s3.id, FAILED, preimage = None, fee = 0L msat)
    ...
    assertThrows[IllegalArgumentException](db.updateOutgoingPayment(s3.id, SUCCEEDED, preimage = None, fee = 10000L msat))

    db.updateOutgoingPayment(s1.id, SUCCEEDED, preimage = Some(ByteVector32.One), fee = 20000L msat)
    assert(db.getOutgoingPayment(s1.id).get.preimage.isDefined)
    assert(db.getOutgoingPayment(s1.id).get.completedAt.isDefined)
    assert(db.getOutgoingPayment(s1.id).get.fee.toLong === 20000)
  }
