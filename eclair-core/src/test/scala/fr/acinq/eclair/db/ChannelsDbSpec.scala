...
class ChannelsDbSpec extends AnyFunSuite {
  ...
  test("add/remove/list channels") {
    forAllDbs { dbs =>
      ...
      val channel2a = ChannelCodecsSpec.normal.modify(_.metaCommitments.main.channelId).setTo(randomBytes32())
  ...
  test("concurrent channel updates") {
    forAllDbs { dbs =>
      ...
      val futures = for (i <- 0 until 10000) yield {
        ...
        Future(db.addOrUpdateChannel(channel.modify(_.metaCommitments.main.channelId).setTo(channelId)))
  ...
  test("channel metadata") {
    forAllDbs { dbs =>
      ...
      val channel1 = ChannelCodecsSpec.normal
      val channel2 = channel1.modify(_.metaCommitments.main.channelId).setTo(randomBytes32())
...
object ChannelsDbSpec {
  ...
  val testCases: Seq[TestCase] = for (_ <- 0 until 10) yield {
    ...
    val data = channelDataCodec.encode(ChannelCodecsSpec.normal.modify(_.metaCommitments.main.channelId).setTo(channelId)).require.bytes
    TestCase(
      channelId = channelId,
      data = data,
