...
import fr.acinq.eclair.{TimestampMilli, randomBytes32}
...
class PendingCommandsDbSpec extends AnyFunSuite {
  ...
  test("add/remove/list messages") {
    forAllDbs { dbs =>
      ...
      val msg2 = CMD_FAIL_HTLC(2, Left(randomBytes32()), useAttributableErrors = false, TimestampMilli.now())
      val msg3 = CMD_FAIL_HTLC(3, Left(randomBytes32()), useAttributableErrors = false, TimestampMilli.now())
  ...
  val testCases: Set[TestCase] = (0 until 100).flatMap { _ =>
    ...
    val cmds = (0 until Random.nextInt(5)).map { _ =>
      Random.nextInt(2) match {
        ...
        case 1 => CMD_FAIL_HTLC(Random.nextLong(100_000), Right(UnknownNextPeer()), useAttributableErrors = false, TimestampMilli.now())
      }
    }
