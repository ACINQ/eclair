...
import fr.acinq.eclair.{TimestampMilli, UInt64, randomBytes}
...
class CommandCodecsSpec extends AnyFunSuite {
  ...
  test("encode/decode all settlement commands") {
    val testCases: Map[HtlcSettlementCommand, ByteVector] = Map(
      ...
      CMD_FAIL_HTLC(253, FailureReason.LocalFailure(TemporaryNodeFailure()), Some(TimestampMilli(123))) -> hex"0005 00000000000000fd 01 0002 2002 ff 000000000000007b",
      CMD_FAIL_HTLC(253, FailureReason.LocalFailure(TemporaryNodeFailure(TlvStream(Set.empty[FailureMessageTlv], Set(GenericTlv(UInt64(17), hex"deadbeef"))))), Some(TimestampMilli(456))) -> hex"0005 00000000000000fd 01 0008 2002 1104deadbeef ff 00000000000001c8",
