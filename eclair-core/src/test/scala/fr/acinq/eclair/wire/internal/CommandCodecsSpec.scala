...
class CommandCodecsSpec extends AnyFunSuite {
  ...
  test("encode/decode all settlement commands") {
    val testCases: Map[HtlcSettlementCommand, ByteVector] = Map(
      ...
      CMD_FAIL_HTLC(253, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli(1234)) -> hex"0003 00000000000000fd ff 0002 2002 00 00000000000004d2",
      CMD_FAIL_HTLC(253, Right(TemporaryNodeFailure(TlvStream(Set.empty[FailureMessageTlv], Set(GenericTlv(UInt64(17), hex"deadbeef"))))), useAttributableErrors = true, TimestampMilli(56789)) -> hex"0003 00000000000000fd ff 0008 2002 1104deadbeef ff 000000000000ddd5",
  ...
  test("backward compatibility") {
    ...
    val testCases = Map(
      ...
      hex"0001 000000000000002a900100" -> CMD_FAIL_HTLC(42, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min),
