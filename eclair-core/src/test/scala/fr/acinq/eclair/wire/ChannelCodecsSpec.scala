...
class ChannelCodecsSpec extends FunSuite {
  ...
  test("HtlcTimeoutTx compatibility") {
    // HtlcTimeoutTx contains a `paymentHash` field now whereas earlier it did not. Since old format of HtlcTimeoutTx is stored in `localCommit.publishableTxs` it should now deserialize as HtlcTimeoutTxLegacy
    ...
    val htlcTimeoutTx2 =
      // This is an old HtlcTimeoutTx without `paymentHash` which should now be HtlcTimeoutTxLegacy
