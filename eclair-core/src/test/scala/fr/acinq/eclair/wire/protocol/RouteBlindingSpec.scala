...
class RouteBlindingSpec extends AnyFunSuiteLike {
  ...
  test("decode encrypted route blinding data") {
    ...
    val BlindedRouteDetails(blindedRoute, lastPathKey) = Sphinx.RouteBlinding.create(sessionKey, nodePrivKeys.map(_.publicKey), payloads.map(_._2))
    val pathKey0 = sessionKey.publicKey
    val Right(RouteBlindingDecryptedData(decryptedPayload0, pathKey1)) = RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys(0), pathKey0, blindedRoute.encryptedPayloads(0))
    val Right(RouteBlindingDecryptedData(decryptedPayload1, pathKey2)) = RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys(1), pathKey1, blindedRoute.encryptedPayloads(1))
    val Right(RouteBlindingDecryptedData(decryptedPayload2, pathKey3)) = RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys(2), pathKey2, blindedRoute.encryptedPayloads(2))
    val Right(RouteBlindingDecryptedData(decryptedPayload3, pathKey4)) = RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys(3), pathKey3, blindedRoute.encryptedPayloads(3))
    val Right(RouteBlindingDecryptedData(decryptedPayload4, _)) = RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys(4), pathKey4, blindedRoute.encryptedPayloads(4))
    assert(Seq(decryptedPayload0, decryptedPayload1, decryptedPayload2, decryptedPayload3, decryptedPayload4) == payloads.map(_._1))
    assert(lastPathKey == pathKey4)
  }
