...
class RouteBlindingSpec extends AnyFunSuiteLike {
  ...
  test("decode encrypted route blinding data") {
    ...
    val payloads = Seq[(TlvStream[RouteBlindingEncryptedDataTlv], ByteVector)](
      (TlvStream(Padding(hex"000000"), OutgoingChannelId(ShortChannelId(561))), hex"0103000000 02080000000000000231"),
      ...
      (TlvStream(Seq(OutgoingChannelId(ShortChannelId(42))), Seq(GenericTlv(UInt64(65535), hex"06c1"))), hex"0208000000000000002a fdffff0206c1"),
    )
  ...
  test("decode invalid encrypted route blinding data") {
    ...
    for (testCase <- testCases) {
      ...
      assert(RouteBlindingEncryptedDataCodecs.decode(nodePrivKeys.last, blinding1, blindedRoute.encryptedPayloads.last).isFailure)
    }
  }
