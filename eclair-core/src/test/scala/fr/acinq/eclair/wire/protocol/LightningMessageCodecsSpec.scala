...
class LightningMessageCodecsSpec extends AnyFunSuite {
  ...
  test("encode/decode live node_announcements") {
    ...
    val node = legacyNodeAnnouncementCodec.decode(bin).require.value
    val bin2 = legacyNodeAnnouncementCodec.encode(node).require
    assert(bin == bin2)
  }
  ...
  test("encode/decode all channel messages") {
    ...
    val msgs = List(
      ...
      LegacyChannelAnnouncement(randomBytes64(), randomBytes64(), randomBytes64(), randomBytes64(), Features(bin(7, 9)), Block.RegtestGenesisBlock.hash, RealShortChannelId(1), randomKey().publicKey, randomKey().publicKey, randomKey().publicKey, randomKey().publicKey),
      LegacyNodeAnnouncement(randomBytes64(), Features(DataLossProtect -> Optional), 1 unixsec, randomKey().publicKey, Color(100.toByte, 200.toByte, 300.toByte), "node-alias", IPv4(InetAddress.getByAddress(Array[Byte](192.toByte, 168.toByte, 1.toByte, 42.toByte)).asInstanceOf[Inet4Address], 42000) :: Nil),
      LegacyChannelUpdate(randomBytes64(), Block.RegtestGenesisBlock.hash, ShortChannelId(1), 2 unixsec, LegacyChannelUpdate.MessageFlags(dontForward = false), LegacyChannelUpdate.ChannelFlags.DUMMY, CltvExpiryDelta(3), 4 msat, 5 msat, 6, 25_000_000 msat),
      LegacyAnnouncementSignatures(randomBytes32(), RealShortChannelId(42), randomBytes64(), randomBytes64()),
  ...
  test("decode channel_update with htlc_maximum_msat") {
    ...
    val update = lightningMessageCodec.decode(bin.bits).require.value.asInstanceOf[LegacyChannelUpdate]
    assert(update == LegacyChannelUpdate(ByteVector64(hex"58fff7d0e987e2cdd560e3bb5a046b4efe7b26c969c2f51da1dceec7bcb8ae1b634790503d5290c1a6c51d681cf8f4211d27ed33a257dcc1102862571bf17923"), BlockHash(ByteVector32(hex"06226e46111a0b59caaf126043eb5bbf28c34f3a5e332a1fc7b2b73cf188910f")), ShortChannelId(0x5a10000020000L), 1539791129 unixsec, LegacyChannelUpdate.MessageFlags(dontForward = false), LegacyChannelUpdate.ChannelFlags(isEnabled = true, isNode1 = false), CltvExpiryDelta(6), 1 msat, 1 msat, 10, 980_000_000 msat))
  ...
  test("non-regression on channel flags") {
    val testCases = Map(
      bin"0000 0000" -> LegacyChannelUpdate.ChannelFlags(isEnabled = true, isNode1 = true),
      bin"0000 0001" -> LegacyChannelUpdate.ChannelFlags(isEnabled = true, isNode1 = false),
      bin"0000 0010" -> LegacyChannelUpdate.ChannelFlags(isEnabled = false, isNode1 = true),
      bin"0000 0011" -> LegacyChannelUpdate.ChannelFlags(isEnabled = false, isNode1 = false),
    )
