...
class LightningMessageCodecsSpec extends AnyFunSuite {
  ...
  test("nonreg generic tlv") {
    ...
    val refs = Map(
      ...
      hex"0084" ++ channelId ++ ByteVector64.Zeroes ++ hex"0000" ++ hex"01 20" ++ txId.value.reverse ++ hex"02 62" ++ partialSig ++ ByteVector(nonce.toByteArray) -> CommitSig(channelId, txId, PartialSignatureWithNonce(partialSig, nonce), Nil, batchSize = 1),
