  ...
  class SqliteChannelsDbWithValidation(innerDb: SqliteChannelsDb) extends SqliteChannelsDb(innerDb.sqlite) {
    override def addOrUpdateChannel(data: PersistentChannelData): Unit = {
      ...
      def freeze3(input: PersistentChannelData): PersistentChannelData = input match {
        case d: ChannelStateData.DATA_WAIT_FOR_FUNDING_CONFIRMED => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_WAIT_FOR_FUNDING_LOCKED => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_WAIT_FOR_REMOTE_PUBLISH_FUTURE_COMMITMENT => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_NORMAL => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_CLOSING => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_NEGOTIATING => d.copy(commitments = freeze2(d.commitments))
        case d: ChannelStateData.DATA_SHUTDOWN => d.copy(commitments = freeze2(d.commitments))
      }
