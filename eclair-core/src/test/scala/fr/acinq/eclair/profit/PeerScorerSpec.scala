...
class PeerScorerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with AnyFunSuiteLike {
  ...
  private def channelInfo(canSend: BtcAmount, canReceive: BtcAmount, channelId: ByteVector32 = randomBytes32(), scid_opt: Option[RealShortChannelId] = None, isPublic: Boolean = true): ChannelInfo = ChannelInfo(
    channelId = channelId,
    shortChannelId_opt = scid_opt,
  ...
  test("simulate exhausted large outgoing flow") {
    // We use a large scoring frequency to remove rate-limits on peers that have exhausted their liquidity.
    val config = defaultConfig.copy(scoringFrequency = 7 days)
  ...
  test("fund channels with whitelisted peers") {
    val config = defaultConfig.copy(topPeersWhitelist = Set(remoteNodeId3), scoringFrequency = 7 days)
    withFixture(onChainBalance = 10 btc, config = config) { f =>
      import f._
  ...
  test("don't fund channels if maximum capacity already exceeded") {
    val config = defaultConfig.copy(topPeersWhitelist = Set(remoteNodeId1), liquidity = defaultConfig.liquidity.copy(maxPerPeerCapacity = 10 btc))
    withFixture(onChainBalance = 10 btc, config = config) { f =>
      import f._

      val c1a = channelInfo(canSend = 1 btc, canReceive = 8 btc)
      val c1b = channelInfo(canSend = 0.5 btc, canReceive = 3 btc)

      // We need to scale the last bucket of test data based on the current timestamp, otherwise we will overestimate
      // the outgoing payment volume of the current bucket and increase relay fees.
      val bucketRatio = Bucket.consumed(TimestampMilli.now())

      // We have a stable, large outgoing flow with our peer: we should add liquidity.
      val peerInfo = PeerInfo(
        remoteNodeId = remoteNodeId1,
        stats = peerStats(totalAmountOut = 0.5.btc * bucketRatio, relayFeeEarned = 0.01.btc * bucketRatio) +: Seq.fill(weeklyBuckets - 1)(peerStats(totalAmountOut = 0.5 btc, relayFeeEarned = 0.01 btc)),
        channels = Seq(c1a, c1b),
        latestUpdate_opt = Some(channelUpdate(c1a.capacity, timestamp = TimestampSecond.now() - Bucket.duration * 2)),
        hasPendingChannel = false,
      )

      // But our total capacity already exceeds our maximum configured amount, so we won't fund a new channel.
      scorer ! ScorePeers(None)
      inside(tracker.expectMessageType[GetLatestStats]) { msg =>
  ...
  test("close idle channels with liquidity") {
    withFixture(onChainBalance = 0 btc) { f =>
      ...
      // We have several old channels where more than 80% of the liquidity is on our side and the remote balance is lower than our threshold.
      ...
      // We don't close channels where 80% of the liquidity is on our side, but the remote balance is still large because those channels are huge.
      val c4a = channelInfo(canSend = 8 btc, canReceive = 0.5 btc)
      val c4b = channelInfo(canSend = 8 btc, canReceive = 0.5 btc)
      // We don't close channel where 80% of the liquidity is on our side, but it is below our local balance threshold because channels are tiny.
      val c5a = channelInfo(canSend = 0.5 btc, canReceive = 0.01 btc)
      val c5b = channelInfo(canSend = 0.05 btc, canReceive = 0.001 btc)
      // We don't close channels that have been opened very recently.
      val c6a = channelInfo(canSend = 0.8 btc, canReceive = 0.01 btc, scid_opt = Some(RealShortChannelId(blockHeight = nodeParams.currentBlockHeight - 1, txIndex = 3, outputIndex = 1)))
      val c6b = channelInfo(canSend = 0.7 btc, canReceive = 0.01 btc, scid_opt = Some(RealShortChannelId(blockHeight = nodeParams.currentBlockHeight - 1, txIndex = 51, outputIndex = 3)))
      ...
      val peerInfo4 = PeerInfo(
        remoteNodeId = randomKey().publicKey,
        stats = Seq.fill(weeklyBuckets)(peerStats(totalAmountOut = 5_000 sat, totalAmountIn = 8_000 sat, relayFeeEarned = 80 sat)),
        channels = Seq(c4a, c4b),
        latestUpdate_opt = None,
        hasPendingChannel = false
      )
      val peerInfo5 = PeerInfo(
        remoteNodeId = randomKey().publicKey,
        stats = Seq.fill(weeklyBuckets)(peerStats(totalAmountOut = 5_000 sat, totalAmountIn = 8_000 sat, relayFeeEarned = 80 sat)),
        channels = Seq(c5a, c5b),
        latestUpdate_opt = None,
        hasPendingChannel = false
      )
      val peerInfo6 = PeerInfo(
        remoteNodeId = randomKey().publicKey,
        stats = Seq.fill(weeklyBuckets)(peerStats(totalAmountOut = 5_000 sat, totalAmountIn = 8_000 sat, relayFeeEarned = 80 sat)),
        channels = Seq(c6a, c6b),
        latestUpdate_opt = None,
        hasPendingChannel = false
      )
      ...
      scorer ! ScorePeers(None)
      inside(tracker.expectMessageType[GetLatestStats]) { msg =>
        msg.replyTo ! LatestStats(Seq(peerInfo1, peerInfo2, peerInfo3, peerInfo4, peerInfo5, peerInfo6))
      }
  ...
  test("don't fund the same peer within cooldown period") {
    withFixture(onChainBalance = 10 btc) { f =>
      ...
      // Second scoring cycle with a lower volume: we don't fund again because the cooldown is active and volume has decreased.
      scorer ! ScorePeers(None)
      inside(tracker.expectMessageType[GetLatestStats]) { msg =>
        val slightlyLowerStats = Seq.fill(weeklyBuckets)(peerStats(totalAmountOut = 0.99 btc, relayFeeEarned = 0.01 btc))
        msg.replyTo ! LatestStats(Seq(PeerInfo(remoteNodeId1, slightlyLowerStats, Seq(channel), None, hasPendingChannel = false)))
      }
      register.expectNoMessage(100 millis)

      // Third scoring cycle: volume improved slightly, so we fund again despite cooldown.
      scorer ! ScorePeers(None)
      inside(tracker.expectMessageType[GetLatestStats]) { msg =>
        val improvedStats = Seq.fill(weeklyBuckets)(peerStats(totalAmountOut = 1.1 btc, relayFeeEarned = 0.011 btc))
