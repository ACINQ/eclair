...
case class SwapOutReceiverSpec() extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("happy path for new swap out") { f =>
    ...
    val agreement = register.expectMessageType[ForwardShortId[SwapOutAgreement]].message
    ...
    val paymentInvoice = Bolt11Invoice.fromString(openingTxBroadcasted.message.payreq).get
