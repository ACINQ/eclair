...
trait TestVectorsSpec extends AnyFunSuite with Logging {
  ...
  val (commitmentInput, redeemInfo) = Funding.makeFundingInputInfo(fundingTx.txid, 0, fundingAmount, Local.funding_pubkey, Remote.funding_pubkey, commitmentFormat)
  ...
  logger.info(s"# funding wscript = $redeemInfo")
  assert(redeemInfo == RedeemInfo.SegwitV0(hex"5221023da092f6980e58d2c037173180e9a465476026ee50f96695963e8efe436f54eb21030e9f7b623d2ccc7c9bd44d66d5ce21ce504c0acf6385a132cec6d3c39fa711c152ae"))
  ...
  def run(name: String, specHtlcs: Set[DirectedHtlc]): (CommitTx, Seq[TransactionWithInputInfo]) = {
    ...
    val signedTxs = htlcTxs.collect {
      case tx: HtlcSuccessTx =>
        ...
        val htlcIndex = htlcScripts.indexOf(Script.parse(tx.redeemInfo.asInstanceOf[RedeemInfo.SegwitV0].redeemScript))
