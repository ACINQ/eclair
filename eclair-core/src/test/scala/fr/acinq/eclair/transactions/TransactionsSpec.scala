...
class TransactionsSpec extends AnyFunSuite with Logging {
  ...
  private def testCommitAndHtlcTxs(commitmentFormat: CommitmentFormat): Unit = {
    ...
    val publicNonces = Seq(localNonce, remoteNonce).map(_.public)
      ...
      val commitTx = commitmentFormat match {
        case _: SimpleTaprootChannelCommitmentFormat =>
          val Right(commitTx) = for {
            ...
            _ = assert(txInfo.checkRemotePartialSignature(localFundingPriv.publicKey, remoteFundingPriv.publicKey, remotePartialSig, localNonce.public))
            invalidRemotePartialSig = ChannelSpendSignature.PartialSignatureWithNonce(randomBytes32(), remotePartialSig.nonce)
            _ = assert(!txInfo.checkRemotePartialSignature(localFundingPriv.publicKey, remoteFundingPriv.publicKey, invalidRemotePartialSig, localNonce.public))
