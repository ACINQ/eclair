...
class TransactionsSpec extends AnyFunSuite with Logging {
  ...
  test("check pre-computed transaction weights") {
    ...
    }
    {
      ...
      val htlcSuccessRedeemScript = toLocalDelayed(localRevocationPriv.publicKey, toLocalDelay, localPaymentPriv.publicKey)
      val htlcSuccessTxOutput = CommitmentOutput.HtlcSuccessOutput(TxOut(20000 sat, pay2wsh(htlcSuccessRedeemScript)), RedeemInfo.SegwitV0(htlcSuccessRedeemScript))
      val htlcSuccessOrTimeoutTx = Transaction(version = 2, txIn = Nil, txOut = htlcSuccessTxOutput.txOut :: Nil, lockTime = 0)
      val Right(htlcDelayedTx) = makeHtlcDelayedTx(htlcSuccessTxOutput, 0, htlcSuccessOrTimeoutTx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      // we use dummy signatures to compute the weight
    ...
    }
    {
      ...
      val Right(htlcPenaltyTx) = makeHtlcPenaltyTx(commitTx, 0, RedeemInfo.SegwitV0(redeemScript), localDustLimit, finalPubKeyScript, feeratePerKw)
      // we use dummy signatures to compute the weight
    ...
    }
    {
      ...
      val commitTx = Transaction(version = 2, txIn = Nil, txOut = TxOut(htlc.amountMsat.truncateToSatoshi, pubKeyScript) :: Nil, lockTime = 0)
      val Right(claimHtlcSuccessTx) = makeClaimHtlcSuccessTx(commitTx, outputs, localDustLimit, finalPubKeyScript, htlc, feeratePerKw, DefaultCommitmentFormat)
      // we use dummy signatures to compute the weight
    ...
    }
    {
      ...
      val commitTx = Transaction(version = 2, txIn = Nil, txOut = TxOut(htlc.amountMsat.truncateToSatoshi, pubKeyScript) :: Nil, lockTime = 0)
      val Right(claimClaimHtlcTimeoutTx) = makeClaimHtlcTimeoutTx(commitTx, outputs, localDustLimit, finalPubKeyScript, htlc, feeratePerKw, DefaultCommitmentFormat)
      // we use dummy signatures to compute the weight
    ...
    }
    {
      ...
      val Right(claimAnchorOutputTx) = makeClaimLocalAnchorOutputTx(commitTx, localFundingPriv.publicKey, ConfirmationTarget.Absolute(BlockHeight(1105)), ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
  ...
  test("generate valid commitment and htlc transactions (default commitment format)") {
    val finalPubKeyScript = Script.write(Script.pay2wpkh(PrivateKey(randomBytes32()).publicKey))
    val (commitInput, redeemInfo) = Funding.makeFundingInputInfo(randomTxId(), 0, Btc(1), localFundingPriv.publicKey, remoteFundingPriv.publicKey, DefaultCommitmentFormat)
    ...
    val commitTx = {
      val txInfo = makeCommitTx(commitInput, redeemInfo, commitTxNumber, localPaymentPriv.publicKey, remotePaymentPriv.publicKey, localIsChannelOpener = true, outputs)
    ...
    }
    {
      ...
      val htlcTimeoutOutputs = outputs.collect { case CommitmentOutput.OutHtlc(_, _, _, Some(htlcTimeoutOutput)) => htlcTimeoutOutput }
      val Right(htlcDelayed) = makeHtlcDelayedTx(htlcTimeoutOutputs(1), 0, htlcTimeoutTxs(1).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      ...
      val htlcDelayed1 = makeHtlcDelayedTx(htlcTimeoutOutputs(0), 0, htlcTimeoutTxs(0).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
    ...
    }
    {
      ...
      for ((htlc, paymentPreimage) <- (htlc1, paymentPreimage1) :: (htlc3, paymentPreimage3) :: Nil) {
        val Right(claimHtlcSuccessTx) = makeClaimHtlcSuccessTx(commitTx.tx, outputs, localDustLimit, finalPubKeyScript, htlc, feeratePerKw, DefaultCommitmentFormat)
    ...
    }
    {
      ...
      for ((htlcSuccessTx, paymentPreimage) <- (htlcSuccessTxs(1), paymentPreimage2) :: (htlcSuccessTxs(0), paymentPreimage4) :: Nil) {
        ...
        assert(checkSpendable(signedTx).isSuccess)
        // check remote sig
        assert(htlcSuccessTx.checkSig(remoteSig, remoteHtlcPriv.publicKey, TxOwner.Remote, DefaultCommitmentFormat, Map.empty))
      }
    }
    {
      // local spends delayed output of htlc2 success tx
      val htlcSuccessOutputs = outputs.collect { case CommitmentOutput.InHtlc(_, _, _, Some(htlcSuccess)) => htlcSuccess }
      val Right(htlcDelayed) = makeHtlcDelayedTx(htlcSuccessOutputs(1), 0, htlcSuccessTxs(1).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      val localSig = htlcDelayed.sign(localDelayedPaymentPriv, TxOwner.Local, DefaultCommitmentFormat, Map.empty)
      val signedTx = addSigs(htlcDelayed, localSig)
      assert(checkSpendable(signedTx).isSuccess)
      // local can't claim delayed output of htlc4 success tx because it is below the dust limit
      val htlcDelayed1 = makeHtlcDelayedTx(htlcSuccessOutputs(0), 0, htlcSuccessTxs(0).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      assert(htlcDelayed1 == Left(AmountBelowDustLimit))
    }
    {
      // local spends main delayed output
      val toLocal = outputs.collectFirst { case t: CommitmentOutput.ToLocal => t }.get
      val outputIndex = outputs.indexOf(toLocal)
      val Right(claimMainOutputTx) = makeClaimLocalDelayedOutputTx(toLocal, outputIndex, commitTx.tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
    ...
    }
    {
      ...
      val Right(claimHtlcTimeoutTx) = makeClaimHtlcTimeoutTx(commitTx.tx, outputs, localDustLimit, finalPubKeyScript, htlc2, feeratePerKw, DefaultCommitmentFormat)
    ...
    }
    {
      ...
      val Seq(Right(claimHtlcDelayedPenaltyTx)) = makeClaimHtlcDelayedOutputPenaltyTxs(htlcTimeoutTxs(1).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, DefaultCommitmentFormat)
      ...
      val claimHtlcDelayedPenaltyTx1 = makeClaimHtlcDelayedOutputPenaltyTxs(htlcTimeoutTxs(0).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, DefaultCommitmentFormat)
    ...
    }
    {
      ...
      val Some(htlcOutputIndex) = outputs.zipWithIndex.find {
        case (o: OutHtlc, _) => o.outgoingHtlc.add.id == htlc1.id
        case _ => false
      }.map(_._2)
      val Right(htlcPenaltyTx) = makeHtlcPenaltyTx(commitTx.tx, htlcOutputIndex, RedeemInfo.SegwitV0(script), localDustLimit, finalPubKeyScript, feeratePerKw)
    ...
    }
    {
      ...
      val Seq(Right(claimHtlcDelayedPenaltyTx)) = makeClaimHtlcDelayedOutputPenaltyTxs(htlcSuccessTxs(1).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, DefaultCommitmentFormat)
      val sig = claimHtlcDelayedPenaltyTx.sign(localRevocationPriv, TxOwner.Local, DefaultCommitmentFormat, Map.empty)
      val signed = addSigs(claimHtlcDelayedPenaltyTx, sig)
      assert(checkSpendable(signed).isSuccess)
      // remote can't claim revoked output of htlc4's htlc-success tx because it is below the dust limit
      val claimHtlcDelayedPenaltyTx1 = makeClaimHtlcDelayedOutputPenaltyTxs(htlcSuccessTxs(0).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, DefaultCommitmentFormat)
      assert(claimHtlcDelayedPenaltyTx1 == Seq(Left(AmountBelowDustLimit)))
    }
    {
      ...
      val Some(htlcOutputIndex) = outputs.zipWithIndex.find {
        case (i: InHtlc, _) => i.incomingHtlc.add.id == htlc2.id
        case _ => false
      }.map(_._2)
      val Right(htlcPenaltyTx) = makeHtlcPenaltyTx(commitTx.tx, htlcOutputIndex, RedeemInfo.SegwitV0(script), localDustLimit, finalPubKeyScript, feeratePerKw)
      val sig = htlcPenaltyTx.sign(localRevocationPriv, TxOwner.Local, DefaultCommitmentFormat, Map.empty)
  ...
  def generateCommitAndHtlcTxs(commitmentFormat: CommitmentFormat): Unit = {
    ...
    val (commitInput, commitRedeemInfo) = Funding.makeFundingInputInfo(fundingTxOutpoint.txid, fundingTxOutpoint.index.toInt, Btc(1), localFundingPriv.publicKey, remoteFundingPriv.publicKey, commitmentFormat)
    ...
    val (commitTx, commitTxOutputs, htlcTimeoutTxs, htlcSuccessTxs) = {
      ...
      val txInfo = makeCommitTx(commitInput, commitRedeemInfo, commitTxNumber, localPaymentPriv.publicKey, remotePaymentPriv.publicKey, localIsChannelOpener = true, outputs)
      val commitTx = commitmentFormat match {
        case SimpleTaprootChannelCommitmentFormat =>
          ...
          val sharedInput = Musig2Input(InputInfo(fundingTxOutpoint, fundingOutput), RedeemInfo.TaprootKeyPath(Taproot.musig2Aggregate(localFundingPriv.publicKey, remoteFundingPriv.publicKey), None), 0, remoteFundingPriv.publicKey)
        ...
        case DefaultCommitmentFormat | _: AnchorOutputsCommitmentFormat =>
          ...
          assertWeightMatches(commitTx.tx.weight(), expectedCommitTxWeight, commitmentFormat)
          val sharedInput = Multisig2of2Input(InputInfo(fundingTxOutpoint, fundingOutput), RedeemInfo.SegwitV0(Scripts.multiSig2of2(localFundingPriv.publicKey, remoteFundingPriv.publicKey)), 0, remoteFundingPriv.publicKey)
          assertWitnessWeightMatches(commitTx.tx.txIn(0).witness, sharedInput.weight, commitmentFormat)
          commitTx
      }
      ...
      val zeroFeeCommitTx = makeCommitTx(commitInput, commitRedeemInfo, commitTxNumber, localPaymentPriv.publicKey, remotePaymentPriv.publicKey, localIsChannelOpener = true, zeroFeeOutputs)
    ...
    {
      // local spends main delayed output
      val toLocal = commitTxOutputs.collectFirst { case t: CommitmentOutput.ToLocal => t }.get
      val outputIndex = commitTxOutputs.indexOf(toLocal)
    ...
    }
    {
      ...
      val Right(claimAnchorOutputTx) = makeClaimLocalAnchorOutputTx(commitTx.tx, anchorKey.publicKey, ConfirmationTarget.Absolute(BlockHeight(0)), commitmentFormat).map(anchorTx => {
    ...
    }
    {
      ...
      for (htlcTimeoutTx <- htlcTimeoutTxs) {
        ...
        signedTx.tx.correctlySpends(Seq(commitTx.tx), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
        assert(checkSpendable(signedTx).isSuccess)
    ...
    {
      val htlcTimeoutOutputs = commitTxOutputs.collect { case CommitmentOutput.OutHtlc(_, _, _, Some(htlcTimeoutOutput)) => htlcTimeoutOutput }
      // local spends delayed output of htlc1 timeout tx
      val Right(htlcDelayed) = makeHtlcDelayedTx(htlcTimeoutOutputs(1), 0, htlcTimeoutTxs(1).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
    ...
    }
    {
      ...
      for ((htlcSuccessTx, paymentPreimage) <- (htlcSuccessTxs(0), paymentPreimage4) :: (htlcSuccessTxs(1), paymentPreimage2) :: (htlcSuccessTxs(2), paymentPreimage2) :: Nil) {
        ...
        assert(checkSpendable(signedTx).isSuccess)
        // check remote sig
        assert(htlcSuccessTx.checkSig(remoteSig, remoteHtlcPriv.publicKey, TxOwner.Remote, commitmentFormat, Map.empty))
        ...
        for (sighash <- invalidSighash) {
          ...
          assert(checkSpendable(invalidTx).isFailure)
          assert(!invalidTx.checkSig(invalidRemoteSig, remoteHtlcPriv.publicKey, TxOwner.Remote, commitmentFormat, Map.empty))
        }
      }
    }
    {
      // local spends delayed output of htlc2a and htlc2b success txs
      val htlcSuccessOutputs = commitTxOutputs.collect { case CommitmentOutput.InHtlc(_, _, _, Some(htlcSuccess)) => htlcSuccess }
      val Right(htlcDelayedA) = makeHtlcDelayedTx(htlcSuccessOutputs(1), 0, htlcSuccessTxs(1).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      val Right(htlcDelayedB) = makeHtlcDelayedTx(htlcSuccessOutputs(2), 0, htlcSuccessTxs(2).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      ...
      // local can't claim delayed output of htlc4 success tx because it is below the dust limit
      val claimHtlcDelayed1 = makeClaimLocalDelayedOutputTx(htlcSuccessOutputs(0), 0, htlcSuccessTxs(0).tx, localDustLimit, toLocalDelay, finalPubKeyScript, feeratePerKw)
      assert(claimHtlcDelayed1 == Left(AmountBelowDustLimit))
    }
    {
      // remote spends local->remote htlc outputs directly in case of success
      for ((htlc, paymentPreimage) <- (htlc1, paymentPreimage1) :: (htlc3, paymentPreimage3) :: Nil) {
        val Right(claimHtlcSuccessTx) = makeClaimHtlcSuccessTx(commitTx.tx, commitTxOutputs, localDustLimit, finalPubKeyScript, htlc, feeratePerKw, commitmentFormat)
    ...
    {
      // remote spends htlc1's htlc-timeout tx with revocation key
      val Seq(Right(claimHtlcDelayedPenaltyTx)) = makeClaimHtlcDelayedOutputPenaltyTxs(htlcTimeoutTxs(1).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, commitmentFormat)
      ...
      assert(checkSpendable(signedTx).isSuccess)
      // remote can't claim revoked output of htlc3's htlc-timeout tx because it is below the dust limit
      val claimHtlcDelayedPenaltyTx1 = makeClaimHtlcDelayedOutputPenaltyTxs(htlcTimeoutTxs(0).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, commitmentFormat)
      assert(claimHtlcDelayedPenaltyTx1 == Seq(Left(AmountBelowDustLimit)))
    }
    {
      // remote spends remote->local htlc output directly in case of timeout
      for (htlc <- Seq(htlc2a, htlc2b)) {
        val Right(claimHtlcTimeoutTx) = makeClaimHtlcTimeoutTx(commitTx.tx, commitTxOutputs, localDustLimit, finalPubKeyScript, htlc, feeratePerKw, commitmentFormat)
    ...
    }
    {
      ...
      val Seq(Right(claimHtlcDelayedPenaltyTxA)) = makeClaimHtlcDelayedOutputPenaltyTxs(htlcSuccessTxs(1).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, commitmentFormat)
      val Seq(Right(claimHtlcDelayedPenaltyTxB)) = makeClaimHtlcDelayedOutputPenaltyTxs(htlcSuccessTxs(2).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, commitmentFormat)
      ...
      // remote can't claim revoked output of htlc4's htlc-success tx because it is below the dust limit
      val claimHtlcDelayedPenaltyTx1 = makeClaimHtlcDelayedOutputPenaltyTxs(htlcSuccessTxs(0).tx, localDustLimit, localRevocationPriv.publicKey, toLocalDelay, localDelayedPaymentPriv.publicKey, finalPubKeyScript, feeratePerKw, commitmentFormat)
      assert(claimHtlcDelayedPenaltyTx1 == Seq(Left(AmountBelowDustLimit)))
    }
    {
    ...
    }
    {
      ...
      val Right(htlcPenaltyTx) = commitmentFormat match {
        case SimpleTaprootChannelCommitmentFormat =>
          ...
          makeHtlcPenaltyTx(commitTx.tx, htlcOutputIndex, RedeemInfo.TaprootKeyPath(localRevocationPriv.publicKey.xOnly, Some(scriptTree)), localDustLimit, finalPubKeyScript, feeratePerKw)
        case DefaultCommitmentFormat | _: AnchorOutputsCommitmentFormat =>
          ...
          makeHtlcPenaltyTx(commitTx.tx, htlcOutputIndex, RedeemInfo.SegwitV0(script), localDustLimit, finalPubKeyScript, feeratePerKw)
      }
    ...
    }
    {
      ...
      for (htlc <- Seq(htlc2a, htlc2b)) {
        val Some(htlcOutputIndex) = commitTxOutputs.zipWithIndex.find {
          case (i: InHtlc, _) => i.incomingHtlc.add.id == htlc.id
          case _ => false
        }.map(_._2)
        val Right(htlcPenaltyTx) = commitmentFormat match {
          case SimpleTaprootChannelCommitmentFormat =>
  ...
  test("sort the htlc outputs using BIP69 and cltv expiry") {
    ...
    val (commitInput, redeemInfo) = Funding.makeFundingInputInfo(TxId.fromValidHex("a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0"), 0, Btc(1), localFundingPriv.publicKey, remoteFundingPriv.publicKey, DefaultCommitmentFormat)
    ...
    assert(outputs.collectFirst { case OutHtlc(txOut, OutgoingHtlc(htlc), _, _) if htlc == htlc2 => txOut.publicKeyScript }.contains(htlcOut2.publicKeyScript))
    assert(outputs.collectFirst { case OutHtlc(txOut, OutgoingHtlc(htlc), _, _) if htlc == htlc3 => txOut.publicKeyScript }.contains(htlcOut3.publicKeyScript))
    assert(outputs.collectFirst { case OutHtlc(txOut, OutgoingHtlc(htlc), _, _) if htlc == htlc4 => txOut.publicKeyScript }.contains(htlcOut4.publicKeyScript))
    assert(outputs.collectFirst { case OutHtlc(txOut, OutgoingHtlc(htlc), _, _) if htlc == htlc5 => txOut.publicKeyScript }.contains(htlcOut5.publicKeyScript))
  }

  test("find our output in closing tx") {
    val (commitInput, redeemInfo) = Funding.makeFundingInputInfo(randomTxId(), 0, Btc(1), localFundingPriv.publicKey, remoteFundingPriv.publicKey, ZeroFeeHtlcTxAnchorOutputsCommitmentFormat)
...

    {
      ...
      val closingTx = makeClosingTx(commitInput, redeemInfo, localPubKeyScript, remotePubKeyScript, localPaysClosingFees = true, localDustLimit, 1000 sat, spec)
    ...
    }
    {
      ...
      val spec = CommitmentSpec(Set.empty, feeratePerKw, 150_000_000 msat, 250_000_000 msat)
      val closingTxs = makeSimpleClosingTxs(commitInput, redeemInfo, spec, SimpleClosingTxFee.PaidByUs(5_000 sat), 0, localPubKeyScript, remotePubKeyScript)
    ...
    }
    {
      ...
      val remotePubKeyScript = Script.write(OP_RETURN :: OP_PUSHDATA(hex"deadbeef") :: Nil)
      val closingTxs = makeSimpleClosingTxs(commitInput, redeemInfo, spec, SimpleClosingTxFee.PaidByThem(5_000 sat), 0, localPubKeyScript, remotePubKeyScript)
      assert(closingTxs.localAndRemote_opt.nonEmpty)
      assert(closingTxs.localOnly_opt.nonEmpty)
      assert(closingTxs.remoteOnly_opt.isEmpty)
      val localAndRemote = closingTxs.localAndRemote_opt.flatMap(_.toLocalOutput).get
    ...
    }
    {
      ...
      val closingTx = makeClosingTx(commitInput, redeemInfo, localPubKeyScript, remotePubKeyScript, localPaysClosingFees = false, localDustLimit, 1000 sat, spec)
      assert(closingTx.tx.txOut.length == 2)
      assert(closingTx.toLocalOutput !== None)
      val toLocal = closingTx.toLocalOutput.get
      assert(toLocal.publicKeyScript == localPubKeyScript)
    ...
    }
    {
      ...
      val closingTxs = makeSimpleClosingTxs(commitInput, redeemInfo, spec, SimpleClosingTxFee.PaidByThem(800 sat), 0, localPubKeyScript, remotePubKeyScript)
    ...
    }
    {
      ...
      val spec = CommitmentSpec(Set.empty, feeratePerKw, 150_000_000 msat, 1_000_000 msat)
      val remotePubKeyScript = Script.write(OP_RETURN :: OP_PUSHDATA(hex"deadbeef") :: Nil)
      val closingTxs = makeSimpleClosingTxs(commitInput, redeemInfo, spec, SimpleClosingTxFee.PaidByThem(1_001 sat), 0, localPubKeyScript, remotePubKeyScript)
      assert(closingTxs.all.size == 1)
      assert(closingTxs.localOnly_opt.nonEmpty)
      val toLocal = closingTxs.localOnly_opt.flatMap(_.toLocalOutput).get
      assert(toLocal.publicKeyScript == localPubKeyScript)
    ...
    }
    {
      ...
      val closingTxs = makeSimpleClosingTxs(commitInput, redeemInfo, spec, SimpleClosingTxFee.PaidByUs(800 sat), 0, localPubKeyScript, remotePubKeyScript)
      assert(closingTxs.all.size == 1)
