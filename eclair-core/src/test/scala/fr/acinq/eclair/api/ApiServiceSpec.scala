  ...
  test("'send' method should correctly forward amount parameters to EclairImpl") {
    ...
    Post("/payinvoice", FormData("invoice" -> invoice).toEntity) ~>
      ...
      check {
        assert(handled)
        assert(status == OK)
        eclair.send(any, 1258000, any, any, any, any, any, any, None)(any[Timeout]).wasCalled(once)
      }
    ...
    Post("/payinvoice", FormData("invoice" -> invoice, "amountMsat" -> "123", "feeThresholdSat" -> "112233", "maxFeePct" -> "2.34").toEntity) ~>
      ...
      check {
        assert(handled)
        assert(status == OK)
        eclair.send(any, 123, any, any, any, any, Some(112233), Some(2.34), None)(any[Timeout]).wasCalled(once)
      }
