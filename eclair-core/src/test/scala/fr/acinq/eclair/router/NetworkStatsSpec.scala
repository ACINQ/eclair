...
object NetworkStatsSpec {
  ...
  def fakeChannelUpdate1(cltv: CltvExpiryDelta, feeBase: MilliSatoshi, feeProportional: Long): ChannelUpdate = {
    ChannelUpdate(randomBytes64(), randomBytes32(), ShortChannelId(42), 0, ChannelUpdate.ChannelFlags(direction = false, disable = false), cltv, 1 msat, feeBase, feeProportional, None)
  }

  def fakeChannelUpdate2(cltv: CltvExpiryDelta, feeBase: MilliSatoshi, feeProportional: Long): ChannelUpdate = {
    ChannelUpdate(randomBytes64(), randomBytes32(), ShortChannelId(42), 0, ChannelUpdate.ChannelFlags(direction = true, disable = false), cltv, 1 msat, feeBase, feeProportional, None)
  }
