...
class RoutingSyncWithChecksumsSpec extends TestKit(ActorSystem("test")) with FunSuiteLike {
  ...
  val initChannels = fakeRoutingInfo.values
    .foldLeft(TreeMap.empty[ShortChannelId, PublicChannel]) {
      case (m, (c, u1, u2, _, _)) => m + (c.shortChannelId -> PublicChannel(c, fakeFundingTx(c).txid, Satoshi(0), Some(u1), Some(u2)))
    }

  def fakeFundingTx(c: ChannelAnnouncement): Transaction = {
  ...
  class FakeWatcher extends Actor {
    override def receive: Receive = {
      case ValidateRequest(c) =>
        val tx = fakeFundingTx(c)
        sender ! ValidateResult(c, Right(tx, UtxoStatus.Unspent))
    }
  }
  ...
  test("handle channel range extended (full sync)") {
    ...
    val block = ReplyChannelRange(chainHash, firstBlockNum, numberOfBlocks, 1,
      ...
      extendedInfo_opt = Some(ExtendedInfo(shortChannelIds.toList.map(Router.getChannelDigestInfo(initChannels))))
    )
    ...
    val updatedChannels = initChannels
        .map {
          case (id, pc) if updatedIds.contains(id) =>
            require(pc.update_1_opt.isDefined)
            val newUpdate1 = RoutingSyncSpec.makeNewerChannelUpdate(pc.update_1_opt.get)
            id -> pc.copy(update_1_opt = Some(newUpdate1))
          case (id, pc) =>
            id -> pc
        }
    // ask router to send a channel range query
    ...
    val block1 = ReplyChannelRange(chainHash, firstBlockNum, numberOfBlocks, 1,
      ...
      Some(ExtendedInfo(shortChannelIds.toList.map(Router.getChannelDigestInfo(updatedChannels)))))
