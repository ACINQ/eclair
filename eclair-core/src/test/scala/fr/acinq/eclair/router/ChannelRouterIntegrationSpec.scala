...
class ChannelRouterIntegrationSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ChannelStateTestsBase {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    val router: TestFSMRef[Router.State, Router.Data, Router] = TestFSMRef(new Router(channels.alice.underlyingActor.nodeParams, channels.alice.underlyingActor.blockchain, initialized = None, eventStream_opt = Some(channels.alice.underlyingActor.eventStream)))
    val rebroadcastListener = TestProbe()
    // router publishes on the system's eventstream
