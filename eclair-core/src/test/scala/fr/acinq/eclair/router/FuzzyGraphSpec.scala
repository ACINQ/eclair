...
class FuzzyGraphSpec extends FunSuite {
  ...
  test("find 500 routes between a fixed set of nodes") {
    ...
    val resultString1 = new StringBuilder
    val resultString2 = new StringBuilder
    ...
    Files.writeString(Paths.get("simulation_1000sat_master.txt"), resultString.toString())

    // with payment amount 100000 sat
    resultString1.append("## 2 random nodes, payment amount 100000 sat ##\n")
    fixedList.foreach { case (node1, node2) =>
      resultString1.append(s"$node1 -> $node2: ")
      Router.findRoute(g.copy(), node1, node2, 100000.sat.toMilliSatoshi, 0, Set.empty, Set.empty, Set.empty, DEFAULT_ROUTE_PARAMS, 123) match {
        case Failure(exception) => resultString1.append("not found")
        case Success(r) => resultString1.append(r.map(hop => s"${hop.nodeId} -> ${hop.nextNodeId}"))
      }
      resultString1.append("\n")
    }
    Files.writeString(Paths.get("simulation_100000sat_master.txt"), resultString1.toString())

    // 1000 sat from ACINQ to a random node
    resultString2.append("## ACINQ -> random node, payment amount 10000 sat ##\n")
    ...
    fixedList.foreach { case (_, node2) =>
      resultString2.append(s"$acinq -> $node2: ")
      Router.findRoute(g.copy(), acinq, node2, AMOUNT_TO_ROUTE.toMilliSatoshi, 0, Set.empty, Set.empty, Set.empty, DEFAULT_ROUTE_PARAMS, 123) match {
        case Failure(exception) => resultString2.append("not found")
        case Success(r) => resultString2.append(r.map(hop => s"${hop.nodeId} -> ${hop.nextNodeId}"))
      }
      resultString2.append("\n")
    }
    Files.writeString(Paths.get("simulation_acinq_1000sat_master.txt"), resultString2.toString())
  }
