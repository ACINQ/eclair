...
class RouteCalculationSpec extends AnyFunSuite with ParallelTestExecution {
  ...
  test("use direct channel when available") {
    ...
    val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, c, DEFAULT_AMOUNT_MSAT, 100_000_000 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(wr)), currentBlockHeight = BlockHeight(400000))
    assert(routes.distinct.length == 1)
    val route :: Nil = routes
  ...
  test("trampoline relay with direct channel to target") {
...

    {
      ...
      assert(findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 100_999 msat, Set.empty, Set.empty, Set.empty, Nil, routeParams = routeParams, currentBlockHeight = BlockHeight(400000)) === Failure(RouteNotFound))
    }
    {
      ...
      assert(findMultiPartRoute(g, BalancesEstimates.baseline(g, 1 day), a, b, amount, 101_000 msat, Set.empty, Set.empty, Set.empty, Nil, routeParams = routeParams, currentBlockHeight = BlockHeight(400000)).isSuccess)
    }
  }
  ...
  test("small local edge with liquidity is better than big remote edge") {
    ...
    val Success(routes) = findRoute(g, BalancesEstimates.baseline(g, 1 day), a, d, 50000 msat, 100000000 msat, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS.copy(heuristics = Left(wr), includeLocalChannelCost = true), currentBlockHeight = BlockHeight(400000))
    val route :: Nil = routes
