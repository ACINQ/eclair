...
class RouteCalculationSpec extends FunSuite {
  ...
  test("find the k-shortest paths in a graph, k=4") {
    ...
    val twoShortestPaths = Graph.yenKshortestPaths(graph, d, f, DEFAULT_AMOUNT_MSAT, numberOfPathsToFind = 4)
  ...
  test("find the k shortest path (wikipedia example)") {
    ...
    val twoShortestPaths = Graph.yenKshortestPaths(graph, c, h, DEFAULT_AMOUNT_MSAT, numberOfPathsToFind = 2)
