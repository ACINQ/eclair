...
import fr.acinq.eclair.router.Router.{Data, GossipDecision, PublicChannel, RemoteGossip, SendChannelQuery, State}
...
class RoutingSyncSpec extends TestKit(ActorSystem("test")) with AnyFunSuiteLike with ParallelTestExecution {
  ...
  def sync(src: TestFSMRef[State, Data, Router], tgt: TestFSMRef[State, Data, Router], extendedQueryFlags_opt: Option[QueryChannelRangeTlv]): SyncResult = {
    ...
    pipe.send(tgt, PeerRoutingMessage(RemoteGossip(pipe.ref, srcId), qcr))
    ...
    rcrs.foreach(rcr => pipe.send(src, PeerRoutingMessage(RemoteGossip(pipe.ref, tgtId), rcr)))
    ...
    while (src.stateData.sync.nonEmpty) {
      ...
      pipe.send(tgt, PeerRoutingMessage(RemoteGossip(pipe.ref, srcId), query))
      ...
      announcements.foreach(ann => pipe.send(src, PeerRoutingMessage(RemoteGossip(pipe.ref, tgtId), ann)))
      // and tgt ends this chunk with a reply_short_channel_id_end
      val rscie = pipe.expectMsgType[ReplyShortChannelIdsEnd]
      pipe.send(src, PeerRoutingMessage(RemoteGossip(pipe.ref, tgtId), rscie))
    }
  ...
  test("sync with standard channel queries") {
    ...
    fakeRoutingInfo.take(10).values.foreach {
      case (pc, na1, na2) =>
        sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), pc.ann))
        sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), pc.update_1_opt.get))
        // we don't send channel_update #2
        sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), na1))
        sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), na2))
    }
    ...
    fakeRoutingInfo.take(10).values.foreach {
      case (pc, _, _) =>
        sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), pc.update_2_opt.get))
    }
  ...
  def syncWithExtendedQueries(requestNodeAnnouncements: Boolean): Unit = {
    ...
    bumpedUpdates.foreach(c => sender.send(bob, PeerRoutingMessage(RemoteGossip(sender.ref, charlieId), c)))
  ...
  test("reset sync state on reconnection") {
    ...
    // send first block
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), block1))
