...
class ChannelRangeQueriesSpec extends FunSuite {
  ...
  test("compute flag tests") {
    ...
    assert(Router.getChannelDigestInfo(channels)(ab.shortChannelId) == TimestampsAndChecksums(now, 714408668, now, 714408668))
    ...
    assert(Router.computeFlag(channels)(ab.shortChannelId, Some(TimestampsAndChecksums(now + 1, 714408668, now + 1, 714408668))) === 0.toByte)
    // different checksums, newer timestamps: we ask for the updates
    assert(Router.computeFlag(channels)(ab.shortChannelId, Some(TimestampsAndChecksums(now + 1, 154654604, now, 714408668))) === INCLUDE_CHANNEL_UPDATE_1)
    ...
    assert(Router.computeFlag(channels)(ab.shortChannelId, Some(TimestampsAndChecksums(now - 1, 154654604, now, 714408668))) === 0.toByte)
    ...
    assert(Router.computeFlag(channels)(cd.shortChannelId, Some(TimestampsAndChecksums(now, 714408668, now, 714408668))) === INCLUDE_CHANNEL_UPDATE_2)
