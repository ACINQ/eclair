...
class RouterSpec extends BaseRouterSpec {
  ...
  test("given a pre-defined channels route with routing hints add the proper channel updates") { fixture =>
...

    {
      ...
      assert(route.hops.head.asInstanceOf[ChannelHop].params == HopRelayParams.FromAnnouncement(update_ab))
      assert(route.hops.last.asInstanceOf[ChannelHop].params == HopRelayParams.FromHint(invoiceRoutingHint))
    }
    {
      ...
      assert(route.hops.collect { case h: ChannelHop => h.params }.dropRight(1) == Seq(HopRelayParams.FromAnnouncement(update_ag_private), HopRelayParams.FromAnnouncement(update_gh)))
      assert(route.hops.last.asInstanceOf[ChannelHop].params == HopRelayParams.FromHint(invoiceRoutingHint))
    }
  }
