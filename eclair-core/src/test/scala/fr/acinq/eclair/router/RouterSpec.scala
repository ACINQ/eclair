...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_ac))
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, update_ac))
      peerConnection.expectMsg(TransportHandler.ReadAck(update_ac))
      peerConnection.expectMsg(GossipDecision.Accepted(update_ac))
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, node_c))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_uc))
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, update_uc))
      peerConnection.expectMsg(TransportHandler.ReadAck(update_uc))
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, node_u))
    ...
    {
      // duplicates
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, node_b))
      peerConnection.expectMsg(TransportHandler.ReadAck(node_b))
      peerConnection.expectMsg(GossipDecision.Duplicate(node_b))
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_ab))
      peerConnection.expectMsg(TransportHandler.ReadAck(chan_ab))
      peerConnection.expectMsg(GossipDecision.Duplicate(chan_ab))
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, update_ab))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, invalid_node_b))
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, invalid_chan_ac))
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, invalid_update_ab))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_vc))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, update_ay))
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, node_y))
...

    {
      ...
      val node_y = makeNodeAnnouncement(priv_y, "node-Y", Color(123, 100, -40), Nil, TestConstants.Bob.nodeParams.features)
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_ay))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_ax))
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, chan_az))
  ...
  test("handle bad signature for ChannelAnnouncement") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, buggy_chan_ac))
  ...
  test("handle bad signature for NodeAnnouncement") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, buggy_ann_b))
  ...
  test("handle bad signature for ChannelUpdate") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, buggy_channelUpdate_ab))
  ...
  test("route not found (channel disabled)") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, channelUpdate_cd1))
  ...
  test("send local channels") { fixture =>
    ...
    val peerConnection = TestProbe()
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, g, remoteInit, update_ga))
    val sender = TestProbe()
  ...
  test("ask for channels that we marked as stale for which we receive a new update") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, announcement))
    watcher.expectMsgType[ValidateRequest]
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, staleUpdate))
    ...
    peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, recentUpdate))
  ...
  test("stream updates to front") { fixture =>
    ...
    val peerConnection = TestProbe()
    router ! PeerRoutingMessage(peerConnection.ref, remoteNodeId, remoteInit, update_ab_2)
    sender.fishForMessage() {
