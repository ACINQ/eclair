...
import fr.acinq.bitcoin.scala.{Block, Transaction, TxOut}
...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
...

    {
      ...
      eventListener.expectMsg(ChannelUpdatesReceived(update_ac.copy(signature = null) :: Nil))
...

    {
      ...
      eventListener.expectMsg(ChannelUpdatesReceived(update_uc.copy(signature = null) :: Nil))
...

    {
      ...
      peerConnection.expectMsg(GossipDecision.Duplicate(update_ab.copy(signature = null)))
      peerConnection.expectNoMsg(100 millis)
       router ! Router.TickBroadcast
       eventListener.expectNoMsg(100 millis)
    }
...
//      eventListener.expectNoMsg(100 millis)
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(peerConnection.ref, remoteNodeId, update_ab))
      peerConnection.expectMsg(TransportHandler.ReadAck(update_ab))
      peerConnection.expectMsg(GossipDecision.Stale(update_ab.copy(signature = null)))
      peerConnection.expectNoMsg(100 millis)
      router ! Router.TickBroadcast
      eventListener.expectNoMsg(100 millis)
    }
