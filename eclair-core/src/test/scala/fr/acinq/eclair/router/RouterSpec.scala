...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_ac)
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), update_ac)
      peerConnection.expectMsg(TransportHandler.ReadAck(update_ac))
      peerConnection.expectMsg(GossipDecision.Accepted(update_ac))
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), node_c)
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_uc)
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), update_uc)
      peerConnection.expectMsg(TransportHandler.ReadAck(update_uc))
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), node_u)
    ...
    {
      // duplicates
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), node_a)
      peerConnection.expectMsg(TransportHandler.ReadAck(node_a))
      peerConnection.expectMsg(GossipDecision.Duplicate(node_a))
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_ab)
      peerConnection.expectMsg(TransportHandler.ReadAck(chan_ab))
      peerConnection.expectMsg(GossipDecision.Duplicate(chan_ab))
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), update_ab)
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), invalid_node_a)
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), invalid_chan_a)
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), invalid_update_ab)
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_vc)
...

    {
      ...
      peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), update_ab))
      peerConnection.expectMsg(TransportHandler.ReadAck(update_ab))
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), update_ay)
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), node_y)
...

    {
      ...
      val node_y = makeNodeAnnouncement(priv_y, "node-Y", Color(123, 100, -40), Nil, hex"0200")
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_ay)
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_ax)
...

    {
      ...
      router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_az)
  ...
  test("handle bad signature for ChannelAnnouncement") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), buggy_chan_ac))
  ...
  test("handle bad signature for NodeAnnouncement") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), buggy_ann_b))
  ...
  test("handle bad signature for ChannelUpdate") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), buggy_channelUpdate_ab))
  ...
  test("route not found (channel disabled)") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), channelUpdate_cd1))
  ...
  test("ask for channels that we marked as stale for which we receive a new update") { fixture =>
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), announcement))
    watcher.expectMsgType[ValidateRequest]
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), staleUpdate))
    ...
    peerConnection.send(router, PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), recentUpdate))
