...
import fr.acinq.eclair.payment.MultiPartPaymentLifecycleSpec
import fr.acinq.eclair.wire.ChannelCodecsSpec
import fr.acinq.eclair.wire.ChannelCodecsSpec.{fundingAmount, fundingTx, keyManager, localParams, remoteParams}
...
class StartupSpec extends FunSuite with IdiomaticMockito {
  ...
  test("reconciliation routine should import PENDING addresses") {
    ...
    channelDb.listLocalChannels() returns Seq(channel1, channel2)

    // channel1 is already IMPORTED, channel2 is PENDING
    ...
    // imnportMulti will be called with 1 element to import: address2 from channel2
    bitcoinClient.importMulti(Seq(ImportMultiItem(address2, "PENDING", Some(mockBlockChannel2.header.time))), true) returns Future.successful(true)
