...
class OfferPaymentSpec extends FixtureSpec with IntegrationPatience {
  ...
  test("basic offer") { f =>
    ...
    val offer = Await.result(carol.eclairImpl.createOffer(description_opt = Some("test offer"), amount_opt = Some(amount), expiry_opt = None, issuer_opt = None, firstNodeId_opt = None, hideNodeId = false), 10 seconds)

    assert(offer.nodeId == Some(carol.nodeId))
    assert(offer.description == Some("test offer"))
    assert(offer.amount == Some(amount))

    val payment = Await.result(alice.eclairImpl.payOfferBlocking(offer, amount, 1), 10 seconds)
  ...
  test("offer without node id (dummy hops only)") { f =>
    ...
    val amount = 20_000_000 msat
    val offer = Await.result(carol.eclairImpl.createOffer(description_opt = Some("test offer"), amount_opt = Some(amount), expiry_opt = None, issuer_opt = None, firstNodeId_opt = Some(carol.nodeId), hideNodeId = true), 10 seconds)
  ...
  test("offer without node id (real and dummy blinded hops)") { f =>
    ...
    val amount = 20_000_000 msat
    val offer = Await.result(carol.eclairImpl.createOffer(description_opt = Some("test offer"), amount_opt = Some(amount), expiry_opt = None, issuer_opt = None, firstNodeId_opt = Some(bob.nodeId), hideNodeId = true), 10 seconds)
    ...
    val payment = Await.result(alice.eclairImpl.payOfferBlocking(offer, amount, 1), 10 seconds)
    assert(payment.isInstanceOf[PaymentSent])
    payment.asInstanceOf[PaymentSent].parts.foreach(println)
    assert(payment.asInstanceOf[PaymentSent].feesPaid == 0.msat)
  }
  ...
  test("offer without node id (payer is first node of blinded path)") { f =>
    ...
    val amount = 20_000_000 msat
    val offer = Await.result(carol.eclairImpl.createOffer(description_opt = Some("test offer"), amount_opt = Some(amount), expiry_opt = None, issuer_opt = None, firstNodeId_opt = Some(alice.nodeId), hideNodeId = true), 10 seconds)
