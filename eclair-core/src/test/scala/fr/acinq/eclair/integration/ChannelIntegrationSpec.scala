...
abstract class ChannelIntegrationSpec extends IntegrationSpec {
  ...
  def prepareForceCloseCF(commitmentFormat: Transactions.CommitmentFormat): ForceCloseFixture = {
    ...
    val dataC = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get
    ...
    val dataF = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get
  ...
  def testDownstreamTimeoutLocalCommit(commitmentFormat: Transactions.CommitmentFormat): Unit = {
    ...
    awaitCond(stateListenerF.expectMsgType[ChannelStateChanged](max = 60 seconds).currentState == CLOSING, max = 60 seconds)
    sender.send(nodes("C").register, Register.Forward(sender.ref, htlc.channelId, CMD_GET_CHANNEL_DATA(ActorRef.noSender)))
    val Some(localCommit) = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Closing]].data.get.localCommitPublished
  ...
  def testDownstreamTimeoutRemoteCommit(commitmentFormat: Transactions.CommitmentFormat): Unit = {
    ...
    awaitCond({
      ...
      sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData]].data match {
        case Some(d: ChannelData.Closing) if d.remoteCommitPublished.nonEmpty => true
        case _ => false
      }
    }, max = 30 seconds, interval = 1 second)
    sender.send(nodes("C").register, Register.Forward(sender.ref, htlc.channelId, CMD_GET_CHANNEL_DATA(ActorRef.noSender)))
    val Some(remoteCommit) = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Closing]].data.get.remoteCommitPublished
  ...
  def testRevokedCommit(commitmentFormat: Transactions.CommitmentFormat): RevokedCommitFixture = {
    ...
    val finalAddressC = scriptPubKeyToAddress(sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get.commitments.localParams.defaultFinalScriptPubKey)
...
class StandardChannelIntegrationSpec extends ChannelIntegrationSpec {
  ...
  test("open a wumbo channel C <-> F, wait for longer than the default min_depth, then close") {
    ...
    val commitmentsC = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get.commitments
    ...
    val finalPubKeyScriptF = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get.commitments.localParams.defaultFinalScriptPubKey
...
abstract class AnchorChannelIntegrationSpec extends ChannelIntegrationSpec {
  ...
  def testOpenPayClose(expectedChannelType: SupportedChannelType): Unit = {
    ...
    val initialStateDataF = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get
    ...
    awaitCond({
      sender.send(nodes("F").register, Register.Forward(sender.ref, channelId, CMD_GET_CHANNEL_DATA(ActorRef.noSender)))
      val stateDataF = sender.expectMsgType[RES_GET_CHANNEL_DATA[ChannelData.Normal]].data.get
