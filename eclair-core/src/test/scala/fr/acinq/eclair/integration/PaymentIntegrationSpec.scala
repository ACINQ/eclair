...
class PaymentIntegrationSpec extends IntegrationSpec {
  ...
  test("start eclair nodes") {
    instantiateEclairNode("A", ConfigFactory.parseMap(Map("eclair.node-alias" -> "A", "eclair.channel.expiry-delta-blocks" -> 130, "eclair.server.port" -> 29730, "eclair.api.port" -> 28080, "eclair.channel.channel-flags.announce-channel" -> false, "eclair.features.trampoline_routing" -> "disabled").asJava).withFallback(withAnchorOutputsZeroFeeHtlcTxs).withFallback(commonConfig)) // A's channels are private
    instantiateEclairNode("B", ConfigFactory.parseMap(Map("eclair.node-alias" -> "B", "eclair.channel.expiry-delta-blocks" -> 131, "eclair.server.port" -> 29731, "eclair.api.port" -> 28081, "eclair.onion-messages.relay-policy" -> "relay-all").asJava).withFallback(withAnchorOutputsZeroFeeHtlcTxs).withFallback(commonConfig))
    ...
    instantiateEclairNode("D", ConfigFactory.parseMap(Map("eclair.node-alias" -> "D", "eclair.channel.expiry-delta-blocks" -> 133, "eclair.server.port" -> 29733, "eclair.api.port" -> 28083).asJava).withFallback(withAnchorOutputsZeroFeeHtlcTxs).withFallback(commonConfig))
  ...
  test("send a trampoline payment B->F1 (via trampoline G)") {
    ...
    inside(holdTimesRecorderB.expectMsgType[Router.ReportedHoldTimes]) { e =>
      ...
      // F may report a hold time, depending on the time it took for the payment to complete.
      assert(e.trampolineHoldTimes.map(_.remoteNodeId).contains(nodes("G").nodeParams.nodeId))
      if (e.trampolineHoldTimes.size > 1) assert(e.trampolineHoldTimes.map(_.remoteNodeId).contains(nodes("F").nodeParams.nodeId))
    }
  }
  ...
  test("send a blinded payment A->D with trampoline") {
    ...
    inside(holdTimesRecorderA.expectMsgType[Router.ReportedHoldTimes]) { e =>
      assert(e.holdTimes.map(_.remoteNodeId) == Seq("B").map(nodes(_).nodeParams.nodeId))
      // C may report a hold time, depending on whether it was used in the failing retry.
      assert(e.trampolineHoldTimes.map(_.remoteNodeId).contains(nodes("B").nodeParams.nodeId))
      if (e.trampolineHoldTimes.size > 1) assert(e.trampolineHoldTimes.map(_.remoteNodeId).contains(nodes("C").nodeParams.nodeId))
    }
  }
