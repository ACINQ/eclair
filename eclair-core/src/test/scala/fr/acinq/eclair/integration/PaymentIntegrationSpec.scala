...
class PaymentIntegrationSpec extends IntegrationSpec {
  ...
  test("send an HTLC A->D with an invalid expiry delta for B") {
    ...
    val commitmentBC = sender.expectMsgType[RES_GET_CHANNEL_INFO].data.get.asInstanceOf[PersistentChannelData].commitments
    ...
    nodes("B").register ! Register.ForwardShortId(sender.ref, shortIdBC, CMD_GET_CHANNEL_INFO(ActorRef.noSender))
    val channelUpdateBC_new = sender.expectMsgType[RES_GET_CHANNEL_INFO].data.get.asInstanceOf[ChannelData.Normal].channelUpdate
