...
class TrampolinePaymentSpec extends FixtureSpec with IntegrationPatience {
  ...
  override def createFixture(testData: TestData): FixtureParam = {
    ...
    val bobParams = nodeParamsFor("bob", ByteVector32(hex"7620226fec887b0b2ebe76492e5a3fd3eb0e47cd3773263f6a81b59a704dc492"))
      ...
      .modify(_.relayParams.minLocalTrampolineFees).setToIf(!testData.tags.contains(IgnoreLocalFees))(RelayFees(feeBase = 1000 msat, feeProportionalMillionths = 2000))
      .modify(_.relayParams.minLocalTrampolineFees).setToIf(testData.tags.contains(IgnoreLocalFees))(RelayFees(feeBase = 0 msat, feeProportionalMillionths = 0))
