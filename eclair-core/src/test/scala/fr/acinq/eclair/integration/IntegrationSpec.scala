...
abstract class IntegrationSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  val commonConfig = ConfigFactory.parseMap(Map(
    ...
    "eclair.offers.message-path-min-length" -> 2,
    "eclair.offers.payment-path-count" -> 2,
    "eclair.offers.payment-path-length" -> 2,
  ...
  val withZeroFeeCommitments = ConfigFactory.parseMap(Map(
    s"eclair.features.${ZeroFeeCommitments.rfcName}" -> "optional"
  ).asJava).withFallback(commonFeatures)
