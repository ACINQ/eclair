...
class IntegrationSpec extends TestKit(ActorSystem("test")) with BitcoindService with FunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("starting eclair nodes") {
    ...
    instantiateEclairNode("C", ConfigFactory.parseMap(Map("eclair.node-alias" -> "C", "eclair.expiry-delta-blocks" -> 132, "eclair.server.port" -> 29732, "eclair.api.port" -> 28082, "eclair.trampoline-payments-enable" -> true, "eclair.max-payment-attempts" -> 10)).withFallback(commonConfig))
  ...
  test("send a multi-part payment B->D") {
    ...
    val Some(received) = nodes("D").nodeParams.db.payments.getIncomingPayment(pr.paymentHash)
    assert(received.status.isInstanceOf[IncomingPaymentStatus.Received], received.status)
    assert(received.status.asInstanceOf[IncomingPaymentStatus.Received].amount === amount)
  }
  ...
  test("send a multi-part payment D->C (local channels only)") {
    ...
    val Some(received) = nodes("C").nodeParams.db.payments.getIncomingPayment(pr.paymentHash)
    assert(received.status.isInstanceOf[IncomingPaymentStatus.Received], received.status)
    assert(received.status.asInstanceOf[IncomingPaymentStatus.Received].amount === amount)
  }
  ...
  test("send a trampoline payment B->F3 (via trampoline G)") {
    ...
    assert(paymentSent.id === paymentId)
    assert(paymentSent.paymentHash === pr.paymentHash)
    assert(paymentSent.amount === amount + payment.trampolineFees)
    assert(paymentSent.feesPaid === 0.msat) // TODO: @t-bast: fix that

    // TODO: @t-bast: verify relay DB at G
    ...
    // TODO: @t-bast: validate fees and route once implemented, change targetNodeId to real recipient
    ...
    assert(outgoingSuccess.forall(p => p.targetNodeId == nodes("G").nodeParams.nodeId))
  ...
  test("send a trampoline payment D->B (via trampoline C)") {
    ...
    // TODO: @t-bast: verify relay DB at C
    ...
    assert(outgoingSuccess.forall(p => p.targetNodeId == nodes("C").nodeParams.nodeId))
    assert(outgoingSuccess.map(_.amount).sum >= amount)
  }
