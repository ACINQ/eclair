...
class IntegrationSpec extends TestKit(ActorSystem("test")) with BitcoindService with FunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("punish a node that has published a revoked commit tx") {
    ...
    awaitCond({
      ...
      println(s"rcps.size = ${rcps.size}")
      rcps.zipWithIndex.foreach { case (rcp, i) =>
        println(s"rcp.[$i].claimMainOutputTx=${rcp.claimMainOutputTx.isDefined}")
        println(s"rcp.[$i].claimHtlcDelayedPenaltyTxs=${rcp.claimHtlcDelayedPenaltyTxs.size}")
        println(s"rcp.[$i].mainPenaltyTx=${rcp.mainPenaltyTx.isDefined}")
        println(s"rcp.[$i].htlcPenaltyTxs=${rcp.htlcPenaltyTxs.size}")
        println(s"rcp.[$i].irrevocablySpent=${rcp.irrevocablySpent.size}")
        println(s"##")
      }
