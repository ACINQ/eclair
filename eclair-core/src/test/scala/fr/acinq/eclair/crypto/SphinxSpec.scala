  ...
  */
  ...
  test("last node replies with a long failure message (reference test vector)") {
    for ((payloads, packetPayloadLength) <- Seq((referencePaymentPayloads, 1300), (paymentPayloadsFull, 1300))) {
      ...
      val attribution = FailurePacket.Attribution.create(None, failurePacket, 1 millisecond, sharedSecret4)
      ...
      val attribution1 = FailurePacket.Attribution.create(Some(attribution), error, 2 milliseconds, sharedSecret3)
