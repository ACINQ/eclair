...
class LocalOnchainKeyManagerSpec extends AnyFunSuite {
  test("sign psbt (non-reg test)") {
    ...
    val Success(psbt1) = onchainKeyManager.signPsbt(psbt, psbt.getInputs.toArray().indices, Seq(0))
  ...
  test("sign psbt") {
...

    {
      ...
      val Success(psbt1) = onchainKeyManager.signPsbt(psbt, Seq(0, 1, 2), Seq(0))
    ...
    }
    {
      ...
      val Success(psbt1) = onchainKeyManager.signPsbt(psbt, Seq(0, 1), Seq(0))
    ...
    }
    {
      ...
      val Failure(error) = onchainKeyManager.signPsbt(updated, Seq(0, 1, 2), Seq(0))
    ...
    }
    {
      ...
      val Failure(error) = onchainKeyManager.signPsbt(psbt, Seq(0, 1, 2), Seq(0))
