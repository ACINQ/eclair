...
class LocalChannelKeyManagerSpec extends AnyFunSuite {
  test("generate the same secrets from the same seed") {
    ...
    assert(channelKeyManager.delayedPaymentBaseKey(keyPath) == PrivateKey(hex"195f8f7de612978117baaa750c0098362eb17ed287161cc84dc03f869e321317"))
    assert(channelKeyManager.htlcBaseKey(keyPath) == PrivateKey(hex"9430df6ca38bf1a00a8ea8f1123ec870ad04c3cbce9641e38b6bf94cb910f7f3"))
    assert(channelKeyManager.paymentBaseKey(keyPath) == PrivateKey(hex"6a2577dbac51e4ddc6fc325ff63f3eba6f37b08c3e5ac173168810d20c5632cd"))
    assert(channelKeyManager.revocationBaseKey(keyPath) == PrivateKey(hex"0871e813ddc7b29bca128c9c9b048f6e60fbe6a53fda1558f33545951e04e1ab"))
  }
  ...
  test("test vectors (testnet, funder)") {
    ...
    val fundingKey = channelKeyManager.fundingKey(fundingKeyPath, fundingTxIndex = 0)

    val localParams = TestConstants.Alice.channelParams.copy(fundingKeyPath = fundingKeyPath)
    val channelKeyPath = channelKeyManager.channelKeyPath(localParams, ChannelConfig.standard)

    assert(fundingKey.publicKey == PrivateKey(hex"216414970b4216b197a1040367419ad6922f80e8b73ced083e9afe5e6ddd8e4c").publicKey)
    assert(channelKeyManager.revocationBaseKey(channelKeyPath).publicKey == PrivateKey(hex"a4e7ab3c54752a3487b3c474467843843f28d3bb9113e65e92056ad45d1e318e").publicKey)
    assert(channelKeyManager.paymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"de24c43d24b8d6bc66b020ac81164206bb577c7924511d4e99431c0d60505012").publicKey)
    assert(channelKeyManager.delayedPaymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"8aa7b8b14a7035540c331c030be0dd73e8806fb0c97a2519d63775c2f579a950").publicKey)
    assert(channelKeyManager.htlcBaseKey(channelKeyPath).publicKey == PrivateKey(hex"94eca6eade204d6e753344c347b46bb09067c92b2fe371cf4f8362c1594c8c59").publicKey)
  ...
  test("test vectors (testnet, fundee)") {
    ...
    assert(channelKeyManager.revocationBaseKey(channelKeyPath).publicKey == PrivateKey(hex"26510db99546c9b08418fe9df2da710a92afa6cc4e5681141610dfb8019052e6").publicKey)
    assert(channelKeyManager.paymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"0766c93fd06f69287fcc7b343916e678b83942345d4080e83f4c8a061b1a9f4b").publicKey)
    assert(channelKeyManager.delayedPaymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"094aa052a9647228fd80e42461cae26c04f6cdd1665b816d4660df686915319a").publicKey)
    assert(channelKeyManager.htlcBaseKey(channelKeyPath).publicKey == PrivateKey(hex"8ec62bd03b241a2e522477ae1a9861a668429ab3e443abd2aa0f2f10e2dc2206").publicKey)
  ...
  test("test vectors (mainnet, funder)") {
    ...
    assert(channelKeyManager.revocationBaseKey(channelKeyPath).publicKey == PrivateKey(hex"ee13db7f2d7e672f21395111ee169af8462c6e8d1a6a78d808f7447b27155ffb").publicKey)
    assert(channelKeyManager.paymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"7fc18e4c925bf3c5a83411eac7f234f0c5eaef9a8022b22ec6e3272ae329e17e").publicKey)
    assert(channelKeyManager.delayedPaymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"c0d9a3e3601d79b11b948db9d672fcddafcb9a3c0873c6a738bb09087ea2bfc6").publicKey)
    assert(channelKeyManager.htlcBaseKey(channelKeyPath).publicKey == PrivateKey(hex"bd3ba7068d131a9ab47f33202d532c5824cc5fc35a9adada3644ac2994372228").publicKey)
  ...
  test("test vectors (mainnet, fundee)") {
    ...
    val fundingPub = channelKeyManager.fundingKey(fundingKeyPath, fundingTxIndex = 0)

    val localParams = TestConstants.Alice.channelParams.copy(fundingKeyPath = fundingKeyPath)
    val channelKeyPath = channelKeyManager.channelKeyPath(localParams, ChannelConfig.standard)

    assert(fundingPub.publicKey == PrivateKey(hex"46a4e818615a48a99ce9f6bd73eea07d5822dcfcdff18081ea781d4e5e6c036c").publicKey)
    assert(channelKeyManager.revocationBaseKey(channelKeyPath).publicKey == PrivateKey(hex"c2cd9e2f9f8203f16b1751bd252285bb2e7fc4688857d620467b99645ebdfbe6").publicKey)
    assert(channelKeyManager.paymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"1e4d3527788b39dc8ebc0ae6368a67e92eff55a43bea8e93054338ca850fa340").publicKey)
    assert(channelKeyManager.delayedPaymentBaseKey(channelKeyPath).publicKey == PrivateKey(hex"6bc30b0852fbc653451662a1ff6ad530f311d58b5e5661b541eb57dba8206937").publicKey)
    assert(channelKeyManager.htlcBaseKey(channelKeyPath).publicKey == PrivateKey(hex"b1be27b5232e3bc5d6a261949b4ee68d96fa61f481998d36342e2ad99444cf8a").publicKey)
