...
class ReputationSpec extends AnyFunSuite {
  ...
  test("basic, single endorsement level") {
    var r = Reputation.init(Config(enabled = true, 1 day, 1 second, 2))
    assert(r.getConfidence(10000 msat, 0) == 0)
    r = r.addPendingHtlc(htlcId1, 10000 msat, 0)
    r = r.settlePendingHtlc(htlcId1, isSuccess = true)
    assert(r.getConfidence(10000 msat, 0) === (1.0 / 3) +- 0.001)
    r = r.addPendingHtlc(htlcId2, 10000 msat, 0)
    assert(r.getConfidence(10000 msat, 0) === (1.0 / 5) +- 0.001)
    r = r.addPendingHtlc(htlcId3, 10000 msat, 0)
    r = r.settlePendingHtlc(htlcId2, isSuccess = true)
    r = r.settlePendingHtlc(htlcId3, isSuccess = true)
    assert(r.getConfidence(1 msat, 0) === 1.0 +- 0.001)
    r = r.addPendingHtlc(htlcId4, 1 msat, 0)
    assert(r.getConfidence(40000 msat, 0) === (3.0 / 11) +- 0.001)
    r = r.addPendingHtlc(htlcId5, 40000 msat, 0)
    assert(r.getConfidence(10000 msat, 0) === (3.0 / 13) +- 0.001)
    r = r.addPendingHtlc(htlcId6, 10000 msat, 0)
    r = r.settlePendingHtlc(htlcId6, isSuccess = false)
    assert(r.getConfidence(10000 msat, 0) === (3.0 / 13) +- 0.001)
  }

  test("long HTLC, single endorsement level") {
    var r = Reputation.init(Config(enabled = true, 1000 day, 1 second, 10))
    assert(r.getConfidence(100000 msat, 1, TimestampMilli(0)) == 0)
    r = r.addPendingHtlc(htlcId1, 100000 msat, 1, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId1, isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 1, TimestampMilli(0)) === (10.0 / 11) +- 0.001)
    r = r.addPendingHtlc(htlcId2, 1000 msat, 1, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId2, isSuccess = false, now = TimestampMilli(0) + 100.seconds)
  ...
  test("exponential decay, single endorsement level") {
    var r = Reputation.init(Config(enabled = true, 100 seconds, 1 second, 1))
    r = r.addPendingHtlc(htlcId1, 1000 msat, 2, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId1, isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 2, TimestampMilli(0)) == 1.0 / 2)
    r = r.addPendingHtlc(htlcId2, 1000 msat, 2, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId2, isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 2, TimestampMilli(0)) == 2.0 / 3)
    r = r.addPendingHtlc(htlcId3, 1000 msat, 2, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId3, isSuccess = true, now = TimestampMilli(0))
  ...
  test("multiple endorsement levels") {
    var r = Reputation.init(Config(enabled = true, 1 day, 1 second, 10))
    assert(r.getConfidence(1 msat, 7, TimestampMilli(0)) == 0)
    r = r.addPendingHtlc(htlcId1, 100000 msat, 0, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId1, isSuccess = true, TimestampMilli(0))
    r = r.addPendingHtlc(htlcId2, 900000 msat, 0, TimestampMilli(0))
    r = r.settlePendingHtlc(htlcId2, isSuccess = false, TimestampMilli(1000))
    r = r.addPendingHtlc(htlcId3, 50000 msat, 4, TimestampMilli(1000))
    r = r.settlePendingHtlc(htlcId3, isSuccess = true, TimestampMilli(1000))
    r = r.addPendingHtlc(htlcId4, 50000 msat, 4, TimestampMilli(1000))
    r = r.settlePendingHtlc(htlcId4, isSuccess = false, TimestampMilli(2000))
