...
class ReputationSpec extends AnyFunSuite {
  ...
  test("exponential decay") {
    ...
    r = r.settlePendingHtlc(HtlcId(add1), isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 0, BlockHeight(0), CltvExpiry(1), TimestampMilli(0)) == 1.0 / 2)
    val add2 = makeAdd(CltvExpiry(2))
    r = r.addPendingHtlc(add2, 1000 msat, 0, TimestampMilli(0))
    r = r.settlePendingHtlc(HtlcId(add2), isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 0, BlockHeight(0), CltvExpiry(1), TimestampMilli(0)) == 2.0 / 3)
    val add3 = makeAdd(CltvExpiry(2))
    r = r.addPendingHtlc(add3, 1000 msat, 0, TimestampMilli(0))
    r = r.settlePendingHtlc(HtlcId(add3), isSuccess = true, now = TimestampMilli(0))
    assert(r.getConfidence(1000 msat, 0, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 100.seconds) == 1.5 / 2.5)
  ...
  test("mix of accountable and non-accountable HTLCs") {
    ...
    r = r.addPendingHtlc(add3, 50000 msat, 1, TimestampMilli(0) + 1.minute)
    r = r.settlePendingHtlc(HtlcId(add3), isSuccess = true, TimestampMilli(0) + 1.minute)
    val add4 = makeAdd(CltvExpiry(6))
    r = r.addPendingHtlc(add4, 50000 msat, 1, TimestampMilli(0) + 1.minute)
    ...
    assert(r.getConfidence(1 msat, 1, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 2.minutes) === 0.5 +- 0.01)
    assert(r.getConfidence(1000 msat, 0, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 2.minutes) === 0.1 +- 0.01)
    assert(r.getConfidence(1000 msat, 1, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 2.minutes) === 5.0 / 11 +- 0.01)
    assert(r.getConfidence(100000 msat, 0, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 2.minutes) === 0.05 +- 0.01)
    assert(r.getConfidence(100000 msat, 1, BlockHeight(0), CltvExpiry(1), TimestampMilli(0) + 2.minutes) === 1.0 / 14 +- 0.01)
  }
}