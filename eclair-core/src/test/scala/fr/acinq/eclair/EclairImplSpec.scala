...
class EclairImplSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with IdiomaticMockito with ParallelTestExecution {
  ...
  test("convert fee rate properly") { f =>
    ...
    // standard conversion
    eclair.open(nodeId, fundingAmount = 10000000L sat, maxExcess_opt = None, pushAmount_opt = None, channelType_opt = None, fundingFeerate_opt = Some(FeeratePerByte(5 sat)), fundingFeeBudget_opt = None, announceChannel_opt = None, openTimeout_opt = None)
    ...
    eclair.open(nodeId, fundingAmount = 10000000L sat, maxExcess_opt = None, pushAmount_opt = None, channelType_opt = Some(ChannelTypes.StaticRemoteKey()), fundingFeerate_opt = Some(FeeratePerByte(1 sat)), fundingFeeBudget_opt = None, announceChannel_opt = None, openTimeout_opt = None)
  ...
  test("open with bad arguments") { f =>
    ...
    eclair.open(randomKey().publicKey, 123456 sat, None, None, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(scidAlias = true, zeroConf = true)), None, None, announceChannel_opt = Some(true), None).pipeTo(sender.ref)
    ...
    eclair.open(randomKey().publicKey, 123456 sat, None, None, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(scidAlias = true, zeroConf = true)), None, None, announceChannel_opt = Some(false), None).pipeTo(sender.ref)
    switchboard.expectMsgType[Peer.OpenChannel]

    eclair.open(randomKey().publicKey, 123456 sat, None, None, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(zeroConf = true)), None, None, announceChannel_opt = Some(true), None).pipeTo(sender.ref)
    switchboard.expectMsgType[Peer.OpenChannel]

    eclair.open(randomKey().publicKey, 123456 sat, None, None, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(zeroConf = true)), None, None, announceChannel_opt = Some(false), None).pipeTo(sender.ref)
    switchboard.expectMsgType[Peer.OpenChannel]
  }
  ...
  test("get channel info (filtered channels)") { f =>
    ...
    c1.message.replyTo ! RES_GET_CHANNEL_INFO(map(c1.channelId), c1.channelId, ActorRef.noSender, NORMAL, ChannelCodecsSpec.normal)
    val c2 = register.expectMsgType[Register.Forward[CMD_GET_CHANNEL_INFO]]
    c2.message.replyTo ! RES_GET_CHANNEL_INFO(map(c2.channelId), c2.channelId, ActorRef.noSender, NORMAL, ChannelCodecsSpec.normal)
  ...
  test("get channel info (using node id)") { f =>
    ...
    val c1 = register.expectMsgType[Register.Forward[CMD_GET_CHANNEL_INFO]]
    c1.message.replyTo ! RES_GET_CHANNEL_INFO(channels2Nodes(c1.channelId), c1.channelId, ActorRef.noSender, NORMAL, ChannelCodecsSpec.normal)
    val c2 = register.expectMsgType[Register.Forward[CMD_GET_CHANNEL_INFO]]
    c2.message.replyTo ! RES_GET_CHANNEL_INFO(channels2Nodes(c2.channelId), c2.channelId, ActorRef.noSender, NORMAL, ChannelCodecsSpec.normal)
    register.expectNoMessage()
