...
class EclairImplSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with IdiomaticMockito with ParallelTestExecution {
  ...
  test("get channel info (filtered channels)") { f =>
    ...
    register.reply(RES_GET_CHANNEL_INFO(map(c1.channelId), c1.channelId, NORMAL, Some(ChannelCodecsSpec.normal)))
    ...
    register.reply(RES_GET_CHANNEL_INFO(map(c3.channelId), c3.channelId, NORMAL, Some(ChannelCodecsSpec.normal)))
    register.expectNoMessage()

    assert(sender.expectMsgType[Iterable[RES_GET_CHANNEL_INFO]].toSet === Set(
      RES_GET_CHANNEL_INFO(a, a1, NORMAL, Some(ChannelCodecsSpec.normal)),
      RES_GET_CHANNEL_INFO(b, b1, NORMAL, Some(ChannelCodecsSpec.normal)),
    ))
  }
  ...
  test("get channel info (using node id)") { f =>
    ...
    val c1 = register.expectMsgType[Register.Forward[CMD_GET_CHANNEL_INFO]]
    register.reply(RES_GET_CHANNEL_INFO(channels2Nodes(c1.channelId), c1.channelId, NORMAL, Some(ChannelCodecsSpec.normal)))
    val c2 = register.expectMsgType[Register.Forward[CMD_GET_CHANNEL_INFO]]
    register.reply(RES_GET_CHANNEL_INFO(channels2Nodes(c2.channelId), c2.channelId, NORMAL, Some(ChannelCodecsSpec.normal)))
    register.expectNoMessage()

    assert(sender.expectMsgType[Iterable[RES_GET_CHANNEL_INFO]].toSet === Set(
      RES_GET_CHANNEL_INFO(a, a1, NORMAL, Some(ChannelCodecsSpec.normal)),
      RES_GET_CHANNEL_INFO(a, a2, NORMAL, Some(ChannelCodecsSpec.normal)),
    ))
  }
  ...
  test("get channel info (using channel id)") { f =>
    ...
    sender.expectMsg(RES_GET_CHANNEL_INFO(a, a2, NORMAL, Some(ChannelCodecsSpec.normal)))
  }
