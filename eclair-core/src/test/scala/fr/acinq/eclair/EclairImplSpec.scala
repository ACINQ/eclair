...
class EclairImplSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with IdiomaticMockito with ParallelTestExecution {
  ...
  test("convert fee rate properly") { f =>
    ...
    eclair.open(nodeId, fundingAmount = 10000000L sat, pushAmount_opt = None, channelType_opt = Some(ChannelTypes.StaticRemoteKey(scidAlias = false, zeroConf = false)), fundingFeeratePerByte_opt = Some(FeeratePerByte(1 sat)), announceChannel_opt = None, openTimeout_opt = None)
    ...
    assert(open1.channelType_opt.contains(ChannelTypes.StaticRemoteKey(scidAlias = false, zeroConf = false)))
  }
