...
import fr.acinq.eclair.blockchain.bitcoind.rpc.{BasicBitcoinJsonRPCClient, RPCPassword}
...
class BitcoinCoreFeeProviderSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("get fee rates") {
    ...
    val mockBitcoinClient = new BasicBitcoinJsonRPCClient(rpcAuthMethod = RPCPassword("", ""), host = "localhost", port = 0) {
