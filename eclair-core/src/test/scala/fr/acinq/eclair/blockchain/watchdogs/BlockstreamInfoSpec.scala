...
import fr.acinq.eclair.blockchain.watchdogs.BlockstreamInfo.CheckLatestHeaders
...
class BlockstreamInfoSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with AnyFunSuiteLike {
  ...
  test("fetch some block headers") {
    val blockstreamInfo = testKit.spawn(BlockstreamInfo(Block.LivenetGenesisBlock.hash, 630450, 13))
    val sender = testKit.createTestProbe[LatestHeaders]()
    blockstreamInfo ! CheckLatestHeaders(sender.ref)
    val expectedHeaders = Set(
      ...
      BlockHeaderAt(630460, BlockHeader.read(hex"000040206e390ff12d1568de2700470ff3417e3ef153c2960df4030000000000000000008638741f5e6870335fb5a129fa22e7dcb295ea8e1daa06fac8a1230c75143c8eee8fbe5e397a11170d01e68e".toArray)),
      BlockHeaderAt(630461, BlockHeader.read(hex"0000402090c95977a62b4f32bec3ed16642dbd33512df20d986400000000000000000000a3ab107298a7191e3aa0d49168db2d6798ec173d19522bec32c393cb31762468eb92be5e397a111719708a4b".toArray)),
      BlockHeaderAt(630462, BlockHeader.read(hex"00e0ff2705182872bc51f1a9896a72aad2ed3c0404386bd2071c09000000000000000000a2af76bc9352b01b6510dd1520542f2898fe3af97ba7a804ee496a00aa27b38f0e93be5e397a1117eb36157a".toArray)),
    )
    sender.expectMessage(LatestHeaders(630450, expectedHeaders, BlockstreamInfo.Source))
  }

  test("fetch future block headers") {
    val blockstreamInfo = testKit.spawn(BlockstreamInfo(Block.LivenetGenesisBlock.hash, 60000000, 2))
    val sender = testKit.createTestProbe[LatestHeaders]()
    blockstreamInfo ! CheckLatestHeaders(sender.ref)
    sender.expectMessage(LatestHeaders(60000000, Set.empty, BlockstreamInfo.Source))
  }
