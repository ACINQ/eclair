...
class FeeEstimatorSpec extends AnyFunSuite {
  ...
  val defaultFeerateTolerance = FeerateTolerance(0.5, 2.0, FeeratePerKw(2500 sat), 15000 sat)
  ...
  test("fee difference too high") {
    val tolerance = FeerateTolerance(ratioLow = 0.5, ratioHigh = 4.0, anchorOutputMaxCommitFeerate = FeeratePerKw(2500 sat), maxDustHtlcExposure = 25000 sat)
    val channelType = ChannelTypes.Standard
    val testCases = Seq(
