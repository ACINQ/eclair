...
class DummyOnChainWallet extends OnChainWallet with OnchainPubkeyCache {
  ...
  override def makeFundingTx(pubkeyScript: ByteVector, amount: Satoshi, feeRatePerKw: FeeratePerKw, maxFundingFee: Satoshi)(implicit ec: ExecutionContext): Future[MakeFundingTxResponse] = {
...
class NoOpOnChainWallet extends OnChainWallet with OnchainPubkeyCache {
  ...
  override def makeFundingTx(pubkeyScript: ByteVector, amount: Satoshi, feeRatePerKw: FeeratePerKw, maxFundingFee: Satoshi)(implicit ec: ExecutionContext): Future[MakeFundingTxResponse] = Promise().future // will never be completed
