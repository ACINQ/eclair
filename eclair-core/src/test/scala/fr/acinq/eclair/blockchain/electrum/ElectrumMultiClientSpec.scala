...
class ElectrumMultiClientSpec extends TestKit(ActorSystem("test")) with FunSuiteLike with Logging with BeforeAndAfterAll {
  ...
  override protected def beforeAll(): Unit = {
    ...
    stream.close()
    router = system.actorOf(Props(new ElectrumMultiClient(addresses)), "electrum-client")
  }
