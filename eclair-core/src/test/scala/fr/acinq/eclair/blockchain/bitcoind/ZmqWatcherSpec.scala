...
class ZmqWatcherSpec extends TestKitBaseClass with AnyFunSuiteLike with BitcoindService with BeforeAndAfterAll with Logging {
  ...
  test("add/remove watches from/to utxo map") {
    ...
    val w1 = WatchSpent(null, txid, outputIndex, randomBytes32, BITCOIN_FUNDING_SPENT, Nil)
    val w2 = WatchSpent(null, txid, outputIndex, randomBytes32, BITCOIN_FUNDING_SPENT, Nil)
  ...
  test("watch for spent transactions") {
    ...
    probe.send(watcher, WatchSpent(listener.ref, tx, outputIndex, BITCOIN_FUNDING_SPENT, Nil))
    listener.expectNoMsg(1 second)
  ...
  test("watch for unknown spent transactions") {
    ...
    probe.send(watcher, WatchSpent(probe.ref, tx1, outputIndex, BITCOIN_FUNDING_SPENT, Nil))
  ...
  test("publish transactions with relative and absolute delays") {
    ...
    probe.send(watcher, WatchSpent(probe.ref, tx2, 0, BITCOIN_FUNDING_SPENT, Nil))
