...
class ExplorerApiSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with AnyFunSuiteLike {
  ...
  test("fetch old block header") {
    for (explorer <- explorers) {
      val api = testKit.spawn(ExplorerApi(Block.LivenetGenesisBlock.hash, 1, 1, explorer))
      ...
      assert(headers.source === explorer.name)
  ...
  test("fetch old block headers") {
    for (explorer <- explorers) {
      val api = testKit.spawn(ExplorerApi(Block.LivenetGenesisBlock.hash, 1, 3, explorer))
      val sender = testKit.createTestProbe[LatestHeaders]()
      api ! CheckLatestHeaders(sender.ref)
  ...
  test("fetch some block headers") {
    for (explorer <- explorers) {
      val api = testKit.spawn(ExplorerApi(Block.LivenetGenesisBlock.hash, 630450, 13, explorer))
      val sender = testKit.createTestProbe[LatestHeaders]()
      api ! CheckLatestHeaders(sender.ref)
      val expectedHeaders = Set(
  ...
  test("fetch future block headers") {
    for (explorer <- explorers) {
      val api = testKit.spawn(ExplorerApi(Block.LivenetGenesisBlock.hash, 60000000, 2, explorer))
      val sender = testKit.createTestProbe[LatestHeaders]()
      api ! CheckLatestHeaders(sender.ref)
