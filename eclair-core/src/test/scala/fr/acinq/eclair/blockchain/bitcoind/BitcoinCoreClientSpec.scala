...
class BitcoinCoreClientSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("encrypt wallet") {
    ...
    bitcoinClient.makeFundingTx(pubkeyScript, 50 millibtc, FeeratePerKw(10000 sat), 100000 sat).pipeTo(sender.ref)
  ...
  test("create/commit/rollback funding txs") {
    ...
    val fundingTxs = for (_ <- 0 to 3) yield {
      val pubkeyScript = Script.write(Script.pay2wsh(Scripts.multiSig2of2(randomKey().publicKey, randomKey().publicKey)))
      bitcoinClient.makeFundingTx(pubkeyScript, Satoshi(500), FeeratePerKw(250 sat), 100000 sat).pipeTo(sender.ref)
      ...
      bitcoinClient.makeFundingTx(pubkeyScript, 50 millibtc, FeeratePerKw(250 sat), 100000 sat).pipeTo(sender.ref)
  ...
  test("ensure feerate is always above min-relay-fee") {
    ...
    bitcoinClient.makeFundingTx(pubkeyScript, 5 millibtc, FeeratePerKw(200 sat), 100000 sat).pipeTo(sender.ref)
  ...
  test("unlock transaction inputs if double-spent") {
    ...
    bitcoinClient.makeFundingTx(pubkeyScript, 250 btc, FeeratePerKw(1000 sat), 100000 sat).pipeTo(sender.ref)
