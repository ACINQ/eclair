...
import fr.acinq.eclair.blockchain.bitcoind.rpc.{BasicBitcoinJsonRPCClient, BitcoinCoreClient, JsonRPCError, RPCPassword}
...
class BitcoinCoreClientSpec extends TestKitBaseClass with BitcoindService with AnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  test("absence of rounding") {
    ...
    0 to 9 foreach { satoshi =>
      ...
      val rpcClient = new BasicBitcoinJsonRPCClient(rpcAuthMethod = RPCPassword("foo", "bar"), host = "localhost", port = 0) {
