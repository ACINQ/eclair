  ...
  test("on initialization create peers and send Reconnect to them") {
    ...
    val nodeParams = Alice.nodeParams.copy(
      db = new Databases {
        ...
        override val hostedChannels: HostedChannelsDb = Alice.nodeParams.db.hostedChannels
    ...
    val switchboard = system.actorOf(Switchboard.props(nodeParams, authenticator.ref, watcher.ref, router.ref, relayer.ref, hostedChannelGateway.ref, wallet))
