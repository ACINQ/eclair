...
class HtlcReaperSpec extends TestKit(ActorSystem("test")) with FunSuiteLike {
  ...
  test("init and cleanup") {
    ...
    val add0 = UpdateAddHtlc(data.channelId, 0, 20000 msat, randomBytes32, CltvExpiry(100), Sphinx.emptyOnionPacket)
    val add1 = UpdateAddHtlc(data.channelId, 1, 30000 msat, randomBytes32, CltvExpiry(100), Sphinx.emptyOnionPacket)

    // unrelated htlc
    val add99 = UpdateAddHtlc(randomBytes32, 0, 12345678 msat, randomBytes32, CltvExpiry(100), Sphinx.emptyOnionPacket)
