...
class OpenChannelInterceptorSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("don't spawn a wumbo channel if wumbo feature isn't enabled", Tag(ChannelStateTestsTags.DisableWumbo)) { f =>
    ...
    openChannelInterceptor ! OpenChannelInitiator(probe.ref, remoteNodeId, Peer.OpenChannel(remoteNodeId, fundingAmountBig, None, None, None, TestConstants.maxFundindFee, None, None), Features.empty, initFeatures().add(Wumbo, Optional))
  ...
  test("don't spawn a wumbo channel if remote doesn't support wumbo", Tag(ChannelStateTestsTags.DisableWumbo)) { f =>
    ...
    val probe = TestProbe[Any]()
    val fundingAmountBig = Channel.MAX_FUNDING_WITHOUT_WUMBO + 10_000.sat
    openChannelInterceptor ! OpenChannelInitiator(probe.ref, remoteNodeId, Peer.OpenChannel(remoteNodeId, fundingAmountBig, None, None, None, TestConstants.maxFundindFee, None, None), initFeatures().add(Wumbo, Optional), Features.empty)
