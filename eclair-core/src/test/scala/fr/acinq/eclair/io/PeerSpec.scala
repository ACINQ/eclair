...
class PeerSpec extends FixtureSpec {
  ...
  test("handle OpenChannelInterceptor spawning a user initiated open channel request ") { f =>
    ...
    val requestRemoteFunding = LiquidityAds.RequestRemoteFunding(50_000 sat, 250 sat, BlockHeight(750_000), 2016)
    val open = Peer.OpenChannel(remoteNodeId, 10_000 sat, None, None, None, None, Some(requestRemoteFunding), None, None, None)
    peerConnection.send(peer, open)
    assert(channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR].requestRemoteFunding_opt.contains(requestRemoteFunding))
  }
