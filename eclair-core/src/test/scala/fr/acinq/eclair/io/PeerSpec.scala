...
class PeerSpec extends FixtureSpec {
  ...
  test("handle OpenChannelInterceptor spawning a user initiated open channel request ") { f =>
    ...
    val open = Peer.OpenChannel(remoteNodeId, 10000 sat, None, None, None, TestConstants.maxFundindFee, None, None)
    peerConnection.send(peer, open)
  ...
  test("use dual-funding when available", Tag(ChannelStateTestsTags.DualFunding)) { f =>
    ...
    assert(peer.stateData.channels.isEmpty)
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 25000 sat, None, None, None, TestConstants.maxFundindFee, None, None))
  ...
  test("use requested channel type when spawning a channel", Tag(ChannelStateTestsTags.StaticRemoteKey)) { f =>
    ...
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, None, None, None, TestConstants.maxFundindFee, None, None))
    ...
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, Some(ChannelTypes.Standard()), None, None, TestConstants.maxFundindFee, None, None))
    ...
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, Some(ChannelTypes.AnchorOutputs()), None, None, TestConstants.maxFundindFee, None, None))
  ...
  test("use correct final script if option_static_remotekey is negotiated", Tag(ChannelStateTestsTags.StaticRemoteKey)) { f =>
    ...
    val probe = TestProbe()
    connect(remoteNodeId, peer, peerConnection, switchboard, remoteInit = protocol.Init(Features(StaticRemoteKey -> Mandatory)))
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 24000 sat, None, None, None, TestConstants.maxFundindFee, None, None))
    val init = channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR]
  ...
  test("compute max-htlc-value-in-flight based on funding amount", Tag("max-htlc-value-in-flight-percent")) { f =>
    ...
    {
      probe.send(peer, Peer.OpenChannel(remoteNodeId, 200_000 sat, None, None, None, TestConstants.maxFundindFee, None, None))
      val init = channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR]
      assert(init.localParams.maxHtlcValueInFlightMsat == 50_000_000.msat) // max-htlc-value-in-flight-percent
    }
    {
      probe.send(peer, Peer.OpenChannel(remoteNodeId, 500_000 sat, None, None, None, TestConstants.maxFundindFee, None, None))
      val init = channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR]
  ...
  test("do not allow option_scid_alias with public channel") { f =>
    ...
    intercept[IllegalArgumentException] {
      Peer.OpenChannel(remoteNodeId, 24000 sat, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(scidAlias = true, zeroConf = true)), None, None, TestConstants.maxFundindFee, Some(ChannelFlags(announceChannel = true)), None)
    }
  }
  ...
  test("set origin_opt when spawning a channel") { f =>
    ...
    val probe = TestProbe()
    connect(remoteNodeId, peer, peerConnection, switchboard)
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, None, Some(100 msat), None, TestConstants.maxFundindFee, None, None))
    val init = channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR]
