...
class PeerSpec extends FixtureSpec {
  ...
  test("handle OpenChannelInterceptor spawning a user initiated open channel request ") { f =>
    ...
    val open = Peer.OpenChannel(remoteNodeId, 10000 sat, None, None, None, None, None, None, None)
    peerConnection.send(peer, open)
  ...
  test("use requested channel type when spawning a channel", Tag(ChannelStateTestsTags.StaticRemoteKey)) { f =>
    ...
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, None, Some(ChannelTypes.Standard()), None, None, None, None, None))
    ...
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, None, Some(ChannelTypes.AnchorOutputs()), None, None, None, None, None))
  ...
  test("do not allow option_scid_alias with public channel") { f =>
    ...
    intercept[IllegalArgumentException] {
      Peer.OpenChannel(remoteNodeId, 24000 sat, None, Some(ChannelTypes.AnchorOutputsZeroFeeHtlcTx(scidAlias = true, zeroConf = true)), None, None, None, Some(ChannelFlags(announceChannel = true)), None)
    }
  }
  ...
  test("set origin_opt when spawning a channel") { f =>
    ...
    val probe = TestProbe()
    connect(remoteNodeId, peer, peerConnection, switchboard)
    probe.send(peer, Peer.OpenChannel(remoteNodeId, 15000 sat, None, None, Some(100 msat), None, None, None, None))
    val init = channel.expectMsgType[INPUT_INIT_CHANNEL_INITIATOR]
