...
class PendingChannelsRateLimiterSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  override protected def withFixture(test: OneArgTest): Outcome = {
    ...
    val channelsBelowLimit2 = Seq(
      ...
      DATA_NORMAL(commitments(peerBelowLimit2, randomBytes32()), ShortIds(RealScidStatus.Unknown, ShortChannelId.generateLocalAlias(), None), null, None, None, None, SpliceStatus.NoSplice),
    ...
    val privateChannels = Seq(
      ...
      DATA_NORMAL(commitments(privatePeer2, randomBytes32()), ShortIds(RealScidStatus.Unknown, ShortChannelId.generateLocalAlias(), None), null, None, None, None, SpliceStatus.NoSplice),
    )
  ...
  test("reject any requests that come in during the restore") { f =>
    ...
    val channels = Seq(
      ...
      DATA_NORMAL(commitments(randomKey().publicKey, randomBytes32()), ShortIds(RealScidStatus.Unknown, ShortChannelId.generateLocalAlias(), None), null, None, None, None, SpliceStatus.NoSplice),
