...
class PendingChannelsRateLimiterSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  override protected def withFixture(test: OneArgTest): Outcome = {
    ...
    val channelsAtLimit2 = Seq(
      DATA_WAIT_FOR_DUAL_FUNDING_CONFIRMED(commitments(peerAtLimit2, channelIdAtLimit2), None, 0 msat, BlockHeight(0), BlockHeight(0), RbfStatus.NoRbf, None),
    ...
    val initiatorChannels = Seq(
      ...
      DATA_WAIT_FOR_DUAL_FUNDING_CONFIRMED(commitments(peerAtLimit1, randomBytes32(), isOpener = true), None, 0 msat, BlockHeight(0), BlockHeight(0), RbfStatus.NoRbf, None),
