...
class ChannelSelectionSpec extends FunSuite {
  ...
  test("convert to CMD_FAIL_HTLC/CMD_ADD_HTLC") {
    ...
    // nominal case
    assert(Relayer.relayOrFail(relayPayload, Some(channelUpdate)) === RelaySuccess(ShortChannelId(12345), CMD_ADD_HTLC(relayPayload.payload.amountToForward, relayPayload.add.paymentHash, relayPayload.payload.outgoingCltv, relayPayload.nextPacket, Upstream.ChannelRelayed(relayPayload.add), commit = true)))
    // no channel_update
    ...
    assert(Relayer.relayOrFail(relayPayload_highfee, Some(channelUpdate)) === RelaySuccess(ShortChannelId(12345), CMD_ADD_HTLC(relayPayload_highfee.payload.amountToForward, relayPayload_highfee.add.paymentHash, relayPayload_highfee.payload.outgoingCltv, relayPayload_highfee.nextPacket, Upstream.ChannelRelayed(relayPayload.add), commit = true)))
  }
