...
class MultiPartHandlerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("PaymentHandler should handle multi-part payment timeout") { f =>
    ...
    assert(commands.toSet[Register.Forward[CMD_FAIL_HTLC]].map(fwd => fwd.copy(message = fwd.message.copy(htlcReceivedAt_opt = None))) == Set(
    ...
    assert(fwd.copy(message = fwd.message.copy(htlcReceivedAt_opt = None)) == Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(42, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true)))
  ...
  test("PaymentHandler should handle multi-part payment success") { f =>
    ...
    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(fwd.copy(message = fwd.message.copy(htlcReceivedAt_opt = None)) == Register.Forward(null, add2.channelId, CMD_FAIL_HTLC(add2.id, FailureReason.LocalFailure(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)), None, commit = true)))
  ...
  test("PaymentHandler should handle multi-part payment timeout then success") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, FinalPayload.Standard.createPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata)))
    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(fwd.copy(message = fwd.message.copy(htlcReceivedAt_opt = None)) == Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(0, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true)))
    awaitCond({
      f.sender.send(handler, GetPendingPayments)
      f.sender.expectMsgType[PendingPayments].paymentHashes.isEmpty
    })
  ...
  test("PaymentHandler should reject KeySend payment when feature is disabled") { f =>
    ...
    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(fwd.copy(message = fwd.message.copy(htlcReceivedAt_opt = None)) == Register.Forward(null, add.channelId, CMD_FAIL_HTLC(add.id, FailureReason.LocalFailure(IncorrectOrUnknownPaymentDetails(42000 msat, nodeParams.currentBlockHeight)), None, commit = true)))
    assert(nodeParams.db.payments.getIncomingPayment(paymentHash).isEmpty)
  }
