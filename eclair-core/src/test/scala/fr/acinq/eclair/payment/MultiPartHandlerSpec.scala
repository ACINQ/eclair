...
import fr.acinq.bitcoin.scalacompat.{Block, Btc, ByteVector32, Crypto}
...
class MultiPartHandlerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("Invoice generation with route blinding support") { f =>
    ...
    val hop_ab = Router.ChannelHop(ShortChannelId(1), a, b, Router.ChannelRelayParams.FromPaymentInfo(PaymentInfo(1000 msat, 699, CltvExpiryDelta(123), 2000 msat, 2_000_000_000 msat, Features.empty)))
    val hop_bc = Router.ChannelHop(ShortChannelId(2), b, c, Router.ChannelRelayParams.FromPaymentInfo(PaymentInfo(800 msat, 455, CltvExpiryDelta(78), 1 msat, 1_000_000_000 msat, Features.empty)))
    val hop_dc = Router.ChannelHop(ShortChannelId(3), d, c, Router.ChannelRelayParams.FromPaymentInfo(PaymentInfo(0 msat, 1700, CltvExpiryDelta(89), 1000 msat, 900_000_000 msat, Features.empty)))
    val hop_cc = Router.ChannelHop(ShortChannelId.toSelf, c, c, Router.ChannelRelayParams.FromPaymentInfo(PaymentInfo(0 msat, 0, CltvExpiryDelta(0), 0 msat, MilliSatoshi.toMilliSatoshi(Btc(1)), Features.empty)))
    ...
    router.send(router.lastSender, RouteResponse(Seq(Router.Route(finalizeRoute1.amount, Seq(hop_ab, hop_bc, hop_cc), finalizeRoute1.recipient))))
    ...
    router.send(router.lastSender, RouteResponse(Seq(Router.Route(finalizeRoute2.amount, Seq(hop_dc, hop_cc, hop_cc), finalizeRoute2.recipient))))
    val invoice = sender.expectMsgType[Bolt12Invoice]
    assert(invoice.amount == 25_000.msat)
    assert(invoice.nodeId == privKey.publicKey)
    assert(invoice.recipients.nonEmpty)
    assert(invoice.features.hasFeature(RouteBlinding, Some(Mandatory)))
    assert(invoice.description == Left("a blinded coffee please"))
    assert(invoice.offerId.contains(offer.offerId))
    assert(invoice.recipients.length == 3)
    val recipient1 = invoice.recipients(0)
    val recipient2 = invoice.recipients(1)
    val recipient3 = invoice.recipients(2)
    assert(recipient1.route.blindedNodeIds.length == 4)
    assert(recipient1.introductionNodeId == a)
    assert(recipient1.paymentInfo == PaymentInfo(1801 msat, 1155, CltvExpiryDelta(201), 2000 msat, 25_000 msat, Features.empty))
    assert(recipient2.route.blindedNodeIds.length == 4)
    assert(recipient2.introductionNodeId == d)
    assert(recipient2.paymentInfo == PaymentInfo(0 msat, 1700, CltvExpiryDelta(89), 1000 msat, 25_000 msat, Features.empty))
    assert(recipient3.route.blindedNodeIds.length == 1)
    assert(recipient3.introductionNodeId == c)
    assert(recipient3.paymentInfo == PaymentInfo(0 msat, 0, CltvExpiryDelta(0), 0 msat, 25_000 msat, Features.empty))
