...
class MultiPartHandlerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("PaymentHandler should handle multi-part payment timeout") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, FinalPayload.Standard.createPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, pr1.paymentSecret, pr1.paymentMetadata), TimestampMilli.now()))
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, FinalPayload.Standard.createPayload(add2.amountMsat, 2000 msat, add2.cltvExpiry, pr2.paymentSecret, pr2.paymentMetadata), TimestampMilli.now()))
    ...
    assert(commands.toSet[Register.Forward[CMD_FAIL_HTLC]].map(ignoreHtlcReceivedAt) == Set(
      Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(0, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true)),
      Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(1, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true))
    ))
    awaitCond({
      f.sender.send(handler, GetPendingPayments)
    ...
    f.sender.send(handler, MultiPartPaymentFSM.ExtraPaymentReceived(pr1.paymentHash, HtlcPart(1000 msat, UpdateAddHtlc(ByteVector32.One, 42, 200 msat, pr1.paymentHash, add1.cltvExpiry, add1.onionRoutingPacket, None, 1.0, None), TimestampMilli.now()), Some(PaymentTimeout())))
    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(ignoreHtlcReceivedAt(fwd) == Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(42, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true)))
  ...
  test("PaymentHandler should handle multi-part payment success") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, FinalPayload.Standard.createPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, FinalPayload.Standard.createPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, invoice.paymentSecret.reverse, invoice.paymentMetadata), TimestampMilli.now()))
    val add3 = add2.copy(id = 43)
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add3, FinalPayload.Standard.createPayload(add3.amountMsat, 1000 msat, add3.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))

    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(ignoreHtlcReceivedAt(fwd) == Register.Forward(null, add2.channelId, CMD_FAIL_HTLC(add2.id, FailureReason.LocalFailure(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)), None, commit = true)))
    ...
    f.sender.send(handler, MultiPartPaymentFSM.ExtraPaymentReceived(invoice.paymentHash, HtlcPart(1000 msat, UpdateAddHtlc(ByteVector32.One, 44, 200 msat, invoice.paymentHash, add1.cltvExpiry, add1.onionRoutingPacket, None, 1.0, None), TimestampMilli.now()), None))
  ...
  test("PaymentHandler should handle multi-part over-payment") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, FinalPayload.Standard.createPayload(add1.amountMsat, 1500 msat, add1.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, FinalPayload.Standard.createPayload(add2.amountMsat, 1500 msat, add2.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))
  ...
  test("PaymentHandler should handle multi-part payment timeout then success") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, FinalPayload.Standard.createPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))
    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(ignoreHtlcReceivedAt(fwd) == Register.Forward(null, ByteVector32.One, CMD_FAIL_HTLC(0, FailureReason.LocalFailure(PaymentTimeout()), None, commit = true)))
    awaitCond({
      f.sender.send(handler, GetPendingPayments)
      f.sender.expectMsgType[PendingPayments].paymentHashes.isEmpty
    })
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, FinalPayload.Standard.createPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, invoice.paymentSecret, invoice.paymentMetadata), TimestampMilli.now()))
  ...
  test("PaymentHandler should reject KeySend payment when feature is disabled") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, amountMsat, paymentHash, defaultExpiry, TestConstants.emptyOnionPacket, None, 1.0, None)
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, payload, TimestampMilli.now()))

    val fwd = f.register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]]
    assert(ignoreHtlcReceivedAt(fwd) == Register.Forward(null, add.channelId, CMD_FAIL_HTLC(add.id, FailureReason.LocalFailure(IncorrectOrUnknownPaymentDetails(42000 msat, nodeParams.currentBlockHeight)), None, commit = true)))
    assert(nodeParams.db.payments.getIncomingPayment(paymentHash).isEmpty)
  }
  ...
  def ignoreHtlcReceivedAt(cmd: CMD_FAIL_HTLC): CMD_FAIL_HTLC = cmd.copy(htlcReceivedAt_opt = None)

  def ignoreHtlcReceivedAt(fwd: Register.Forward[CMD_FAIL_HTLC]): Register.Forward[CMD_FAIL_HTLC] = fwd.copy(message = ignoreHtlcReceivedAt(fwd.message))
}
