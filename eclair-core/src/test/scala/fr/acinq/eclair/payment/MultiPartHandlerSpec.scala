...
class MultiPartHandlerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("PaymentHandler should reject incoming multi-part payment if the invoice is expired") { f =>
    ...
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 1000 msat, add.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment if the invoice does not allow it") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, invoice.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket, None)
    sender.send(handlerWithoutMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 1000 msat, add.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.reason == Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
    assert(nodeParams.db.payments.getIncomingPayment(invoice.paymentHash).get.status == IncomingPaymentStatus.Pending)
  }
  ...
  test("PaymentHandler should reject incoming multi-part payment with a total amount too low") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, invoice.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket, None)
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 999 msat, add.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment with a total amount too high") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, invoice.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket, None)
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 2001 msat, add.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment with an invalid payment secret") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, invoice.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket, None)
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 1000 msat, add.cltvExpiry, invoice.paymentSecret.get.reverse, invoice.paymentMetadata)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.reason == Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
    assert(nodeParams.db.payments.getIncomingPayment(invoice.paymentHash).get.status == IncomingPaymentStatus.Pending)
  }
  ...
  test("PaymentHandler should handle multi-part payment timeout") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, PaymentOnion.createFinalTlvPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, pr1.paymentSecret.get, pr1.paymentMetadata)))
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, PaymentOnion.createFinalTlvPayload(add2.amountMsat, 2000 msat, add2.cltvExpiry, pr2.paymentSecret.get, pr2.paymentMetadata)))
  ...
  test("PaymentHandler should handle multi-part payment success") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add1, PaymentOnion.createFinalTlvPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, PaymentOnion.createFinalTlvPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, invoice.paymentSecret.get.reverse, invoice.paymentMetadata)))
    val add3 = add2.copy(id = 43)
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add3, PaymentOnion.createFinalTlvPayload(add3.amountMsat, 1000 msat, add3.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
  ...
  test("PaymentHandler should handle multi-part payment timeout then success") { f =>
    ...
    f.sender.send(handler, IncomingPaymentPacket.FinalPacket(add2, PaymentOnion.createFinalTlvPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, invoice.paymentSecret.get, invoice.paymentMetadata)))
  ...
  test("PaymentHandler should reject incoming multi-part payment if the invoice doesn't exist") { f =>
    ...
    sender.send(handlerWithMpp, IncomingPaymentPacket.FinalPacket(add, PaymentOnion.createFinalTlvPayload(add.amountMsat, 1000 msat, add.cltvExpiry, paymentSecret, Some(hex"012345"))))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.id == add.id)
    assert(cmd.reason == Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
  }
