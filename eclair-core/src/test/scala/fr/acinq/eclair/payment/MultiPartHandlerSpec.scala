...
class MultiPartHandlerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("PaymentHandler should reply with a fulfill/fail, emit a PaymentReceived and add payment in DB") { f =>
...

    {
      ...
      sender.send(handlerWithoutMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createSinglePartPayload(add.amountMsat, add.cltvExpiry, pr.paymentSecret.get)))
...

    {
      ...
      val add = UpdateAddHtlc(ByteVector32.One, 0, amountMsat, pr.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket)
      sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createSinglePartPayload(add.amountMsat, add.cltvExpiry, pr.paymentSecret.get)))
      register.expectMsgType[Register.Forward[CMD_FULFILL_HTLC]]
  ...
  test("PaymentHandler should reject incoming multi-part payment if the payment request is expired") { f =>
    ...
    sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 1000 msat, add.cltvExpiry, pr.paymentSecret.get)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment if the payment request does not allow it") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, pr.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket)
    sender.send(handlerWithoutMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 1000 msat, add.cltvExpiry, pr.paymentSecret.get)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.reason == Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
    assert(nodeParams.db.payments.getIncomingPayment(pr.paymentHash).get.status === IncomingPaymentStatus.Pending)
  }
  ...
  test("PaymentHandler should reject incoming multi-part payment with a total amount too low") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, pr.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket)
    sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 999 msat, add.cltvExpiry, pr.paymentSecret.get)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment with a total amount too high") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, pr.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket)
    sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 2001 msat, add.cltvExpiry, pr.paymentSecret.get)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment with an invalid payment secret") { f =>
    ...
    val add = UpdateAddHtlc(ByteVector32.One, 0, 800 msat, pr.paymentHash, defaultExpiry, TestConstants.emptyOnionPacket)
    sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 1000 msat, add.cltvExpiry, pr.paymentSecret.get.reverse)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.reason == Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
    assert(nodeParams.db.payments.getIncomingPayment(pr.paymentHash).get.status === IncomingPaymentStatus.Pending)
  }
  ...
  test("PaymentHandler should handle multi-part payment timeout") { f =>
    ...
    f.sender.send(handler, IncomingPacket.FinalPacket(add1, PaymentOnion.createMultiPartPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, pr1.paymentSecret.get)))
    ...
    f.sender.send(handler, IncomingPacket.FinalPacket(add2, PaymentOnion.createMultiPartPayload(add2.amountMsat, 2000 msat, add2.cltvExpiry, pr2.paymentSecret.get)))
  ...
  test("PaymentHandler should handle multi-part payment success") { f =>
    ...
    f.sender.send(handler, IncomingPacket.FinalPacket(add1, PaymentOnion.createMultiPartPayload(add1.amountMsat, 1000 msat, add1.cltvExpiry, pr.paymentSecret.get)))
    ...
    f.sender.send(handler, IncomingPacket.FinalPacket(add2, PaymentOnion.createMultiPartPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, pr.paymentSecret.get.reverse)))
    val add3 = add2.copy(id = 43)
    f.sender.send(handler, IncomingPacket.FinalPacket(add3, PaymentOnion.createMultiPartPayload(add3.amountMsat, 1000 msat, add3.cltvExpiry, pr.paymentSecret.get)))
  ...
  test("PaymentHandler should handle multi-part payment timeout then success") { f =>
    ...
    f.sender.send(handler, IncomingPacket.FinalPacket(add2, PaymentOnion.createMultiPartPayload(add2.amountMsat, 1000 msat, add2.cltvExpiry, pr.paymentSecret.get)))
  ...
  test("PaymentHandler should reject incoming payments if the payment request doesn't exist") { f =>
    ...
    sender.send(handlerWithoutMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createSinglePartPayload(add.amountMsat, add.cltvExpiry, paymentSecret)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
  ...
  test("PaymentHandler should reject incoming multi-part payment if the payment request doesn't exist") { f =>
    ...
    sender.send(handlerWithMpp, IncomingPacket.FinalPacket(add, PaymentOnion.createMultiPartPayload(add.amountMsat, 1000 msat, add.cltvExpiry, paymentSecret)))
    val cmd = register.expectMsgType[Register.Forward[CMD_FAIL_HTLC]].message
    assert(cmd.id === add.id)
    assert(cmd.reason === Right(IncorrectOrUnknownPaymentDetails(1000 msat, nodeParams.currentBlockHeight)))
  }
