...
class MultiPartPaymentLifecycleSpec extends TestKit(ActorSystem("test")) with fixture.FunSuiteLike {
  ...
  def initPayment(f: FixtureParam, request: SendPaymentRequest, networkStats: NetworkStats, localChannels: OutgoingChannels): Unit = {
    import f._
  ...
  test("send to remote node via single big channel") { f =>
    import f._
    val pr = createMultiPartInvoice(3000 * 1000 msat)
    val payment = SendPaymentRequest(3500 * 1000 msat, paymentHash, e, 3, CltvExpiryDelta(12), Some(pr))
    ...
    initPayment(f, payment, emptyStats.copy(capacity = Stats(Seq(1000), d => Satoshi(d.toLong))), localChannel)
    waitUntilAmountSent(f, payment.amount)
  ...
  test("split fees between child payments") { f =>
    ...
    val pr = createMultiPartInvoice(3000 * 1000 msat)
    val payment = SendPaymentRequest(3000 * 1000 msat, paymentHash, e, 3, CltvExpiryDelta(12), Some(pr), routeParams = Some(routeParams))
    initPayment(f, payment, emptyStats.copy(capacity = Stats(Seq(1000), d => Satoshi(d.toLong))), localChannels())
  ...
  test("cannot send (fee rate too high)") { f =>
    import f._
    val pr = createMultiPartInvoice(2500 * 1000 msat)
    val payment = SendPaymentRequest(2500 * 1000 msat, paymentHash, e, 3, CltvExpiryDelta(12), Some(pr))
    initPayment(f, payment, emptyStats.copy(capacity = Stats(Seq(1000), d => Satoshi(d.toLong))), OutgoingChannels(Seq(
