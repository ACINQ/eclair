...
class MultiPartPaymentLifecycleSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("successful first attempt (single part)") { f =>
    ...
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(finalAmount, expiry, paymentSecret, None)
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount, expiry, 1, routeParams = routeParams.copy(randomize = true))
    ...
    assert(childPayment.finalPayload.asInstanceOf[FinalPayload.Standard].expiry == expiry)
    assert(childPayment.finalPayload.asInstanceOf[FinalPayload.Standard].paymentSecret == paymentSecret)
    assert(childPayment.finalPayload.asInstanceOf[FinalPayload.Standard].amount == finalAmount)
    assert(childPayment.finalPayload.asInstanceOf[FinalPayload.Standard].totalAmount == finalAmount)
  ...
  test("successful first attempt (multiple parts)") { f =>
    ...
    assert(payFsm.stateName == WAIT_FOR_PAYMENT_REQUEST)
    val paymentSecret = randomBytes32()
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(1200000 msat, expiry, paymentSecret, Some(hex"012345"))
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, 1200000 msat, expiry, 1, routeParams = routeParams.copy(randomize = false))
    sender.send(payFsm, payment)
    ...
    assert(childPayments.map(_.finalPayload.asInstanceOf[FinalPayload.Standard].expiry).toSet == Set(expiry))
    childPayments.foreach(childPayment => assert(childPayment.finalPayload.isInstanceOf[FinalPayload.Standard]))
    assert(childPayments.map(_.finalPayload.asInstanceOf[FinalPayload.Standard].paymentSecret).toSet == Set(paymentSecret))
    assert(childPayments.map(_.finalPayload.asInstanceOf[FinalPayload.Standard].paymentMetadata).toSet == Set(Some(hex"012345")))
    assert(childPayments.map(_.finalPayload.asInstanceOf[FinalPayload.Standard].amount).toSet == Set(500000 msat, 700000 msat))
    assert(childPayments.map(_.finalPayload.asInstanceOf[FinalPayload.Standard].totalAmount).toSet == Set(1200000 msat))
    assert(payFsm.stateName == PAYMENT_IN_PROGRESS)
  ...
  test("send custom tlv records") { f =>
    ...
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(finalAmount + 1000.msat, expiry, randomBytes32(), None,  additionalTlvs = Seq(trampolineTlv), userCustomTlvs = Seq(userCustomTlv))
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount + 1000.msat, expiry, 1, routeParams = routeParams)
    sender.send(payFsm, payment)
  ...
  test("successful retry") { f =>
    ...
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(finalAmount, expiry, randomBytes32(), None)
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount, expiry, 3, routeParams = routeParams)
    sender.send(payFsm, payment)
    router.expectMsgType[RouteRequest]
  ...
  test("retry with updated routing hints") { f =>
    ...
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount, expiry, 3, routeParams = routeParams, extraEdges = List(extraEdge))
    sender.send(payFsm, payment)
  ...
  test("abort after too many failed attempts") { f =>
    ...
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(finalAmount, expiry, randomBytes32(), None)
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount, expiry, 2, routeParams = routeParams)
    sender.send(payFsm, payment)
    router.expectMsgType[RouteRequest]
  ...
  test("abort if no routes found") { f =>
    ...
    sender.watch(payFsm)
    val finalPayload = FinalPayload.Standard.createMultiPartPayload(finalAmount, expiry, randomBytes32(), None)
    val payment = SendMultiPartPayment(sender.ref, e, finalPayload, finalAmount, expiry, 5, routeParams = routeParams)
    sender.send(payFsm, payment)
    router.expectMsgType[RouteRequest]
    router.send(payFsm, Status.Failure(RouteNotFound))
