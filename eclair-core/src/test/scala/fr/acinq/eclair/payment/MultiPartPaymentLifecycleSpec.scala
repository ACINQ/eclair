...
class MultiPartPaymentLifecycleSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("update routing hints") { () =>
...

    {
      val failures = Seq(
        ...
        UnreadableRemoteFailure(finalAmount, Nil, randomBytes(292), Nil)
      )
      val extraEdges1 = Seq(
        ExtraEdge(a, b, ShortChannelId(1), 10 msat, 0, CltvExpiryDelta(12), 1 msat, None),
  ...
  test("abort after too many failed attempts") { f =>
    ...
    childPayFsm.send(payFsm, PaymentFailed(failedId1, paymentHash, Seq(UnreadableRemoteFailure(failedRoute1.amount, failedRoute1.hops, randomBytes(292), Nil))))
    router.expectMsgType[RouteRequest]
    ...
    val result = abortAfterFailure(f, PaymentFailed(failedId2, paymentHash, Seq(UnreadableRemoteFailure(failedRoute2.amount, failedRoute2.hops, randomBytes(292), Nil))))
  ...
  test("receive partial success after retriable failure (recipient spec violation)") { f =>
    ...
    childPayFsm.send(payFsm, PaymentFailed(failedId, paymentHash, Seq(UnreadableRemoteFailure(failedRoute.amount, failedRoute.fullRoute, randomBytes(292), Nil))))
    router.expectMsgType[RouteRequest]
