...
class MultiPartPaymentLifecycleSpec extends TestKit(ActorSystem("test")) with fixture.FunSuiteLike {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    class TestMultiPartPaymentLifecycle extends MultiPartPaymentLifecycle(nodeParams, id, relayer.ref, router.ref, TestProbe().ref) {
      override def spawnChildPaymentFsm(progressHandler: PaymentProgressHandler): ActorRef = childPayFsm.ref
    }
  ...
  test("fail after too many attempts") { f =>
    ...
    assert(result.failures.last.asInstanceOf[LocalFailure].t.getMessage === "payments attempts exhausted without success")
  }
