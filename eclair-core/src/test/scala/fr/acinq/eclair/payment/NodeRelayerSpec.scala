...
class NodeRelayerSpec extends TestkitBaseClass {
  ...
  test("fail to relay when incoming multi-part payment times out") { f =>
    ...
    val parts = incomingMultiPart.dropRight(1).map(i => MultiPartPaymentFSM.PendingPayment(i.add.id, PaymentReceived.PartialPayment(i.add.amountMsat, i.add.channelId), sender.ref))
  ...
  test("fail all extraneous multi-part incoming HTLCs") { f =>
    ...
    val sender = TestProbe()
    val partial = MultiPartPaymentFSM.PendingPayment(15, PaymentReceived.PartialPayment(100 msat, randomBytes32), sender.ref)
