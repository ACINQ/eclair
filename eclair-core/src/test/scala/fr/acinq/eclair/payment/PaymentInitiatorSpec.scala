...
class PaymentInitiatorSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("forward payment with pre-defined route") { f =>
    ...
    payFsm.expectMsg(SendPaymentConfig(payment.paymentId, payment.parentId, None, paymentHash, c, Upstream.Local(payment.paymentId, upgradeAccountability = false), Some(invoice), None, storeInDb = true, publishEvent = true, recordPathFindingMetrics = false))
  ...
  test("forward single-part payment when multi-part deactivated", Tag(Tags.DisableMPP)) { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    payFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, c, Upstream.Local(id, upgradeAccountability = false), Some(invoice), None, storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
  ...
  test("forward multi-part payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    multiPartPayFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, c, Upstream.Local(id, upgradeAccountability = false), Some(invoice), None, storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
  ...
  test("forward single-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    payFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, invoice.nodeId, Upstream.Local(id, upgradeAccountability = false), Some(invoice), Some(payerKey), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
  ...
  test("forward multi-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    multiPartPayFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, invoice.nodeId, Upstream.Local(id, upgradeAccountability = false), Some(invoice), Some(payerKey), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true))
    val payment = multiPartPayFsm.expectMsgType[SendMultiPartPayment]
    assert(payment.recipient.nodeId == invoice.nodeId)
    assert(payment.recipient.totalAmount == finalAmount)
    assert(payment.recipient.extraEdges.nonEmpty)
