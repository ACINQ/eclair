...
class PaymentInitiatorSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("forward payment with user custom tlv records") { f =>
    ...
    assert(payment.targetNodeId == invoice.nodeId)
  ...
  test("forward single-part payment when multi-part deactivated", Tag(Tags.DisableMPP)) { f =>
    ...
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some MPP invoice"), finalExpiryDelta, features = featuresWithMpp.remove(RouteBlinding))
  ...
  test("forward multi-part payment") { f =>
    import f._
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some invoice"), CltvExpiryDelta(18), features = featuresWithMpp.remove(RouteBlinding))
    val req = SendPaymentToNode(finalAmount + 100.msat, invoice, 1, routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
  ...
  test("forward multi-part payment with randomized final expiry", Tag(Tags.RandomizeFinalExpiry)) { f =>
    ...
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some invoice"), invoiceFinalExpiryDelta, features = featuresWithMpp.remove(RouteBlinding))
    val req = SendPaymentToNode(finalAmount, invoice, 1, routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    ...
    assert(nodeParams.currentBlockHeight + invoiceFinalExpiryDelta.toInt + 50 <= payment.recipient.expiry.blockHeight)
    assert(payment.recipient.expiry.blockHeight <= nodeParams.currentBlockHeight + invoiceFinalExpiryDelta.toInt + 200)
  }
  ...
  def createBolt12Invoice(features: Features[InvoiceFeature]): Bolt12Invoice = {
    ...
    val routeExpiry = CltvExpiry(500_000)
    val blindedPayloads = encodeBlindedPayloads(Seq(
      intermediateBlindedPayloadFromUpdate(channelUpdate_cd, routeExpiry),
      intermediateBlindedPayloadFromUpdate(channelUpdate_de, routeExpiry),
      finalBlindedPayload(hex"2a2a2a2a", routeExpiry)
    ))
    val blindedRoute = Sphinx.RouteBlinding.create(randomKey(), Seq(c, d, e), blindedPayloads).route
    val paymentInfo = OfferTypes.PaymentInfo(1_000_000 msat, 0, CltvExpiryDelta(24), 0 msat, finalAmount, Features.empty)
  ...
  test("forward single-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    payFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, finalAmount, invoice.nodeId, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true, Nil))
  ...
  test("forward multi-part blinded payment") { f =>
    ...
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
    multiPartPayFsm.expectMsg(SendPaymentConfig(id, id, None, paymentHash, finalAmount, invoice.nodeId, Upstream.Local(id), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = true, Nil))
    val payment = multiPartPayFsm.expectMsgType[SendMultiPartPayment]
    assert(payment.recipient.isInstanceOf[BlindedRecipient])
    val recipient = payment.recipient.asInstanceOf[BlindedRecipient]
    assert(recipient.nodeId == invoice.nodeId)
  ...
  test("forward trampoline payment") { f =>
    ...
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some phoenix invoice"), CltvExpiryDelta(9), features = featuresWithTrampoline.remove(RouteBlinding), extraHops = ignoredRoutingHints)
    ...
    assert(msg.amountToSend == finalAmount + trampolineFees)
  ...
  test("reject trampoline to legacy payment for 0-value invoice") { f =>
    ...
    val invoice = Bolt11Invoice(Block.RegtestGenesisBlock.hash, None, paymentHash, priv_a.privateKey, Left("#abittooreckless"), CltvExpiryDelta(18), None, None, routingHints, features = featuresWithMpp.remove(RouteBlinding))
    val trampolineFees = 21_000 msat
    val req = SendTrampolinePayment(finalAmount, invoice, b, Seq((trampolineFees, CltvExpiryDelta(12))), routeParams = nodeParams.routerConf.pathFindingExperimentConf.getRandomConf().getDefaultRouteParams)
    sender.send(initiator, req)
    val id = sender.expectMsgType[UUID]
  ...
  test("retry trampoline payment") { f =>
    import f._
    val invoice = Bolt11Invoice(Block.LivenetGenesisBlock.hash, Some(finalAmount), paymentHash, priv_c.privateKey, Left("Some phoenix invoice"), CltvExpiryDelta(18), features = featuresWithTrampoline.remove(RouteBlinding))
  ...
  test("retry trampoline payment and fail (route not found)") { f =>
    ...
    assert(failure.failures == Seq(LocalFailure(finalAmount, Seq(NodeHop(nodeParams.nodeId, b, nodeParams.channelConf.expiryDelta, 0 msat), NodeHop(b, c, CltvExpiryDelta(24), 25_000 msat)), RouteNotFound)))
  ...
  test("forward trampoline payment with pre-defined route") { f =>
    ...
    val trampolineExpiryDelta = CltvExpiryDelta(144)
    val route = PredefinedNodeRoute(Seq(a, b))
    val req = SendPaymentToRoute(finalAmount + trampolineFees, finalAmount, invoice, route, None, None, None, trampolineFees, trampolineExpiryDelta, Seq(b, c))
    sender.send(initiator, req)
    val payment = sender.expectMsgType[SendPaymentToRouteResponse]
    assert(payment.trampolineSecret.nonEmpty)
    payFsm.expectMsg(SendPaymentConfig(payment.paymentId, payment.parentId, None, paymentHash, finalAmount, c, Upstream.Local(payment.paymentId), Some(invoice), storeInDb = true, publishEvent = true, recordPathFindingMetrics = false, Seq(NodeHop(b, c, trampolineExpiryDelta, trampolineFees))))
    val msg = payFsm.expectMsgType[PaymentLifecycle.SendPaymentToRoute]
    assert(msg.route == Left(route))
