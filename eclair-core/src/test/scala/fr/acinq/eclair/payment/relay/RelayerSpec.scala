...
class RelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("relay an htlc-add") { f =>
    ...
    val add_ab = UpdateAddHtlc(randomBytes32(), 123456, payment.cmd.amount, payment.cmd.paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion, None, None)
  ...
  test("relay an htlc-add at the final node to the payment handler") { f =>
    ...
    val add_ab = UpdateAddHtlc(channelId_ab, 123456, payment.cmd.amount, payment.cmd.paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion, None, None)
    relayer ! RelayForward(add_ab)
  ...
  test("fail to relay an htlc-add when the onion is malformed") { f =>
    ...
    val add_ab = UpdateAddHtlc(channelId_ab, 123456, payment.cmd.amount, payment.cmd.paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion.copy(hmac = payment.cmd.onion.hmac.reverse), None, None)
    relayer ! RelayForward(add_ab)
  ...
  test("fail to relay an htlc-add with invalid blinding data (introduction node)") { f =>
    ...
    val add_ab = UpdateAddHtlc(channelId_ab, 0, payment.cmd.amount, payment.cmd.paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion, payment.cmd.nextBlindingKey_opt, payment.cmd.fundingFee_opt)
    relayer ! RelayForward(add_ab)
  ...
  test("relay htlc settled") { f =>
    ...
    val add_ab = UpdateAddHtlc(channelId_ab, 42, 11000000 msat, ByteVector32.Zeroes, CltvExpiry(4200), TestConstants.emptyOnionPacket, None, None)
    val add_bc = UpdateAddHtlc(channelId_bc, 72, 1000 msat, paymentHash, CltvExpiry(1), TestConstants.emptyOnionPacket, None, None)
    val channelOrigin = Origin.Hot(replyTo.ref.toClassic, Upstream.Hot.Channel(add_ab, TimestampMilli.now()))
    val trampolineOrigin = Origin.Hot(replyTo.ref.toClassic, Upstream.Hot.Trampoline(List(Upstream.Hot.Channel(add_ab, TimestampMilli.now()))))
  ...
  test("store on-the-fly funding preimage") { f =>
    ...
    val replyTo = TestProbe[Any]()
    val add_ab = UpdateAddHtlc(channelId_ab, 17, 50_000 msat, paymentHash, CltvExpiry(800_000), TestConstants.emptyOnionPacket, None, None)
    val add_bc = UpdateAddHtlc(channelId_bc, 21, 45_000 msat, paymentHash, CltvExpiry(799_000), TestConstants.emptyOnionPacket, None, Some(LiquidityAds.FundingFee(1000 msat, TxId(randomBytes32()))))
    val originHot = Origin.Hot(replyTo.ref.toClassic, Upstream.Hot.Channel(add_ab, TimestampMilli.now()))
    ...
    for (res <- addFulfilled) {
      ...
      assert(nodeParams.db.onTheFlyFunding.getPreimage(paymentHash).isEmpty)
      relayer ! res
      awaitCond(nodeParams.db.onTheFlyFunding.getPreimage(paymentHash).contains(preimage), 10 seconds)
    }
  }
