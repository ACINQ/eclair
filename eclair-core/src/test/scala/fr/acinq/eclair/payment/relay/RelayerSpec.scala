...
import fr.acinq.eclair.payment.{ClearRecipient, OutgoingPaymentPacket, PaymentPacketSpec}
...
class RelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("relay an htlc-add") { f =>
    ...
    val Success((cmd, _, _)) = buildCommand(randomKey(), ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, hops, ClearRecipient(e, paymentSecret, None)), finalAmount, finalAmount, finalExpiry)
  ...
  test("relay an htlc-add at the final node to the payment handler") { f =>
    ...
    val Success((cmd, _, _)) = buildCommand(randomKey(), ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, hops.take(1), ClearRecipient(b, paymentSecret, None)), finalAmount, finalAmount, finalExpiry)
  ...
  test("relay a trampoline htlc-add at the final node to the payment handler") { f =>
    ...
    val Success((trampolineAmount, trampolineExpiry, trampolineOnion)) = OutgoingPaymentPacket.buildTrampolinePacket(paymentHash, trampolineHops, ClearRecipient(b, paymentSecret, None), finalAmount, totalAmount, finalExpiry)
  ...
  test("fail to relay a trampoline htlc-add when trampoline is disabled", Tag("trampoline-disabled")) { f =>
    ...
    val Success((trampolineAmount, trampolineExpiry, trampolineOnion)) = OutgoingPaymentPacket.buildTrampolinePacket(paymentHash, trampolineHops, ClearRecipient(c, paymentSecret, None), finalAmount, finalAmount, finalExpiry)
    val Success((cmd, _, _)) = buildCommand(randomKey(), ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(trampolineAmount, channelHopFromUpdate(a, b, channelUpdate_ab) :: Nil, ClearRecipient(b, randomBytes32(), None, additionalTlvs = Seq(OnionPaymentPayloadTlv.TrampolineOnion(trampolineOnion.packet)))), trampolineAmount, trampolineAmount, trampolineExpiry)
