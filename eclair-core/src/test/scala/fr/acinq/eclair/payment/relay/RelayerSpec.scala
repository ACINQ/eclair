...
class RelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("fail to relay an htlc-add with invalid blinding data (introduction node)") { f =>
    ...
    val routeExpiry = CltvExpiry(nodeParams.currentBlockHeight - 1)
    ...
    val blindedPayloads = encodeBlindedPayloads(Seq(
      intermediateBlindedPayloadFromUpdate(channelUpdate_bc, routeExpiry),
      finalBlindedPayload(hex"deadbeef", routeExpiry)
    ))
    val blindedRoute = Sphinx.RouteBlinding.create(randomKey(), Seq(b, c), blindedPayloads).route
    val paymentInfo = OfferTypes.PaymentInfo(channelUpdate_bc.feeBaseMsat, channelUpdate_bc.feeProportionalMillionths, channelUpdate_bc.cltvExpiryDelta, 0 msat, finalAmount, Features.empty)
    ...
    val route = Route(finalAmount, Seq(channelHopFromUpdate(priv_a.publicKey, b, channelUpdate_ab), BlindedHop(recipient.extraEdges.head.dummyId, blindedRoute, paymentInfo)))
  ...
  test("fail to relay a trampoline htlc-add when trampoline is disabled", Tag("trampoline-disabled")) { f =>
    ...
    val recipient = ClearTrampolineRecipient(invoice, finalAmount, finalExpiry, Seq(NodeHop(b, c, channelUpdate_bc.cltvExpiryDelta, fee_b)), randomBytes32())
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, priv_a.privateKey, Upstream.Local(UUID.randomUUID()), paymentHash, Route(recipient.trampolineAmount, Seq(channelHopFromUpdate(priv_a.publicKey, b, channelUpdate_ab))), recipient)
