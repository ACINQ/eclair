...
import fr.acinq.eclair.{Alias, BlockHeight, Bolt11Feature, Bolt12Feature, CltvExpiry, CltvExpiryDelta, EncodedNodeId, FeatureSupport, Features, MilliSatoshi, MilliSatoshiLong, NodeParams, RealShortChannelId, ShortChannelId, TestConstants, TimestampMilli, TimestampMilliLong, UInt64, randomBytes, randomBytes32, randomKey}
...
class NodeRelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("fail all additional incoming HTLCs once already relayed out") { f =>
    ...
    val i2 = IncomingPaymentPacket.RelayToTrampolinePacket(
      UpdateAddHtlc(randomBytes32(), Random.nextInt(100), 1500 msat, paymentHash, CltvExpiry(499990), TestConstants.emptyOnionPacket, None, accountable = false, None),
  ...
  test("fail to relay when expiry is too soon (single-part)") { f =>
    ...
    val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
    assert(fwd.channelId == p.add.channelId)
    assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TrampolineFeeOrExpiryInsufficient(1_096_000 msat, 60, CltvExpiryDelta(288))), Some(FailureAttributionData(p.receivedAt, Some(p.receivedAt))), commit = true))
  ...
  test("fail to relay when expiry is too soon (multi-part)") { f =>
    ...
    p.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TrampolineFeeOrExpiryInsufficient(1_096_000 msat, 60, CltvExpiryDelta(288))), Some(FailureAttributionData(p.receivedAt, Some(TimestampMilli(20)))), commit = true))
    }
  ...
  test("relay the payment immediately when the async payment feature is disabled") { f =>
    ...
    nodeRelayerAdapters ! PreimageReceived(paymentHash, paymentPreimage, None, None)
    incomingAsyncPayment.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FULFILL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FULFILL_HTLC(p.add.id, paymentPreimage, None, Some(FulfillAttributionData(p.receivedAt, Some(incomingAsyncPayment.last.receivedAt), None)), commit = true))
    }
  ...
  test("fail to relay when fees are insufficient (multi-part)") { f =>
    ...
    p.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TrampolineFeeOrExpiryInsufficient(1_096_000 msat, 60, CltvExpiryDelta(288))), Some(FailureAttributionData(p.receivedAt, Some(TimestampMilli(486)))), commit = true))
    }
  ...
  test("fail to relay when amount is 0 (single-part)") { f =>
    ...
    val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
    assert(fwd.channelId == p.add.channelId)
    assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(InvalidOnionPayload(UInt64(2), 0)), Some(FailureAttributionData(p.receivedAt, Some(p.receivedAt))), commit = true))
  ...
  test("fail to relay when amount is 0 (multi-part)") { f =>
    ...
    p.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(InvalidOnionPayload(UInt64(2), 0)), Some(FailureAttributionData(p.receivedAt, Some(TimestampMilli(9)))), commit = true))
    }
  ...
  test("fail to relay because outgoing balance isn't sufficient (low fees)") { f =>
    ...
    nodeRelayerAdapters ! PaymentFailed(relayId, paymentHash, LocalFailure(outgoingAmount, Nil, BalanceTooLow) :: Nil, 100 unixms, 150 unixms)
    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TrampolineFeeOrExpiryInsufficient(2_740_000 msat, 150, CltvExpiryDelta(720))), Some(FailureAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt))), commit = true))
    }
  ...
  test("fail to relay because outgoing balance isn't sufficient (high fees)") { f =>
    ...
    incoming.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TemporaryTrampolineFailure()), Some(FailureAttributionData(p.receivedAt, Some(incoming.last.receivedAt))), commit = true))
    }
  ...
  test("fail to relay because incoming fee isn't enough to find routes downstream") { f =>
    ...
    val failures = LocalFailure(outgoingAmount, Nil, RouteNotFound) :: RemoteFailure(outgoingAmount, Nil, Sphinx.DecryptedFailurePacket(outgoingNodeId, 1, PermanentNodeFailure()), 100 unixms, 150 unixms) :: LocalFailure(outgoingAmount, Nil, RouteNotFound) :: Nil
    payFSM ! PaymentFailed(relayId, incomingMultiPart.head.add.paymentHash, failures, 100 unixms, 150 unixms)
  ...
  test("fail to relay because of downstream failures") { f =>
    ...
    val encryptedFailure = Sphinx.CannotDecryptFailurePacket(randomBytes(292), None)
    val failures = RemoteFailure(outgoingAmount, Nil, Sphinx.DecryptedFailurePacket(outgoingNodeId, 1, PaymentTimeout()), 100 unixms, 150 unixms) :: UnreadableRemoteFailure(outgoingAmount, Nil, encryptedFailure, 100 unixms, 150 unixms, Nil) :: Nil
    payFSM ! PaymentFailed(relayId, incomingMultiPart.head.add.paymentHash, failures, 100 unixms, 150 unixms)

    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.EncryptedDownstreamFailure(encryptedFailure.unwrapped, encryptedFailure.attribution_opt), Some(FailureAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt))), commit = true))
    }
  ...
  test("relay incoming multi-part payment") { f =>
    ...
    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FULFILL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FULFILL_HTLC(p.add.id, paymentPreimage, None, Some(FulfillAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt), None)), commit = true))
    }
  ...
  test("relay incoming multi-part payment with fulfillment payload") { f =>
    import f._
    ...
    // The downstream payment is fulfilled with a fulfillment payload that we must relay upstream.
    val fulfillmentPayload = randomBytes(500)
    val attribution = randomBytes(Sphinx.Attribution.totalLength)
    nodeRelayerAdapters ! PreimageReceived(paymentHash, paymentPreimage, Some(fulfillmentPayload), Some(attribution))
    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FULFILL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FULFILL_HTLC(p.add.id, paymentPreimage, Some(fulfillmentPayload), Some(FulfillAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt), Some(attribution))), commit = true))
    }

    nodeRelayerAdapters ! createSuccessEvent()
    eventListener.expectMessageType[TrampolinePaymentRelayed]
    parent.expectMessageType[NodeRelayer.RelayComplete]
    register.expectNoMessage(100 millis)
  }

  test("relay incoming multi-part payment with fulfillment payload (without preimage event)") { f =>
    import f._
    ...
    // Some payment FSMs only send us the final PaymentSent event, without a preceding PreimageReceived event: we must
    // then take the fulfillment payload and the attribution data from that event, without mixing them up.
    val fulfillmentPayload = randomBytes(500)
    val attribution = randomBytes(Sphinx.Attribution.totalLength)
    nodeRelayerAdapters ! createSuccessEvent().copy(fulfillmentPayload_opt = Some(fulfillmentPayload), remainingAttribution_opt = Some(attribution))
    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FULFILL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FULFILL_HTLC(p.add.id, paymentPreimage, Some(fulfillmentPayload), Some(FulfillAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt), Some(attribution))), commit = true))
  ...
  test("ignore downstream failures after fulfill") { f =>
    ...
    val downstreamHtlc = UpdateAddHtlc(randomBytes32(), 7, outgoingAmount, paymentHash, outgoingExpiry, TestConstants.emptyOnionPacket, None, accountable = false, None)
    val failure = LocalFailure(outgoingAmount, Nil, HtlcOverriddenByLocalCommit(randomBytes32(), downstreamHtlc))
    nodeRelayerAdapters ! PaymentFailed(relayId, incomingMultiPart.head.add.paymentHash, Seq(failure), 100 unixms, 150 unixms)
  ...
  test("relay incoming single-part payment") { f =>
    ...
    assert(fwd.message == CMD_FULFILL_HTLC(incomingAdd.id, paymentPreimage, None, Some(FulfillAttributionData(incomingSinglePart.receivedAt, Some(incomingSinglePart.receivedAt), None)), commit = true))
  ...
  test("relay incoming multi-part payment with on-the-fly funding", Tag(wakeUpEnabled), Tag(onTheFlyFunding)) { f =>
    ...
    outgoingPayment.replyTo ! PaymentFailed(relayId, paymentHash, LocalFailure(outgoingAmount, Nil, BalanceTooLow) :: Nil, 100 unixms, 150 unixms)
  ...
  test("relay incoming multi-part payment with on-the-fly funding (non-liquidity failure)", Tag(wakeUpEnabled), Tag(onTheFlyFunding)) { f =>
    ...
    outgoingPayment.replyTo ! PaymentFailed(relayId, paymentHash, RemoteFailure(outgoingAmount, Nil, Sphinx.DecryptedFailurePacket(outgoingNodeId, 1, TemporaryNodeFailure()), 100 unixms, 150 unixms) :: Nil, 100 unixms, 150 unixms)
    incomingMultiPart.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(TemporaryTrampolineFailure()), Some(FailureAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt))), commit = true))
    }
    parent.expectMessageType[NodeRelayer.RelayComplete]
  }
  ...
  test("fail to relay to blinded path when wake-up fails", Tag(wakeUpEnabled), Tag(wakeUpTimeout)) { f =>
    ...
    incomingPayments.foreach { p =>
      val fwd = register.expectMessageType[Register.Forward[CMD_FAIL_HTLC]]
      assert(fwd.channelId == p.add.channelId)
      assert(fwd.message == CMD_FAIL_HTLC(p.add.id, FailureReason.LocalTrampolineFailure(UnknownNextPeer()), Some(FailureAttributionData(p.receivedAt, Some(incomingMultiPart.last.receivedAt))), commit = true))
    }
  }
  ...
  test("relay to blinded path with on-the-fly funding failure", Tag(wakeUpEnabled), Tag(onTheFlyFunding)) { f =>
    ...
    outgoingPayment.replyTo ! PaymentFailed(relayId, paymentHash, LocalFailure(outgoingAmount, Nil, BalanceTooLow) :: Nil, 100 unixms, 110 unixms)
    val fwd = register.expectMessageType[Register.ForwardNodeId[Peer.ProposeOnTheFlyFunding]]
...
object NodeRelayerSpec {
  ...
  val paymentSecret: ByteVector32 = randomBytes32()

  val outgoingAmount: MilliSatoshi = 40_000_000 msat
  val outgoingExpiry: CltvExpiry = CltvExpiry(490000)
  val outgoingNodeKey: PrivateKey = randomKey()
  val outgoingNodeId: PublicKey = outgoingNodeKey.publicKey

  val incomingAmount: MilliSatoshi = 41_000_000 msat
  val incomingSecret: ByteVector32 = randomBytes32()
  val incomingMultiPart: Seq[RelayToTrampolinePacket] = Seq(
    createValidIncomingPacket(15_000_000 msat, incomingAmount, CltvExpiry(500000), outgoingAmount, outgoingExpiry, TimestampMilli(1000)),
    createValidIncomingPacket(15_000_000 msat, incomingAmount, CltvExpiry(499999), outgoingAmount, outgoingExpiry, TimestampMilli(2000)),
    createValidIncomingPacket(11_000_000 msat, incomingAmount, CltvExpiry(499999), outgoingAmount, outgoingExpiry, TimestampMilli(3000))
  )
  val incomingSinglePart: RelayToTrampolinePacket = createValidIncomingPacket(incomingAmount, incomingAmount, CltvExpiry(500000), outgoingAmount, outgoingExpiry, TimestampMilli(5000))
  ...
  def createSuccessEvent(): PaymentSent =
    PaymentSent(relayId, paymentPreimage, outgoingAmount, outgoingNodeId, Seq(PaymentSent.PaymentPart(UUID.randomUUID(), PaymentEvent.OutgoingPayment(randomBytes32(), randomKey().publicKey, outgoingAmount, 50 unixms), 10 msat, None, 0 unixms)), None, None, 0 unixms)
