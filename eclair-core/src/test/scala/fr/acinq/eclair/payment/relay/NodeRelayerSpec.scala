...
class NodeRelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("fail all additional incoming HTLCs once already relayed out") { f =>
    ...
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingMultiPart.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 3)
  ...
  test("relay the payment immediately when the async payment feature is disabled") { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingAsyncPayment.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 6)
    val outgoingPayment = mockPayFSM.expectMessageType[SendMultiPartPayment]
    validateOutgoingPayment(outgoingPayment)
  ...
  test("relay to non-trampoline recipient supporting multi-part") { f =>
    ...
    val incomingPayments = incomingMultiPart.map(incoming => {
      ...
      RelayToNonTrampolinePacket(incoming.add, incoming.outerPayload, innerPayload, TimestampMilli.now())
    })
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingMultiPart.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 2)
    val outgoingPayment = mockPayFSM.expectMessageType[SendMultiPartPayment]
  ...
  test("relay to non-trampoline recipient without multi-part") { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingMultiPart.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 0)
  ...
  test("relay to blinded paths without multi-part") { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingPayments.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 7, ignoreNodeId = true)
    val outgoingPayment = mockPayFSM.expectMessageType[SendPaymentToNode]
    assert(outgoingPayment.amount == outgoingAmount)
    assert(outgoingPayment.recipient.expiry == outgoingExpiry)
  ...
  test("relay to blinded paths with multi-part") { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingPayments.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 1, ignoreNodeId = true)
    val outgoingPayment = mockPayFSM.expectMessageType[SendMultiPartPayment]
    assert(outgoingPayment.recipient.totalAmount == outgoingAmount)
    assert(outgoingPayment.recipient.expiry == outgoingExpiry)
    assert(outgoingPayment.recipient.isInstanceOf[BlindedRecipient])
  ...
  test("relay to blinded path with wake-up", Tag(wakeUpEnabled)) { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingPayments.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 3, ignoreNodeId = true)
    val outgoingPayment = mockPayFSM.expectMessageType[SendMultiPartPayment]
    assert(outgoingPayment.recipient.totalAmount == outgoingAmount)
    assert(outgoingPayment.recipient.expiry == outgoingExpiry)
    assert(outgoingPayment.recipient.isInstanceOf[BlindedRecipient])
  ...
  test("relay to compact blinded paths") { f =>
    ...
    val outgoingCfg = mockPayFSM.expectMessageType[SendPaymentConfig]
    validateOutgoingCfg(outgoingCfg, Upstream.Hot.Trampoline(incomingPayments.map(p => Upstream.Hot.Channel(p.add, TimestampMilli.now(), randomKey().publicKey)).toList), 4, ignoreNodeId = true)
    val outgoingPayment = mockPayFSM.expectMessageType[SendPaymentToNode]
    assert(outgoingPayment.amount == outgoingAmount)
    assert(outgoingPayment.recipient.expiry == outgoingExpiry)
    assert(outgoingPayment.recipient.isInstanceOf[BlindedRecipient])
