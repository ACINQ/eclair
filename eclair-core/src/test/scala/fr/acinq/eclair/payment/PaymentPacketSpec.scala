...
class PaymentPacketSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("build outgoing trampoline payment (reference test vector)") {
    ...
    val onionForCarol = {
      ...
      val add = UpdateAddHtlc(randomBytes32(), 1, 100_006_000 msat, paymentHash, CltvExpiry(800_300), onionForBob, None, Reputation.maxEndorsement, None)
    ...
    val trampolineOnionForEve = {
      ...
      val eve = PublicKey(hex"02edabbd16b41c8371b92ef2f04c1185b4f03b6dcd52ba9b78d9d7c89c8f221145")
      val add = UpdateAddHtlc(randomBytes32(), 1, 100_005_000 msat, paymentHash, CltvExpiry(800_250), onionForCarol, None, Reputation.maxEndorsement, None)
    ...
    val onionForEve = {
      ...
      val add = UpdateAddHtlc(randomBytes32(), 1, 100_001_000 msat, paymentHash, CltvExpiry(800_100), onionForDave, None, Reputation.maxEndorsement, None)
    ...
    val add = UpdateAddHtlc(randomBytes32(), 1, 100_000_000 msat, paymentHash, CltvExpiry(800_000), onionForEve, None, Reputation.maxEndorsement, None)
  ...
  test("build outgoing trampoline payment to blinded recipient (reference test vector)") {
    ...
    val onionForCarol = {
      ...
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_155_000 msat, paymentHash, CltvExpiry(800_100), onionForBob, None, Reputation.maxEndorsement, None)
    ...
    val onionForDave = {
      ...
      val dave = PublicKey(hex"032c0b7cf95324a07d05398b240174dc0c2be444d96b159aa6c7f7b1e668680991")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_153_000 msat, paymentHash, CltvExpiry(800_060), onionForCarol, None, Reputation.maxEndorsement, None)
    ...
    val onionForEve = {
      ...
      val add = UpdateAddHtlc(randomBytes32(), 3, 150_150_500 msat, paymentHash, CltvExpiry(800_036), onionForDave, None, Reputation.maxEndorsement, None)
    ...
    val add = UpdateAddHtlc(randomBytes32(), 1, 150_000_000 msat, paymentHash, CltvExpiry(800_000), onionForEve, None, Reputation.maxEndorsement, None)
  ...
  test("build outgoing trampoline payment to blinded paths (reference test vector)") {
    ...
    val onionForEve = {
      val davePriv = PrivateKey(hex"4444444444444444444444444444444444444444444444444444444444444444")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_150_500 msat, paymentHash, CltvExpiry(800_036), onionForDave, None, Reputation.maxEndorsement, None)
    ...
    val add = UpdateAddHtlc(randomBytes32(), 1, 150_000_000 msat, paymentHash, CltvExpiry(800_000), onionForEve, Some(pathKey), Reputation.maxEndorsement, None)
    val Right(FinalPacket(_, payload, _)) = decrypt(add, evePriv, Features(Features.RouteBlinding -> FeatureSupport.Optional))
    assert(payload.isInstanceOf[FinalPayload.Blinded])
    assert(payload.asInstanceOf[FinalPayload.Blinded].pathId == pathId)
    assert(payload.asInstanceOf[FinalPayload.Blinded].amount == 150_000_000.msat)
  ...
  test("build htlc failure onion with attribution data (trampoline payment)") {
    ...
    val add_c = UpdateAddHtlc(randomBytes32(), 0, payment.trampolineAmount, paymentHash, payment.trampolineExpiry, payment.onion.packet, None, Reputation.maxEndorsement, None)
    ...
    val (add_d, sharedSecrets_c) = {
      ...
      val add_d = UpdateAddHtlc(randomBytes32(), 0, payload_c.amountToForward + 500.msat, paymentHash, payload_c.outgoingCltv + CltvExpiryDelta(36), onion_d.packet, None, Reputation.maxEndorsement, None)
      (add_d, onion_d.sharedSecrets)
    }
    val Right(ChannelRelayPacket(_, _, packet_e, _)) = decrypt(add_d, priv_d.privateKey, Features.empty)
    val add_e = UpdateAddHtlc(randomBytes32(), 3, payload_c.amountToForward, paymentHash, payload_c.outgoingCltv, packet_e, None, Reputation.maxEndorsement, None)
    val Right(FinalPacket(_, payload_e, _)) = decrypt(add_e, priv_e.privateKey, Features.empty)
    assert(payload_e.isInstanceOf[FinalPayload.Standard])
  ...
  test("build htlc failure onion (blinded trampoline payment)") {
    ...
    val (htlcForBob, sharedSecretsAlice) = {
      ...
      (UpdateAddHtlc(randomBytes32(), 1, 150_010_000 msat, paymentHash, CltvExpiry(800_150), onion.packet, None, Reputation.maxEndorsement, None), onion.sharedSecrets)
    }

    // Bob decrypts the onion and relays to Carol.
    val htlcForCarol = {
      val Right(packetForBob: ChannelRelayPacket) = decrypt(htlcForBob, priv_b.privateKey, Features.empty)
      UpdateAddHtlc(randomBytes32(), 1, packetForBob.amountToForward, paymentHash, packetForBob.outgoingCltv, packetForBob.nextPacket, None, Reputation.maxEndorsement, None)
    }
    ...
    val (htlcForDave, sharedSecretsCarol) = {
      ...
      (UpdateAddHtlc(randomBytes32(), 1, outgoingAmount, paymentHash, outgoingExpiry, onion.packet, None, Reputation.maxEndorsement, None), onion.sharedSecrets)
    }
  ...
  test("build htlc failure onion (trampoline reference test vector)") {
    ...
    val (htlcForBob, sharedSecretsAlice) = {
      ...
      (UpdateAddHtlc(randomBytes32(), 1, 100_006_000 msat, paymentHash, CltvExpiry(800_300), onion.packet, None, Reputation.maxEndorsement, None), onion.sharedSecrets)
    }
    ...
    val htlcForCarol = {
      ...
      UpdateAddHtlc(randomBytes32(), 1, 100_005_000 msat, paymentHash, CltvExpiry(800_250), onionForCarol, None, Reputation.maxEndorsement, None)
    }
    ...
    val (htlcForDave, sharedSecretsCarol) = {
      ...
      val sessionKey = PrivateKey(hex"0505050505050505050505050505050505050505050505050505050505050505")
      val onion = OutgoingPaymentPacket.buildOnion(sessionKey, payloads, paymentHash, Some(PaymentOnionCodecs.paymentOnionPayloadLength)).toOption.get
      (UpdateAddHtlc(randomBytes32(), 1, 100_001_000 msat, paymentHash, CltvExpiry(800_100), onion.packet, None, Reputation.maxEndorsement, None), onion.sharedSecrets)
    }
    ...
    val htlcForEve = {
      ...
      UpdateAddHtlc(randomBytes32(), 1, 100_000_000 msat, paymentHash, CltvExpiry(800_000), onionForEve, None, Reputation.maxEndorsement, None)
    }
  ...
  test("build htlc failure onion (trampoline reference test vector, blinded payment)") {
    ...
    val onionForCarol = {
      val bobPriv = PrivateKey(hex"4242424242424242424242424242424242424242424242424242424242424242")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_010_000 msat, paymentHash, CltvExpiry(800_150), onionForBob, None, Reputation.maxEndorsement, None)
      val Right(packetForBob: ChannelRelayPacket) = decrypt(add, bobPriv, Features.empty)
      assert(packetForBob.payload.outgoing.contains(ShortChannelId.fromCoordinates("572330x42x2821").get))
    ...
    val onionForDave = {
      val carolPriv = PrivateKey(hex"4343434343434343434343434343434343434343434343434343434343434343")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_005_000 msat, paymentHash, CltvExpiry(800_100), onionForCarol, None, Reputation.maxEndorsement, None)
    ...
    val failureForCarol = {
      val davePriv = PrivateKey(hex"4444444444444444444444444444444444444444444444444444444444444444")
      val add = UpdateAddHtlc(randomBytes32(), 3, 150_002_000 msat, paymentHash, CltvExpiry(800_030), onionForDave, None, Reputation.maxEndorsement, None)
