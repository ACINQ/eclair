...
class PaymentPacketSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("build outgoing trampoline payment") {
    ...
    val Success(payment) = buildOutgoingPayment(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(recipient.targetTotalAmount, trampolineChannelHops), recipient)
    assert(payment.outgoingChannel == channelUpdate_ab.shortChannelId)
    assert(payment.cmd.amount == amount_ab)
    assert(payment.cmd.cltvExpiry == expiry_ab)
    ...
    val Success(payment_e) = buildOutgoingPayment(ActorRef.noSender, Upstream.Trampoline(Seq(add_c)), paymentHash, Route(inner_c.amountToForward, afterTrampolineChannelHops), recipient_e)
  ...
  test("fail to build outgoing trampoline payment when too much invoice data is provided") {
    ...
    val recipient = ClearTrampolineRecipient(invoice, finalAmount, finalAmount, finalExpiry, trampolineHop, randomBytes32())
    assert(buildOutgoingPayment(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(recipient.targetTotalAmount, trampolineChannelHops), recipient).isFailure)
  }
  ...
  test("fail to decrypt at the final trampoline node when amount has been decreased by next-to-last trampoline") {
    ...
    val Success(payment_e) = buildOutgoingPayment(ActorRef.noSender, Upstream.Trampoline(Seq(add_c)), paymentHash, Route(invalidTotalAmount, afterTrampolineChannelHops), recipient_e)
    val add_d = UpdateAddHtlc(randomBytes32(), 3, payment_e.cmd.amount, paymentHash, payment_e.cmd.cltvExpiry, payment_e.cmd.onion, None)
