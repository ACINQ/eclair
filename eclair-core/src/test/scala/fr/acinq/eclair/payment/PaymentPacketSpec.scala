...
class PaymentPacketSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  def testPeelOnion(packet_b: OnionRoutingPacket): Unit = {
    ...
    assert(payload_e.paymentSecret contains paymentSecret)
  }
  ...
  test("build a command including the onion") {
    val Success((add, _)) = buildCommand(ActorRef.noSender, Upstream.Local(UUID.randomUUID), paymentHash, hops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, Some(paymentSecret), None))
  ...
  test("build a command with no hops") {
    val Success((add, _)) = buildCommand(ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, hops.take(1), PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, Some(paymentSecret), Some(paymentMetadata)))
    assert(add.amount == finalAmount)
    ...
    assert(payload_b.paymentSecret contains  paymentSecret)
  ...
  test("build a trampoline payment") {
    ...
    val Success((amount_ac, expiry_ac, trampolineOnion)) = buildTrampolinePacket(paymentHash, trampolineHops, PaymentOnion.createMultiPartPayload(finalAmount, finalAmount * 3, finalExpiry, Some(paymentSecret), Some(hex"010203")))
  ...
  test("build a trampoline payment with non-trampoline recipient") {
    ...
    val Success((amount_ac, expiry_ac, trampolineOnion)) = buildTrampolineToLegacyPacket(invoice, trampolineHops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, invoice.paymentSecret, None))
    assert(amount_ac == amount_bc)
    assert(expiry_ac == expiry_bc)
  ...
  test("fail to build a trampoline payment when too much invoice data is provided") {
    ...
    assert(buildTrampolineToLegacyPacket(invoice, trampolineHops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, invoice.paymentSecret, invoice.paymentMetadata)).isFailure)
  }

  test("fail to decrypt when the onion is invalid") {
    val Success((firstAmount, firstExpiry, onion)) = buildPaymentPacket(paymentHash, hops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, Some(paymentSecret), None))
  ...
  test("fail to decrypt when the trampoline onion is invalid") {
    val Success((amount_ac, expiry_ac, trampolineOnion)) = buildTrampolinePacket(paymentHash, trampolineHops, PaymentOnion.createMultiPartPayload(finalAmount, finalAmount * 2, finalExpiry, Some(paymentSecret), None))
  ...
  test("fail to decrypt when payment hash doesn't match associated data") {
    val Success((firstAmount, firstExpiry, onion)) = buildPaymentPacket(paymentHash.reverse, hops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, Some(paymentSecret), None))
  ...
  test("fail to decrypt at the final trampoline node when amount has been modified by next-to-last trampoline") {
    val Success((amount_ac, expiry_ac, trampolineOnion)) = buildTrampolinePacket(paymentHash, trampolineHops, PaymentOnion.createMultiPartPayload(finalAmount, finalAmount, finalExpiry, Some(paymentSecret), None))
    val Success((firstAmount, firstExpiry, onion)) = buildPaymentPacket(paymentHash, trampolineChannelHops, PaymentOnion.createTrampolinePayload(amount_ac, amount_ac, expiry_ac, randomBytes32(), trampolineOnion.packet))
  ...
  test("fail to decrypt at intermediate trampoline node when amount is invalid") {
    val Success((amount_ac, expiry_ac, trampolineOnion)) = buildTrampolinePacket(paymentHash, trampolineHops, PaymentOnion.createSinglePartPayload(finalAmount, finalExpiry, Some(paymentSecret), None))
    val Success((firstAmount, firstExpiry, onion)) = buildPaymentPacket(paymentHash, trampolineChannelHops, PaymentOnion.createTrampolinePayload(amount_ac, amount_ac, expiry_ac, randomBytes32(), trampolineOnion.packet))
    val Right(ChannelRelayPacket(_, _, packet_c)) = decrypt(UpdateAddHtlc(randomBytes32(), 1, firstAmount, paymentHash, firstExpiry, onion.packet), priv_b.privateKey)
