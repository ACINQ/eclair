...
class PaymentPacketSpec extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("build outgoing trampoline payment (reference test vector)") {
    ...
    val onionForCarol = {
      ...
      val Right(ChannelRelayPacket(_, _, onionForCarol)) = decrypt(add, priv, Features.empty)
    ...
    val trampolineOnionForEve = {
      ...
      val Right(RelayToTrampolinePacket(_, outer_c, inner_c, trampolineOnionForEve)) = decrypt(add, priv, Features.empty)
    ...
    val onionForEve = {
      ...
      val Right(ChannelRelayPacket(_, _, onionForEve)) = decrypt(add, priv, Features.empty)
    ...
    val Right(FinalPacket(_, payloadForEve)) = decrypt(add, priv, Features.empty)
  ...
  test("build outgoing trampoline payment to blinded recipient (reference test vector)") {
    ...
    val onionForDave = {
      ...
      val Right(RelayToTrampolinePacket(_, payload, trampolinePayload, trampolineOnionForDave)) = decrypt(add, carolPriv, Features.empty)
    ...
    val onionForEve = {
      ...
      val Right(RelayToBlindedTrampolinePacket(_, payload, innerPayload, trampolineOnionForEve)) = decrypt(add, davePriv, Features(Features.RouteBlinding -> FeatureSupport.Optional))
    ...
    val Right(FinalPacket(_, payload)) = decrypt(add, evePriv, Features(Features.RouteBlinding -> FeatureSupport.Optional))
  ...
  test("build outgoing trampoline payment to blinded paths (reference test vector)") {
    ...
    val onionForDave = {
      ...
      val dave = PublicKey(hex"032c0b7cf95324a07d05398b240174dc0c2be444d96b159aa6c7f7b1e668680991")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_153_000 msat, paymentHash, CltvExpiry(800_060), onionForCarol, None, 1.0, None)
      val Right(RelayToBlindedPathsPacket(_, payload, trampolinePayload)) = decrypt(add, carolPriv, Features(Features.RouteBlinding -> FeatureSupport.Optional))
      assert(payload.amount == 150_153_000.msat)
      assert(payload.expiry == CltvExpiry(800_060))
      assert(payload.paymentSecret == ByteVector32.fromValidHex("7494b65bc092b48a75465e43e29be807eb2cc535ce8aaba31012b8ff1ceac5da"))
  ...
  test("build htlc failure onion (trampoline payment)") {
    // Create a trampoline payment to e:
    //        .--> d --.
    //       /          \
    // b -> c            e
    ...
    val Right(RelayToTrampolinePacket(_, _, payload_c, trampolinePacket_e)) = decrypt(add_c, priv_c.privateKey, Features.empty)
    ...
    val failure = IncorrectOrUnknownPaymentDetails(finalAmount, BlockHeight(currentBlockCount))
    val Right(fail_e: UpdateFailHtlc) = buildHtlcFailure(priv_e.privateKey, CMD_FAIL_HTLC(add_e.id, FailureReason.LocalTrampolineFailure(failure)), add_e)
    assert(fail_e.id == add_e.id)
    val Right(fail_d: UpdateFailHtlc) = buildHtlcFailure(priv_d.privateKey, CMD_FAIL_HTLC(add_d.id, FailureReason.EncryptedDownstreamFailure(fail_e.reason)), add_d)
    assert(fail_d.id == add_d.id)
    // c tries to decrypt the failure but cannot because it's encrypted for b, so she relays it upstream.
    val Left(Sphinx.CannotDecryptFailurePacket(unwrapped_c)) = Sphinx.FailurePacket.decrypt(fail_d.reason, sharedSecrets_c)
    val Right(fail_b: UpdateFailHtlc) = buildHtlcFailure(priv_c.privateKey, CMD_FAIL_HTLC(add_c.id, FailureReason.EncryptedDownstreamFailure(unwrapped_c)), add_c)
    // b decrypts the failure with the outer onion secrets *and* trampoline onion secrets
    val Left(Sphinx.CannotDecryptFailurePacket(unwrapped_b)) = Sphinx.FailurePacket.decrypt(fail_b.reason, payment.onion.sharedSecrets)
    val Right(Sphinx.DecryptedFailurePacket(failingNode, decryptedFailure)) = Sphinx.FailurePacket.decrypt(unwrapped_b, payment.trampolineOnion.sharedSecrets)
    assert(failingNode == e)
    assert(decryptedFailure == failure)
  }
  ...
  test("build htlc failure onion (blinded trampoline payment)") {
    ...
    val (htlcForDave, sharedSecretsCarol) = {
      val Right(RelayToBlindedTrampolinePacket(_, _, innerPayload, trampolineOnionForDave)) = decrypt(htlcForCarol, priv_c.privateKey, Features(Features.RouteBlinding -> FeatureSupport.Optional))
    ...
    val (htlcForEve, sharedSecretsDave) = {
      val Right(RelayToBlindedTrampolinePacket(_, _, innerPayload, trampolineOnionForEve)) = decrypt(htlcForDave, priv_d.privateKey, Features(Features.RouteBlinding -> FeatureSupport.Optional))
    ...
    val failureForDave = {
      val Right(FinalPacket(_, payload)) = decrypt(htlcForEve, priv_e.privateKey, Features(Features.RouteBlinding -> FeatureSupport.Optional))
      assert(payload.isInstanceOf[FinalPayload.Blinded])
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv_e.privateKey, CMD_FAIL_HTLC(htlcForEve.id, FailureReason.LocalTrampolineFailure(failure)), htlcForEve)
      fail
    }

    // Dave cannot decrypt the failure, so he forwards it to Carol.
    val failureForCarol = {
      val Left(Sphinx.CannotDecryptFailurePacket(unwrapped)) = Sphinx.FailurePacket.decrypt(failureForDave.reason, sharedSecretsDave)
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv_d.privateKey, CMD_FAIL_HTLC(failureForDave.id, FailureReason.EncryptedDownstreamFailure(unwrapped)), htlcForDave)
      fail
    }

    // Carol cannot decrypt the failure, so she forwards it to Bob.
    val failureForBob = {
      val Left(Sphinx.CannotDecryptFailurePacket(unwrapped)) = Sphinx.FailurePacket.decrypt(failureForCarol.reason, sharedSecretsCarol)
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv_c.privateKey, CMD_FAIL_HTLC(failureForCarol.id, FailureReason.EncryptedDownstreamFailure(unwrapped)), htlcForCarol)
      fail
    }

    // Bob cannot decrypt the failure, so he forwards it to Alice.
    val failureForAlice = {
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv_b.privateKey, CMD_FAIL_HTLC(failureForBob.id, FailureReason.EncryptedDownstreamFailure(failureForBob.reason)), htlcForBob)
      fail
    }

    // Alice is able to decrypt the failure using the outer onion and trampoline onion shared secrets.
    val Right(decrypted) = Sphinx.FailurePacket.decrypt(failureForAlice.reason, sharedSecretsAlice ++ trampolineOnion.sharedSecrets)
  ...
  test("build htlc failure onion (trampoline reference test vector)") {
    ...
    val htlcForCarol = {
      val priv = PrivateKey(hex"4242424242424242424242424242424242424242424242424242424242424242")
      val Right(ChannelRelayPacket(_, _, onionForCarol)) = decrypt(htlcForBob, priv, Features.empty)
    ...
    val trampolineOnionForEve = {
      val priv = PrivateKey(hex"4343434343434343434343434343434343434343434343434343434343434343")
      val Right(RelayToTrampolinePacket(_, _, _, trampolineOnionForEve)) = decrypt(htlcForCarol, priv, Features.empty)
      trampolineOnionForEve
    }
    ...
    val htlcForEve = {
      val priv = PrivateKey(hex"4444444444444444444444444444444444444444444444444444444444444444")
      val Right(ChannelRelayPacket(_, _, onionForEve)) = decrypt(htlcForDave, priv, Features.empty)
    ...
    val failureForDave = {
      ...
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv, CMD_FAIL_HTLC(htlcForEve.id, FailureReason.LocalTrampolineFailure(failure)), htlcForEve)
    ...
    val failureForCarol = {
      val priv = PrivateKey(hex"4444444444444444444444444444444444444444444444444444444444444444")
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv, CMD_FAIL_HTLC(failureForDave.id, FailureReason.EncryptedDownstreamFailure(failureForDave.reason)), htlcForDave)
    ...
    val failureForBob = {
      ...
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv, CMD_FAIL_HTLC(failureForCarol.id, FailureReason.EncryptedDownstreamFailure(unwrapped)), htlcForCarol)
    ...
    val failureForAlice = {
      val priv = PrivateKey(hex"4242424242424242424242424242424242424242424242424242424242424242")
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(priv, CMD_FAIL_HTLC(failureForBob.id, FailureReason.EncryptedDownstreamFailure(failureForBob.reason)), htlcForBob)
    ...
    val Left(Sphinx.CannotDecryptFailurePacket(trampolineFailure)) = Sphinx.FailurePacket.decrypt(failureForAlice.reason, sharedSecretsAlice)
    ...
    val Right(f) = Sphinx.FailurePacket.decrypt(trampolineFailure, trampolineOnionForCarol.sharedSecrets)
  ...
  test("build htlc failure onion (trampoline reference test vector, blinded payment)") {
    ...
    val onionForDave = {
      ...
      val Right(RelayToBlindedTrampolinePacket(_, payload, innerPayload, trampolineOnionForDave)) = decrypt(add, carolPriv, Features.empty)
    ...
    val failureForCarol = {
      ...
      val Right(RelayToBlindedTrampolinePacket(_, payload, innerPayload, _)) = decrypt(add, davePriv, Features(Features.RouteBlinding -> FeatureSupport.Optional))
    ...
    val failureForBob = {
      val carolPriv = PrivateKey(hex"4343434343434343434343434343434343434343434343434343434343434343")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_005_000 msat, paymentHash, CltvExpiry(800_100), onionForCarol, None, 1.0, None)
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(carolPriv, CMD_FAIL_HTLC(add.id, FailureReason.LocalTrampolineFailure(TemporaryTrampolineFailure())), add)
    ...
    val failureForAlice = {
      val bobPriv = PrivateKey(hex"4242424242424242424242424242424242424242424242424242424242424242")
      val add = UpdateAddHtlc(randomBytes32(), 1, 150_010_000 msat, paymentHash, CltvExpiry(800_150), onionForBob, None, 1.0, None)
      val Right(fail: UpdateFailHtlc) = buildHtlcFailure(bobPriv, CMD_FAIL_HTLC(add.id, FailureReason.EncryptedDownstreamFailure(failureForBob.reason)), add)
    ...
    val Right(f) = Sphinx.FailurePacket.decrypt(trampolineFailure, trampolineOnion.sharedSecrets)
