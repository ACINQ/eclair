...
class PostRestartHtlcCleanerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ParallelTestExecution with ChannelStateTestsBase {
  ...
  test("clean up upstream HTLCs that weren't relayed downstream") { f =>
    ...
    channel.expectMsgAllOf(
      CMD_FAIL_HTLC(1, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true),
    ...
    channel.expectMsgAllOf(
      CMD_FAIL_HTLC(0, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true),
      CMD_FAIL_HTLC(4, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true)
    )
    channel.expectNoMessage(100 millis)
  ...
  test("clean up upstream HTLCs for which we're the final recipient") { f =>
    ...
    val expected1 = Set(
      ...
      CMD_FAIL_HTLC(7, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true)
    )
    ...
    val expected2 = Set(
      CMD_FAIL_HTLC(1, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true),
      CMD_FAIL_HTLC(3, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true),
      CMD_FULFILL_HTLC(4, preimage, commit = true),
      CMD_FAIL_HTLC(9, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true)
    )
  ...
  test("ignore htlcs in closing downstream channels that have been settled on-chain") { f =>
    ...
    val upstream_1 = Upstream.Trampoline(Upstream.ReceivedHtlc(htlc_upstream_1.head.add, TimestampMilli(1687345927000L), useAttributableErrors = false) :: Upstream.ReceivedHtlc(htlc_upstream_2.head.add, TimestampMilli(1687345967000L), useAttributableErrors = false) :: Nil)
    val upstream_2 = Upstream.Trampoline(Upstream.ReceivedHtlc(htlc_upstream_1(1).add, TimestampMilli(1687345902000L), useAttributableErrors = false) :: Upstream.ReceivedHtlc(htlc_upstream_2(1).add, TimestampMilli(1687345999000L), useAttributableErrors = false) :: Nil)
    val upstream_3 = Upstream.Trampoline(Upstream.ReceivedHtlc(htlc_upstream_3.head.add, TimestampMilli(1687345980000L), useAttributableErrors = false) :: Nil)
    ...
    channel_upstream_1.expectMsg(CMD_FAIL_HTLC(0, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true))
    channel_upstream_2.expectMsg(CMD_FAIL_HTLC(7, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true))
  ...
  test("ignore htlcs that have a pending settlement command") { f =>
    ...
    nodeParams.db.pendingCommands.addSettlementCommand(channelId_ab_1, CMD_FAIL_HTLC(4, Right(PermanentChannelFailure()), useAttributableErrors = false, TimestampMilli.min))
  ...
  test("handle a trampoline relay htlc-fail") { f =>
    ...
    assert(fails.toSet == testCase.origin_1.htlcs.map {
      case (channelId, htlcId) => Register.Forward(null, channelId, CMD_FAIL_HTLC(htlcId, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true))
    }.toSet)
  ...
  test("relayed standard->non-standard HTLC is retained") { f =>
    ...
    channel.expectMsg(CMD_FAIL_HTLC(1L, Right(TemporaryNodeFailure()), useAttributableErrors = false, TimestampMilli.min, commit = true))
    channel.expectNoMessage(100 millis)
  }
  ...
  test("non-standard HTLC CMD_FAIL in relayDb is retained") { f =>
    ...
    val cmd1 = CMD_FAIL_HTLC(id = 0L, reason = Left(ByteVector.empty), useAttributableErrors = false, TimestampMilli.min, replyTo_opt = None)
    val cmd2 = CMD_FAIL_HTLC(id = 1L, reason = Left(ByteVector.empty), useAttributableErrors = false, TimestampMilli.min, replyTo_opt = None)
    val nodeParams1 = nodeParams.copy(pluginParams = List(pluginParams))
