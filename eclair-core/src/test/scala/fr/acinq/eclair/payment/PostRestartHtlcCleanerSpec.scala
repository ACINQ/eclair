...
class PostRestartHtlcCleanerSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike with ParallelTestExecution with ChannelStateTestsBase {
  ...
  test("clean up upstream HTLCs that weren't relayed downstream") { f =>
    ...
    val relayed = Origin.Cold(Upstream.Cold.Channel(channelId_ab_1, a, 5, 10 msat))
  ...
  test("ignore htlcs in downstream revoked close") { f =>
    ...
    val origins: Map[Long, Origin] = Map(
      2L -> Origin.Cold(Upstream.Cold.Trampoline(Upstream.Cold.Channel(channelId_ab_1, a, 1, 500 msat) :: Upstream.Cold.Channel(channelId_ab_1, a, 2, 200 msat) :: Nil)),
      3L -> Origin.Cold(Upstream.Cold.Trampoline(Upstream.Cold.Channel(channelId_ab_1, a, 1, 500 msat) :: Upstream.Cold.Channel(channelId_ab_1, a, 2, 200 msat) :: Nil)),
      4L -> Origin.Cold(Upstream.Cold.Trampoline(Upstream.Cold.Channel(channelId_ab_1, a, 1, 500 msat) :: Upstream.Cold.Channel(channelId_ab_1, a, 2, 200 msat) :: Nil)),
      5L -> Origin.Cold(Upstream.Cold.Channel(channelId_ab_1, a, 4, 550 msat)),
    )
  ...
  test("handle a trampoline relay htlc-fail") { f =>
    ...
    assert(fails.toSet == testCase.upstream_1.originHtlcs.map {
      case Upstream.Cold.Channel(channelId, _, htlcId, _) => Register.Forward(null, channelId, CMD_FAIL_HTLC(htlcId, FailureReason.LocalTrampolineFailure(TemporaryTrampolineFailure()), None, commit = true))
    }.toSet)
  ...
  test("handle a trampoline relay htlc-fulfill") { f =>
    ...
    assert(fulfills.toSet == testCase.upstream_1.originHtlcs.map {
      case Upstream.Cold.Channel(channelId, _, htlcId, _) => Register.Forward(null, channelId, CMD_FULFILL_HTLC(htlcId, preimage1, None, None, commit = true))
    }.toSet)
    ...
    register.expectMsg(testCase.upstream_2.originHtlcs.map {
      case Upstream.Cold.Channel(channelId, _, htlcId, _) => Register.Forward(null, channelId, CMD_FULFILL_HTLC(htlcId, preimage2, None, None, commit = true))
    }.head)
  ...
  test("ignore relayed htlc-fail for on-the-fly funding") { f =>
    ...
    val upstreamChannel = Upstream.Cold.Channel(htlc_ab(1).add, a)
    val upstreamTrampoline = Upstream.Cold.Trampoline(Upstream.Cold.Channel(htlc_ab(2).add, a) :: Nil)
...
object PostRestartHtlcCleanerSpec {
  ...
  val channelId_ab_1: ByteVector32 = randomBytes32()
  val channelId_ab_2: ByteVector32 = randomBytes32()
  val channelId_bc_1: ByteVector32 = randomBytes32()
  val channelId_bc_2: ByteVector32 = randomBytes32()
  val channelId_bc_3: ByteVector32 = randomBytes32()
  val channelId_bc_4: ByteVector32 = randomBytes32()
  val channelId_bc_5: ByteVector32 = randomBytes32()
  ...
  def buildHtlc(htlcId: Long, channelId: ByteVector32, paymentHash: ByteVector32, blinded: Boolean = false): UpdateAddHtlc = {
    val Right(payment) = if (blinded) {
      ...
      buildOutgoingPayment(TestConstants.emptyOrigin, paymentHash, route, recipient, Reputation.Score.max(accountable = false))
    }
    UpdateAddHtlc(channelId, htlcId, payment.cmd.amount, paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion, payment.cmd.nextPathKey_opt, accountable = false, payment.cmd.fundingFee_opt)
  }
  ...
  def buildFinalHtlc(htlcId: Long, channelId: ByteVector32, paymentHash: ByteVector32): DirectedHtlc = {
    val Right(payment) = buildOutgoingPayment(TestConstants.emptyOrigin, paymentHash, Route(finalAmount, Seq(channelHopFromUpdate(a, b, channelUpdate_ab)), None), SpontaneousRecipient(b, finalAmount, finalExpiry, randomBytes32()), Reputation.Score.max(accountable = false))
    IncomingHtlc(UpdateAddHtlc(channelId, htlcId, payment.cmd.amount, paymentHash, payment.cmd.cltvExpiry, payment.cmd.onion, None, accountable = false, None))
  }

  def buildForwardFail(add: UpdateAddHtlc, upstream: Upstream.Cold): RES_ADD_SETTLED[Origin.Cold, HtlcResult.Fail] =
    RES_ADD_SETTLED(Origin.Cold(upstream), randomKey().publicKey, add, HtlcResult.RemoteFail(UpdateFailHtlc(add.channelId, add.id, ByteVector.empty)))

  def buildForwardOnChainFail(add: UpdateAddHtlc, upstream: Upstream.Cold): RES_ADD_SETTLED[Origin.Cold, HtlcResult.Fail] =
    RES_ADD_SETTLED(Origin.Cold(upstream), randomKey().publicKey, add, HtlcResult.OnChainFail(HtlcsTimedoutDownstream(add.channelId, Set(add))))
  ...
  def buildForwardFulfill(add: UpdateAddHtlc, upstream: Upstream.Cold, preimage: ByteVector32): RES_ADD_SETTLED[Origin.Cold, HtlcResult.Fulfill] =
    RES_ADD_SETTLED(Origin.Cold(upstream), randomKey().publicKey, add, HtlcResult.RemoteFulfill(UpdateFulfillHtlc(add.channelId, add.id, preimage)))
  ...
  def setupLocalPayments(nodeParams: NodeParams): LocalPaymentTest = {
    ...
    val add1 = UpdateAddHtlc(channelId_bc_1, 72, 561 msat, paymentHash1, CltvExpiry(4200), onionRoutingPacket = TestConstants.emptyOnionPacket, pathKey_opt = None, accountable = false, fundingFee_opt = None)
    val origin1 = Origin.Cold(Upstream.Local(id1))
    val add2 = UpdateAddHtlc(channelId_bc_1, 75, 1105 msat, paymentHash2, CltvExpiry(4250), onionRoutingPacket = TestConstants.emptyOnionPacket, pathKey_opt = None, accountable = false, fundingFee_opt = None)
    val origin2 = Origin.Cold(Upstream.Local(id2))
    val add3 = UpdateAddHtlc(channelId_bc_1, 78, 1729 msat, paymentHash2, CltvExpiry(4300), onionRoutingPacket = TestConstants.emptyOnionPacket, pathKey_opt = None, accountable = false, fundingFee_opt = None)
  ...
  def setupChannelRelayedPayments(): ChannelRelayedPaymentTest = {
    ...
    val upstream_1 = Upstream.Cold.Channel(htlc_ab_1.head.add, a)
  ...
  def setupTrampolinePayments(): TrampolinePaymentTest = {
    ...
    val upstream_3 = Upstream.Cold.Trampoline(Upstream.Cold.Channel(channelId_ab_1, a, 57, 100 msat) :: Nil)
    val upstream_4 = Upstream.Cold.Channel(channelId_ab_2, a, 57, 150 msat)
    ...
    val downstream_1_1 = UpdateAddHtlc(channelId_bc_1, 6L, finalAmount, paymentHash1, finalExpiry, TestConstants.emptyOnionPacket, None, accountable = false, None)
    val downstream_2_1 = UpdateAddHtlc(channelId_bc_1, 8L, finalAmount, paymentHash2, finalExpiry, TestConstants.emptyOnionPacket, None, accountable = false, None)
    val downstream_2_2 = UpdateAddHtlc(channelId_bc_2, 1L, finalAmount, paymentHash2, finalExpiry, TestConstants.emptyOnionPacket, None, accountable = false, None)
    val downstream_2_3 = UpdateAddHtlc(channelId_bc_3, 4L, finalAmount, paymentHash2, finalExpiry, TestConstants.emptyOnionPacket, None, accountable = false, None)
