...
object PostRestartHtlcCleanerSpec {
  ...
  def buildHtlc(htlcId: Long, channelId: ByteVector32, paymentHash: ByteVector32): UpdateAddHtlc = {
    val Success((_, cmd, _)) = buildCommand(ActorRef.noSender, randomKey(), Upstream.Local(UUID.randomUUID()), paymentHash, hops, PaymentOnion.FinalPayload.Standard.createSinglePartPayload(finalAmount, finalExpiry, randomBytes32(), None), finalAmount, finalExpiry)
