...
object PostRestartHtlcCleanerSpec {
  ...
  def buildHtlc(htlcId: Long, channelId: ByteVector32, paymentHash: ByteVector32): UpdateAddHtlc = {
    val Success((cmd, _, _)) = buildCommand(randomKey(), ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, hops, ClearRecipient(e, randomBytes32(), None)), finalAmount, finalAmount, finalExpiry)
  ...
  def buildFinalHtlc(htlcId: Long, channelId: ByteVector32, paymentHash: ByteVector32): DirectedHtlc = {
    val Success((cmd, _, _)) = buildCommand(randomKey(), ActorRef.noSender, Upstream.Local(UUID.randomUUID()), paymentHash, Route(finalAmount, channelHopFromUpdate(a, TestConstants.Bob.nodeParams.nodeId, channelUpdate_ab) :: Nil, ClearRecipient(b, randomBytes32(), None)), finalAmount, finalAmount, finalExpiry)
