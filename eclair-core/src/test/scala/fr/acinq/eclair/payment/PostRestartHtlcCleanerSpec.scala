...
class PostRestartHtlcCleanerSpec extends TestkitBaseClass {
  ...
  test("ignore HTLCs stuck in closing downstream channels that have been settled upstream") { f =>
    import f._
  ...
  test("handle a trampoline relay htlc-fail") { f =>
    ...
    sender.send(relayer, buildForwardOverridden(testCase.downstream_2_1, testCase.upstream_2))
    ...
    sender.send(relayer, buildForwardTimeout(testCase.downstream_2_3, testCase.upstream_2))
...
object PostRestartHtlcCleanerSpec {
  ...
  def buildForwardTimeout(add: UpdateAddHtlc, origin: Origin) =
    Status.Failure(AddHtlcFailed(add.channelId, add.paymentHash, HtlcTimedout(add.channelId, Set(add)), origin, None, None))

  def buildForwardOverridden(add: UpdateAddHtlc, origin: Origin) =
    Status.Failure(AddHtlcFailed(add.channelId, add.paymentHash, HtlcOverriddenByLocalCommit(add.channelId, Set(add)), origin, None, None))
   ...
   * correctly resolved when the channel went to closing, so we should ignore that payment (downstream will
   * eventually settle on-chain).
   *  - it also contains a normal relayed payment stuck in a closing channel downstream: the upstream HTLC has been
   * correctly resolved, so we should ignore that payment (downstream will eventually settle on-chain).
   */
