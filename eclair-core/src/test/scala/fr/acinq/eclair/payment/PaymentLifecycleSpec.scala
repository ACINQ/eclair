...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("payment failed (unparsable failure)") { fixture =>
    ...
    routerForwarder.expectMsg(RouteRequest(a, d, DEFAULT_AMOUNT_MSAT, ignoreNodes = Set.empty, ignoreChannels = Set.empty))
    ...
    routerForwarder.expectMsg(RouteRequest(a, d, DEFAULT_AMOUNT_MSAT, ignoreNodes = Set(c), ignoreChannels = Set.empty))
  ...
  test("payment failed (local error)") { fixture =>
    ...
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    val WaitingForRoute(_, _, Nil) = paymentFSM.stateData
    routerForwarder.expectMsg(RouteRequest(a, d, DEFAULT_AMOUNT_MSAT, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set.empty))
    routerForwarder.forward(router)
    awaitCond(paymentFSM.stateName == WAITING_FOR_PAYMENT_COMPLETE)
    val WaitingForComplete(_, _, cmd1, Nil, _, _, _, hops) = paymentFSM.stateData
    ...
    routerForwarder.expectMsg(RouteRequest(a, d, DEFAULT_AMOUNT_MSAT, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set(ChannelDesc(channelId_ab, a, b))))
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
  }
  ...
  test("payment failed (PermanentChannelFailure)") { fixture =>
    ...
    // payment lifecycle forwards the embedded channelUpdate to the router
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    routerForwarder.expectMsg(RouteRequest(a, d, DEFAULT_AMOUNT_MSAT, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set(ChannelDesc(channelId_bc, b, c))))
    routerForwarder.forward(router)
