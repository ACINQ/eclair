...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("payment succeeded to a channel with fees=0") { routerFixture =>
    ...
    router ! PeerRoutingMessage(origin, chan_bg)
    router ! PeerRoutingMessage(origin, ann_g)
    router ! PeerRoutingMessage(origin, channelUpdate_bg)
    router ! PeerRoutingMessage(origin, channelUpdate_gb)
