...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("send to route") { _ =>
    ...
    assert(outgoing.copy(createdAt = TimestampMilli(0)) === OutgoingPayment(id, parentId, Some(defaultExternalId), defaultPaymentHash, PaymentType.Standard, defaultAmountMsat, defaultAmountMsat, d, TimestampMilli(0), Some(defaultInvoice), OutgoingPaymentStatus.Pending))
