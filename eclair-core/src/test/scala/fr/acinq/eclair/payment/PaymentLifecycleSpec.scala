...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  val defaultOrigin = Origin.Cold(Upstream.Local(UUID.randomUUID(), upgradeAccountability = false))
  ...
  def createPaymentLifecycle(invoice: Invoice, storeInDb: Boolean = true, publishEvent: Boolean = true, recordMetrics: Boolean = true): PaymentFixture = {
    ...
    val cfg = SendPaymentConfig(id, parentId, Some(defaultExternalId), defaultPaymentHash, invoice.nodeId, Upstream.Local(id, upgradeAccountability = false), Some(invoice), None, storeInDb, publishEvent, recordMetrics)
