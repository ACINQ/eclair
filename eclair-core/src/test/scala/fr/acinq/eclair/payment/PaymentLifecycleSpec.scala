...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("payment failed (route not found)") { case (router, _) =>
    ...
    val request = SendPayment(142000L, "42" * 32, Nil, f)
  ...
  test("payment failed (first hop returns an UpdateFailMalformedHtlc)") { case (router, _) =>
    ...
    val request = SendPayment(142000L, "42" * 32, Nil, d, maxAttempts = 2)
    sender.send(paymentFSM, request)
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    val WaitingForRoute(_, _, Nil) = paymentFSM.stateData
    routerForwarder.expectMsg(RouteRequest(a, d, Nil, ignoreNodes = Set.empty, ignoreChannels = Set.empty))
    ...
    routerForwarder.expectMsg(RouteRequest(a, d, Nil, ignoreNodes = Set.empty, ignoreChannels = Set(channelId_ab)))
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
  ...
  test("payment failed (PermanentChannelFailure)") { case (router, _) =>
    ...
    // payment lifecycle forwards the embedded channelUpdate to the router
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    routerForwarder.expectMsg(RouteRequest(a, d, Nil, ignoreNodes = Set.empty, ignoreChannels = Set(channelId_bc)))
    routerForwarder.forward(router)
  ...
  test("payment succeeded") { case (router, _) =>
    ...
    val request = SendPayment(142000L, "42" * 32, Nil, d)
    sender.send(paymentFSM, request)
    val Transition(_, WAITING_FOR_REQUEST, WAITING_FOR_ROUTE) = monitor.expectMsgClass(classOf[Transition[_]])
