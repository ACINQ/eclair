...
import fr.acinq.eclair.router.Router.{ChannelDesc, ChannelHop, ExcludeChannel, FinalizeRoute, RemoteGossip, RouteParams, RouteRequest, RouteResponse}
...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("payment succeeded to a channel with fees=0") { routerFixture =>
    ...
    router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), chan_bg)
    router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), ann_g)
    router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), channelUpdate_bg)
    router ! PeerRoutingMessage(RemoteGossip(peerConnection.ref, remoteNodeId), channelUpdate_gb)
