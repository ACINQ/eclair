...
class InvoicePurgerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with AnyFunSuiteLike {
  ...
  test("purge invoices on startup") {
    ...
    assert(db.listIncomingPayments(0 unixms, now, None, None) == expiredPayments ++ pendingPayments ++ paidPayments)
    assert(db.listIncomingPayments(now - 100.days, now, None, None) == pendingPayments ++ paidPayments)
    ...
    assert(db.listIncomingPayments(0 unixms, now, None, None) == pendingPayments ++ paidPayments)
