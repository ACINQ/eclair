...
class PaymentRequestSpec extends AnyFunSuite {
  ...
  test("Please make a donation of any amount using payment_hash 0001020304050607080900010203040506070809000102030405060708090102 to me @03e7156ae33b0a208d0744199163177e909e80176e55d97a2f221ede0f934dd9ad") {
    ...
    assert(pr.paymentSecret.map(_.toHex) === Some("1111111111111111111111111111111111111111111111111111111111111111"))
  ...
  test("correctly serialize/deserialize variable-length tagged fields") {
    ...
    val pr = PaymentRequest(chainHash = Block.LivenetGenesisBlock.hash, amount = Some(123 msat), paymentHash = ByteVector32.fromValidHex("01" * 32), privateKey = priv, description = Left("Some invoice"), minFinalCltvExpiryDelta = CltvExpiryDelta(18), expirySeconds = Some(123456), timestamp = 12345 unixsec)
