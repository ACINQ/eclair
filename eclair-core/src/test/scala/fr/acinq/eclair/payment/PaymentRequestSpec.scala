...
import fr.acinq.eclair.payment.PaymentRequest.Codecs.dataCodec
...
class PaymentRequestSpec extends FunSuite {
  ...
  test("supported payment request features") {
    ...
    val featureBits = Map(
      ...
      Features(bin"          0001000000") -> Result(allowMultiPart = false, requirePaymentSecret = false, areSupported = false), // those are useful for nonreg testing of the areSupported method (which needs to be updated with every new supported mandatory bit)
  ...
  test("payment secret") {
    ...
    assert(pr.features === Features(Features.PAYMENT_SECRET_OPTIONAL))
