...
class OnTheFlyFundingSpec extends TestKitBaseClass with FixtureAnyFunSuiteLike {
  ...
  test("receive remote failure") { f =>
    ...
    upstream4.received.map(_.add).foreach(add => {
      ...
      val Left(Sphinx.CannotDecryptFailurePacket(expected)) = Sphinx.FailurePacket.decrypt(fail4.reason, onionSharedSecrets)
    ...
    upstream5.received.map(_.add).foreach(add => {
      ...
      assert(fwd.message.reason.isInstanceOf[FailureReason.EncryptedDownstreamFailure])
      // This is a trampoline payment: we unwrap the failure packet before forwarding upstream.
      val Left(Sphinx.CannotDecryptFailurePacket(expected)) = Sphinx.FailurePacket.decrypt(fail5.reason, onionSharedSecrets)
      assert(fwd.message.reason.asInstanceOf[FailureReason.EncryptedDownstreamFailure].packet == expected)
    })
  }
