...
import fr.acinq.eclair.{CltvExpiry, LongToBtcAmount, MilliSatoshi, NodeParams, TestConstants, randomBytes32}
...
class MultiPartPaymentHandlerSpec extends TestKit(ActorSystem("test")) with FunSuiteLike {
  ...
  test("timeout waiting for first htlc") {
    ...
    f.parent.expectMsg(MultiPartHtlcFailed(paymentHash))
    f.eventListener.expectNoMsg(10 millis)
  }

  test("timeout waiting for more htlcs") {
    val f = createFixture(100 millis)
    ...
    f.sender1.expectMsg(CMD_FAIL_HTLC(1, Right(IncorrectOrUnknownPaymentDetails(250 msat, f.currentBlockHeight)), commit = true))
    f.sender2.expectMsg(CMD_FAIL_HTLC(2, Right(IncorrectOrUnknownPaymentDetails(250 msat, f.currentBlockHeight)), commit = true))
    f.parent.expectMsg(MultiPartHtlcFailed(paymentHash))
    f.eventListener.expectNoMsg(10 millis)
  }
  ...
  test("fail additional htlcs after timeout") {
    ...
    f.sender1.expectMsg(CMD_FAIL_HTLC(1, Right(IncorrectOrUnknownPaymentDetails(250 msat, f.currentBlockHeight)), commit = true))
    f.sender1.expectMsg(CMD_FAIL_HTLC(2, Right(IncorrectOrUnknownPaymentDetails(250 msat, f.currentBlockHeight)), commit = true))
    f.parent.expectMsg(MultiPartHtlcFailed(paymentHash))

    f.sender1.send(f.handler, createMultiPartHtlc(1000 msat, 300 msat, 3))
    f.sender1.expectMsg(CMD_FAIL_HTLC(3, Right(IncorrectOrUnknownPaymentDetails(250 msat, f.nodeParams.currentBlockHeight)), commit = true))
    f.parent.expectNoMsg(25 millis)
    f.eventListener.expectNoMsg(10 millis)
  }
  ...
  test("fail all if total amount is not consistent") {
    ...
    f.sender1.expectMsg(CMD_FAIL_HTLC(1, Right(IncorrectOrUnknownPaymentDetails(600 msat, f.currentBlockHeight)), commit = true))
    f.sender2.expectMsg(CMD_FAIL_HTLC(2, Right(IncorrectOrUnknownPaymentDetails(600 msat, f.currentBlockHeight)), commit = true))
    f.parent.expectMsg(MultiPartHtlcFailed(paymentHash))
    f.eventListener.expectNoMsg(10 millis)
  }
