...
class ChannelRelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  def basicRelayTest(f: FixtureParam)(relayPayloadScid: ShortChannelId, lcu: LocalChannelUpdate, success: Boolean): Unit = {
    ...
    if (success) {
      ...
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(UnknownNextPeer()), useAttributableErrors = false, TimestampMilli.now(), commit = true))
    }
  }
  ...
  test("relay with retries") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(TemporaryChannelFailure(u1.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("fail to relay when channel is disabled") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(ChannelDisabled(u.channelUpdate.messageFlags, u.channelUpdate.channelFlags, u.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("fail to relay when amount is below minimum") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(AmountBelowMinimum(outgoingAmount, u.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("fail to relay when expiry is too small") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(IncorrectCltvExpiry(r.outgoingCltv, u.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("fail to relay when fee is insufficient") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(FeeInsufficient(r.add.amountMsat, u.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("relay that would fail (fee insufficient) with a recent channel update but succeed with the previous update") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(FeeInsufficient(r.add.amountMsat, u3.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
  }
  ...
  test("fail to relay when there is a local error") { f =>
    ...
    testCases.foreach { testCase =>
      ...
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(testCase.failure), useAttributableErrors = false, TimestampMilli.now(), commit = true))
    }
  }
  ...
  test("select preferred channels") { f =>
...

    {
      ...
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(TemporaryChannelFailure(channelUpdates(ShortChannelId(12345)).channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
    }
    {
    ...
    }
    {
      ...
      val r = createValidIncomingPacket(payload, 1000000 msat, CltvExpiry(70))
      channelRelayer ! Relay(r)
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(IncorrectCltvExpiry(CltvExpiry(61), channelUpdates(ShortChannelId(12345)).channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true))
    }
  }
  ...
  test("settlement failure") { f =>
    ...
    val testCases = Seq(
      TestCase(HtlcResult.RemoteFail(UpdateFailHtlc(channelId1, downstream_htlc.id, hex"deadbeef")), CMD_FAIL_HTLC(r.add.id, Left(hex"deadbeef"), useAttributableErrors = false, TimestampMilli.now(), commit = true)),
      TestCase(HtlcResult.RemoteFailMalformed(UpdateFailMalformedHtlc(channelId1, downstream_htlc.id, ByteVector32.One, FailureMessageCodecs.BADONION | FailureMessageCodecs.PERM | 5)), CMD_FAIL_HTLC(r.add.id, Right(InvalidOnionHmac(ByteVector32.One)), useAttributableErrors = false, TimestampMilli.now(), commit = true)),
      TestCase(HtlcResult.OnChainFail(HtlcOverriddenByLocalCommit(channelId1, downstream_htlc)), CMD_FAIL_HTLC(r.add.id, Right(PermanentChannelFailure()), useAttributableErrors = false, TimestampMilli.now(), commit = true)),
      TestCase(HtlcResult.DisconnectedBeforeSigned(u_disabled.channelUpdate), CMD_FAIL_HTLC(r.add.id, Right(TemporaryChannelFailure(u_disabled.channelUpdate)), useAttributableErrors = false, TimestampMilli.now(), commit = true)),
      TestCase(HtlcResult.ChannelFailureBeforeSigned, CMD_FAIL_HTLC(r.add.id, Right(PermanentChannelFailure()), useAttributableErrors = false, TimestampMilli.now(), commit = true))
    )
