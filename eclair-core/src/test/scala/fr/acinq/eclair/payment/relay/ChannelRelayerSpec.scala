...
class ChannelRelayerSpec extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike {
  ...
  test("relay with retries") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(TemporaryChannelFailure(u1.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("fail to relay when channel is disabled") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(ChannelDisabled(u.channelUpdate.messageFlags, u.channelUpdate.channelFlags, u.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("fail to relay when amount is below minimum") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(AmountBelowMinimum(outgoingAmount, u.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("fail to relay when expiry is too small") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(IncorrectCltvExpiry(r.outgoingCltv, u.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("fail to relay when fee is insufficient") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(FeeInsufficient(r.add.amountMsat, u.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("relay that would fail (fee insufficient) with a recent channel update but succeed with the previous update") { f =>
    ...
    expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(FeeInsufficient(r.add.amountMsat, u3.channelUpdate)), commit = true), reputationRecorder)
  }
  ...
  test("select preferred channels") { f =>
...

    {
      ...
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(TemporaryChannelFailure(channelUpdates(ShortChannelId(12345)).channelUpdate)), commit = true), reputationRecorder)
    }
    {
    ...
    }
    {
      ...
      channelRelayer ! Relay(r, TestConstants.Alice.nodeParams.nodeId)
      setConfidence(f)(1.0)
      expectFwdFail(register, r.add.channelId, CMD_FAIL_HTLC(r.add.id, Right(IncorrectCltvExpiry(CltvExpiry(61), channelUpdates(ShortChannelId(12345)).channelUpdate)), commit = true), reputationRecorder)
    }
  }
