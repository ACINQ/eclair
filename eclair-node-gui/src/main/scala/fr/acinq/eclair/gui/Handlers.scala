  ...
  def send(overrideAmountMsat_opt: Option[Long], req: PaymentRequest) = {
    ...
    (for {
      ...
      sendPayment = req.minFinalCltvExpiry match {
        case None => SendPayment(amountMsat, req.paymentHash, req.nodeId, assistedRoutes = req.routingInfo, maxAttempts = kit.nodeParams.maxPaymentAttempts)
        case Some(minFinalCltvExpiry) => SendPayment(amountMsat, req.paymentHash, req.nodeId, assistedRoutes = req.routingInfo, finalCltvExpiry = minFinalCltvExpiry, maxAttempts = kit.nodeParams.maxPaymentAttempts)
      }
