...
class MainController(val handlers: Handlers, val hostServices: HostServices) extends Logging {
  ...
  @FXML var networkChannelsFeeBaseMsatColumn: TableColumn[ChannelInfo, Number] = _
  @FXML var networkChannelsFeeProportionalMillionthsColumn: TableColumn[ChannelInfo, Number] = _
  @FXML var networkChannelsCapacityColumn: TableColumn[ChannelInfo, Number] = _
  ...
  @FXML def initialize(): Unit = {
    ...
    networkChannelsFeeBaseMsatColumn.setCellValueFactory(new Callback[CellDataFeatures[ChannelInfo, Number], ObservableValue[Number]]() {
      def call(pc: CellDataFeatures[ChannelInfo, Number]) = new SimpleLongProperty(pc.getValue.feeBaseMsat)
    })
    networkChannelsFeeProportionalMillionthsColumn.setCellValueFactory(new Callback[CellDataFeatures[ChannelInfo, Number], ObservableValue[Number]]() {
      def call(pc: CellDataFeatures[ChannelInfo, Number]) = new SimpleLongProperty(pc.getValue.feeProportionalMillionths)
    })
    networkChannelsCapacityColumn.setCellValueFactory(new Callback[CellDataFeatures[ChannelInfo, Number], ObservableValue[Number]]() {
      def call(pc: CellDataFeatures[ChannelInfo, Number]) = new SimpleLongProperty(pc.getValue.capacity)
    })
