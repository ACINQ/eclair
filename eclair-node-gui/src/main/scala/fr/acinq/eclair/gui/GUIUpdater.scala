...
class GUIUpdater(mainController: MainController) extends Actor with ActorLogging {
  ...
  def main(m: Map[ActorRef, ChannelPaneController]): Receive = {
    ...
    case ChannelRestored(channel, channelId, peer, remoteNodeId, isFunder, currentData: HasCommitments) => // We are specifically interested in normal Commitments with funding txid here
      context.watch(channel)
