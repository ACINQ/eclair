...
class GUIUpdater(mainController: MainController) extends Actor with ActorLogging {
  ...
  def main(m: Map[ActorRef, ChannelPaneController]): Receive = {
    ...
    case ChannelRestored(channel, peer, remoteNodeId, isFunder, channelId, commitments: Commitments) =>
      context.watch(channel)
      ...
      runInGuiThread(() => {
        ...
        channelPaneController.txId.setText(commitments.commitInput.outPoint.txid.toHex)
