...
class HelpersSpec extends FunSuite {
  test("add, fulfill and fail htlcs") {
    val spec = CommitmentSpec(Set(), Set(), 1000, 2000 * 1000, 0, 2000 * 1000, 0)
    ...
    assert(spec1.htlcs_out.size == 1 && spec1.htlcs_out.head.id == 1 && spec1.htlcs_out.head.rHash == H1)
    ...
    assert(spec2.htlcs_out.isEmpty && spec2.amount_them_msat == 1000 && spec2.totalFunds == spec.totalFunds)
