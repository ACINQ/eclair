  ...
  def wait(a: ActorRef, b: ActorRef, script: List[String]): Receive = {
    ...
    case d: DATA_NORMAL if script.head.endsWith(":dump") =>
      ...
      val l = List(
        ...
        s"  Offered htlcs: ${d.ourCommit.spec.htlcs_out.map(h => (h.id, h.amountMsat)).mkString(" ")}",
        s"  Received htlcs: ${d.ourCommit.spec.htlcs_in.map(h => (h.id, h.amountMsat)).mkString(" ")}",
        ...
        s"  Offered htlcs: ${d.theirCommit.spec.htlcs_out.map(h => (h.id, h.amountMsat)).mkString(" ")}",
        s"  Received htlcs: ${d.theirCommit.spec.htlcs_in.map(h => (h.id, h.amountMsat)).mkString(" ")}",
