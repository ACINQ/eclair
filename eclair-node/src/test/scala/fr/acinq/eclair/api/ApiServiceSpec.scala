...
class ApiServiceSpec extends FunSuite with ScalatestRouteTest with IdiomaticMockito with Matchers {
  ...
  test("'usablebalances' asks relayer for current usable balances") {
    val eclair = mock[Eclair]
    val mockService = new MockService(eclair)
    val mockChannelUpdate = ChannelUpdate(ByteVector64.Zeroes, ByteVector32.Zeroes, ShortChannelId(0), 0, 0, 0, CltvExpiryDelta(12), 10 msat, 1 msat, 1, None)
    eclair.usableBalances()(any[Timeout]) returns Future.successful(UsableBalances(List(
      UsableBalance(aliceNodeId, ShortChannelId(1), 100000000 msat, 20000000 msat, isPublic = true, mockChannelUpdate.copy(shortChannelId = ShortChannelId(1))),
      UsableBalance(aliceNodeId, ShortChannelId(2), 400000000 msat, 30000000 msat, isPublic = false, mockChannelUpdate.copy(shortChannelId = ShortChannelId(2)))
    )))
