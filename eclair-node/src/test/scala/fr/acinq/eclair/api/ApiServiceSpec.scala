...
class ApiServiceSpec extends FunSuite with ScalatestRouteTest with IdiomaticMockito with Matchers {
  ...
  test("'usablebalances' asks router for current usable balances") {
    ...
    eclair.usableBalances()(any[Timeout]) returns Future.successful(List(
      UsableBalances(canSend = 100000000 msat, canReceive = 20000000 msat, shortChannelId = ShortChannelId(1), remoteNodeId = aliceNodeId, isPublic = true, isEnabled = false),
      UsableBalances(canSend = 400000000 msat, canReceive = 30000000 msat, shortChannelId = ShortChannelId(2), remoteNodeId = aliceNodeId, isPublic = false, isEnabled = true)
    ))
