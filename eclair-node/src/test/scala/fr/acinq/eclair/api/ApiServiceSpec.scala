  ...
  test("'open' channels with liquidity ads") {
    ...
    Post("/open", FormData("nodeId" -> nodeId.toString(), "fundingSatoshis" -> "50000", "requestRemoteFundingSatoshis" -> "50000", "remoteFundingMaxFeeSatoshis" -> "500", "remoteFundingDurationBlocks" -> "1008").toEntity) ~>
      ...
      check {
        assert(handled)
        assert(status == OK)
        eclair.open(nodeId, 50_000 sat, None, None, None, Some(LiquidityAds.RequestRemoteFundingParams(50_000 sat, 1008, 500 sat)), None, None)(any[Timeout]).wasCalled(once)
      }
    ...
    Post("/open", FormData("nodeId" -> nodeId.toString(), "fundingSatoshis" -> "50000", "requestRemoteFundingSatoshis" -> "50000", "remoteFundingMaxFeeSatoshis" -> "500").toEntity) ~>
      ...
      check {
        assert(handled)
        assert(status == OK)
        eclair.open(nodeId, 50_000 sat, None, None, None, Some(LiquidityAds.RequestRemoteFundingParams(50_000 sat, 4032, 500 sat)), None, None)(any[Timeout]).wasCalled(once)
      }
