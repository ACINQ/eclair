...
trait Channel {
  ...
  val open: Route = postRequest("open") { implicit t =>
    formFields(nodeIdFormParam, "fundingSatoshis".as[Satoshi], "pushMsat".as[MilliSatoshi].?, "channelType".?, "fundingFeerateSatByte".as[FeeratePerByte].?, "announceChannel".as[Boolean].?, "openTimeoutSeconds".as[Timeout].?) {
      (nodeId, fundingAmount, pushAmount, channelTypeName_opt, fundingFeerate, announceChannel_opt, openTimeout_opt) =>
        withRequestedRemoteFunding { requestRemoteFunding_opt =>
          ...
          if (!channelTypeOk) {
            ...
            complete(eclairApi.open(nodeId, fundingAmount, pushAmount, channelType_opt, fundingFeerate, requestRemoteFunding_opt, announceChannel_opt, openTimeout_opt))
          }
        }
    }
  }
  ...
  val rbfOpen: Route = postRequest("rbfopen") { implicit f =>
    formFields(channelIdFormParam, "targetFeerateSatByte".as[FeeratePerByte], "lockTime".as[Long].?) { (channelId, targetFeerateSatByte, lockTime_opt) =>
      withRequestedRemoteFunding { requestRemoteFunding_opt =>
        complete(eclairApi.rbfOpen(channelId, FeeratePerKw(targetFeerateSatByte), requestRemoteFunding_opt, lockTime_opt))
      }
    }
  }
