...
trait Channel {
  ...
  val open: Route = postRequest("open") { implicit t =>
    formFields(nodeIdFormParam, "fundingSatoshis".as[Satoshi], "pushMsat".as[MilliSatoshi].?, "channelType".?, "fundingFeerateSatByte".as[FeeratePerByte].?, "maxFundingFeeSat".as[Satoshi].?, "announceChannel".as[Boolean].?, "openTimeoutSeconds".as[Timeout].?) {
      (nodeId, fundingSatoshis, pushMsat, channelTypeName_opt, fundingFeerateSatByte, maxFundingFee_opt, announceChannel_opt, openTimeout_opt) =>
        ...
        if (!channelTypeOk) {
          ...
          complete(eclairApi.open(nodeId, fundingSatoshis, pushMsat, channelType_opt, fundingFeerateSatByte, maxFundingFee_opt, announceChannel_opt, openTimeout_opt))
        }
    }
  }
