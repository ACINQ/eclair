...
trait Channel {
  ...
  val rbfOpen: Route = postRequest("rbfopen") { implicit f =>
    formFields(channelIdFormParam, "targetFeerateSatByte".as[FeeratePerByte], "fundingFeeBudgetSatoshis".as[Satoshi], "requestFundingSatoshis".as[Satoshi]?, "lockTime".as[Long].?) {
      (channelId, targetFeerateSatByte, fundingFeeBudget, requestFunding_opt, lockTime_opt) => complete(eclairApi.rbfOpen(channelId, FeeratePerKw(targetFeerateSatByte), fundingFeeBudget, requestFunding_opt, lockTime_opt))
    }
  }
  ...
  val spliceIn: Route = postRequest("splicein") { implicit f =>
    formFields(channelIdFormParam, "amountIn".as[Satoshi], "requestFundingSatoshis".as[Satoshi]?, "pushMsat".as[MilliSatoshi].?) {
      (channelId, amountIn, requestFunding_opt, pushMsat_opt) => complete(eclairApi.spliceIn(channelId, amountIn, requestFunding_opt, pushMsat_opt))
    }
  }
  ...
  val spliceOut: Route = postRequest("spliceout") { implicit f =>
    formFields(channelIdFormParam, "amountOut".as[Satoshi], "scriptPubKey".as[ByteVector](bytesUnmarshaller), "requestFundingSatoshis".as[Satoshi]?) {
      (channelId, amountOut, scriptPubKey, requestFunding_opt) => complete(eclairApi.spliceOut(channelId, amountOut, Left(scriptPubKey), requestFunding_opt))
    } ~
      formFields(channelIdFormParam, "amountOut".as[Satoshi], "address".as[String], "requestFundingSatoshis".as[Satoshi]?) {
        (channelId, amountOut, address, requestFunding_opt) => complete(eclairApi.spliceOut(channelId, amountOut, Right(address), requestFunding_opt))
      }
  }
