...
Eclair will use any BTC it finds in the Bitcoin Core wallet to fund any channels you choose to open. Eclair will return BTC from closed channels to this wallet. Eclair will stop with an error if the wallet contains any outputs that are not either `p2sh-segwit` addresses or `bech32` addresses. In this case, you must send all your bitcoins to a new `p2sh-segwit` or `bech32` address before starting Eclair.
