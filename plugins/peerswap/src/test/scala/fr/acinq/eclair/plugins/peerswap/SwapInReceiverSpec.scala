...
import fr.acinq.eclair.plugins.peerswap.wire.protocol.PeerSwapMessageCodecs.swapInAgreementCodec
...
case class SwapInReceiverSpec() extends ScalaTestWithActorTestKit(ConfigFactory.load("application")) with FixtureAnyFunSuiteLike with BeforeAndAfterAll with Logging {
  ...
  override def withFixture(test: OneArgTest): Outcome = {
    ...
    val swapInReceiver = testKit.spawn(Behaviors.monitor(monitor.ref, SwapTaker(TestConstants.Bob.nodeParams, paymentInitiator.ref.toClassic, watcher.ref, register.ref.toClassic, wallet, keyManager, db)), "swap-in-receiver")
  ...
  test("happy path from restored swap in") { f =>
    import f._
  ...
  test("happy path for new swap in") { f =>
    ...
    val agreement = swapInAgreementCodec.decode(expectUnknownMessage(register).data.drop(2).toBitVector).require.value
