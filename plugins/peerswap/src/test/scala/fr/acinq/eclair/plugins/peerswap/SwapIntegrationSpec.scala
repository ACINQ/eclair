...
import fr.acinq.eclair.plugins.peerswap.wire.protocol.SwapInAgreement
import fr.acinq.eclair.testutils.FixtureSpec
import fr.acinq.eclair.{BlockHeight, ShortChannelId, randomKey}
...
class SwapIntegrationSpec extends FixtureSpec with IntegrationPatience {
  ...
  test("eclair forwards swap messages to the SwapRegister") { f =>
    ...
    // TODO: add message to SwapRegister which forwards messages to channel peer
    // alice.peer.send(peer, swapInAgreement)
    //val messageReceived = alice.swapRegister.expectMsgType[MessageReceived]
